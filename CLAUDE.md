...
The rule that outlives the specifics: before claiming a branch change is visible through a link, check whether it lives in **lib** (previewable with `?use=`) or in a **page shell** (previewable only when the page is the toss subject, which for toss-render means nesting). Headless evidence is the substitute; say so rather than implying the link will show it.
