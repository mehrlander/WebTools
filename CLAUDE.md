...
**It is a git hook now, and that is the fix for the failure it used to have.** It was a Claude Code `PreToolUse(Bash)` hook until 2026-08-06, declared in `.claude/settings.json`, which the harness reads **only when the session's project root IS this repo**. In a multi-repo session the root sits above the checkouts, so the declaration was never read and the hook never ran, silently (cause and tells: [docs/environment/extending.md](docs/environment/extending.md)). Git resolves its hooks from the repository being committed to and has no notion of a project root, so a git hook fires from any root. The one thing git will not do is find a *committed* hooks folder, since `.git/hooks/` is local and absent on clone; [`.claude/hooks/session-githooks.sh`](.claude/hooks/session-githooks.sh) sets `core.hooksPath` and is itself discovered by filename by the `portable` plugin's session dispatcher, so it too runs from any root. Home has carried the identical pair since 2026-07-31. Bypass with `git commit --no-verify`.

**Best-effort still, so the lockstep keeps its second owner.** A clone whose `core.hooksPath` was never set runs nothing, and that is now the only gap rather than the common case. So `npm test` carries [`tools/test/artifacts-lockstep.test.mjs`](tools/test/artifacts-lockstep.test.mjs), which re-runs each deterministic generator in `--check` mode and fails if a tracked artifact is behind its source. When it fails, run the command it names and commit the result.
