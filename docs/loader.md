...
| `sessionRender` | `session-render.js` | a session record (web-tools-private `sessions/**.json`) as a paged conversation: merges the record's three parallel turn lists on `at`, groups a card per exchange, folding the work in two levels (a run of calls plus the sentence that introduced it is a step; a run of steps is one summary line), and names what the record could not capture. Loads after `chat-render.js` |
