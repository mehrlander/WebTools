...
| `sessionRender` | `session-render.js` | a session record (web-tools-private `sessions/**.json`) as a paged conversation: merges the record's three parallel turn lists on `at`, groups a card per exchange, folding each run of tool calls and the short sentence that introduced it behind one summary line, and names what the record could not capture. Loads after `chat-render.js` |
