...
| `Annotate` | `kits/annotate.js` | notes pinned to selections/elements/regions of a target document, or to the page as a whole; the set (or one note of it) serializes to markdown/JSON, and saves as a jot. The card reads its own set behind an expander. FAB take "Annotate" and `pages/annotate.html` drive it. Chain `kits/dictate.js` before it for the voice composer |
