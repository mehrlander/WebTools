...
- **A whitespace-only region is not a decision.** A version that reflowed a paragraph turns every line wrap into a change, and a word diff that keeps whitespace as content reports each one: on a 1,827 word essay with four versions, 15 of 88 regions were a newline against a space. Those are marked `trivial` and dropped from the decision list, but kept as segments, because flattening them away would silently substitute the original's wrapping into every version.
