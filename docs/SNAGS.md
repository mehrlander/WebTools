...
**61 snags**, repeats first. Read this before adding one: a trip that is
...
| `injection-budget-counts-the-checkout-path` | 2026-08-31 | a green local run went red on CI, by 8 bytes |
...
### injection-budget-counts-the-checkout-path: a green local run went red on CI, by 8 bytes

`inject-conventions.sh` prints an absolute path into its recovery preamble
(`Read CONVENTIONS.md and SURFACING.md from $DOCS`), so the payload it measures
against the budget grows one byte per character of the checkout root. The
sandbox root is `/home/user/web-tools` at 20 characters and the runner's is
`/home/runner/work/web-tools/web-tools` at 37, a 17-byte gap that is invisible
from either side alone.

On 2026-08-31 a branch grew the primitives by 66 bytes. `npm test` passed here
with 9 bytes to spare and the same commit failed on CI at 8 bytes over, dropping
SURFACING.md from the payload entirely rather than trimming it. The number in
the failure (25,451 against 25,443) is arithmetic on the path, not on the prose.

The branch bought margin instead of chasing the fit, and the check that proves
it is running the injector from a padded root rather than trusting a local pass:
at a 106-character root the rung still holds. The path dependence itself is
unfixed, and a budget whose value depends on where the repo happens to sit will
do this again to the next branch that lands within ~20 bytes of the
ceiling. *(seen: 2026-08-31)*
→ [environment/capabilities.md](environment/capabilities.md); the path dependence itself is unowned
