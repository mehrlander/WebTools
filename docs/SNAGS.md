...
**58 snags**, repeats first. Read this before adding one: a trip that is
...
| `untracked-file-invisible-to-the-suite` **×2** | 2026-08-30 | a local green over a file the scanner never enumerated |
...
on nothing you changed since. Twice now, and the second time is the more useful
one: on 2026-08-30 a new `skills/state-the-rule/runs/…/standoff.json` passed
2810/0 locally and failed the same gate on the same push, with this entry
already the newest in the log and its corrected move already written down. So
the entry is not the fix. What would be is running the check against the index
the gate uses, which `git add -A` before the final suite run does for free.
*(seen: 2026-08-28, 2026-08-30)*
