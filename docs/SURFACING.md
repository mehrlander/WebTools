...
`docs/MERGE-GUIDE.md` is a durable newest-first log of merged PRs, generated by a repo-owned script; web-tools supplies [`scripts/build-merge-guide.py`](https://github.com/mehrlander/web-tools/blob/main/scripts/build-merge-guide.py). There is no hand-written merge-guide step. Run it where API access exists and commit the output. Where a sandbox blocks `api.github.com`, the GitHub MCP's `list_pull_requests` returns objects of the shape `--from-json` reads, so the generator runs there too.

The merge guide keys on the **PR**, a unit of delivery; [TRACKER.md](https://github.com/mehrlander/web-tools/blob/main/docs/TRACKER.md) keys on the **task**, a unit of intent. Choose one primary historical axis to avoid competing logs. Keeping neither is also an answer: `mehrlander/home` runs the task axis alone and holds no merge guide.

**Two projections, chosen once per repo.**

| | `--index` | full entries |
| --- | --- | --- |
| A row is | date, number, title, PR link | the PR's guide region, copied in |
| Covers | every merged PR | only PRs whose body followed the convention |
| Ages by | falling behind, visibly | falling behind, and by drifting from a body edited after merge |
| Costs | about 15 words per PR | about 400 words per PR |
| Answers | what shipped, when, and where to read the account | what shipped, in full, with no network |

**Prefer `--index`, and reach for full entries only where offline full-text delivery history is something the repo actually uses.** The two failure modes are not symmetric. An index can be incomplete, and that shows: the newest row names the last PR it covers. A copy can be incomplete *and* wrong, because `extract()` yields nothing for a terse body and drops that PR silently, and because a body edited after merge leaves the copy disagreeing with its source until someone runs `--refresh`. A copy also carries the PR's per-file blob links, which later renames break, while a PR URL never moves.

Full entries are non-destructive and keyed by PR number: add uncovered PRs, preserve existing entries, `--refresh` to regenerate covered ones. `--index` rewrites wholesale, preserving only a trailing `## Merges without a pull request` section for merges the pulls endpoint cannot see.

Under either projection, main can show that a represented PR is included, but absence is not proof of non-merge; git and GitHub remain authoritative.

A full entry mirrors the guide region, puts the result and primary file first, preserves `renders on:`, rewrites branch URLs to main, and drops branch-only next steps.
