...
`?view=guides`, and `?view=chats` deep-link directly. Where a pane has an as-of
readout and a Refresh, they ride the pill row, since the panes read different
caches on different throttles.
...
- **There is no Refresh, deliberately.** The month shards are immutable once
  committed and the frontier moves only when an export lands, which is a commit
  to another repo rather than a crawl this page could run. A control that did
