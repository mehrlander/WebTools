...
if git status --porcelain -- tracker/tasks/ | grep -q .; then
  if npm run tracker-board --silent >/dev/null 2>&1; then
    git add tracker/board.md tracker/board.json 2>/dev/null || true
  else
