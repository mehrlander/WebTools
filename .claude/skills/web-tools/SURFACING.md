...
* **Run a shortcut 📲:** the same link shape, payoff anything but the clipboard. The payload is legible, so the caption stays short.
...
**A third measurement contradicted both** (measured 2026-08-09, web-tools PR #384, three writes to the **body**; the evidence is in that body): a slashless 40-character SHA carrying `:pages/session.html`, a one-slash path, was wrapped, and percent-encoding the colon as `%3A` did not help. That is the case the fifth measurement records as passing, so the two could not both be describing the same rule. The suspicion was that the **surface** was the uncontrolled variable, #384 having written to a body and #385 to comments.

**The controlled probe settles it, and the surface was not the variable** (measured 2026-08-10, web-tools PR #401: four addresses written to the body and to a comment in one pass, read back through the MCP). **Body and comment came back identical in all four cells.** Only the `:path` depth matters:

| Probe | Ref | `:path` | Body | Comment |
| --- | --- | --- | --- | --- |
| P1 | SHA | `pages/annotate.html` | passes | passes |
| P2 | SHA | `pages/show-repo/show-repo.html` | **wrapped** | **wrapped** |
| P3 | `claude/<branch>` | `pages/annotate.html` | passes | passes |
| P4 | SHA | none | passes | passes |

So the operative rule is one line: **a `#gh=` address is wrapped when its `:path` carries more than one slash.** Ref shape is irrelevant (P3 has a slashed ref *and* a path, the fourth trigger's exact case, and passes), and so is the surface. **The fourth trigger is retired**; the fifth's path-depth finding is what survives, now measured twice.

What is *not* explained is #384's result, since P1 is its case and passes here. Either the sanitizer changed between the two days or that probe carried something its write-up does not record. So a nested page still cannot be tossed from a body, a one-slash path is fine, and a body toss is cheap to read back once rather than trust. Link the branch page, which carries no `:path` and passed every time, or hand the reader a `#gz=`. Chat replies are unaffected, so the full toss still belongs there.
