...
**A third measurement contradicts both** (measured 2026-08-09, web-tools PR #384, three writes to the **body**; the evidence is in that body, folded in here 2026-08-10): a slashless 40-character SHA carrying `:pages/session.html`, a one-slash path, was wrapped identically, and percent-encoding the colon as `%3A` did not help either. That is precisely the case the fifth measurement records as passing. The variable nobody has controlled is the **surface**: #384 wrote to a PR **body**, #385 to PR **comments**. If those sanitize differently, all three results are correct and the missing term is where the text was written, not what it said. A controlled probe, the same address written to both surfaces in one pass, is the outstanding work.

**Until then, take the rule that survives every measurement: a PR body cannot carry a `#gh=` address with a `:path`, on any ref, at any depth.** Link the branch page, which carries no `:path` and passed clean in all three, or hand the reader a `#gz=`. Chat replies are unaffected, so the full toss still belongs there, and this is one more reason the render link lives in the chat caption rather than the body.
