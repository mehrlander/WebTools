...
// docs/properties.json (fetched relative, same origin) through a stubbed GH.get.
...
export default async function (page) {
  const props = await page.evaluate(() => fetch('../../docs/properties.json').then(r => r.text()));
  const ok = await page.evaluate((propsText) => {
    ...
    window.GH.prototype.get = async function (name) {
      if (name === 'docs/properties.json') return { text: propsText };
