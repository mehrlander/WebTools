...
// scenario serves the REAL committed docs/harness.csv through GH.get, with no
...
export default async function (page) {
  const reg = await page.evaluate(() => fetch('../../docs/harness.csv').then(r => r.text()));
  const ok = await page.evaluate((regText) => {
    ...
    window.GH.prototype.get = async function (name) {
      if (name === 'docs/harness.csv') return { text: regText };
