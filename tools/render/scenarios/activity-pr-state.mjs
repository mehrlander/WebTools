  ...
  await page.evaluate(() => {
    ...
    d.activity = {
      'mehrlander/web-tools': {
        ...
        counts: { branches: 24, active: 5, landed: 6, stranded: 1, scanned: 8, older: 12, openPRs: 1 },
        openPRs: [
        ...
        branchPRs: [
          ...
          { head: 'claude/cem-scan-powershell-oxl71l', number: 426, state: 'closed', draft: true, count: 1 },
        ],
        prReach: iso(21 * 24),
        scan: { scannedAt: iso(1), cap: 30, scanned: 8, older: 12, truncated: false, branches: [
          ...
          br('claude/cem-scan-powershell-oxl71l', iso(14), 'conventions: define a local term or drop it'),
          br('claude/scratch-no-pr-yet', iso(20), 'A branch pushed without a pull request'),
        ] },
      },
      'mehrlander/home': {
        pushedAt: iso(14), defaultBranch: 'main',
        counts: { branches: 31, active: 1, landed: 9, stranded: 2, scanned: 10, older: 18, openPRs: 0 },
        openPRs: [],
        // An index that WAS capped, and a branch older than it reaches: the row
        // that must not claim there is no PR.
        branchPRs: [{ head: 'claude/cem-scan-powershell-oxl71l', number: 214, state: 'merged', draft: false, count: 1 }],
        prReach: iso(10),
        scan: { scannedAt: iso(14), cap: 30, scanned: 10, older: 18, truncated: false, branches: [
          br('claude/cem-scan-powershell-oxl71l', iso(14), 'full-picture: restamp the as-of date at wrap-up'),
