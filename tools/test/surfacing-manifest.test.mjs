// docs/surfacing.json — the machine index of the surfacing primitives. Unlike
...
// unchecked, which docs/docs.json's claims table says out loud.
...
const manifest = JSON.parse(readFileSync(path.join(repoRoot, 'docs', 'surfacing.json'), 'utf8'));
...
test('the primitives section parses and both sides are non-trivial', () => {
  ...
  assert.equal(manifest.doc, 'docs/SURFACING.md', 'the manifest names its authoritative carrier');
});
