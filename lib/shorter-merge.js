...
(() => {
  ...
  //
  //   { kind: 'equal',  id, text }
  //   { kind: 'choice', id, a, b, options[], selected }
  //
  ...
  function build(original, proposals, diffFn, opts) {
    ...
    for (const [A, B] of regions) {
      ...
      segs.push({ id: id++, kind: 'choice', a: A, b: B, options, selected: options[0].id });
      cursor = B;
    }
  ...
  window.ShorterMerge = { opsFor, regionsFrom, absorbGaps, textForRegion, optionsFor, build, words, GAP };
})();
