...
(() => {
  ...
  function checks(p, r) {
    ...
    add('readable', p.kind === 'set-json-field' ? 'Target is readable JSON' : 'Target is readable',
      'pass', r.exists ? '' : 'the file does not exist yet, so applying creates it');
  ...
  window.RepoProposals = {
    PENDING_DIR, APPLIED_DIR, ATTEMPT_DIR, KINDS, DELIVERIES,
