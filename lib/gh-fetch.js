(() => {
  ...
  proto.graphql = async function(query, variables = {}) {
    try {
      ...
      // ran and the nulls already carry the misses, so hand it back and note
      // the partials; no data at all is the real failure.
      if (json.errors && json.data != null) {
        try { console.warn(`gh-fetch: GraphQL ${opOf(query)} returned partial data (${json.errors.length} field error(s); first: ${json.errors[0]?.message})`); } catch {}
        return json.data;
      }
