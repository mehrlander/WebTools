...
(() => {
  ...
  const paint = (host, o = {}) => {
    ...
    if (interim) {
      host.appendChild(mk('interim', (text ? ' ' : '') + interim,
        o.interimCss || 'color:#a1a1aa;font-style:italic;'));
    }
    // The handles come LAST and out of the flow. Inline they were part of the
    // line, so arriving at a selection shoved the text sideways and reading
    // it meant reading a paragraph that had just moved; absolute positioning
    // over the selection's own rects costs a measure and moves nothing.
    //
    // Above on the left and below on the right, which is the arrangement every
    // phone uses and worth matching rather than inventing: the two are then
    // distinguishable at a glance, and neither sits where the word it marks
    // is being read.
    if (sel) {
      host.style.position = host.style.position || 'relative';
      const rects = sel.getClientRects ? sel.getClientRects() : [];
      const hb = host.getBoundingClientRect ? host.getBoundingClientRect() : { left: 0, top: 0 };
      const first = rects[0], last = rects[rects.length - 1];
      // A dot on a STEM, not a dot alone. The stem runs the height of the line
      // down the edge of the selection, which is what every platform draws and
      // what makes the pair read as brackets around the text rather than as
      // two loose markers near it: the eye gets the boundary from the line and
      // the target from the ball.
      const D = 18, BAR = 3, PAD = D + 4;
      const put = (edge, x, top, h) => {
        const box = mk('handle-' + edge, '',
          'position:absolute;width:32px;cursor:pointer;z-index:2;'
          + 'left:' + Math.round(x - 16) + 'px;top:' + Math.round(top - PAD) + 'px;'
          + 'height:' + Math.round(h + PAD * 2) + 'px;');
        box.setAttribute('data-edge', edge);
        const paint = o.armed === edge
          ? 'background:#dc2626;' : 'background:#2563eb;';
        ...
        dot.setAttribute('style', 'position:absolute;pointer-events:none;border-radius:50%;'
          ...
          + 'width:' + D + 'px;height:' + D + 'px;' + paint
          + (o.armed === edge ? 'box-shadow:0 0 0 4px #fecaca;' : ''));
        box.appendChild(bar);
        box.appendChild(dot);
        host.appendChild(box);
      };
      // No rects means no layout (jsdom, or a hidden card): the handles still
      // exist so the state is readable and the arming target is there, they
      // simply have nowhere to be yet.
      const sx = host.scrollLeft || 0, sy = host.scrollTop || 0;
      if (first) put('start', first.left - hb.left + sx, first.top - hb.top + sy, first.height);
      else put('start', 0, 0, 0);
      if (last) put('end', last.right - hb.left + sx, last.top - hb.top + sy, last.height);
      else put('end', 0, 0, 0);
    }
    return host;
  };
  ...
  window.Dictate = { create, available, paint, offsetAt, SUPPRESS, _SPOKEN: SPOKEN };
})();
