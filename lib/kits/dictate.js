...
(() => {
  ...
  const paint = (host, o = {}) => {
    ...
    if (!r) {
      ...
      // A selection, with a handle at each edge. Tapping a handle ARMS it and
      // the next tap in the text places it: dragging is what puts a finger
      // over the words being aimed at, which is the whole reason this is not
      // the browser's selection.
      const hCss = (armed) => 'display:inline-block;width:13px;height:13px;border-radius:50%;'
        + 'vertical-align:middle;margin:0 -2px;cursor:pointer;'
        + (armed ? 'background:#dc2626;box-shadow:0 0 0 3px #fecaca;' : 'background:#2563eb;');
      host.appendChild(mk('text', text.slice(0, r.start)));
      const h1 = mk('handle-start', '', hCss(o.armed === 'start'));
      h1.setAttribute('data-edge', 'start');
      host.appendChild(h1);
      host.appendChild(mk('sel', text.slice(r.start, r.end), o.selCss
        || 'background:#bfdbfe;border-radius:2px;'));
      const h2 = mk('handle-end', '', hCss(o.armed === 'end'));
      h2.setAttribute('data-edge', 'end');
      host.appendChild(h2);
  ...
  const offsetAt = (host, node, nodeOffset) => {
    ...
    for (const part of host.childNodes) {
      ...
      if (part === n) {
        if (kind === 'caret' || (kind || '').startsWith('handle')) return total;
