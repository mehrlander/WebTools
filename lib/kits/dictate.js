...
(() => {
  ...
  const paint = (host, o = {}) => {
    ...
    if (sel && o.handles !== false) {
      ...
      // WHICH SIDE OF ITS LINE EACH BALL HANGS ON. The default is the platform's
      // split, above on the left and below on the right, which tells the two
      // apart at a glance and is right for a surface where a pin is TAPPED.
      //
      // `pinsBelow` puts both underneath, for a surface where a pin is DRAGGED.
      // There the ball is a grip, the thumb goes wherever the ball is, and a
      // grip that is above the line on one edge and below it on the other means
      // the thumb covers the words on one of them and the same drag reads
      // differently depending on which end you grabbed. Below for both puts the
      // hand in the same place either way.
      const BELOW = o.pinsBelow === true;
      const put = (edge, x, top, h) => {
        const below = BELOW || edge === 'end';
        ...
        // Each ball touches the far end of its own stem, on whichever side
        // `pinsBelow` put it.
        //
