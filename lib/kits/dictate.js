...
//
//   AND `jumpBack()`, WHICH IS HOW THE CARET REACHES AN OLDER ONE. The stitch
//   reached backwards by itself for a few hours on 2026-09-06 and no ordering
//   of its targets could be right: the buffer almost always ends in a full
//   stop, so "the most recent break" is that one, and whether the reader wants
//   it gone or wants to keep it and fix an earlier one is a fact about what
//   they are about to say. Walking the caret back shows the target before
//   anything changes. See "The seam" below.
//
...
//   .stitch() .canStitch    undo the sentence break the caret is at
//   .stitchAim              which repair that is: 'caret' / 'end' / ''
  ...
  const create = (opts = {}) => {
    ...
    const api = {
      ...
      stitch() {
        ...
        range = borrowed || at >= text.length ? null : { start: at, end: at };
        jumped = false;
