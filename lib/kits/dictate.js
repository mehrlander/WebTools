  ...
  const paint = (host, o = {}) => {
    ...
    if (sel && o.handles !== false) {
      ...
      const put = (edge, x, top, h) => {
        ...
        const up = edge === 'start' ? out : 0, down = edge === 'end' ? out : 0;
        ...
        // record button already uses for "this one is live". The stalk above
        // does not reopen that: it moves the ball OUT of the text rather than
        // growing it in place, which was the objection.
        const dy = edge === 'start' ? PAD - D : PAD + Math.round(h) + down;
