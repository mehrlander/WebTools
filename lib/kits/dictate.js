    ...
    const api = {
      ...
      start() {
        ...
        r.onresult = (ev) => {
          ...
          // More is being said, so the period the last pause earned is not the
          // end of anything. Take it back before the reader sees it sitting
          // between the settled text and the words still arriving.
          if (live && provisional) { dropProvisional(); onText(text); }
          interim = live;
          onInterim(interim);
        };
        r.onerror = (ev) => {
