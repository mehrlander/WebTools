...
(() => {
  ...
  const paint = (host, o = {}) => {
    ...
    // ── The pending stitch ────────────────────────────────────────────────
    // A seam a surface has ARMED but not committed, painted as the edit it
    // would make rather than as a highlight over the characters it covers.
    // The full stop is struck through and the capital behind it is drawn in
    // the lower case it would take, so the marker is a preview: what is on
    // screen is what the next tap produces, and there is nothing to interpret.
    //
    // A highlight was the obvious move and says less. Three tinted characters
    // answer "here" and leave "and then what" to be remembered, which is the
    // question a confirm step exists to answer.
    //
    // OFFSETS SURVIVE IT, which is what lets this ride the same host the hit
    // test walks: the cut span holds the seam's own characters and the case
    // span holds exactly one, swapped for its lower-case twin rather than
    // removed. Every text node keeps its length, so an offset found by walking
    // them is the offset it was before the marker went on.
    //
    // AND IT TAKES THE SCREEN FROM THE CARET while it is up. A caret sitting
    // inside the marker (which is where the step-back key leaves it) reads as
    // a second thing to look at during a moment that has one question in it.
    // The insertion point has not moved and comes back when the marker goes.
    const mendSpans = (m) => {
      const out = [];
      let cut = m.start;
      while (cut < m.end && TERM.test(text[cut] || '')) cut++;
      out.push(mk('text', text.slice(0, m.start)));
      out.push(mk('mend-cut', text.slice(m.start, m.end), o.mendCutCss
        || 'background:#fef3c7;color:#b45309;text-decoration:line-through;'
        + 'text-decoration-thickness:2px;border-radius:2px 0 0 2px;'));
      const after = text.slice(m.end);
      if (after) {
        // The same test `stitch()` applies, so a name or an acronym is shown
        // standing rather than promised a change it will not get.
        const head = /^[A-Z][a-z]/.test(after) ? after[0].toLowerCase() : after[0];
        out.push(mk('mend-case', head, o.mendCaseCss
          || 'background:#fef3c7;color:#b45309;border-radius:0 2px 2px 0;'));
        out.push(mk('text', after.slice(1)));
      }
      return out;
    };
