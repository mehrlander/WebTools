    ...
    if (sel) {
      ...
      const put = (edge, x, top, h) => {
        ...
        dot.setAttribute('style', 'position:absolute;pointer-events:none;border-radius:50%;'
          ...
          + (o.armed === edge ? 'box-shadow:0 0 0 3px #fecaca;' : ''));
        box.appendChild(bar);
        box.appendChild(dot);
      ...
      if (first) { const f = off(first); put('start', f.l, f.t, first.height); }
      else put('start', 0, 0, 0);
      if (last) { const l = off(last); put('end', l.r, l.t, last.height); }
      else put('end', 0, 0, 0);
      ...
      if (o.armed) {
        ...
        if (r) {
          ...
          const cluster = mk('nudge', '',
            ...
            + 'left:' + Math.round(x - 37) + 'px;top:' + Math.round(y) + 'px;');
          for (const [dir, glyph] of [[-1, '\u2039'], [1, '\u203a']]) {
            const b = doc.createElement('button');
            b.setAttribute('data-nudge', String(dir));
            b.setAttribute('type', 'button');
            b.setAttribute('style', 'width:34px;height:34px;border-radius:8px;cursor:pointer;'
              + 'border:1px solid #fecaca;background:#fff;color:#dc2626;'
              + 'font:700 20px/1 ui-sans-serif,system-ui;box-shadow:0 1px 3px rgba(0,0,0,.12);');
            b.textContent = glyph;
