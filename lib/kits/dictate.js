...
(() => {
  ...
  const paint = (host, o = {}) => {
    ...
    if (sel) {
      ...
      const put = (edge, x, y) => {
        const h = mk('handle-' + edge, '',
          'position:absolute;width:30px;height:30px;margin:-15px 0 0 -15px;cursor:pointer;'
          + 'display:flex;align-items:center;justify-content:center;z-index:2;'
          + 'left:' + Math.round(x) + 'px;top:' + Math.round(y) + 'px;');
        h.setAttribute('data-edge', edge);
        const dot = doc.createElement('span');
        dot.setAttribute('style', 'width:13px;height:13px;border-radius:50%;pointer-events:none;'
          + (o.armed === edge ? 'background:#dc2626;box-shadow:0 0 0 3px #fecaca;' : 'background:#2563eb;'));
        h.appendChild(dot);
        host.appendChild(h);
      };
      ...
      if (first) put('start', first.left - hb.left + sx, first.top - hb.top + sy - 8);
      else put('start', 0, 0);
      if (last) put('end', last.right - hb.left + sx, last.bottom - hb.top + sy + 8);
      else put('end', 0, 0);
    }
    return host;
  };
