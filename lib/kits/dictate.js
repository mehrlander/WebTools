...
//   the repair is two edits at one seam, and `stitch()` makes them one.
//   It took a CARET dropped in the gap until 2026-09-06 and now takes none:
//   with no caret placed it reaches the most recent invented break, which is
//   the only reading that survives contact with dictation, since by the time
//   a thumb could aim at the gap the reader is three words past it. A caret
//   still wins where one is placed. See "The seam" below.
//
...
//   .stitch() .canStitch    undo an invented sentence break: the one the caret
//                           is in, else the most recent one in the buffer
//   .stitchAim              which of those it would do: caret / end / back / ''
...
(() => {
  ...
  const create = (opts = {}) => {
    ...
    // THE MOST RECENT SEAM, WHEREVER THE CARET IS. `seamAt` answers "is the
    // caret in one", which was the whole model until it met the way dictation
    // actually runs: the pause writes a full stop, you HEAR it land, and you
    // are already saying the next words. By the time a thumb could reach the
    // buffer the seam is three words back, and repairing it meant leaving the
    // flow, aiming a caret at a gap, tapping, and finding your place again.
    //
    // So the repair stops needing an address. Scanning right to left for the
    // first mark run that IS a seam finds the break the reader just heard,
    // because the newest one is the rightmost one, and a reader who taps twice
    // walks back through them in the order they were invented.
    //
    // It tests each run rather than taking the first: a mark ending the buffer
    // has nothing after it to join to (that is `unendable`, below), and a mark
    // with no word in front of it is not a sentence ending at all.
    const lastSeam = () => {
      let i = text.length;
      while (i > 0) {
        let e = i;
    ...
    const api = {
      ...
      // WHAT THE KEY WOULD DO, so a surface can present it honestly instead of
      // inferring it from a boolean. The three readings are not equally cheap
      // to offer: 'caret' and 'end' are states where a full stop key is either
      // aimed away from or inert, so a pad can let the stitch borrow that cell
      // and lose nothing, while 'back' is true of almost any buffer and a cell
      // borrowed for it would never give the full stop back. A surface with
      // room for a key of its own takes all three; one without takes the two
      // that cost nothing. Both call `stitch()`, which does the right thing
      // either way.
      //
      //   'caret'  the caret is in a seam: the reader aimed
      //   'end'    a full stop ends the buffer with nothing after it (unend)
      //   'back'   neither, but there is an earlier break to reach
      //   ''       nothing to undo
      get stitchAim() {
        if (range && range.start !== range.end) return '';
        if (seamAt(range ? range.start : text.length)) return 'caret';
        if (unendable()) return 'end';
        return lastSeam() ? 'back' : '';
      },
      get canStitch() { return !!api.stitchAim; },
      stitch() {
        if (range && range.start !== range.end) return false;
        const aimed = seamAt(range ? range.start : text.length);
        // The end of the buffer is not an aim: a caret resting there is where
        // dictation leaves it, not somewhere the reader put it, so it must not
        // shadow the two readings below.
        const seam = aimed || (unendable() ? null : lastSeam());
        if (!seam) return unendable() ? api.unend() : false;
        record();
        ...
        // THE CARET MOVES ONLY WHERE THE READER PUT IT. Landing it on the join
        // is right when they aimed at that gap: it shows the repair and leaves
        // them where they were working. It is wrong for the reaching-back case,
        // which exists precisely so that nobody has to stop dictating, and
        // where the resting null range means "append at the end". A caret
        // sitting past the seam keeps its place in the text by shifting with
        // it rather than by holding an offset the edit moved.
