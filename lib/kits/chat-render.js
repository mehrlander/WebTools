...
(() => {
  ...
  // THE CLOCK BELONGS TO THE ASK. In a log it says where an exchange begins,
  // and a reply's own clock restates that a minute or two later: two numbers
  // down the left edge where one is the landmark. So only a turn that OPENS an
  // exchange prints one, and the reply keeps its time on the icon's title,
  // where it costs nothing and is still recoverable.
  const denseHead = (m, meta, role) => {
    const showWord = m.label || (role !== 'user' && role !== 'assistant');
    const showTs = !!m.ts && role === 'user';
    const icon = h('i', { class: `ph ${meta.icon} text-[11px] opacity-50 not-prose` });
    if (m.ts && !showTs) icon.setAttribute('title', String(m.ts));
    const lead = h('span', { class: 'inline-flex items-baseline gap-1 mr-1.5 align-baseline' }, icon);
    if (showWord) lead.append(h('span',
      { class: 'font-mono text-[9.5px] tracking-widest uppercase opacity-50' }, m.label || meta.label));
    if (showTs) lead.append(h('span', { class: 'font-mono text-[9.5px] opacity-40' }, String(m.ts)));
    return lead;
  };
  ...
  const moreChip = (n) => h('span', {
    class: 'inline-block align-baseline ml-1.5 rounded px-1 bg-base-content/10'
         + ' font-mono text-[9.5px] leading-[1.4] text-base-content/50 not-prose',
