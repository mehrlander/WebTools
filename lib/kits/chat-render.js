...
(() => {
  ...
  // the text column is their sum. Both are smaller than LEAD_INDENT on purpose:
  // the icon occupies 0 to 11px, so a fill at 10 tucks under its trailing edge
  // and the ask sits as close to the gutter as it can without a gap that reads
  // as a second indent.
  //
  ...
  function message(m, o = {}) {
    ...
    // was a second voice for one fact and now it is the only one. Every turn
    // sits on the same left edge, and the ask's fill bleeds 8px past the text
    // with -mx-2/px-2 so the two text columns still line up exactly.
    //
