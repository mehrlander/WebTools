...
(() => {
  ...
  function message(m, o = {}) {
    ...
    // NO FILL EITHER, so a dense turn's whole frame is one word: where it sits.
    // The ask carried a band for one round, and it did find the exchanges, but
    // it found them the way a rail did, by drawing a shape around the text
    // rather than letting the text make one. What is left says the same thing
    // with no ink at all: the ask flush and the reply indented, mono against
    // prose, a tinted glyph apiece, and a gap before each ask twice the gap
    // inside an exchange.
