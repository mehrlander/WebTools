...
(() => {
  ...
  function markdown(md, o = {}) {
    ...
    const flush = () => {
      ...
      if (o.dense) el.style.fontSize = DENSE_PX + 'px';
      wrap.append(el);
      run = [];
    };
  ...
  const denseHead = (m, meta, role) => {
    ...
    const lead = h('span', { class: 'inline-flex items-baseline gap-1 mr-1.5 align-baseline' },
      h('i', { class: `ph ${meta.icon} text-[11px] opacity-50 not-prose` }));
    if (showWord) lead.append(h('span',
      { class: 'font-mono text-[9.5px] tracking-widest uppercase opacity-50' }, m.label || meta.label));
    if (m.ts) lead.append(h('span', { class: 'font-mono text-[9.5px] opacity-40' }, String(m.ts)));
    return lead;
  };
  ...
  function message(m, o = {}) {
    ...
    // A dense assistant turn hangs off the ask above it: indented, and its rail
    // a hairline rather than the 2px every turn carries at full size. The user
    // turn keeps the full rail flush left, which is what the indent is measured
    // against.
    const frame = !dense ? `border-l-2 ${meta.edge} pl-3.5 py-0.5`
      : role === 'assistant' ? 'ml-5 border-l border-base-content/30 pl-3'
      : `border-l-2 ${meta.edge} pl-3`;
