...
(() => {
  ...
  function message(m, o = {}) {
    ...
    // THE ASK IS BANDED, the reply is not, and the asymmetry is the point. A
    // band on every turn is stripes; a band on the half that OPENS each
    // exchange turns the card's structure into something the eye finds without
    // reading, which is what the removed rail was failing to do at a tenth of
    // the ink. It bleeds past the text with -mx-2/px-2, so the text's own left
    // edge is unmoved and the indent below it still measures against it.
    const frame = !dense ? `border-l-2 ${meta.edge} pl-3.5 py-0.5`
      : role === 'user' ? `${meta.band} rounded -mx-2 px-2 py-1`
      : 'ml-5';
