...
(() => {
  ...
  const ROLES = {
    user:      { label: 'You',       icon: 'ph-user',       edge: 'border-primary/50',
                 tint: 'var(--color-primary, currentColor)' },
  ...
  function message(m, o = {}) {
    ...
    // A DENSE TURN HAS NO FRAME AT ALL. It lost the rail, then the fill, and now
    // the reply's indent: every turn sits on the same left edge and the card is
    // one column of text with a glyph hung off it. Each of the three was doing
    // the same job, saying which voice this is by putting the turn somewhere,
    // and four carriers were already saying it without moving anything: mono
    // against prose, a tinted glyph apiece, the size step between them, and a
    // gap before each ask twice the gap inside an exchange.
    //
    // What the removed indent cost is real and worth naming: two replies in a
    // row, or two asks, no longer step. The gap rhythm is what separates them,
    // and it is the carrier to strengthen if that ever reads as one turn.
