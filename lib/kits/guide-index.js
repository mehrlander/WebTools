...
// BranchStatus.sessionsIn already parses, and the activity cache's open-PR rows
...
(() => {
  ...
  // branch resolves its own branch's thumb: pages/pages.csv is generated from the
  ...
  /**
   * main   [{ repo, path, ref?, sha? }]  guides on each repo's default branch
   * onPrs  [{ repo, path, pr, sha? }]    guides found on an open PR's head
   *
   ...
   *
   * A PR entry counts as IN FLIGHT only when its blob sha differs from the
   * default branch's. Presence alone is not authorship and never was: this
   * builds from a directory listing of the PR head, which inherits every guide
   * already on main, so without the comparison a guide lands on every open PR
   * cut after it merged, each one crediting sessions that never opened the
   * file. Measured 2026-08-09: one guide existed estate-wide and all five open
   * web-tools PRs predated it, so the fault was invisible and would have
   * arrived with the next branch. The sha is free, since the contents listing
   * that finds the file already carries it; an entry with no sha at all is
   * taken at face value, so a caller that cannot supply one is not silently
   * emptied.
   */
  function build({ main = [], onPrs = [] } = {}) {
    const rows = new Map();
    // The default branch's blob per guide, for the comparison below.
    const onDefault = new Map();
    for (const e of main) {
      if (isGuidePath(e?.path) && e?.sha) onDefault.set(key(e.repo, e.path), str(e.sha));
    }
    ...
    for (const e of onPrs) {
      if (!isGuidePath(e?.path)) continue;
      // Unchanged against the default branch: the branch merely contains the
      // file, which says nothing about this PR and would put a landed guide
      // back "in flight" on every open branch in the estate.
      const def = onDefault.get(key(e.repo, e.path));
      if (def && e?.sha && str(e.sha) === def) continue;
      const r = touch(e.repo, e.path);
