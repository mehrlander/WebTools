...
(() => {
  ...
  // wording: `entry.icon`, `entry.cls(tone)`, `entry.title(count, noun)`.
  ...
  // control aimed squarely at the reader who is holding the thing.
  const entryCls = (tone = 'primary') =>
    'btn btn-sm btn-square max-sm:h-11 max-sm:w-11 '
    + (tone === 'ghost' ? 'btn-ghost' : 'btn-soft btn-primary');
  ...
  const entry = (o = {}) => {
    const b = h('button', {
      type: 'button', class: entryCls(o.tone),
      title: entryTitle(o.count, o.noun),
      'aria-label': entryTitle(o.count, o.noun),
    }, h('i', { class: `ph ${o.icon || ENTRY_ICON} text-lg max-sm:text-xl` }));
