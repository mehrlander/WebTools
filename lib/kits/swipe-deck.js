...
(() => {
  ...
  function open(o = {}) {
    ...
    // ONE `bg-[…]` CANNOT CARRY A GRADIENT AND A COLOUR. Tailwind resolves an
    // arbitrary background to `background-color` OR `background-image` by
    // looking at the value; a comma-joined "gradient, colour" is neither, so
    // the whole utility compiled to nothing and this overlay was fully
    // transparent from the day it was written. Nothing reported it: the class
    // is present in the markup and real CSS never arrives, the same silent
    // drop as `divide-base-200` (daisy-alpine conventions, 7). Measured
    // 2026-08-18: computed backgroundColor rgba(0,0,0,0), backgroundImage none.
    // Two utilities, two properties, and they no longer have to be one value.
    const overlay = h('div', { class: 'fixed inset-0 z-[70] overflow-hidden bg-base-200 bg-[radial-gradient(circle_at_top,color-mix(in_srgb,var(--color-primary)_7%,transparent),transparent_30rem)]' });
