...
//                          scrolling DOWN inside a slide, and bring both back
//                          on the first upward scroll, on a tap anywhere in
//                          the panel, and on every slide change. Opt-in: a
...
(() => {
  ...
  function open(o = {}) {
    ...
    if (o.immersive) {
      ...
      // A tap brings it back, and only back: while the chrome is showing this
      // does nothing, so it can never take a control away mid-reach. It does
      // not preventDefault either, so a tap that was meant for a link in the
      // slide still lands.
      //
      ...
      panel.addEventListener('click', (e) => {
        if (hidden && !header.contains(e.target) && !footer.contains(e.target)) setHidden(false);
      }, true);
  ...
  // `max-sm:h-11 max-sm:w-11` is the phone hit target and is not decoration:
  // btn-sm is 32px, under the 44px both platforms ask for, and this is a
  // control aimed squarely at the reader who is holding the thing. A size 'md'
  // button is already past 44 and needs no bump.
  const entryCls = (tone = 'primary', size = 'sm') =>
    'btn btn-square ' + (size === 'md' ? '' : 'btn-sm max-sm:h-11 max-sm:w-11 ')
    + (tone === 'ghost' ? 'btn-ghost hover:text-primary' : 'btn-soft btn-primary');
