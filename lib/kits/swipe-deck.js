...
(() => {
  ...
  function open(o = {}) {
    ...
    // FOUR ROWS, and the second is usually empty. `auto` on an empty div is
    // zero, so a deck with no subheader lays out exactly as it did with three
    // rows; nothing needs to branch on whether the slot is in use.
    ...
    // many there are.
    const subWrap = h('div', { class: 'sd-subheader empty:hidden flex shrink-0 flex-wrap items-center gap-1 border-b border-base-300 bg-base-100 px-4 py-1.5 sm:px-6' });
