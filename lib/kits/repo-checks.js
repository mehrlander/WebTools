...
//     { "kind": "tracker",    "path": "projects/budget-drs/tracker/board.csv",
...
(() => {
  ...
  async function probeOne(c, reader) {
    ...
    if (c.kind === 'tracker') {
      const text = await reader.text(c.path);
      if (text == null) { out.error = `${c.path} not found`; return out; }
      // Delegated to kits/csv.js, which every caller of this file loads first
      // (it rides in the pre-build's boot list).
      let tasks = null;
      try { tasks = window.Csv.rows(text); }
      catch { out.error = `${c.path} is not readable as CSV`; return out; }
      // A well-formed file of the wrong shape is not a projection, and reading
      // it as an empty tracker would report a healthy board for one that has
      // tasks. `status` is the column every projection carries and the one this
      // check reads, so its absence is the tell.
      if (!tasks.length || !('status' in tasks[0])) {
        out.error = `${c.path} carries no task rows`; return out;
      }
