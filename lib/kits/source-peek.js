...
(() => {
  ...
  // ── The read ────────────────────────────────────────────────────────────
  // `kind` overrides the rendition the extension would pick. A seeder that made
  // the bytes knows something the name cannot say: the stage converts markup to
  // markdown and opens the result RAW, because a conversion is a payload to copy
  // rather than a document to read, and a card rendering what the reader is
  // about to see as source would preview a different thing. Omitted, the
  // extension decides as it always has.
  ...
  // the column fills it. HTML-STYLE's "don't narrow text to a reading column"
