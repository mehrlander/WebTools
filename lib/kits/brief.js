...
(() => {
  ...
  const MACHINERY = new Set(['kits/brief.js', 'kits/io.js', 'kits/export.js', 'kits/build.js']);
  ...
  const plan = (opts = {}) => {
    ...
    const entries = (window.__loadedScripts || [])
    ...
    const reads = (window.__reads || []).map(r => r.path);
    return {
      path,
      repo: (window.gh && window.gh.repo) || 'mehrlander/web-tools',
      ref: (window.gh && window.gh.ref) || window.__bundleRef || 'main',
      own, floor, reads,
      vendor: vendors(),
