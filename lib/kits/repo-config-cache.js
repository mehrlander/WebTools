...
(() => {
  ...
  // membership changes either way, plus any repo whose config hash or alignment
  // grade moved. A changed grade is a changed cache: a repo that wired the
  // conventions in without touching its manifest is precisely the transition
  // worth writing. Sorted, so a caller can name them.
  //
  // The activity cache has carried this shape since it needed to answer "did
  // that long refresh find anything"; this one gained it when the crawl started
  // recording what each run cost, so the figure in the run record and the gate
  // that decided to write it are one derivation rather than two that can part.
  function changedRepos(prev, next) {
    const pr = prev?.repos || {}, nr = next?.repos || {};
    const keys = [...new Set([...Object.keys(pr), ...Object.keys(nr)])].sort();
    return keys.filter(k => !(k in pr) || !(k in nr)
      || pr[k]?.hash !== nr[k]?.hash || pr[k]?.alignHash !== nr[k]?.alignHash);
  }
  ...
  window.RepoConfigCache = { CACHE_PATH, HISTORY_CAP, hashConfig, mergeRepo, buildCache, changedRepos, cacheChanged };
})();
