...
(() => {
  ...
  function open(o = {}) {
    ...
    const chromeFor = (i) => {
      ...
      if (!d) return [];
      const out = [];
      // IDENTITY FIRST, THEN ARRANGEMENT, which is the order the card's own row
      // reads (alpineComponents/file-review.js: the github menu is order-2 on a
      // reading surface, beside the name, and order-11 in a list, at the far
      // end). This header IS the name, so its github mark belongs at the end
      // nearest the title rather than trailing the layouts. It shipped
      // trailing them on 2026-09-07 and contradicted that rule for a day.
      if (d.ghLinks.length) out.push({
        icon: 'ph-github-logo', title: 'This file on GitHub',
        onClick: (deck, btn) => deck.menu(btn, d.ghLinks.map(l => ({
