...
(() => {
  ...
  const held = { saved: false, subject: null, frame: null };
  const announce = (subject) => {
    if (!held.saved) {
      held.saved = true;
      held.subject = window.__tossSubject || null;
      held.frame = window.__tossFrame || null;
    }
    window.__tossSubject = subject;
    window.__tossFrame = null;
    window.dispatchEvent(new CustomEvent('toss-subject'));
  };
  const release = () => {
    if (!held.saved) return;
    window.__tossSubject = held.subject;
    window.__tossFrame = held.frame;
    held.saved = false; held.subject = null; held.frame = null;
    window.dispatchEvent(new CustomEvent('toss-subject'));
  };
