...
(() => {
  ...
  // choice travels the other way on its own channel: `window.__compareRef`
  // plus a `web-tools:compare-ref` event, read by the cards themselves.
  //
  // In one window that is the whole mechanism and this code is not needed. It
  // exists for the framed case, which is the same asymmetry `hosts()` handles
  // above: inside a toss the deck runs in the FRAME and the listening fab is
  // the SHELL's, so the fab dispatches on a window the cards are not in. The
  // deck bridges it, since the deck is the only thing that knows both.
  //
  // One direction only, shell to frame, so there is no loop to guard against
  // beyond the flag: the fab never listens for this event.
  let bridged = [];
  const bridge = () => {
  ...
  const held = [];   // one saved set per window we wrote to
  // Take a copy of everything this deck is about to overwrite, once per
  // window and BEFORE the first write of any of it. The navigate handle is
  // installed before the first announcement, so remembering lazily inside
  // announce() would save the deck's own handle as the thing to restore.
  const remember = (w) => {
    if (!held.some(h => h.w === w)) {
      held.push({ w, subject: w.__tossSubject || null, frame: w.__tossFrame || null,
                  nav: w.__deckNavigate || null });
    }
  };
  ...
  function open(o = {}) {
    ...
    const say = (i) => {
      ...
      announce({ repo: o.repo || '', ref, path: f.path, route: 'deck',
                 base: o.base || '', baseName: o.baseName || o.base || '' });
    };
    if (o.announce !== false) bridge();
    ...
    for (const w of (o.announce === false ? [] : hosts())) { remember(w); w.__deckNavigate = navigate; }
