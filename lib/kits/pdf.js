...
(() => {
  ...
  const firstLook = async (src) => {
    ...
    return {
      pages: doc.numPages,
      // Page `n`'s shape, in PDF units, WITHOUT rendering it. A continuous
      // reader needs this and only this: to reserve the right height for a
      // page it has not drawn, so the scrollbar is honest from the first
      // frame and does not lengthen under a moving thumb.
      //
      // `getViewport` rather than the raw MediaBox, because a page carrying
      // /Rotate 90 is a landscape page inside a portrait box and the box alone
      // reports the wrong axis. `getPage` parses the page dictionary and not
      // its content stream, which is what makes asking for every page in a
      // document affordable where `open()` would not be.
      async size(n) {
        const vp = (await doc.getPage(n)).getViewport({ scale: 1 });
        return { width: vp.width, height: vp.height, ratio: vp.height / vp.width };
      },
      // Draw page `n` into `canvas`, fitted to `width` CSS pixels. Returns the
      // CSS width used, so a caller can size a container around it.
  ...
  const flow = async (look, host, opts = {}) => {
    ...
    const buildScroll = (start) => {
      ...
      const applyZoom = (nz, ax, ay) => {
        ...
        box.scrollTop = Math.max(0, cy * z - py);
        at = activeAt();
        announce(at);
        showZoom();
        ensure();
  ...
  window.pdf = { open, pick, firstLook, flow, geom, stream, lattice, view, doc: docOps, config, PdfDocument, CDN,
