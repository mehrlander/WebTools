...
(() => {
  ...
  const lookAt = (doc) => {
    ...
    return {
      ...
      // space (lower-left origin, y up) plus glyphs split evenly across the run.
      // The even split is approximate for a proportional font, which only ever
      // narrows a sub-run highlight inside a line the merge widens anyway.
      async items(n) {
        ...
        const items = content.items.filter(it => it.str !== undefined).map((it) => {
          ...
          const glyphs = [...it.str].map((char, i) => ({ char, x1: x1 + per * i, x2: x1 + per * (i + 1) }));
  ...
  const flow = async (look, host, opts = {}) => {
    ...
    const buildScroll = (start) => {
        ...
        for (const hit of onPage) {
          ...
          for (const r of hit.rects) {
            ...
            mark.style.cssText = HL + (active
              ? 'background:rgba(251,146,60,.62);box-shadow:0 0 0 1px rgba(234,88,12,.85);'
      ...
      return {
        ...
        // Make hit `k` current and scroll it to the middle of the pane,
        // wrapping past either end. Centring rather than aligning the page top,
        // because a hit low on a page is off-screen when its top is at the top.
        findGo: (k) => {
          ...
          (async () => {
            ...
            if (dead() || !box.isConnected) return;
            const target = Math.max(0, offsets[pi] + topWithin - box.clientHeight / 2);
