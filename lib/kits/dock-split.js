...
(() => {
  const CSS = `
.dk-split{position:relative;touch-action:none;--dk-line:var(--color-base-300)}
...
.dk-split::before{content:"";position:absolute;top:50%;left:50%;translate:-50% -50%;
  border-radius:999px;background:var(--color-base-100);border:1px solid var(--dk-line)}
...
.dk-split.is-col.is-dragging::before{height:60px}
.dk-split.is-row.is-dragging::before{width:60px}
.dk-readout{position:absolute;pointer-events:none;white-space:nowrap;
  padding:.125rem .375rem;border-radius:.25rem;font-size:.75rem;font-variant-numeric:tabular-nums;
  background:var(--color-primary);color:var(--color-primary-content);opacity:0;transition:opacity .1s}
...
`;
  ...
  function attach(handle, o = {}) {
    ...
    const vert = axis === 'col';
    const min = Number.isFinite(o.min) ? o.min : 20;
    const max = Number.isFinite(o.max) ? o.max : 80;
    const step = Number.isFinite(o.step) ? o.step : 2;
    const from = o.from === 'start' ? 'start' : 'end';
    const read = () => clamp(Number(o.value ? o.value() : 50) || 0, min, max);
    ...
    function onPointerDown(ev) {
      ...
      const move = (e) => {
        ...
        last = clamp(Math.round(away / span * 100), min, max);
        apply(last);
      };
      const up = () => {
    ...
    function onKeyDown(ev) {
      ...
      else if (k === 'Home') next = from === 'end' ? max : min;
      else if (k === 'End') next = from === 'end' ? min : max;
      if (next == null) return;
      ev.preventDefault();
      const pct = clamp(Math.round(next), min, max);
      apply(pct);
      o.onCommit?.(pct);
    }
