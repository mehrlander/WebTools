...
(() => {
  ...
  // The form's own note about a cell: Excel's input message, and the choices a
  // list validation allows. Attached as `cell.note`, which a renderer shows
  // however it likes; nothing here decides that.
  function markValidations(rows, sheet, xl) {
    ...
    for (const row of rows) {
      for (const cell of row.cells) {
        ...
        if (!hit.prompt && !hit.promptTitle && !options?.length) continue;
        cell.note = {
          title: hit.promptTitle || '', prompt: hit.prompt || '',
