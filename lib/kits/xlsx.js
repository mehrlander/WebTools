...
(() => {
  ...
  function analyze(parts) {
    ...
    function recordConnections(node, fn, path) {
      ...
      if (sheetName) {
        ...
        if (node.nodeName === 'row') {
          ...
          for (const c of directChildren(node, 'c')) {
            ...
            cols.set(idx, { shared: t === 's', raw, style: c.getAttribute('s') || null });
          }
      ...
      if (fn.includes('styles.xml')) {
        ...
        if (path.endsWith('cellXfs.xf')) xl.cellXfs.push(node.getAttribute('numFmtId') || '0');
      }
  ...
  // A cell's value printed the way its format says to read it. Deliberately
  // NOT a format-code interpreter: it decides the KIND of thing the value is
  // and prints that plainly, in ISO for anything temporal. Reproducing Excel's
  // exact glyphs would mean implementing the whole format grammar, and a reader
  // comparing a browser table to a workbook is better served by an unambiguous
  // date than by an approximation of the one Excel drew.
  function formatValue(raw, format, date1904) {
    const kind = format?.kind;
    if (raw === '' || raw == null) return '';
    if (!kind || kind === 'general' || kind === 'text' || kind === 'number' || kind === 'currency') {
      return String(raw);
    }
    const n = Number(raw);
    ...
    if (kind === 'percent') {
      const scaled = n * 100;
      // Keep the precision the value actually has rather than inventing decimals.
