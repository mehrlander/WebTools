  ...
  const mountUI = () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      b.addEventListener('pointerdown', (e) => {
        e.preventDefault();
        const spec = (S.compShift ? SHIFTED : PRIMARY)[i];
        if (S.dict) S.dict.punct(spec.m);
    ...
    S.compShiftBtn = el('button', key('#f4f4f5') + 'font-size:12px;letter-spacing:.04em;height:30px;');
    S.compShiftBtn.title = 'The other three marks';
    S.compShiftBtn.addEventListener('pointerdown', (e) => {
      e.preventDefault(); S.compShift = !S.compShift; paintPunct();
    });
    S.compPunct.appendChild(S.compShiftBtn);

    // One painter for the pad, so the shift state has exactly one reading.
    paintPunct = () => {
      const set = S.compShift ? SHIFTED : PRIMARY;
      keys.forEach((b, i) => {
        const spec = set[i];
        b.textContent = '';
        if (spec.icon) { setIcon(b, spec.icon); b.style.fontSize = ''; }
        else { b.textContent = spec.m; b.style.fontSize = '19px'; }
        b.title = spec.title || ('Insert ' + spec.m);
      });
      S.compShiftBtn.textContent = S.compShift ? 'abc' : '·!¶';
      S.compShiftBtn.style.background = S.compShift ? '#fef9c3' : '#f4f4f5';
      S.compShiftBtn.style.borderColor = S.compShift ? '#eab308' : '#e4e4e7';
    };
    paintPunct();
