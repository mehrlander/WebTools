...
(() => {
  ...
  const openBubble = (vx, vy, onSave) => {
    closeBubble();
    const d = S.doc;
    const b = d.createElement('div');
    S.bubble = b;
    b.setAttribute(UI_ATTR, '');
    const x = Math.min(Math.max(8, vx), (win().innerWidth || 800) - 248);
    const y = Math.min(Math.max(8, vy), (win().innerHeight || 600) - 80);
    b.style.cssText = `position:fixed;left:${x}px;top:${y}px;z-index:${Z + 2};width:240px;`
      + 'background:#18181b;border:1px solid #3f3f46;border-radius:10px;padding:6px;'
      + 'box-shadow:0 8px 24px rgba(0,0,0,.5);display:flex;gap:6px;align-items:flex-end;';
    const ta = d.createElement('textarea');
    ta.setAttribute(UI_ATTR, '');
    ta.placeholder = 'Note…';
    ta.rows = 2;
    ...
    ta.style.cssText = 'flex:1;min-width:0;background:#27272a;color:#fafafa;border:1px solid #3f3f46;'
      + 'border-radius:6px;padding:5px 7px;font:16px/1.35 ui-sans-serif,system-ui;resize:none;';
    const ok = d.createElement('button');
    ok.setAttribute(UI_ATTR, '');
    ok.title = 'Save note (Enter)';
    ok.textContent = '✓';
    ok.style.cssText = BTN + 'background:#facc15;color:#18181b;padding:4px 10px;font-size:14px;';
  ...
  const startPick = () => {
    ...
    const click = (e) => {
      ...
      endMode();
      openBubble(r.left, Math.min(r.bottom + 6, (win().innerHeight || 600) - 130), (note) => {
  ...
  const startRegion = () => {
    ...
    const up = (e) => {
      ...
      openBubble(vx, Math.min(vy + vh + 6, (w.innerHeight || 600) - 130), (note) => {
