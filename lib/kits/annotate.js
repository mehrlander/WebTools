...
(() => {
  ...
  const mountUI = () => {
    ...
    // sitting.
    //
    // The cell costs nothing to borrow in exactly the states that offer it: a
    // full stop is either aimed away from, or already sitting where a second
    // one would go and doing nothing (measured, kits/dictate.js: punct('.') on
    // such a buffer normalizes the spacing and adds no mark).
    //
    // WHAT THIS CARD DOES NOT HAVE IS THE STEP-BACK KEY. pages/dictate.html
    // pairs the stitch with one that walks the caret to an older break, so a
    // reader there can keep a trailing full stop and repair the one before it.
    // Its keys lie ACROSS, where a seventh cell costs width off six. Here they
    // stack in a 56px column beside a text box capped at 172px, so a fifth
    // cell is 34px tall against the 43 these have, and this column's own
    // argument (above) is that three keys is what makes each big enough to hit
    // without looking. So the card repairs the break at the caret, which
    // during dictation is the full stop just written, and reaching an older
    // one means tapping the text first.
    //
