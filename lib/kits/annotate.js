...
(() => {
  ...
  const startPick = () => {
    ...
    setStatus('Element: tap to select, then “+ note”. Tap ⌖ Element again to exit; drag the header to move this card.');
    const d = S.doc;
    const mkBox = (css) => {
  ...
  const startRegion = () => {
    ...
    S.mode = 'region';
    syncChips();
    setStatus('Region: drag a rectangle, then “+ note”. Tap ▭ Region again to exit; drag the header to move this card.');
    const d = S.doc;
