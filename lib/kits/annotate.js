...
(() => {
  ...
  const mountUI = () => {
    ...
    // Six lines of 26px plus the 7px padding, so the bottom line is a line.
    S.compView = el('div', 'flex:1;min-width:0;min-height:66px;max-height:170px;overflow-y:auto;background:#fafafa;'
  ...
  function bindComposeGestures(surface, host) {
    ...
    on(surface, 'pointerup', (e) => {
      ...
      if (!window.Dictate.hitsText(host, e.clientX, e.clientY)) { taps = 0; toEnd(); return; }
      ...
      const t = Date.now();
      taps = (t - lastTap) < DBL_MS ? taps + 1 : 1;
      lastTap = t;
