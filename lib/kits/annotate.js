...
(() => {
  ...
  const domText = () => {
    ...
    if (!sub) return 'Aim at something, or select a note, and this reads the DOM under it.';
  ...
  const renderDom = () => {
    ...
    const say = (t) => { S.domBody.innerHTML =
      `<div style="${DS.lab}text-transform:none;letter-spacing:0;font-size:11.5px;`
      + `font-style:italic;padding:6px 2px">${esc(t)}</div>`; };
    ...
    if (!sub) return say('Aim at something, or select a note, and this reads the DOM under it.');
    ...
    if (sub.target && sub.target.type === 'page') {
      ...
      S.domBody.innerHTML =
        ...
        + domGrid(rows)
        + `<div style="${DS.lab}text-transform:none;letter-spacing:0;font-size:11.5px;`
        + `font-style:italic;padding-top:2px">No anchor: this note is about the page as a whole.</div>`;
      return;
    }
    ...
    if (sub.kind === 'rect') {
      ...
      if (!found.length) { S.domBody.innerHTML = head
        + `<div style="${DS.lab}text-transform:none;letter-spacing:0;font-size:11.5px;font-style:italic">`
        + `covers nothing on the page.</div>`; return; }
