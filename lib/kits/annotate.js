...
(() => {
  ...
  const mountUI = () => {
    ...
    S.compBack = icon('backspace', BTN + 'padding:8px 13px;border-radius:8px;',
    ...
    // Footer: one door, and it leads out of here.
    //
    // This card is for CAPTURE. Everything you do with a finished set (read it,
    // edit it, copy it in either format, save the jot, clear it) lives in the
    // drawer's Notes tab, which has the room and the house styling. So the
    // footer is Review alone. Copy markdown was the last holdout, kept for the
    // one-tap case, but a copy button here and a copy button there is two
    // places to look for the same thing and one of them is 360px wide.
    //
    // Review is a REQUEST, not a surface: announcing the intent and letting the
    // drawer answer keeps one implementation of that view. A second one living
    // here would be the copy that ages.
    ...
    const primary = el('button', 'border:1px solid #eab308;background:#facc15;color:#18181b;border-radius:8px;'
      + 'padding:5px 12px;font:600 12px ui-sans-serif,system-ui;cursor:pointer;', 'Review');
    primary.title = 'Open the drawer’s Notes tab: read, edit, copy, or save the set';
    primary.addEventListener('click', review);
  ...
  const beginDraft = (target, caption, rectDoc) => {
    cancelDraft();
    S.draft = { target };
    if (rectDoc) {
  ...
  const disable = () => {
    ...
    S.compView = S.compBody = S.compGhost = S.compHint = S.compPunct = S.compEdit = S.compBack = null;
    S.dict = null;
    S.editing = false;
