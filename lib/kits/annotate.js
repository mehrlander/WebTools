...
(() => {
  ...
  const sizeExpanded = () => {
    ...
    const h = Math.max(240, Math.min(760, vh - bottom - 12));
  ...
  const mountUI = () => {
    ...
    S.serial = el('div', 'display:none;flex-direction:column;gap:5px;padding:0 8px 8px;'
    ...
    // over: once by the tab you are standing in and again on the button. Inside
    // the tab, the tab is the qualifier, so the button is one word and the
    // reader never has to check which of two they are about to press.
    S.serialCopy = icon('copy', 'margin-left:auto;box-sizing:border-box;min-height:24px;'
      + 'padding:0 9px 0 7px;gap:5px;border:1px solid #d4d4d8;border-radius:7px;'
      + 'background:#f4f4f5;color:#27272a;cursor:pointer;display:flex;align-items:center;'
