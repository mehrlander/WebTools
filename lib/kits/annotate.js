...
(() => {
  ...
  const onKey = (e) => {
    ...
    if (e.key && e.key.length === 1) d.type(e.key);
    else if (e.key === 'Enter') d.type('\n');
    else if (e.key === 'Backspace') d.erase();
    else if (e.key === 'ArrowLeft') d.caretAt(d.hasSelection ? at() : at() - 1);
    else if (e.key === 'ArrowRight') d.caretAt(d.hasSelection ? end() : end() + 1);
    else if (e.key === 'Home') d.caretAt(0);
    else if (e.key === 'End') d.caretAt(d.text.length);
