...
(() => {
  ...
  const mountUI = () => {
    ...
    S.modeChips = {
      ...
      // CONDITIONAL, and it is the only chip that is. The other three work on
      // any page; this one needs a kits/md-doc.js render to aim at, and a chip
      // that lights up over prose it cannot resolve is the dead control the
      // section menu already refuses to be. Shown when the target document
      // holds a declared render, re-checked whenever one is declared (md-doc
      // announces) and on every mode change.
      section: chip('Section', 'Tap a section of the document, then + note (tap again to exit)',
        () => S.mode === 'section' ? endMode() : startPick({ aim: 'section' })),
    };
    capture.appendChild(S.modeChips.pick);
    capture.appendChild(S.modeChips.section);
    ...
    // for. The title still says which format and which scope, for a pointer
    // that hovers; a finger has the chips a thumb's width to its left.
    S.serialCopy = icon('copy', 'display:none;box-sizing:border-box;min-height:26px;'
      + 'padding:0 8px;border:1px solid #d4d4d8;border-radius:7px;'
  ...
  const syncChips = () => {
    for (const [k, b] of Object.entries(S.modeChips || {})) {
      ...
      b.style.background = on ? '#facc15' : '#fff';
      b.style.color = on ? '#18181b' : '#3f3f46';
      // The Section chip is present only where it has something to aim at.
      // Kept lit while its own mode runs, so a render that disappears mid-pick
      // cannot pull the exit out from under the reader.
      if (k === 'section') b.style.display = (on || hasRender()) ? '' : 'none';
    }
  };
