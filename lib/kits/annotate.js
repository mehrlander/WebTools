...
(() => {
  ...
  const hasSections = () => {
    try {
      for (const box of S.doc.querySelectorAll('[data-md-doc]')) {
        if (box.__mdDoc && (box.__mdDoc.sections || []).length) return true;
      }
    } catch { }
    return false;
  };
  ...
  const syncChips = () => {
    for (const [k, b] of Object.entries(S.modeChips || {})) {
      ...
      if (k === 'section') b.style.display = (on || hasSections()) ? 'flex' : 'none';
    }
    const b = S.aimBtn;
    if (!b) return;
    const on = !!S.mode;
  ...
  const placeOffer = (btn, { left, tops }) => {
    ...
    btn.style.left = Math.max(6, Math.min(left, vpW - 70)) + 'px';
    btn.style.top = Math.min(top, vpH - 36) + 'px';
  };
  ...
  const startPick = (o = {}) => {
    ...
    const commit = () => {
      ...
      // Above the element, else below it, else clear of the card.
