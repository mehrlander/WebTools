...
(() => {
  ...
  const syncCopyKey = () => {
    ...
    if (!b) return;
    const shown = S.expanded && !S.inPlace;
    const serial = shown && !!S.items.length && S.reading !== 'notes';
    // TWO LEVELS, and the second one is a reservation that has now been right
    // twice for different reasons. While the readings are on screen the key
    // holds its place and only its ink goes, because the group beside it is
    // CENTRED in the header's leftover width: a key that left the row would
    // widen that span and slide all three keys on every switch into a
    // serialization. It went to `display:none` two commits ago, correctly, when
    // the keys were content-sized and left-aligned and nothing could move; the
    // centring brought the reason back. Off the readings entirely, it leaves
    // the row outright rather than reserving 30px of a header it has no
    // business in. `visibility` also takes it out of the tab order.
    b.style.display = shown ? 'flex' : 'none';
