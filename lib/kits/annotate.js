...
(() => {
  ...
  const domSubject = () => {
    if (S.aimEl && S.aimEl.isConnected) return { kind: 'el', el: S.aimEl };
    ...
    if (S.holdEl && S.holdEl.isConnected) return { kind: 'el', el: S.holdEl };
    if (S.holdRect) return { kind: 'rect', rect: S.holdRect };
    const it = S.items.find(i => i.id === S.selId);
    if (it) return { kind: 'target', target: it.target };
    if (S.draft && S.draft.target) return { kind: 'target', target: S.draft.target };
    return S.items.length ? { kind: 'target', target: S.items[S.items.length - 1].target } : null;
  };
  ...
  // The ancestor trail, outermost first, the subject filled. Tappable: this is
  // the one thing in the pane a reader can act on, and it is the same movement
  // the repeat tap makes, offered to a finger that would rather aim once.
  const domTrail = (el2) => {
    const chain = window.Peek.chainOf(el2);       // innermost first
    if (chain.length < 2) return '';
    const rows = chain.slice().reverse().map((n, ri) => {
      const i = chain.length - 1 - ri;
      const on = i === 0;
      return `<button data-peek-crumb="${i}" style="${DS.chip}white-space:nowrap;border:0;cursor:pointer;`
        + (on ? 'background:#facc15;color:#18181b;font-weight:600;' : '')
        + `">${esc(window.Peek.shortAtom(n))}</button>`;
    });
  ...
  const aimAncestor = (i) => {
    const sub = domSubject();
    const cur = sub && (sub.kind === 'el' ? sub.el : sub.kind === 'target' ? elementOf(sub.target) : null);
    if (!cur || !window.Peek) return;
    const chain = window.Peek.chainOf(cur);
    const next = chain[i];
    if (!next) return;
    if (S.restage && S.restage(next)) return;   // the pick owns the outline
    S.holdEl = next; S.holdRect = null; S.aimEl = null; S.aimRect = null;
    renderDom();
  };
  ...
  const startPick = (o = {}) => {
    ...
    //
    // Section is deliberately not included. Its notes are about markdown SOURCE,
    // and its aim resolves to the heading that owns a run, so the DOM reading
    // would answer with an <h2> where the reader is asking about a passage.
    if (!section && window.Peek) {
    ...
    const stage = (x, y) => {
      ...
      lastPt = { x, y };
      if (!section && near && chain.length && stepUp()) {
        sel = cur = chain[at];
      } else {
        aim(x, y);
        if (!cur) return;
        // The chain stops at <body>: a note pinned to <html> names the document
        // rather than anything in it, which is what the page aim is for.
        chain = [];
        for (let n = cur; n && n.tagName && n !== d.documentElement; n = n.parentElement) chain.push(n);
        at = 0;
      }
      commit();
    };
    ...
    const commit = () => {
      ...
      S.aimEl = sel; S.aimRect = null; dropHold();
      refreshDom();
