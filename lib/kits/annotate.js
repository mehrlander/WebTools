...
(() => {
  ...
  const syncBody = () => {
    ...
    const serial = set && !bare && S.reading !== 'notes';   // `bare` already exempts dom
  ...
  const refreshDom = () => {
    if (S.ui && S.expanded && S.reading === 'dom') renderSerial();
  };
  ...
  const renderSerial = () => {
    ...
    const text = S.reading === 'dom' ? domText()
      : S.reading === 'json' ? JSON.stringify(toJSON(), null, 2)
      : toMarkdown();
  ...
  const select = (id, opts = {}) => {
    ...
    // never needed before.
    if (S.ui && S.reading === 'dom') renderSerial();
    emit();
    const it = S.items.find(i => i.id === S.selId);
  ...
  const endMode = () => {
    ...
    // rather than describing an outline that is no longer on screen.
    clearAim();
    closeAim();
    syncChips();
  };
