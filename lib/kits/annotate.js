...
(() => {
  ...
  const mountUI = () => {
    ...
    const head = el('div', 'display:flex;align-items:center;gap:6px;flex-wrap:wrap;padding:9px 12px 8px;'
    ...
    const chip = (label, title, fn) => {
      const b = el('button', 'border:1px solid #e4e4e7;background:#fff;color:#3f3f46;border-radius:999px;'
        + 'padding:3px 9px;font:600 11px ui-sans-serif,system-ui;cursor:pointer;white-space:nowrap;', label);
      b.title = title;
    ...
    const capture = el('div', 'display:flex;gap:6px;align-items:center;margin-left:auto;');
    ...
    // It takes the CLOSE BUTTON'S place, at the top right, and the × is gone.
    // Two controls in one corner, one of which dismissed the whole card, was a
    // slip waiting to happen; and the drawer's Notes tab already carries Off,
    // which is the same act said where the set is being read. So the corner
    // holds the way TO the drawer, and putting the annotator away is done from
    // the drawer.
    S.reviewBtn = icon('sidebar-simple', 'border:1px solid #e4e4e7;background:#fff;color:#3f3f46;'
      + 'border-radius:999px;padding:4px 9px;cursor:pointer;display:flex;align-items:center;margin-left:2px;');
    S.reviewBtn.addEventListener('click', review);
    // The drawer button joins the CHIP GROUP rather than the header directly,
    // so a card too narrow for one line wraps all four together and keeps them
    // right-aligned. Left in the header it wrapped alone, and landed under the
    // title on the far side of the card from where it had been (measured at a
    // 360px viewport, where the card is 324).
    ...
    S.compView = el('div', 'flex:1;min-width:0;box-sizing:border-box;'
      + 'min-height:68px;max-height:172px;overflow-y:auto;background:#fff;'
    ...
    const key = (bg) => 'border:0;background:' + (bg || '#fafafa') + ';color:#3f3f46;'
      ...
      // "Done" label did exactly that to the pad beside it.
    ...
    S.compBack = icon('backspace', key('#fef2f2') + 'color:#dc2626;',
    ...
    // Sized for a thumb rather than a toolbar: 20px glyphs in a 46px cell.
    const CBTN = key() + 'min-height:40px;';
    ...
    // Small: the label shares a cell with its glyph, and the cell is a quarter
    // of the row.
    S.compEditTxt = el('span', 'display:none;margin-left:5px;font:600 11px ui-sans-serif,system-ui;');
    ...
    // corner is where a thumb rests without covering anything. Delete moves
    // inboard to where the pad was, still beside save, still one deliberate
    // reach from the marks it used to sit among. Every cell in the row is
    // placed EXPLICITLY rather than left to flow, so a missing microphone (a
    // browser with no recognizer) cannot slide the whole row one column left.
    ...
    // shoved the rest of the row sideways. The columns are equal by decree.
    const frame = el('div', 'display:grid;grid-template-columns:repeat(4,minmax(0,1fr)) 46px;gap:1px;'
      + `background:${GRID_LINE};border:1px solid #d4d4d8;border-radius:10px;overflow:hidden;`);
    S.compFrame = frame;
    S.compView.style.gridColumn = '1 / 5';
    S.compTa.style.gridColumn = '1 / 6';
    ...
    place(S.compMic || el('div', key() + 'cursor:default;'), 1);   // an absent mic still holds its column
    place(S.compEdit, 2);
    place(S.compBack, 3);
    place(S.compSave, 4);
    place(S.compPad, 5);
  ...
  const paintDraft = () => {
    ...
    S.compView.style.gridColumn = marks ? '1 / 5' : '1 / 6';
    paintPunct();     // the pad's face follows the selection, so it repaints with the text
    // Save STAYS live while a pin is armed: a reader who is done adjusting can
    // ship without putting the pin down first.
    ...
    // In edit mode the keyboard brings its own delete and its own caret, so
    // both cells stand down. They DIM rather than vanish: a grid cell that
    // disappears takes its column with it, and the frame's whole point is that
    // the border never jogs. Same reading the microphone already gets.
    const idle = (b) => {
      if (!b) return;
      b.disabled = !!S.editing;
      b.style.opacity = S.editing ? '.3' : '';
      b.style.cursor = S.editing ? 'default' : 'pointer';
    };
    idle(S.compBack);
    idle(S.compPad);
  };
  ...
  window.Annotate = {
    ...
    startPick, startRegion, notePage, editNote, review,
