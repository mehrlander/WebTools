...
(() => {
  ...
  const mountUI = () => {
    ...
    S.compView = el('div', 'flex:1;min-width:0;min-height:44px;max-height:22vh;overflow-y:auto;background:#fafafa;'
      // Roomier top and bottom than the text needs: the selection handles sit
      // above and below the line they mark, and on the first or last line they
      // would otherwise clip against the box (this box scrolls, so clipping is
      // what it does). The padding is the reservation.
      + 'border:1px solid #e4e4e7;border-radius:8px;padding:15px 9px;'
    ...
    if (window.Dictate && window.Dictate.SUPPRESS) {
      S.compView.setAttribute('style', (S.compView.getAttribute('style') || '') + window.Dictate.SUPPRESS);
    }
    S.compView.addEventListener('contextmenu', (e) => e.preventDefault());
    bindComposeGestures(S.compBody);
    ...
    // In edit mode the button grows the word DONE. The glyph flip alone was
    // not enough: a microphone reads as "start dictating", not as "finish
    // typing and go back", so the way out of the keyboard looked like a way
    // deeper in and readers reported finding no confirm at all. The icon still
    // says where the tap lands; the word says what the tap is.
    ...
    // With a pin armed, the row's business is the pin. Save steps aside for
    // two arrows that walk the armed edge one character at a time, because
    // aiming at a character boundary is the one thing a finger cannot do: the
    // tap gets it close and these settle it. Save is not merely disabled but
    // GONE, since the two are answers to different questions and a save
    // sitting there mid-adjustment invites the tap that ends the note.
    S.compNudge = el('div', 'display:none;gap:6px;');
    for (const [dir, name] of [[-1, 'caret-left'], [1, 'caret-right']]) {
      const b = grow(icon(name, CBTN + 'background:#fef2f2;border-color:#fecaca;color:#dc2626;',
        dir < 0 ? 'Move the armed pin left' : 'Move the armed pin right'));
      b.addEventListener('pointerdown', (e) => {
        e.preventDefault();
        if (S.dict && S.compArmed) { S.dict.nudge(S.compArmed, dir); paintDraft(); }
      });
      S.compNudge.appendChild(b);
    }
    cRow.appendChild(S.compNudge);
    ...
    const stack = el('div', 'display:flex;gap:6px;align-items:stretch;');
  ...
  const paintDraft = () => {
    ...
    const armed = !S.editing && !!(S.dict && S.compArmed && S.dict.hasSelection);
    if (S.compNudge) S.compNudge.style.display = armed ? 'flex' : 'none';
    if (S.compSave) S.compSave.style.display = armed ? 'none' : 'flex';
  ...
  function bindComposeGestures(host) {
    let timer = null, downX = 0, downY = 0, longFired = false, lastTap = 0;
    const clear = () => { if (timer) { clearTimeout(timer); timer = null; } };

    on(host, 'pointerdown', (e) => {
      if (S.editing || !S.dict) return;
      downX = e.clientX; downY = e.clientY; longFired = false;
      // A handle is a target for ARMING, never for placing: catching it here
      // rather than on the tap keeps a handle from being read as "put the
      // other edge on top of me".
      const edge = e.target && e.target.getAttribute && e.target.getAttribute('data-edge');
      if (edge) {
        e.preventDefault();
        S.compArmed = (S.compArmed === edge) ? null : edge;   // tapping it again disarms
        paintDraft();
        longFired = true;                                     // the pointerup has nothing left to do
        return;
      }
      clear();
      timer = setTimeout(() => {
    ...
    on(host, 'pointercancel', clear);

    on(host, 'pointerup', (e) => {
      clear();
      if (S.editing || !S.dict || longFired) return;
      const i = offsetFromPoint(host, e.clientX, e.clientY);
      if (i == null) return;
      // A DOUBLE TAP takes the word, the same as a long press. It is the
      // faster of the two when the target is already in view, and it costs
      // nothing to offer both: one is quick, the other is sure.
      const t = Date.now(), dbl = (t - lastTap) < DBL_MS;
      lastTap = t;
      if (dbl) { S.dict.selectWordAt(i); S.compArmed = null; paintDraft(); return; }
  ...
  const disable = () => {
    ...
    S.compEdit = S.compEditTxt = S.compBack = S.compShiftBtn = null;
    S.compNudge = S.compSave = null;
    S.compShift = false;
    S.dict = null;
    S.editing = false;
