...
(() => {
  ...
  const mountUI = () => {
    ...
    // Review rides the chip row rather than a footer band of its own. Two
    // capture modes and one way out is a single row's worth of control, and a
    // band under the list bought nothing but height on a card that already
    // covers a third of a phone.
    const goReview = el('button', 'border:1px solid #eab308;background:#facc15;color:#18181b;'
      + 'border-radius:999px;padding:4px 12px;font:600 12px ui-sans-serif,system-ui;cursor:pointer;', 'Review');
    goReview.title = 'Open the drawer’s Notes tab: read, edit, copy, or save the set';
    goReview.addEventListener('click', review);
    capture.appendChild(goReview);
