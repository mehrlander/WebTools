...
(() => {
  ...
  const placeOffer = (btn, { left, tops }) => {
    ...
    btn.style.left = (Math.max(6, Math.min(left, vpW - 70)) + w.scrollX) + 'px';
  ...
  const startPick = (o = {}) => {
    ...
    //
    // Two marks and no words, because a mode a reader is already inside cannot
    // stop to explain itself. The declared box gets a boundary, which answers
    // the first question. Each section gets a rule down the left of its run,
    // indented by RANK rather than by DOM depth, which answers the second: the
    // rules nest the way the prose does, so the outer section is the longest
    // and leftmost one, and a second tap in the same spot grows the outline to
    // exactly it. That is the gesture made visible rather than described.
    //
    ...
    const paintStructure = () => {
      ...
      // THE BOUNDARY IS DRAWN ONLY WHERE THERE IS ROOM FOR IT. The pad is wide
      // enough that the section rules sit inside it: at 8 the boundary ran
      // within a couple of pixels of the depth-1 rule and the two read as one
      // smudged line. But a render flush against a narrow viewport has no such
      // margin, and at 390px the left edge simply went off the page, leaving a
      // three-sided box. A mark that cannot be drawn whole is worse than one
      // that is not drawn: the rules carry the structure either way, and where
      // the markdown fills a phone screen the region was never in doubt.
      const pad = 16;
      const room = r && r.x >= pad + 6
        && (r.x + r.w + pad) <= Math.max(d.documentElement.scrollWidth, w.innerWidth || 0);
      if (r && room) {
        Object.assign(mark('border:1px dashed rgba(250,204,21,.5);border-radius:6px;').style, {
    ...
    const commit = () => {
      ...
      btn.style.cssText = NOTE_BTN + 'pointer-events:auto;';
      ...
      placeOffer(btn, { left: r.right - 60, tops: [r.top - 30, r.bottom + 6] });
  ...
  const startRegion = () => {
    ...
    const up = (e) => {
      ...
      // can avoid it.
      const placeBtn = () => placeOffer(btn, { left: vx + vw - 60, tops: [vy + vh + 6, vy - 32] });
