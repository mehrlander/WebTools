...
(() => {
  ...
  const mountUI = () => {
    ...
    // The period leads. It was third, behind comma and semicolon, on the
    // reading order of a punctuation chart rather than on how often a thumb
    // reaches for each one. The full stop is the most-tapped mark by a wide
    // margin and it sits at the end of every sentence, which is exactly when
    // the reader is looking away from the screen.
    for (const mark of ['.', ',', ';', '?', '!']) {
