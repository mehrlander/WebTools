...
(() => {
  ...
  const syncCopyKey = () => {
    ...
    // HIDDEN, NOT REMOVED. The key holds its place on the Notes reading, where
    // it has no bytes to take, because the group beside it stretches: dropping
    // the key out of the row would widen the group by the key's width and slide
    // all three glyphs sideways on every switch into a serialization. This card
    // already learned that lesson once, when the scope line appearing and
    // vanishing moved everything under it. `visibility` also takes the button
    // out of the tab order, so nothing invisible is still reachable.
    b.style.visibility = serial ? 'visible' : 'hidden';
  ...
  const mountUI = () => {
    ...
    // would be asking the reader to learn the shape twice. That idiom is why
    // the cells can go to glyphs at all: the group's own frame says these
    // three are one question, so no cell has to carry a word to prove it.
    // ONE ROW, and one question on it. It was a strip and a bar on two lines,
    // 58px of card spent on which reading and what it was a reading OF, with
    // the second line appearing and vanishing as the reading changed, which
    // moved everything under it. The second question is gone entirely now: the
    // format group leads, Copy rides the right end, and the row holds one line
    // at the card's 360px.
    S.readBar = el('div', 'display:none;gap:6px;align-items:stretch;margin:0 8px 8px;');
    // The group STRETCHES, and that is the whole cost of going to glyphs. Three
    // words sized the group by their own width; three glyphs collapse it to
    // about a third of that, which on a card meant for a thumb turns a 90px
    // target into a 34px one. `flex:1` gives the row back: same cells, same
    // reach, one glyph in each instead of a word.
    S.readGroup = el('div', 'flex:1;display:flex;gap:1px;align-items:stretch;'
      + 'background:#e4e4e7;border:1px solid #e4e4e7;border-radius:7px;overflow:hidden;');
    // THE READINGS ARE GLYPHS. `Notes | Markdown | JSON` spent most of a
    // 360px row spelling out three things a reader picks between constantly
    // and learns once, and the two that matter have icons the whole industry
    // already agreed on: the markdown mark and a pair of curly braces say
    // their format faster than the words do. A bulleted list says the third.
    // The words survive as titles, and the lit cell says which is showing,
    // which is the state a label was never carrying anyway.
    const readChip = (key, glyph, title) => {
      const b = icon(glyph, 'flex:1;box-sizing:border-box;min-height:26px;border:0;'
        + 'background-color:#fff;color:#3f3f46;padding:3px 9px;cursor:pointer;'
    ...
    // NO BOX AROUND IT. A bordered chip beside a bordered segmented group reads
    // as a fourth option in the group, which is exactly what it is not: the
    // chips choose a reading and this one performs an errand. Bare glyph, no
    // border and no fill, pushed to the right end of the row by its own margin
    // so the gap says the same thing the border was trying to.
    ...
    S.serialCopy = icon('copy', 'display:flex;visibility:hidden;margin-left:auto;box-sizing:border-box;'
