...
(() => {
  ...
  const mountUI = () => {
    ...
    S.compView = el('div', 'flex:1;min-width:0;min-height:44px;max-height:22vh;overflow-y:auto;background:#fafafa;'
      ...
      + 'font:17px/1.5 ui-sans-serif,system-ui;color:#18181b;white-space:pre-wrap;word-break:break-word;');
    ...
    bindComposeGestures(S.compView, S.compBody);
  ...
  function bindComposeGestures(surface, host) {
    ...
    on(surface, 'pointerup', (e) => {
      ...
      if (!window.Dictate.hitsText(host, e.clientX, e.clientY)) { toEnd(); return; }
