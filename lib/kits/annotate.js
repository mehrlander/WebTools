  ...
  const mountUI = () => {
    ...
    const chip = (label, title, fn) => {
      const b = el('button', 'border:1px solid #e4e4e7;background:#fff;color:#3f3f46;border-radius:999px;'
        + 'padding:3px 10px;font:600 11px ui-sans-serif,system-ui;cursor:pointer;', label);
      b.title = title;
    ...
    const CBTN = key() + 'min-height:46px;';
    ...
    //
    // The two most consequential keys share the bottom-right: save, then
    // delete in the corner. Both are placed EXPLICITLY rather than left to
    // flow, so a missing microphone (a browser with no recognizer) cannot
    // slide the corner one column left.
    ...
    if (S.compMic) frame.appendChild(S.compMic);
    else frame.appendChild(el('div', key() + 'cursor:default;'));   // hold the column
    frame.appendChild(S.compEdit);
    frame.appendChild(S.compPad);
    S.compSave.style.gridColumn = '4';
    S.compBack.style.gridColumn = '5';
    frame.appendChild(S.compSave);
    frame.appendChild(S.compBack);
