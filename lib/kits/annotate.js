...
(() => {
  ...
  // The set changed. Announced on the KIT's own window (the shell's, even when
  // the annotated document is a subject frame), so a reader elsewhere in the
  // page can hold a view of the notes without polling. The FAB's Notes tab is
  // the one that does; nothing here knows about it.
  const emit = () => {
    try { window.dispatchEvent(new CustomEvent('annotate:change')); } catch { }
  };
  ...
  const review = () => {
    let heard = false;
    try {
      const ev = new CustomEvent('annotate:review', { cancelable: true });
      heard = !window.dispatchEvent(ev);   // a handler that preventDefaults has taken it
    } catch { }
    if (!heard) setStatus('No drawer on this page to review in; Copy markdown still works', true);
  };
  ...
  const mountUI = () => {
    ...
    // Review is a REQUEST, not a surface this kit owns. The roomy read-and-edit
    // view is the FAB drawer's Notes tab, which has real estate this 360px card
    // never will and the house styling besides. Announcing the intent and
    // letting the drawer answer keeps one implementation of that view; a second
    // one living here would be the copy that ages.
    quiet('Review', 'Open the drawer’s Notes tab: the whole set, editable, in either format', review);
