...
(() => {
  ...
  const handoffText = () => {
    ...
    try {
      const subj = domSubject();
      const el2 = subj && (subj.el || (subj.target && elementOf(subj.target)));
  ...
  const mountUI = () => {
    ...
    //
    // `arrows-out` rather than a second chevron: the header already spends one
    // on `Notes ⌄`, which folds the list open INSIDE the card, and two chevrons
    // meaning open-here and open-elsewhere would be the redundancy the title
    // merge was made to remove.
    S.outBtn = icon('arrows-out', 'box-sizing:border-box;min-height:30px;padding:0 8px;'
      + 'border:1px solid #e4e4e7;background:#fff;border-radius:7px;color:#3f3f46;cursor:pointer;'
      + 'display:flex;align-items:center;');
    ...
    // sitting.
    //
    // TWO OF THE KIT'S THREE AIMS, AND THE THIRD IS DELIBERATELY NOT HERE.
    // kits/dictate.js `stitchAim` distinguishes a caret dropped in the gap
    // ('caret'), a full stop ending the buffer with nothing after it ('end'),
    // and an earlier break reached with no caret at all ('back'). The first
    // two are exactly the states where the cell costs nothing to borrow: the
    // reader has aimed away from the end, or a second full stop would be inert
    // there. 'back' is true of almost any buffer with two sentences in it, so
    // a cell borrowed for it would never hand the full stop back.
    //
    // The dictation page takes all three, because its keys lie ACROSS and a
    // sixth costs 13px of width off five that had 73. Here they stack in a
    // 56px column beside a text box capped at 172px, so a fifth cell is 34px
    // tall against the 43 these have, and the column's own argument (see
    // above) is that three keys is what makes each big enough to hit without
    // looking. So this card gets the repair at the moment it is heard, and
    // reaching three words back still wants a caret here.
    //
    // ONE KEY, not a set: the aim is a position, so `,` and `?` stay where
    // they were and the pad does not rearrange itself under a thumb. The
    // casing swap can take all three because a selection means the reader is
    // fixing a word rather than writing.
  ...
  const startPick = (o = {}) => {
    ...
    const paintStructure = () => {
      ...
      const w = win();
      const found = window.srcDoc.declaredIn(d);
      if (!found || !found.box) return;
      const r = unionRect([found.box], w);
  ...
  const disable = () => {
    ...
    S.pageChip = S.placeBtn = S.expandBtn = S.jotBtn = S.outBtn = null;
