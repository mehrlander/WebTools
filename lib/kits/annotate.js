...
(() => {
  ...
  const clear = () => { S.items = []; S.selId = null; if (S.ui) { paint(); renderList(); } };
  ...
  const mountUI = () => {
    ...
    S.compView = el('div', 'min-height:44px;max-height:22vh;overflow-y:auto;background:#fafafa;'
      + 'border:1px solid #e4e4e7;border-top:0;border-radius:0 0 8px 8px;padding:7px 9px;'
      + 'font:15px/1.4 ui-sans-serif,system-ui;color:#18181b;white-space:pre-wrap;word-break:break-word;');
    S.compBody = el('span', '');
    S.compGhost = el('span', 'color:#a1a1aa;font-style:italic;');
    S.compHint = el('span', 'color:#a1a1aa;', 'Listening. Tap the pencil to type instead.');
    S.compView.appendChild(S.compBody);
    S.compView.appendChild(S.compGhost);
    ...
    S.compTa = el('textarea', 'display:none;min-height:30vh;background:#fff;color:#18181b;'
      ...
      + 'font:16px/1.45 ui-sans-serif,system-ui;resize:none;overflow-y:auto;');
    S.compTa.placeholder = 'Note…';
    S.compTa.addEventListener('keydown', (e) => { if (e.key === 'Escape') closeEditor(); });

    // The punctuation toolbar, ABOVE the text the way a formatting bar sits
    // above an editor rather than floating beside it: one segmented strip,
    // seams shared, so six marks and a delete cost one control's worth of
    // height. It is furniture, present through dictation rather than
    // appearing only mid-utterance, since a mark tapped while idle appends
    // just as well.
    S.compPunct = el('div', 'display:none;align-items:stretch;border:1px solid #e4e4e7;'
      + 'border-radius:8px 8px 0 0;overflow:hidden;background:#fff;');
    const seg = (label, title, fn, iconName) => {
      const b = iconName
        ? icon(iconName, 'flex:1;border:0;border-left:1px solid #f0f0f2;background:#fff;color:#3f3f46;'
            + 'padding:6px 0;cursor:pointer;display:flex;align-items:center;justify-content:center;', title)
        : el('button', 'flex:1;border:0;border-left:1px solid #f0f0f2;background:#fff;color:#3f3f46;'
            + 'padding:6px 0;font:600 15px/1 ui-sans-serif,system-ui;cursor:pointer;', label);
      if (!iconName) b.title = title;
      if (!S.compPunct.childNodes.length) b.style.borderLeft = '0';
    ...
    for (const mark of [',', ';', '.', '?', '!']) {
      seg(mark, 'Insert ' + mark, () => S.dict && S.dict.punct(mark));
    }
    seg('', 'New paragraph', () => S.dict && S.dict.punct('¶'), 'paragraph');
    seg('', 'Delete the last word (dictation keeps running)', () => S.dict && S.dict.backWord(), 'backspace');

    const cRow = el('div', 'display:flex;gap:6px;align-items:center;');
    if (hasDictation()) {
      S.compMic = icon('microphone', BTN + 'padding:8px 13px;border-radius:8px;', 'Dictate (tap again to stop)');
    ...
    S.compEdit = icon('pencil-simple', BTN + 'padding:8px 13px;border-radius:8px;', 'Type instead (opens the keyboard)');
    ...
    const cSave = icon('check', BTN + 'background:#facc15;border-color:#eab308;color:#18181b;'
      + 'padding:8px 16px;border-radius:8px;', 'Save note');
    cSave.addEventListener('click', saveDraft);
    cRow.appendChild(cSave);

    // The bar and the text are one stack with NO gap; the compose column's
    // own gap still separates the caption above and the controls below, which
    // are separate things and should look it.
    ...
    // "JSON" used to sit here and copy on tap without saying so, which is a
    // button whose label names its format and hides its verb. The formats now
    // live behind Review, where every action says what it does and the text
    // it acts on is visible above it.
    quiet('Review', 'Read the whole set, edit any note, copy in either format', openSheet);
  ...
  // ── The review sheet ──────────────────────────────────────────────────────
  // One roomy surface for everything that was cramped: read the whole set,
  // edit any note in a box that is not 44 pixels tall, and copy in either
  // format with the text you are copying visible above the button. It is a
  // takeover rather than a third panel because a phone has room for one thing
  // at a time, and the page underneath is not what you are looking at here.
  const SHEET_TABS = [['notes', 'Notes'], ['md', 'Markdown'], ['json', 'JSON']];

  const buildSheet = () => {
    S.sheetTab = 'notes';
    const s = el('div', `display:none;position:fixed;z-index:${Z + 2};inset:3vh 3vw;`
      + 'max-width:720px;margin:0 auto;flex-direction:column;overflow:hidden;background:#fff;'
      + 'color:#27272a;border:1px solid #d4d4d8;border-radius:14px;box-shadow:0 14px 44px rgba(0,0,0,.3);');

    const head = el('div', 'display:flex;align-items:center;gap:8px;padding:11px 14px;'
      + 'border-bottom:1px solid #f0f0f2;background:#fafafa;');
    head.appendChild(el('span', 'font:700 13px ui-sans-serif,system-ui;color:#18181b;', '✎ Review'));
    S.sheetCount = el('span', 'color:#71717a;font-size:12px;', '');
    head.appendChild(S.sheetCount);
    const x = el('button', 'margin-left:auto;border:0;background:none;color:#a1a1aa;'
    ...
    const tabs = el('div', 'display:flex;padding:8px 14px 0;gap:0;');
    S.sheetTabs = {};
    for (const [key, label] of SHEET_TABS) {
      const b = el('button', 'flex:1;border:1px solid #e4e4e7;background:#fff;color:#3f3f46;'
        + 'padding:6px 0;font:600 12px ui-sans-serif,system-ui;cursor:pointer;', label);
      b.addEventListener('click', () => { S.sheetTab = key; renderSheet(); });
      S.sheetTabs[key] = b;
      tabs.appendChild(b);
    }
    S.sheetTabs.notes.style.borderRadius = '8px 0 0 8px';
    S.sheetTabs.json.style.borderRadius = '0 8px 8px 0';
    S.sheetTabs.md.style.borderLeft = S.sheetTabs.md.style.borderRight = '0';
    s.appendChild(tabs);

    S.sheetBody = el('div', 'flex:1;min-height:0;overflow-y:auto;padding:10px 14px;'
      + 'display:flex;flex-direction:column;gap:8px;');
    s.appendChild(S.sheetBody);

    // Every action here says its verb. The old footer had a button labeled by
    // its format that copied without saying so.
    const foot = el('div', 'display:flex;align-items:center;gap:6px;padding:9px 14px;'
      + 'border-top:1px solid #f0f0f2;background:#fafafa;flex-wrap:wrap;');
    const act = (label, title, fn, primary) => {
      const b = el('button', primary
        ? 'border:1px solid #eab308;background:#facc15;color:#18181b;border-radius:8px;'
          + 'padding:6px 13px;font:600 12px ui-sans-serif,system-ui;cursor:pointer;'
        : 'border:1px solid #d4d4d8;background:#fff;color:#3f3f46;border-radius:8px;'
          + 'padding:6px 11px;font:600 12px ui-sans-serif,system-ui;cursor:pointer;', label);
      b.title = title;
      b.addEventListener('click', fn);
      foot.appendChild(b);
      return b;
    };
    act('Copy markdown', 'Copy every note as markdown, ready to paste to a model', async () => {
      try { const t = await copy('md'); sheetStatus(`Copied ${Math.max(1, Math.round(t.length / 1024))}K markdown`); }
      catch (e) { sheetStatus(e.message, true); }
    }, true);
    act('Copy JSON', 'Copy the set as JSON (annotate/1)', async () => {
      try { await copy('json'); sheetStatus('Copied JSON'); }
      catch (e) { sheetStatus(e.message, true); }
    });
    act('Save jot', 'Save the set as one jot in the estate registry', async () => {
      try { sheetStatus('Saving jot…'); await saveJot(); sheetStatus('Jot saved'); }
      catch (e) { sheetStatus('Jot: ' + (e.message || e), true); }
    });
    S.sheetStatus = el('div', 'margin-left:auto;color:#71717a;font-size:11px;');
    foot.appendChild(S.sheetStatus);
    S.sheetFoot = foot;
    s.appendChild(foot);
  ...
  const sheetStatus = (msg, isErr) => {
    if (!S.sheetStatus) return;
    S.sheetStatus.textContent = msg || '';
    S.sheetStatus.style.color = isErr ? '#b91c1c' : '#71717a';
  };

  const openSheet = () => {
    if (!S.sheet) return;
    // A sheet is a reading surface; a live microphone behind it is not.
    stopDictation();
    S.sheet.style.display = 'flex';
    S.sheetTab = 'notes';
    sheetStatus('');
    renderSheet();
  };

  const closeSheet = () => { if (S.sheet) S.sheet.style.display = 'none'; };

  const renderSheet = () => {
    if (!S.sheet || S.sheet.style.display === 'none') return;
    S.sheetCount.textContent = S.items.length
      ? S.items.length + ' note' + (S.items.length === 1 ? '' : 's') : 'no notes yet';
    for (const [key] of SHEET_TABS) {
      const on = S.sheetTab === key;
      const b = S.sheetTabs[key];
      b.style.background = on ? '#facc15' : '#fff';
      b.style.borderColor = on ? '#eab308' : '#e4e4e7';
      b.style.color = '#18181b';
    }
    S.sheetBody.textContent = '';
    if (S.sheetTab === 'notes') return renderSheetNotes();
    const text = S.sheetTab === 'json' ? JSON.stringify(toJSON(), null, 2) : toMarkdown();
    // A div, not a textarea: a focused input under 16px is an iOS zoom, and
    // the point of showing the text is to read it, not to type into it. It
    // stays selectable for anyone who would rather select than tap Copy.
    S.sheetBody.appendChild(el('div', 'white-space:pre-wrap;word-break:break-word;user-select:text;'
      + '-webkit-user-select:text;color:#3f3f46;background:#fafafa;border:1px solid #f0f0f2;'
      + 'border-radius:8px;padding:10px 12px;'
      + 'font:12px/1.5 ui-monospace,SFMono-Regular,Menlo,monospace;', text));
  };

  const renderSheetNotes = () => {
    if (!S.items.length) {
      S.sheetBody.appendChild(el('div', 'color:#a1a1aa;font-size:12.5px;padding:8px 2px;',
        'Select text on the page, or capture a unit, and the notes will collect here.'));
      return;
    }
    S.items.forEach((it, i) => {
      const t = it.target;
      const card = el('div', 'border:1px solid #ececee;border-radius:10px;padding:9px 11px;'
        + 'display:flex;flex-direction:column;gap:6px;background:#fff;');
      const top = el('div', 'display:flex;align-items:flex-start;gap:8px;');
      const cap = el('div', 'flex:1;min-width:0;color:#a16207;font-size:11.5px;font-style:italic;',
        (i + 1) + '. ' + (t.type === 'text' ? '“' + clip(t.quote.exact, 160) + '”'
          : t.type === 'element' ? '⌖ ' + clip(t.excerpt || t.selector, 160)
          : '▭ ' + clip(t.excerpt, 160)));
      top.appendChild(cap);
      const del = el('button', 'border:0;background:none;color:#a1a1aa;font-size:16px;line-height:1;'
        + 'cursor:pointer;width:30px;height:26px;display:flex;align-items:center;justify-content:center;', '×');
      del.title = 'Remove this note';
      del.addEventListener('click', () => { remove(it.id); renderSheet(); });
      top.appendChild(del);
      card.appendChild(top);

      const addr = addressText(t);
      if (addr) {
        card.appendChild(el('div', 'color:#a1a1aa;word-break:break-all;user-select:text;'
          + 'font:11px/1.35 ui-monospace,SFMono-Regular,Menlo,monospace;', addr));
      }

      // The roomy editor the compose box could not be. 16px, so focusing it
      // does not zoom the page out from under the reader on iOS.
      const ta = el('textarea', 'width:100%;box-sizing:border-box;min-height:66px;resize:vertical;'
        + 'border:1px solid #e4e4e7;border-radius:8px;padding:7px 9px;background:#fafafa;color:#18181b;'
        + 'font:16px/1.4 ui-sans-serif,system-ui;');
      ta.value = it.note || '';
      ta.placeholder = 'Note…';
      // Writes through update(), which repaints the list behind the sheet but
      // deliberately does NOT re-render the sheet: rebuilding the card under a
      // cursor is how an in-place editor loses a keystroke.
      ta.addEventListener('input', () => update(it.id, ta.value));
      ta.addEventListener('focus', () => select(it.id, { scroll: false }));
      card.appendChild(ta);
      S.sheetBody.appendChild(card);
    });
  };
  ...
  const renderList = () => {
    ...
    S.items.forEach((it, i) => {
      ...
      // Edit and remove sit apart, not stacked two pixels from each other: on
      // a phone the tap that meant "fix this wording" was landing on "delete
      // it," and the two actions are not equally recoverable.
      const acts = el('div', 'display:flex;flex-direction:column;gap:10px;margin:-2px -2px 0 0;');
      const ACT = 'border:0;background:none;color:#a1a1aa;cursor:pointer;'
        + 'width:30px;height:26px;display:flex;align-items:center;justify-content:center;';
      const pencil = icon('pencil-simple', ACT, 'Edit this note');
      pencil.addEventListener('click', (e) => { e.stopPropagation(); editItem(it); });
      const del = el('button', ACT + 'font-size:16px;line-height:1;', '×');
      del.title = 'Remove this note';
  ...
  const paintDraft = () => {
    ...
    S.compBody.textContent = text;
    S.compHint.style.display = (text || (S.compGhost.textContent || '')) ? 'none' : 'inline';
    // The toolbar belongs to dictation mode, not to the moment of speaking:
    // the keyboard carries its own punctuation, so typing mode hides it. A
    // browser with no recognizer never shows it, and the text box closes its
    // own top edge in that case rather than sitting there missing a border.
    const bar = !S.editing && !!S.dict;
    S.compPunct.style.display = bar ? 'flex' : 'none';
    S.compView.style.borderTopWidth = bar ? '0' : '1px';
    S.compView.style.borderRadius = bar ? '0 0 8px 8px' : '8px';
    const listening = !!(S.dict && S.dict.listening);
    if (S.compMic) {
      S.compMic.style.background = listening ? '#facc15' : '#f4f4f5';
      S.compMic.style.borderColor = listening ? '#eab308' : '#d4d4d8';
      setIcon(S.compMic, listening ? 'stop-circle' : 'microphone');
      S.compMic.title = listening ? 'Stop dictating' : 'Dictate (tap again to stop)';
      S.compMic.style.display = S.editing ? 'none' : 'flex';
    }
    S.compView.scrollTop = S.compView.scrollHeight;
  };
  ...
  const openEditor = () => {
    ...
    S.compEdit.style.background = '#facc15';
    S.compEdit.style.borderColor = '#eab308';
    S.compEdit.title = 'Back to dictation';
    setIcon(S.compEdit, 'microphone');
    paintDraft();
    S.compTa.focus();
  };
  const closeEditor = () => {
    ...
    paintDraft();
    // Returning to dictation mode resumes listening: the mode IS the intent,
    // so making the reader tap the mic again to get back where they were is a
    // tax on the round trip.
    if (S.dict && S.dict.available()) S.dict.start();
  };
  ...
  const cancelDraft = () => {
    ...
    if (S.compose) {
      ...
      S.compGhost.textContent = '';
    }
    if (S.hint) S.hint.style.display = S.items.length ? 'none' : 'block';
  };

  // `edit` is { id, note } when reopening an existing note, absent for a new one.
  const beginDraft = (target, caption, rectDoc, edit) => {
    cancelDraft();
    S.draft = { target, editId: (edit && edit.id) || null };
    if (rectDoc) {
    ...
    S.compCap.textContent = (edit ? 'Editing · ' : '') + clip(caption, 110);
    ...
    // Reopening a note that already has words goes straight to the editor:
    // starting the mic first and stopping it a tick later would prompt for
    // the microphone on a path that never wanted it.
    if (edit && edit.note) {
      if (S.dict) S.dict.text = edit.note;
      openEditor();
      return;
    }
    if (edit && S.dict) S.dict.text = edit.note || '';
    paintDraft();
    ...
    if (!S.editing && S.dict && S.dict.available()) S.dict.start();
  };
  ...
  const saveDraft = () => {
    ...
    cancelDraft();
    if (editId) { update(editId, note); select(editId, { scroll: false }); setStatus('Note updated'); }
    else { const it = add(target, note); select(it.id, { scroll: false }); setStatus('Note added'); }
  };

  // Reopen an existing note for editing. It opens in the EDITOR rather than in
  // dictation, unlike a fresh note: there is already text, and the reason to
  // come back to a note is usually to fix a word in it rather than to say more
  // after it. The mic is one tap away either way.
  const editItem = (it) => {
    const t = it.target;
    const caption = t.type === 'text' ? t.quote.exact : (t.excerpt || t.selector || 'region');
    beginDraft(t, caption, t.type === 'region' ? t.rect : null, { id: it.id, note: it.note });
    select(it.id);
  };

  // ── Dictation ─────────────────────────────────────────────────────────────
  // A voice-first composer, ported from the prototype dropped at
  // dump/2026-08-08-paste.html. Four ideas from it are the whole value, and
  // none is about speech recognition itself:
  //
  ...
  //   focused input, so on a phone the compose surface costs no viewport. The
  //   keyboard is a deliberate second act (the pencil), not the default one.
  //
  // The prototype carried a fifth idea, a contenteditable with live Range
  // bookkeeping, that this deliberately drops: a note is a flat string, so an
  // insertion point is a cursor into text and the interim can render in its
  // own element instead of inside the document being edited.
  //
  // Written as a factory over callbacks with no reference to the annotator, so
  // lifting it into lib/kits/dictate.js later is a move, not a rewrite.
  const makeDictation = ({ onText, onInterim, onState, onError }) => {
    const W = () => win() || window;
    const SR = () => { const w = W(); return w.SpeechRecognition || w.webkitSpeechRecognition; };
    let rec = null, pending = null, continuation = false, text = '';

    // Recognized punctuation becomes its spoken name; the row is the only
    // source of real marks.
  ...
  const onDocClick = (e) => {
    if (S.mode || !S.items.length) return;        // pick/region modes own the pointer
    const t = e.target;
  ...
  const startPick = () => {
    ...
    S.mode = 'pick';
    syncChips();
    // The panel hides while a capture mode runs: on a phone it covers a third
    // of the page, which is exactly where the element you want may sit.
    // beginDraft reopens it; the pill brings it back after an Esc.
  ...
  const enable = (opts = {}) => {
    ...
    ensureIcons();
    mountUI();
    S.dict = makeDictation({
      onText: () => paintDraft(),
      onInterim: (t) => { if (S.compGhost) { S.compGhost.textContent = t ? (S.dict.text ? ' ' : '') + t : ''; paintDraft(); } },
      onState: () => paintDraft(),
    ...
    on(doc, 'keydown', (e) => {
      if (e.key === 'Escape' && S.sheet && S.sheet.style.display !== 'none') closeSheet();
    });
    const w = win();
  ...
  const disable = () => {
    ...
    if (S.ui) S.ui.remove();
    S.ui = S.panel = S.badge = S.status = S.listEl = null;
    S.compose = S.compTa = S.compCap = S.compMic = S.countEl = S.hint = null;
    S.compView = S.compBody = S.compGhost = S.compHint = S.compPunct = S.compEdit = null;
    S.dict = null;
    S.editing = false;
    S.selId = S.selRow = null;
    S.sheet = S.sheetBody = S.sheetCount = S.sheetStatus = null;
    S.sheetTabs = {};
    S.modeChips = {};
    S.doc = null;
  };
  ...
  window.Annotate = {
    ...
    startPick, startRegion, review: openSheet, closeReview: closeSheet,
