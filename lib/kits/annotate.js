...
(() => {
  ...
  const syncBody = () => {
    ...
    const list = !S.inPlace && (!S.expanded || S.reading === 'notes');
    const serial = set && S.reading !== 'notes';
  ...
  // chevron so it reads as a control rather than a label, and it goes with an
  // empty set, since a set with nothing in it has no reading to open.
  const syncExpander = () => {
    const b = S.expandBtn;
    if (!b) return;
    const offer = !!S.items.length && !S.inPlace;
    b.style.display = offer ? 'flex' : 'none';
