...
(() => {
  ...
  //
  // THE QUERY LIVES IN kits/md-doc.js, which is the kit that writes the
  // declaration; this reads it. No md-doc means nothing has declared, which is
  // the same answer as nothing declared.
  //
  ...
  const declaredKind = () => {
    const md = window.mdDoc;
    if (!md || !md.declaredIn) return null;
    const d = md.declaredIn(S.doc);
  ...
  const startPick = (o = {}) => {
    ...
    //
    // Section mode is exempt: its resolve() answers with the heading that owns
    // a section, so every point inside one already resolves to the same node
    // and stepping would leave the aim the chip is named for.
    const STEP_SLOP = 14;
