...
(() => {
  ...
  const syncCopyKey = () => {
    ...
    // It simply goes on the Notes reading, where it has no bytes to take. It
    // briefly held its place instead, back when the readings were a stretched
    // segmented group that would have widened into the vacated space and slid
    // sideways. The keys are content-sized and left-aligned now, so nothing
    // moves when this leaves the right end, and the reservation was mechanism
    // for a problem that had already been designed out.
  ...
  const mountUI = () => {
    ...
    // moved everything under it. The second question is gone entirely: the
    // readings lead, Copy rides the right end, one line at the card's 360px.
