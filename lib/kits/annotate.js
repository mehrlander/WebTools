...
(() => {
  ...
  const mountUI = () => {
    ...
    S.compHint = el('span', 'color:#a1a1aa;', 'Tap 🎤 to dictate, or ✎ to type.');
    ...
    // Punctuation, only while listening: the marks the engine is no longer
    // allowed to guess. Wrapped so it can sit out of the way and still be one
    // tap from the text it punctuates.
    S.compPunct = el('div', 'display:none;gap:4px;flex-wrap:wrap;');
    for (const mark of [',', ';', '.', '?', '!', '¶']) {
      const b = el('button', BTN + 'padding:5px 0;min-width:34px;font-size:15px;border-radius:8px;', mark);
      b.title = mark === '¶' ? 'New paragraph' : 'Insert ' + mark;
      // pointerdown, not click: a tap must not blur or scroll before the mark
      // registers, and the engine restart is already async.
      b.addEventListener('pointerdown', (e) => { e.preventDefault(); S.dict && S.dict.punct(mark); });
      S.compPunct.appendChild(b);
    }

    const cRow = el('div', 'display:flex;gap:6px;align-items:center;');
    if (hasDictation()) {
      S.compMic = el('button', BTN + 'padding:7px 12px;font-size:15px;border-radius:8px;', '🎤');
      S.compMic.title = 'Dictate (tap again to stop)';
      S.compMic.addEventListener('click', () => S.dict && S.dict.toggle());
      cRow.appendChild(S.compMic);
    }
    S.compEdit = el('button', BTN + 'padding:7px 12px;font-size:15px;border-radius:8px;', '✎');
    S.compEdit.title = 'Edit the text (opens the keyboard)';
    ...
    const cSave = el('button', BTN + 'background:#facc15;border-color:#eab308;color:#18181b;'
      + 'padding:7px 15px;font-size:15px;border-radius:8px;', '✓');
    cSave.title = 'Save note';
