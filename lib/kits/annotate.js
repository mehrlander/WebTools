...
(() => {
  ...
  const S = {
    ...
    inPlace: false,
    // WHICH READING FILLS THE CARD: the list, either serialization exactly as
    // Copy hands it over, or the structure under what a note points at. The
    // first three are of the SET, and the pane once narrowed to a single
    // selected note behind a scope chip, which put a second question on the row
    // and gave a reader who had tapped a row for some other reason a
    // serialization of one note they had not asked for.
    reading: 'notes',
  ...
  const syncBody = () => {
    ...
    // The DOM reading is not of the set, so an empty set does not empty it: a
    // draft in progress still has a target, and with none it prints the line
    // saying where one comes from. Only the set's own readings go bare.
    const bare = set && !S.items.length && S.reading !== 'dom';
    const list = set && !bare && S.reading === 'notes';
    // Two panes now: the <pre> for the two serializations, the drawn one for
    // the DOM reading. `bare` already exempts dom, which is not of the set.
    const dom = set && S.reading === 'dom';
    const serial = set && !bare && S.reading !== 'notes' && !dom;
    ...
    if (S.domPane) S.domPane.style.display = dom ? 'flex' : 'none';
  ...
  // THE DOM READING, which is the one reading that is NOT of the set.
  //
  // Markdown and JSON serialize every note, because that is what Copy hands
  // over. Structure cannot work that way: "what is this element and what
  // contains it" is a question about one node, and a set of eight notes has no
  // single answer to it. So this reading keys on the SELECTED note, falling
  // back to the draft being written, and says so when neither is there.
  //
  // kits/peek.js is a soft dependency, the way dictate.js is: load annotate
  // alone and everything works except this chip, which never appears. Peek's
  // computations read the element's own document and need no enable(), so
  // nothing of Peek's own UI is mounted by asking.
  // THE LIVE AIM COMES FIRST, and getting that wrong is what made the first
  // version of this reading useless. It keyed on the filed note, so a reader
  // aiming at things watched the pane say "select a note" through an entire
  // pick, cycling included: the answer arrived only after the question had
  // stopped being asked. Reported and reproduced 2026-08-29.
  //
  // What a reader wants named is what they are POINTING AT. So: the staged aim
  // while a mode is running, then the selected note, then the draft being
  // written, then the most recent note.
  //
  // The aim is a live element or a live rectangle rather than a target, since a
  // staged pick has not been committed to one yet and may never be.
  const domSubject = () => {
    if (S.aimEl && S.aimEl.isConnected) return { kind: S.aimKind || 'el', el: S.aimEl };
    if (S.aimRect) return { kind: 'rect', rect: S.aimRect };
    // THE HELD AIM, which is what makes this an inspector rather than a
    // live-only view. Ending a mode used to drop the subject, so a reader who
    // aimed at something, read it, and dismissed the mode watched the answer
    // vanish with the outline. The outline belongs to the mode; the reading
    // does not. Superseded by the next aim and dropped when a note is chosen.
    if (S.holdEl && S.holdEl.isConnected) return { kind: S.holdKind || 'el', el: S.holdEl };
    if (S.holdRect) return { kind: 'rect', rect: S.holdRect };
    // A FILED note already says which reading it wants, and the type is what
    // says it. A section target names a passage, so it reads as one even though
    // the element it resolves to is an ordinary heading. A region target names
    // a RECTANGLE, so it reads as one too: filed regions used to fall through
    // to the element branch, where elementOf resolves a rect to the first
    // element it covers, and a note on a 380x200 area came back describing a
    // single link inside it. The same region answered two different ways
    // depending on whether the drag had been committed yet, which is the one
    // thing a reading of a note must not do. Measured 2026-09-07: live, "region
    // 380 x 200, contains 2 elements"; filed, an element reading of one <a>.
    const asTarget = (t) => {
      if (!t) return null;
      if (t.type === 'section') return { kind: 'section', target: t };
      if (t.type === 'region' && t.rect) return { kind: 'rect', rect: t.rect, target: t };
      return { kind: 'target', target: t };
    };
    const it = S.items.find(i => i.id === S.selId);
    if (it) return asTarget(it.target);
    if (S.draft && S.draft.target) return asTarget(S.draft.target);
    return S.items.length ? asTarget(S.items[S.items.length - 1].target) : null;
  };

  // Every stage and every ancestor step calls this. Cheap when the reading is
  // not showing, which is most of the time.
  const refreshDom = () => {
    if (S.ui && S.reading === 'dom') renderDom();
  };

  // The mode ends, the outline goes, and the SUBJECT is kept: what was aimed at
  // is still the answer to "what am I looking at" until something replaces it.
  const clearAim = () => {
    if (!S.aimEl && !S.aimRect) return;
    S.holdEl = S.aimEl; S.holdRect = S.aimRect; S.holdKind = S.aimKind;
    S.aimEl = null; S.aimRect = null; S.aimKind = null;
    refreshDom();
  };

  // Choosing a note is an explicit request for that note, so it wins over
  // whatever was last aimed at rather than losing to it.
  const dropHold = () => { S.holdEl = null; S.holdRect = null; S.holdKind = null; };

  const domText = () => {
    const P = window.Peek;
    if (!P) return 'kits/peek.js is not loaded.';
    const sub = domSubject();
    if (!sub) return 'Nothing aimed at yet. Pick an aim, then tap the page; '
      + 'or select a note to read what it is pinned to.';

    // The page's own reading, matching the drawn one above.
    if (sub.target && sub.target.type === 'page') {
      const b = S.doc.body;
      const f = P.facts(b);
      const w = win();
      const L = ['page  ' + ((S.doc.title || '').trim() || '(untitled)'), ''];
      L.push(`size     ${f.rect.w} × ${f.rect.h}`);
      L.push(`holds    ${b.querySelectorAll('*').length} elements, ${f.children} top level`);
      const url = (w && w.location && w.location.href) || '';
      if (url) L.push(`at       ${url}`);
      L.push('', 'No anchor: this note is about the page as a whole.');
      return L.join('\n');
    }

    // A RECTANGLE is not a node, so it gets its own answer: what it contains,
    // reduced to roots. Listing every descendant would answer a question nobody
    // drew a box to ask.
    if (sub.kind === 'rect') {
      const r = sub.rect;
      // Contains first, touches second, and the heading says which. A box drawn
      // with a thumb is narrower than a text column, so the precise reading is
      // usually empty and reporting only that answers nothing.
      let found = P.covers(r, { doc: S.doc, mode: 'contain' });
      let verb = 'contains';
      if (!found.length) { found = P.covers(r, { doc: S.doc, mode: 'touch' }); verb = 'touches'; }
      const L = [`region  ${r.w} \u00d7 ${r.h} at ${r.x}, ${r.y}`, ''];
      if (!found.length) return L.concat('covers nothing on the page.').join('\n');
      L.push(`${verb} ${found.length} element${found.length === 1 ? '' : 's'}`);
      for (const n of found) {
        const txt = clip(n.innerText || n.textContent || '', 56);
        L.push('  ' + P.atom(n) + (txt ? `  "${txt}"` : ''));
      }
      return L.join('\n');
    }

    const el2 = sub.kind === 'el' ? sub.el : elementOf(sub.target);
    if (!el2) return `This ${sub.target.type} note does not resolve to an element on the page right now.`;
    const f = P.facts(el2);
    const row = (k, v) => k.padEnd(9) + v;
    const L = [f.atom, ''];
    L.push(row('selector', f.selector + (f.matches === 1 ? '   unique' : `   matches ${f.matches}`)));
    L.push(row('box', `${f.rect.w} \u00d7 ${f.rect.h} at ${f.rect.x}, ${f.rect.y}`));
    L.push(row('layout', Object.entries(f.layout).map(([k, v]) => `${k}: ${v}`).join('  \u00b7  ')));
    L.push(row('tree', `depth ${f.depth} \u00b7 child ${f.index + 1} of ${f.siblings} \u00b7 ${f.children} children`));
    L.push(row('path', f.path));
    if (f.text) L.push(row('text', clip(f.text, 120)));
    if (f.attrs.length) L.push(row('attrs',
      f.attrs.map(a => a.name + (a.value ? '=' + clip(a.value, 40) : '')).join('   ')));
    L.push('', 'under', ...P.tree(el2, { depth: 3 }).split('\n').map(l => '  ' + l));
    // What CONTAINS it, outermost first, so the card answers both halves of
    // the question rather than only the one the note was pinned to.
    const chain = P.chainOf(el2).slice(1).reverse();
    if (chain.length) L.push('', 'contained by',
      ...chain.map((n, i) => '  ' + '  '.repeat(i) + P.atom(n)));
    return L.join('\n');
  };

  // ── The DOM reading, drawn ────────────────────────────────────────────────
  // Delegated, not defined: window.esc is the one escape (lib/vanilla-bundle.js),
  // held there by tools/test/one-escape-helper. A bare `esc` would resolve off
  // the global in a browser and off nothing under the node harness, which runs
  // a kit through new Function with `window` as a parameter.
  const esc = (t) => window.esc(t);

  // One palette, taken from the card around it: yellow is the accent the aim
  // chips and outlines already use, so the selected crumb and the tag pill read
  // as the same idea. Everything else is zinc, and the only two colours that
  // mean anything are the selector's verdict (green unique, amber ambiguous).
  const DS = {
    lab: 'font:9.5px/1.5 ui-sans-serif,system-ui;letter-spacing:.07em;text-transform:uppercase;'
       + 'color:#a1a1aa;white-space:nowrap;',
    mono: 'font:11.5px/1.55 ui-monospace,SFMono-Regular,Menlo,monospace;',
    chip: 'display:inline-flex;align-items:center;border-radius:5px;padding:0 5px;'
        + 'background:#f4f4f5;color:#52525b;'
        + 'font:11px/1.7 ui-monospace,SFMono-Regular,Menlo,monospace;',
    box: 'background:#fafafa;border:1px solid #e4e4e7;border-radius:8px;padding:7px 9px;',
    // A SENTENCE, not a row label, and it needs its own entry because it was
    // built out of `lab` three times over: same four properties overridden
    // inline at each site, and all three inherited the one property a label
    // wants and a sentence must not have. `white-space:nowrap` is right for
    // `BOX` and `PLACE` in a two-column grid; on a full sentence it runs off
    // the card's edge with no wrap and no scroll. Reported 2026-09-07 from a
    // phone, the empty reading's line cut mid-word at the right border.
    say: 'font:italic 11.5px/1.5 ui-sans-serif,system-ui;color:#a1a1aa;white-space:normal;',
  };
  const dchip = (t, extra = '') => `<span style="${DS.chip}${extra}">${esc(t)}</span>`;

  // The identity line: what this node IS, in the notation a selector uses.
  const domIdentity = (el2) => {
    const tag = el2.tagName.toLowerCase();
    const cls = [...el2.classList];
    const shown = cls.slice(0, 5);
    return `<div style="display:flex;flex-wrap:wrap;align-items:center;gap:4px">`
      + `<span style="${DS.chip}background:#facc15;color:#18181b;font-weight:600;padding:1px 7px">${esc(tag)}</span>`
      + (el2.id ? dchip('#' + el2.id, 'background:#eef2ff;color:#4338ca;') : '')
      + shown.map(c => dchip('.' + c)).join('')
      + (cls.length > shown.length ? dchip('+' + (cls.length - shown.length)) : '')
      + `</div>`;
  };

  // The section a node belongs to, with the render it came from. One lookup,
  // since the reading, the trail and the crumb tap all need the same three
  // things and re-locating in each is how they come to disagree.
  const mdAt = (el2) => {
    const loc = window.mdDoc && window.mdDoc.locate ? window.mdDoc.locate(el2) : null;
    return loc && loc.section ? loc : null;
  };

  // THE CHAIN, whichever hierarchy the subject belongs to. `[{ el, label }]`,
  // innermost first, so one trail renderer and one crumb handler serve both.
  // The two structures over a rendered markdown document do not agree, and the
  // aim's kind is what says which one the reader meant.
  const domChain = (sub) => {
    if (sub.kind === 'section') {
      const loc = mdAt(sub.el);
      if (!loc) return [{ el: sub.el, label: sub.el.tagName.toLowerCase() }];
      return window.mdDoc.chain(loc.sections, loc.section.index).map(sec => ({
        el: window.mdDoc.headOf(loc.box, sec.index) || sub.el,
        label: 'h' + sec.depth + ' ' + clip(sec.title, 22),
      }));
    }
    return window.Peek.chainOf(sub.el).map(n => ({ el: n, label: window.Peek.shortAtom(n) }));
  };

  // The trail, outermost first, the subject filled. Tappable: this is the one
  // thing in the pane a reader can act on, and it is the same movement the
  // repeat tap makes, offered to a finger that would rather aim once.
  const domTrail = (sub) => {
    const chain = domChain(sub);
    if (chain.length < 2) return '';
    const rows = chain.slice().reverse().map((c, ri) => {
      const i = chain.length - 1 - ri;
      return `<button data-peek-crumb="${i}" style="${DS.chip}white-space:nowrap;border:0;cursor:pointer;`
        + (i === 0 ? 'background:#facc15;color:#18181b;font-weight:600;' : '')
        + `">${esc(c.label)}</button>`;
    });
    return `<div data-peek-trail style="display:flex;align-items:center;gap:3px;overflow-x:auto;`
      + `overscroll-behavior-x:contain;padding-bottom:1px">`
      + rows.join(`<span style="color:#d4d4d8;flex:none;font-size:10px">›</span>`)
      + `</div>`;
  };

  // ── The markdown reading ──────────────────────────────────────────────────
  // Everything here is counted in markdown's units rather than the DOM's,
  // because that is the whole point of the section aim: a reader asking about a
  // passage wants its rank, its line span in the file, and that it is four
  // paragraphs and a table. That it renders as a div holding nine elements is
  // true and answers nothing.
  const renderMd = (el2) => {
    const loc = mdAt(el2);
    if (!loc) return false;
    const sec = loc.section;
    const st = window.mdDoc.stats(sec.raw);
    const kids = window.mdDoc.children(loc.sections, sec.index);
    const addr = loc.addr || {};

    // The counts worth showing are the ones that are there. A passage with no
    // tables should not carry a zero saying so.
    const parts = [
      [st.paragraphs, 'paragraph'], [st.listItems, 'list item'], [st.code, 'code block'],
      [st.tables, 'table'], [st.quotes, 'quote'], [st.links, 'link'],
    ].filter(([n]) => n > 0)
     .map(([n, w]) => dchip(`${n} ${w}${n === 1 ? '' : 's'}`)).join(' ');

    const rows = [];
    rows.push(domRow('lines', `<span style="${DS.mono}color:#3f3f46">${sec.startLine}–${sec.endLine}</span>`
      + `<span style="${DS.mono}color:#a1a1aa"> of ${addr.path ? esc(addr.path) : 'the source'}</span>`));
    rows.push(domRow('size', `<span style="${DS.mono}color:#3f3f46">${st.words} words</span>`
      + `<span style="${DS.mono}color:#a1a1aa"> · ${st.lines} lines · ${st.chars} chars</span>`));
    if (parts) rows.push(domRow('holds', parts));
    rows.push(domRow('slug', `<span style="${DS.mono}color:#a1a1aa;word-break:break-all">#${esc(sec.slug)}</span>`));

    const head = `<div style="display:flex;flex-wrap:wrap;align-items:center;gap:4px">`
      + `<span style="${DS.chip}background:#facc15;color:#18181b;font-weight:600;padding:1px 7px">`
      + `h${sec.depth}</span>`
      + `<span style="font:600 12.5px/1.5 ui-sans-serif,system-ui;color:#18181b;min-width:0;`
      + `overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(sec.title)}</span></div>`;

    // The first lines of the section's own MARKDOWN, hashes and pipes intact.
    // The rendered prose is on the page behind this card; what a reader cannot
    // see from there is the source, which is also the thing the note carries.
    const raw = sec.raw.split('\n').slice(0, 6).join('\n');
    const src = `<pre style="margin:0;${DS.mono}color:#52525b;background:#fafafa;`
      + `border:1px solid #e4e4e7;border-radius:8px;padding:7px 9px;overflow:auto;`
      + `white-space:pre;max-height:96px">${esc(raw)}</pre>`;

    S.domBody.innerHTML = head
      + domTrail({ kind: 'section', el: el2 })
      + `<div style="${DS.box}${DS.mono}color:#3f3f46;word-break:break-all">`
      + esc(window.mdDoc.sourceRef(loc)) + `</div>`
      + domGrid(rows)
      + domSection('source', src)
      + (kids.length ? domSection(`${kids.length} subsection${kids.length === 1 ? '' : 's'}`,
          kids.map(k => `<div style="${DS.mono}color:#71717a;white-space:nowrap;overflow:hidden;`
            + `text-overflow:ellipsis;padding-left:${(k.depth - sec.depth - 1) * 11}px">`
            + `<span style="color:#a1a1aa">h${k.depth}</span> `
            + `<span style="color:#3f3f46">${esc(k.title)}</span></div>`).join('')) : '');
    const trail = S.domBody.querySelector('[data-peek-trail]');
    if (trail) trail.scrollLeft = trail.scrollWidth;
    return true;
  };

  // Label and value, aligned. This is the row the old blob was faking with
  // `padEnd(9)`, which held its columns only in a monospace face and only until
  // a value wrapped.
  const domRow = (label, value) =>
    `<div style="${DS.lab}padding-top:2px">${esc(label)}</div><div style="min-width:0">${value}</div>`;

  const domGrid = (rows) =>
    `<div style="display:grid;grid-template-columns:auto 1fr;gap:5px 10px;align-items:start">`
    + rows.join('') + `</div>`;

  // The subtree, indented by padding rather than by spaces, so a wrapped line
  // stays inside its own level instead of falling back to column zero.
  const domTree = (el2) => {
    const P = window.Peek;
    const lines = P.tree(el2, { depth: 3, nodes: 60 }).split('\n');
    return lines.map((l) => {
      const d = (l.match(/^ */)[0].length) / 2;
      const t = l.trim();
      const m = /^(\S+)(?:\s{2}"(.*)")?$/.exec(t);
      const name = m ? m[1] : t, txt = m && m[2];
      return `<div style="${DS.mono}padding-left:${d * 11}px;color:#71717a;white-space:nowrap;`
        + `overflow:hidden;text-overflow:ellipsis">`
        + `<span style="color:#3f3f46">${esc(name)}</span>`
        + (txt ? `<span style="color:#a1a1aa"> ${esc('"' + txt + '"')}</span>` : '')
        + `</div>`;
    }).join('');
  };

  const domSection = (label, inner) =>
    `<div><div style="${DS.lab}margin-bottom:4px">${esc(label)}</div>${inner}</div>`;

  const renderDom = () => {
    if (!S.domBody) return;
    const P = window.Peek;
    const say = (t) => { S.domBody.innerHTML =
      `<div style="${DS.say}padding:6px 2px">${esc(t)}</div>`; };

    const sub = domSubject();

    // THE CHIP IS ICON-ONLY, SO ITS TOOLTIP IS ITS LABEL, and it named the DOM
    // on a reading that is markdown's whenever the aim is a section. The icon
    // stays put: a section reading is structure too, and `tree-structure` is
    // honest about both, while a second icon would make one reading look like
    // two. The wording comes from the declared kind, so it is the registry's
    // vocabulary rather than a third place holding the word "markdown".
    const chip = S.readChips && S.readChips.dom;
    if (chip) {
      const kind = sub && sub.kind === 'section' ? declaredKind() : null;
      chip.title = kind
        ? `${kind.label} structure: the ${kind.unit} and what contains it`
        : 'The DOM under the selected note, and what contains it';
    }

    // THE EMPTY READING NAMES THE CONTROL, AND CARRIES IT. It said "Aim at
    // something, or select a note, and this reads the DOM under it", which
    // reads as an instruction to tap the page and is not one: nothing is armed
    // in this state, so a tap does nothing at all. Reported 2026-09-07, "I
    // might think tapping the page would do something but it doesn't seem to",
    // and that is the sentence's fault rather than the reader's.
    //
    // Aiming takes a control the reader has to find first, so the offer is the
    // control: one key that opens the aim menu, which is the same menu the
    // header's own button opens. A pane whose only content is an instruction
    // pointing somewhere else is the dead control this card keeps refusing to
    // be, and the empty SET line beside it has always named its gestures
    // ("Select text on the page, or use Page, Element or Region above").
    //
    // NOT auto-arming the element pick instead. That would mount a cover over
    // the page, so a reader who tapped this key to see what it does would find
    // the document unclickable and no word saying why.
    if (!sub) {
      S.domBody.innerHTML =
        `<div style="${DS.say}padding:6px 2px">Nothing aimed at yet. Pick an aim, then tap `
        + `the page; or select a note to read what it is pinned to.</div>`;
      const b = el('button', 'align-self:flex-start;margin:2px;box-sizing:border-box;'
        + 'min-height:28px;display:flex;align-items:center;gap:5px;padding:2px 10px;'
        + 'background-color:#fff;border:1px solid #e4e4e7;border-radius:7px;color:#3f3f46;'
        + 'cursor:pointer;font:600 11px ui-sans-serif,system-ui;');
      const g = el('i', 'font-size:14px;line-height:1;');
      g.className = 'ph ' + AIM_ICON.pick;
      b.appendChild(g);
      b.appendChild(el('span', '', 'Pick an aim'));
      b.title = 'Open the aim menu: the page, an element, a region, or a section';
      b.addEventListener('click', (e) => { e.stopPropagation(); openAim(); });
      S.domBody.appendChild(b);
      return;
    }

    // EACH BRANCH GUARDS ON WHAT IT NEEDS. A section reading is markdown's and
    // wants kits/md-doc.js; the element and region readings are the DOM's and
    // want kits/peek.js. One check at the top refused a section on a page that
    // had everything a section needs, which the loaders hide by chaining the
    // pair but which is wrong on its own terms.
    if (sub.kind === 'section' && window.mdDoc) {
      const el3 = sub.el || elementOf(sub.target);
      if (el3 && renderMd(el3)) return;
    }
    if (!P) return say('kits/peek.js is not loaded.');

    // THE PAGE IS NOT AN ELEMENT, whatever elementOf answers. A page target is
    // the ABSENCE of an anchor (see PAGE_TARGET), and reading it through the
    // element branch meant reading <body>: a selector of `body.bg-base-100`
    // called unique, `depth 1 · 2 of 2`, and the whole subtree. Every one of
    // those facts is true and every one is about a node's place inside
    // something, which is the question a page note has already declined to ask.
    //
    // What replaces them is what a reader might actually want to know about the
    // document they pinned a note to: how big it is, how much is in it, and
    // where it lives. Same pane, same helpers, the element vocabulary dropped.
    if (sub.target && sub.target.type === 'page') {
      const b = S.doc.body;
      const f = P.facts(b);
      const w = win();
      const title = (S.doc.title || '').trim();
      const url = (w && w.location && w.location.href) || '';
      const rows = [];
      rows.push(domRow('size', `<span style="${DS.mono}color:#3f3f46">${f.rect.w} × ${f.rect.h}</span>`
        + `<span style="${DS.mono}color:#a1a1aa"> of page</span>`));
      rows.push(domRow('holds', dchip(`${b.querySelectorAll('*').length} elements`) + ' '
        + dchip(`${f.children} top level`)));
      if (url) rows.push(domRow('at', `<span style="${DS.mono}color:#a1a1aa;word-break:break-all">${esc(url)}</span>`));
      S.domBody.innerHTML =
        `<div style="display:flex;flex-wrap:wrap;align-items:center;gap:4px">`
        + `<span style="${DS.chip}background:#facc15;color:#18181b;font-weight:600;padding:1px 7px">page</span>`
        + (title ? `<span style="font:600 12.5px/1.5 ui-sans-serif,system-ui;color:#18181b;min-width:0;`
                 + `overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(title)}</span>` : '')
        + `</div>`
        + domGrid(rows)
        + `<div style="${DS.say}padding-top:2px">No anchor: this note is about the page as a whole.</div>`;
      return;
    }
    ...
    if (sub.kind === 'rect') {
      ...
      if (!found.length) { found = P.covers(r, { doc: S.doc, mode: 'touch' }); verb = 'touches'; }
      const head = `<div style="display:flex;flex-wrap:wrap;align-items:center;gap:4px">`
        + `<span style="${DS.chip}background:#facc15;color:#18181b;font-weight:600;padding:1px 7px">region</span>`
        + dchip(`${r.w} × ${r.h}`) + dchip(`at ${r.x}, ${r.y}`) + `</div>`;
      if (!found.length) { S.domBody.innerHTML = head
        + `<div style="${DS.say}">covers nothing on the page.</div>`; return; }
      const rows = found.map(n => {
        const txt = clip(n.innerText || n.textContent || '', 70);
        return `<div style="${DS.box}display:flex;flex-direction:column;gap:2px">`
          + `<div style="${DS.mono}color:#3f3f46">${esc(P.atom(n))}</div>`
          + (txt ? `<div style="font:11px/1.45 ui-sans-serif,system-ui;color:#a1a1aa;`
                 + `overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical">`
                 + esc(txt) + `</div>` : '')
          + `</div>`;
      }).join('');
      S.domBody.innerHTML = head
        + domSection(`${verb} ${found.length} element${found.length === 1 ? '' : 's'}`,
            `<div style="display:flex;flex-direction:column;gap:5px">${rows}</div>`);
      return;
    }

    const el2 = sub.el || elementOf(sub.target);
    if (!el2) return say(`This ${sub.target.type} note does not resolve to an element on the page right now.`);

    // A section that reached here has no render to read (the branch above tried
    // and the document has moved on), so it falls through to the element
    // reading: a note filed against a passage still describes a node, and the
    // honest answer is the one that can still be computed.

    const f = P.facts(el2);
    const rows = [];
    rows.push(domRow('box', `<span style="${DS.mono}color:#3f3f46">${f.rect.w} × ${f.rect.h}</span>`
      + `<span style="${DS.mono}color:#a1a1aa"> at ${f.rect.x}, ${f.rect.y}</span>`));
    rows.push(domRow('place', `<span style="${DS.mono}color:#3f3f46">depth ${f.depth}`
      + ` · ${f.index + 1} of ${f.siblings}`
      + ` · ${f.children} child${f.children === 1 ? '' : 'ren'}</span>`));
    // Bare where the value names itself (`flex`, `relative`, `row wrap`),
    // labelled where it does not: with the flex and grid facts in here, a row
    // reading `center center 6px` cannot say which is justify and which align.
    const BARE = { display: 1, position: 1, flow: 1, cols: 1, overflow: 1 };
    rows.push(domRow('layout', Object.entries(f.layout).map(([k, v]) =>
      dchip(BARE[k] ? v : k === 'parent' ? 'in ' + v : `${k} ${v}`)).join(' ')));
    rows.push(domRow('path', `<span style="${DS.mono}color:#a1a1aa;word-break:break-all">${esc(f.path)}</span>`));
    if (f.attrs.length) rows.push(domRow('attrs',
      f.attrs.map(a => dchip(a.name + (a.value ? '=' + clip(a.value, 28) : ''))).join(' ')));

    // Marked, because adjacent spans concatenate in textContent and a test
    // asserting on the flattened pane would be reading `#li1unique`.
    const verdict = f.matches === 1
      ? `<span data-peek-verdict="unique" style="${DS.chip}background:#dcfce7;color:#15803d;flex:none">unique</span>`
      : `<span data-peek-verdict="${f.matches}" style="${DS.chip}background:#fef3c7;color:#b45309;flex:none">${f.matches} matches</span>`;

    S.domBody.innerHTML = domIdentity(el2)
      + domTrail({ kind: 'el', el: el2 })
      + `<div style="${DS.box}display:flex;align-items:flex-start;gap:7px">`
      +   `<span data-peek-sel style="${DS.mono}color:#3f3f46;word-break:break-all;flex:1;min-width:0">${esc(f.selector)}</span>`
      +   verdict
      + `</div>`
      + domGrid(rows)
      + (f.text ? domSection('text',
          `<div style="font:11.5px/1.5 ui-sans-serif,system-ui;color:#52525b;overflow:hidden;`
          + `display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical">`
          + esc(clip(f.text, 240)) + `</div>`) : '')
      + domSection('under', domTree(el2));

    // THE SUBJECT IS THE LAST CRUMB, so a trail longer than the pane hides the
    // one crumb the reader came for. Outermost-first reads the way a path does
    // and is worth keeping; scrolling to the end costs one line and keeps both.
    const trail = S.domBody.querySelector('[data-peek-trail]');
    if (trail) trail.scrollLeft = trail.scrollWidth;
  };

  // A crumb taps to its ancestor. With a mode running that MOVES THE AIM, so
  // the outline follows and a "+ note" would file the thing now named; with no
  // mode it just re-points the reading, which is the inspector case.
  const aimAncestor = (i) => {
    const sub = domSubject();
    if (!sub || sub.kind === 'rect' || !window.Peek) return;
    const cur = sub.el || elementOf(sub.target);
    if (!cur) return;
    const kind = sub.kind === 'section' ? 'section' : 'el';
    const next = (domChain({ kind, el: cur })[i] || {}).el;
    if (!next) return;
    if (S.restage && S.restage(next)) return;   // the pick owns the outline
    S.holdEl = next; S.holdKind = kind; S.holdRect = null;
    S.aimEl = null; S.aimRect = null; S.aimKind = null;
    renderDom();
  };
  ...
  const syncReadBar = () => {
    ...
    // Peek can arrive after the card is built (a page that loads it late, a
    // ?use= chain), so this is read here rather than at construction.
    const hasPeek = !!window.Peek;
    if (!hasPeek && S.reading === 'dom') S.reading = 'notes';
    for (const [key, b] of Object.entries(S.readChips)) {
      if (key === 'dom') b.style.display = hasPeek ? 'flex' : 'none';
      litKey(b, S.reading === key);
    }
    // The hairline exists to separate the DOM key from the three; with no DOM
    // key it separates nothing and would read as a stray mark.
    if (S.readRule) S.readRule.style.display = hasPeek ? 'block' : 'none';
  };
  ...
  const syncCopyKey = () => {
    ...
    if (!b) return;
    // The DOM reading is not of the set, so it earns the key on a draft with no
    // notes filed yet. copyShown() takes what is on screen, so it needs nothing else.
    const serial = !S.inPlace && S.reading !== 'notes'
      && (!!S.items.length || S.reading === 'dom');
    ...
    b.style.visibility = '';
    b.title = 'Copy this ' + (S.reading === 'json' ? 'JSON' : S.reading === 'dom' ? 'DOM reading' : 'markdown')
      + ', exactly as shown';
  };
  ...
  const copyShown = async () => {
    ...
    const text = S.reading === 'dom' ? domText()
      : S.serialPre ? S.serialPre.textContent : '';
  ...
  const select = (id, opts = {}) => {
    ...
    if (S.selId) dropHold();
    if (S.ui) { paint(); renderList(); }
    // The DOM reading is of the SELECTED note, so moving the selection changes
    // what it says. The set's own readings do not move, which is why this was
    // never needed before.
    if (S.ui && S.reading === 'dom') renderDom();
    emit();
    const it = S.items.find(i => i.id === S.selId);
  ...
  const mountUI = () => {
    ...
    // ── The fourth key is not a fourth reading ────────────────────────────
    //
    // THREE READINGS ARE OF THE SET; THIS ONE IS OF WHAT A NOTE POINTS AT.
    // Asked 2026-09-06 whether it is really a peer of the three beside it, and
    // it is not: a list, a markdown serialization and a JSON serialization are
    // three answers to "show me my notes"; structure answers "show me what this
    // note is pinned to". Same pane, same exclusivity, different subject.
    //
    // The code had been conceding this in three places without the row ever
    // saying it: `bare` exempts dom from the empty-set rule, syncCopyKey gives
    // it the copy key on a draft with nothing filed, and the comment here
    // called it the one reading about a node. Three workarounds for a grouping
    // that was never true.
    //
    // A HAIRLINE, not a move. It stays a reading, since it fills the same pane
    // and only one reading shows at a time; what it stops claiming is
    // membership in the set of three. Moving it out to sit beside the aim
    // button (the other control whose subject is a target rather than the set)
    // is the larger version of this and is not what a separator costs.
    S.readRule = el('div', 'flex:0 0 1px;align-self:center;height:16px;'
      + 'margin:0 5px;background-color:#e4e4e7;');
    S.readGroup.appendChild(S.readRule);
    // Offered only where kits/peek.js is aboard, the way the Section aim is
    // offered only where a declared render is. syncReadBar hides the hairline
    // with it, so a page without Peek gets three keys and no dangling rule.
    S.readChips.dom = readChip('dom', 'tree-structure',
      'The DOM under the selected note, and what contains it');
    ...
    // THE DOM READING GETS ITS OWN SURFACE, beside the serialization rather
    // than inside it. The other two readings ARE bytes: a <pre> is the honest
    // shape for something whose whole point is what Copy hands over. This one
    // is a description, and a description rendered as a monospace blob with
    // `padEnd(9)` columns is a serialization pretending to be a layout. Copy
    // still hands over the text form (domText), so nothing is lost by drawing
    // the on-screen one properly.
    S.domPane = el('div', 'display:none;flex-direction:column;padding:0 8px 8px;'
      + 'flex-grow:1;flex-shrink:1;flex-basis:0;min-height:0;');
    S.domBody = el('div', 'flex:1;min-height:0;overflow:auto;touch-action:pan-y;'
      + 'overscroll-behavior:contain;display:flex;flex-direction:column;gap:9px;padding:1px;');
    S.domPane.appendChild(S.domBody);
    p.appendChild(S.domPane);
    // The trail is the one thing here a reader can act on, so the whole pane
    // takes one delegated listener rather than the rows growing handlers.
    S.domPane.addEventListener('click', (e) => {
      const b = e.target.closest ? e.target.closest('[data-peek-crumb]') : null;
      if (!b) return;
      e.stopPropagation();
      aimAncestor(+b.dataset.peekCrumb);
    });
  ...
  const endMode = () => {
    ...
    S.cleanupMode = null;
    S.mode = null;
    // The aim goes with the mode, so the DOM reading falls back to the note
    // rather than describing an outline that is no longer on screen. The
    // restage hook goes with it: nothing owns an outline any more.
    S.restage = null;
    clearAim();
    closeAim();
    syncChips();
  ...
  const startPick = (o = {}) => {
    ...
    syncChips();
    // CHOOSING ELEMENT OPENS THE READING, the way a region drag does. Picking
    // this aim is one deliberate act with one question behind it, what is this
    // thing, and the pane that answers it should be open before the first tap
    // rather than found afterwards. Asked for 2026-08-29.
    //
    // The earlier objection was to expanding on every TAP, which would grow the
    // card under a reader mid-gesture; arming the mode is a different moment
    // and the objection does not reach it.
    //
    // SECTION IS INCLUDED NOW, and the reason it was not is the reason it is.
    // It was left out because this reading would have answered with an <h2> in
    // a div where the reader is asking about a passage: a true answer to the
    // wrong question. The fix is not to withhold the reading, it is to give a
    // section its OWN, in markdown's units. The pane branches on the aim's kind
    // (2026-08-30).
    if (window.Peek) setReading('dom');
    ...
    const commit = () => {
      sel = cur;
      // The staged node IS the DOM reading's subject while this mode runs, so
      // the pane follows the aim and follows every ancestor step with it.
      // THE AIM CARRIES ITS KIND. A heading element can be aimed at two ways and
      // the answer differs: as an element it is an <h2> in a div; as a section
      // it is a passage of markdown with a rank, a line span and subsections.
      // The node alone cannot say which was meant.
      S.aimEl = sel; S.aimKind = section ? 'section' : 'el'; S.aimRect = null; dropHold();
      refreshDom();
  ...
  const startRegion = () => {
    ...
    const up = (e) => {
      ...
      S.aimRect = rectDoc0(); S.aimEl = null; dropHold();
      if (window.Peek) setReading('dom'); else refreshDom();
      placeBtn();
  ...
  const disable = () => {
    ...
    S.pageChip = S.placeBtn = S.readRule = S.jotBtn = null;
    S.readBar = S.readGroup = S.readChips = S.empty = null;
    S.serial = S.serialPre = S.serialCopy = S.setActs = null;
    S.domPane = S.domBody = null;
    S.aimKind = S.holdKind = null;
