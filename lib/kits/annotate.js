...
(() => {
  ...
  // The ceiling, in pixels, and it has to be computed rather than declared: the
  ...
  // edge to grow into, and a vh-based cap would run off the top.
  const PANEL_MAX = 'min(480px,70vh)';
  const sizeExpanded = () => {
    const p = S.panel;
    if (!p) return;
    if (!S.expanded) { p.style.maxHeight = PANEL_MAX; return; }
    const w = win() || window;
    const vh = (w.visualViewport && w.visualViewport.height) || w.innerHeight || 0;
    const bottom = parseFloat(S.ui && S.ui.style.bottom) || 0;
    p.style.maxHeight = Math.max(240, Math.min(760, vh - bottom - 12)) + 'px';
  };
  ...
  // The scope chips: the set, always, and the selected note when there is one.
  // The single-note chip is LABELLED BY THE SELECTION ("Note 2") rather than
  // standing there greyed out with nothing behind it, which is the same rule
  // the review button follows: a control that cannot say what it would do is a
  // control the reader stops trusting.
  const syncScope = () => {
    if (!S.scopeSet) return;
    const id = scopeId();
    const i = S.items.findIndex(x => x.id === S.selId);
    const has = i >= 0;
    S.scopeNote.style.display = has ? 'block' : 'none';
    if (has) S.scopeNote.textContent = 'Note ' + (i + 1);
    const on = (b, lit) => {
      b.style.background = lit ? '#facc15' : '#fff';
      b.style.color = lit ? '#18181b' : '#3f3f46';
    };
    on(S.scopeSet, !id);
    on(S.scopeNote, !!id);
    if (S.serialCopy) {
  ...
  const mountUI = () => {
    ...
    S.serial = el('div', 'display:none;flex-direction:column;gap:5px;flex:1;min-height:0;padding:0 8px 8px;');
    const scopeBar = el('div', 'display:flex;align-items:center;gap:6px;');
    const scopeGroup = el('div', 'display:flex;gap:1px;align-items:stretch;'
      + 'background:#e4e4e7;border:1px solid #e4e4e7;border-radius:7px;overflow:hidden;');
    const scopeChip = (key, label) => {
      const b = el('button', 'box-sizing:border-box;min-height:24px;border:0;background:#fff;color:#3f3f46;'
        + 'padding:2px 9px;font:600 11px ui-sans-serif,system-ui;cursor:pointer;white-space:nowrap;', label);
      b.addEventListener('click', () => setScope(key));
      scopeGroup.appendChild(b);
      return b;
    };
    S.scopeSet = scopeChip('set', 'Set');
    S.scopeSet.title = 'Serialize every note in the set';
    S.scopeNote = scopeChip('note', 'Note');
    S.scopeNote.title = 'Serialize the selected note on its own, with the page it came from';
    scopeBar.appendChild(scopeGroup);
    // Copy WHAT IS SHOWN, which is the pane's own subject and not the set's:
    // the footer below carries the set's copies and never changes with the
    // reading, so neither button has to be read twice to know what it takes.
    S.serialCopy = icon('copy', 'margin-left:auto;box-sizing:border-box;min-height:24px;padding:0 8px;'
      + 'border:1px solid #d4d4d8;border-radius:7px;background:#f4f4f5;color:#3f3f46;cursor:pointer;'
      + 'display:flex;align-items:center;');
    S.serialCopy.addEventListener('click', () => copyShown());
    scopeBar.appendChild(S.serialCopy);
    S.serial.appendChild(scopeBar);
    ...
    // The actions on the whole set, which had lived only in the drawer. They
    // arrive with the expander because they are the set's, not the note's: the
    // collapsed card is where ONE note is written, and its per-row keys carry
    // that note's verbs already.
    ...
    setAct('Copy markdown', 'Copy the whole set as markdown, for pasting to a model',
      () => copy('md').then(() => setStatus('Set copied as markdown'))
        .catch(() => setStatus('Copy failed', true)));
    setAct('Copy JSON', 'Copy the whole set as annotate/1 JSON',
      () => copy('json').then(() => setStatus('Set copied as JSON'))
        .catch(() => setStatus('Copy failed', true)));
  ...
  const disable = () => {
    ...
    S.readBar = S.readChips = S.empty = null;
    S.serial = S.serialPre = S.serialCopy = S.scopeSet = S.scopeNote = S.setActs = null;
