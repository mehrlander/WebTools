...
(() => {
  ...
  // Selected, then the draft being written, then the most recent note. The
  // last rung is the one that matters in practice: a reader who opens this
  // chip is asking about the thing they just aimed at, and asking them to
  // select a row first would be a prompt where an answer was expected.
  // Measured 2026-08-29: with one note filed and nothing selected, the pane
  // showed instructions.
  const domSubject = () => {
    const it = S.items.find(i => i.id === S.selId);
    if (it) return it.target;
    if (S.draft && S.draft.target) return S.draft.target;
    return S.items.length ? S.items[S.items.length - 1].target : null;
  };
  ...
  const domText = () => {
    ...
    const t = domSubject();
    if (!t) return 'Select a note, or aim at something, and this reads the DOM under it.';
    const el2 = elementOf(t);
    if (!el2) return `This ${t.type} note does not resolve to an element on the page right now.`;
    const f = P.facts(el2);
