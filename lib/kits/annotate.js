...
(() => {
  ...
  const mountUI = () => {
    ...
    // Compose: the panel IS the input surface. Appears only while a draft is
    // staged; the page keeps nothing floating over it but the + note chip.
    ...
    const cRow = el('div', 'display:flex;gap:6px;align-items:flex-end;');
    // 16px: the iOS threshold below which Safari zooms the page on focus.
    S.compTa = el('textarea', 'flex:1;min-width:0;background:#fafafa;color:#18181b;border:1px solid #d4d4d8;'
    ...
    S.compTa.rows = 1;
    S.compTa.addEventListener('input', growCompTa);
    S.compTa.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); saveDraft(); }
      if (e.key === 'Escape') cancelDraft();
    });
    cRow.appendChild(S.compTa);
    if (hasDictation()) {
      S.compMic = el('button', BTN + 'padding:7px 10px;font-size:14px;border-radius:8px;', '🎤');
      S.compMic.title = 'Dictate the note (tap again to stop)';
      S.compMic.addEventListener('click', toggleDictation);
  ...
  const growCompTa = () => {
    ...
    ta.style.height = Math.min(ta.scrollHeight, Math.round(((win() || {}).innerHeight || 600) * 0.3)) + 'px';
  };
  ...
  const saveDraft = () => {
    ...
    const note = S.compTa.value;
    cancelDraft();
    add(t, note);
    setStatus('Note added');
  };

  // Dictation: SpeechRecognition where the browser has it (iOS Safari 14.5+,
  // Chrome; Siri-backed on iOS). Feature-detected, permission-gated, and a
  // failure lands on the status line; the keyboard's own mic key remains the
  ...
  const stopDictation = () => {
    ...
    if (S.compMic) S.compMic.style.background = '#f4f4f5';
  };
  const toggleDictation = () => {
    if (rec) { stopDictation(); setStatus(''); return; }
    const W = win();
    const SR = W.SpeechRecognition || W.webkitSpeechRecognition;
    if (!SR) return;
    rec = new SR();
    rec.lang = (W.navigator && W.navigator.language) || 'en-US';
    rec.interimResults = true;
    const base = S.compTa.value;
    rec.onresult = (ev) => {
      let text = '';
      for (const r of ev.results) text += r[0].transcript;
      S.compTa.value = base ? base.replace(/\s*$/, ' ') + text : text;
      growCompTa();
    };
    rec.onerror = (ev) => setStatus('Dictation: ' + (ev.error || 'failed'), true);
    rec.onend = () => { rec = null; if (S.compMic) S.compMic.style.background = '#f4f4f5'; };
    try {
      rec.start();
      S.compMic.style.background = '#fef3c7';
      setStatus('Listening…');
    } catch (e) {
      rec = null;
      setStatus('Dictation unavailable', true);
    }
  };
