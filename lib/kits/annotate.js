...
(() => {
  ...
  const syncBody = () => {
    ...
    // The readings strip arrives on the same condition, so the header's width
    // problem arrives with it: the word yields the row to the controls.
    if (S.expandWord) S.expandWord.style.display = set ? 'none' : '';
  ...
  const domSubject = () => {
    ...
    // A FILED note already says which reading it wants: a section target names
    // a passage, so it reads as one even though the element it resolves to is
    // an ordinary heading.
    const asTarget = (t) => t && ({ kind: t.type === 'section' ? 'section' : 'target', target: t });
  ...
  const refreshDom = () => {
    if (S.ui && S.expanded && S.reading === 'dom') renderDom();
  };
  ...
  const syncCopyKey = () => {
    ...
    const serial = S.expanded && !S.inPlace && S.reading !== 'notes'
  ...
  const mountUI = () => {
    ...
    // reading this button spent two rounds shedding. HEAD_KEY carries that
    // height, and the border it used to carry with it is gone: see the note on
    // HEAD_KEY for why one row wants one key shape.
    S.expandBtn = icon('caret-up', HEAD_KEY + 'font:700 13px ui-sans-serif,system-ui;');
    S.expandBtn._icon.style.fontSize = '13px';
    // THE WORD GOES WHEN THE ROW FILLS UP, which is the one state that grows
    // this header from three controls to five: expanding it brings the readings
    // strip and the copy key, and the five together overrun a phone's row (see
    // the aim button below for the measurements). The word costs 48px and it is
    // the least load-bearing thing on the row when the card is open, since what
    // is open under it is the set. Collapsed, there is room and it reads as it
    // was designed to, `Notes 3 ⌄`. syncBody owns the switch, beside the other
    // things that arrive and leave with the expanded body.
    //
    // THE COST IS THE EMPTY SET, and it is worth naming rather than pretending
    // away: with notes the button reads `3 ⌄`, and with none it is a lit
    // chevron alone. That is a disclosure control in the position and the state
    // a disclosure control is read in, sitting directly above the body it
    // closes, and the title still names the errand. Restoring the word costs
    // the one-line row back at 360px, which was measured, so the trade is a
    // known one rather than an oversight.
    S.expandWord = el('span', '', 'Notes');
    S.expandBtn.insertBefore(S.expandWord, S.expandBtn._icon);
    // The number rides between the word and the chevron, and is empty until
    // there is one, so an untouched card reads simply `Notes ⌄`. Muted by
    // OPACITY rather than by a colour of its own, so it follows the key from
    // grey to near-black when the set opens; a fixed grey stayed grey on the
    // lit ground and read as switched off inside a switched-on control.
    S.countEl = el('span', 'font:400 12px ui-sans-serif,system-ui;opacity:.65;', '');
    ...
    S.readChips = {
      ...
      // The one reading that is about a NODE rather than the set, so it is the
      // one whose subject is the selected note. Offered only where kits/peek.js
      // is aboard, the way the Section chip is offered only where a declared
      // render is.
      dom: readChip('dom', 'tree-structure', 'The DOM under the selected note, and what contains it'),
    };
  ...
  const disable = () => {
    ...
    S.pageChip = S.placeBtn = S.expandBtn = S.expandWord = S.jotBtn = null;
