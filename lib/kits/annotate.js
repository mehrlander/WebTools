...
(() => {
  ...
  const S = {
    ...
    // the whole set. `reading` picks which of the three is showing and `scope`
    // says whether a serialization is of the set or of the one selected note,
    // which is the reading no other surface offers.
    expanded: false,
    reading: 'notes',
    scope: 'set',
  ...
  //
  // An EMPTY set is not one of the three and does not take the window. It has
  // one line to show, and a 700px card holding one italic sentence is not a
  // window, it is a hole. The arrival state stays small for the same reason.
  const PANEL_MAX = 'min(480px,70vh)';
  const windowed = () => !!(S.expanded && !S.inPlace && S.items.length);
  const sizeExpanded = () => {
    const p = S.panel;
    if (!p) return;
    ...
    // A WINDOW OVER THE DOCUMENT, not a takeover of it. The first figure took
    // whatever room was above the card's bottom edge, which on a 720px screen
    // was 696 of it: the page the notes are about went off screen behind the
    // thing describing it, and the reader could no longer see what any of it
    // pointed at. Two thirds, and never a card taller than 560px, so the
    // document stays visible on every screen and a tall desktop does not get a
    // column of monospace a foot high.
    //
    // Floored at the collapsed card's OWN ceiling, since opening the set has
    // to buy room and must never take it away: without the floor, a card
    // already full of notes on a short screen shrank when it was expanded.
    const collapsed = Math.min(480, vh * 0.7);
    const h = Math.max(260, Math.min(
      Math.max(collapsed, Math.round(vh * 0.66)),
      560,
      vh - bottom - 12));
  ...
  const syncBody = () => {
    ...
    if (S.empty) S.empty.style.display = bare ? 'block' : 'none';
    ...
    syncExpander();
    syncReadBar();
    // Unconditionally: the scope chip and Copy ride the format row now, so
    // leaving a serialization has to take them off it. Skipping this when the
    // pane was hidden left both sitting beside the Notes chip with nothing to
    // act on.
    syncScope();
    if (serial) renderSerial();
  };

  // The scope a serialization is of. `note` means the SELECTED note, so it
  // cannot outlive a selection: clearing one drops the scope back to the set
  // rather than leaving the pane showing a note nothing is pointing at.
  const scopeId = () => (S.scope === 'note' && S.selId && S.items.some(i => i.id === S.selId)) ? S.selId : null;

  const renderSerial = () => {
    if (!S.serialPre) return;
    const id = scopeId();
    if (S.scope === 'note' && !id) S.scope = 'set';
    const json = S.reading === 'json';
    const text = id ? (json ? JSON.stringify(noteJSON(id), null, 2) : noteMarkdown(id))
      : (json ? JSON.stringify(toJSON(), null, 2) : toMarkdown());
  ...
  //
  // The number is what comes and goes, and at nothing filed the button is a
  // bare chevron, narrower than any state that carries a count.
  const syncExpander = () => {
    const b = S.expandBtn;
    if (!b) return;
    b.style.display = S.inPlace ? 'none' : 'flex';
    setIcon(b, S.expanded ? 'caret-down' : 'caret-up');
    b.style.background = S.expanded ? '#fef9c3' : 'transparent';
    b.style.borderColor = S.expanded ? '#fde68a' : 'transparent';
  ...
  // THE SCOPE CHIP, which rides the format row and says which of the two it is
  // currently showing. It appears only where it means something: a
  // serialization, with a note selected to narrow to. Labelled by the
  // SELECTION ("Note 2") rather than by the word "Note", so a reader who has
  // forgotten which row they tapped is told, and lit while narrowed, since a
  // pane showing part of a set has to look different from one showing all of
  // it.
  const syncScope = () => {
    const b = S.scopeBtn;
    if (!b) return;
    ...
    const id = scopeId();
    const i = S.items.findIndex(x => x.id === S.selId);
    const has = i >= 0;
    b.style.display = (serial && has) ? 'flex' : 'none';
    if (has) {
      b.textContent = id ? 'Note ' + (i + 1) : 'Set';
      b.style.background = id ? '#facc15' : '#fff';
      b.style.borderColor = id ? '#eab308' : '#d4d4d8';
      b.style.color = id ? '#18181b' : '#3f3f46';
      b.title = id ? 'Showing note ' + (i + 1) + ' alone; tap for the whole set'
        : 'Showing the whole set; tap for note ' + (i + 1) + ' on its own';
    }
    if (S.serialCopy) {
      S.serialCopy.style.display = serial ? 'flex' : 'none';
      S.serialCopy.title = 'Copy this ' + (S.reading === 'json' ? 'JSON' : 'markdown')
        + ', exactly as shown, for ' + (id ? 'this one note' : 'the whole set');
    }
  };

  // A toggle, since exactly one of the two is ever live. Taking a key would be
  // a choice between two things one control already holds.
  const setScope = () => {
    S.scope = S.scope === 'note' ? 'set' : 'note';
    renderSerial();
  };

  // Copy WHAT IS SHOWN. The footer's two copy buttons are the set's, always,
  // and never change with the reading; this one is the pane's, and its subject
  // is whatever the reader is looking at. Two buttons, two subjects, and
  // neither has to be read carefully to know which one it is.
  const copyShown = async () => {
    ...
    try {
      ...
      setStatus('Copied ' + (S.reading === 'json' ? 'JSON' : 'markdown')
        + ' for ' + (scopeId() ? 'one note' : S.items.length + ' notes'));
      return true;
  ...
  // One note, in the shape the set uses, carrying the same preamble: a note
  // lifted out of the shelf and pasted somewhere still says which page it came
  // from, which is the whole reason the preamble exists. The lead line reads
  // "Note 2 of 5" rather than "1 note", since the count is a claim about the
  // SET and this is an excerpt from one, not a set of one.
  const noteMarkdown = (id) => {
    const i = S.items.findIndex(x => x.id === id);
    if (i < 0) return '';
    const L = mdHead(`Note ${i + 1} of ${S.items.length} · ${today()}`);
    L.push(...itemMarkdown(S.items[i], i + 1));
    return L.join('\n');
  };
  ...
  // Still annotate/1, with one note in it. A single note that serialized to a
  // bare object would be a second format nothing declares, and the reader who
  // pastes it wants the receiving end to parse it the same way.
  const noteJSON = (id) => {
    const it = S.items.find(x => x.id === id);
    return it ? jsonFor([it]) : null;
  };
  ...
  const mountUI = () => {
    ...
    // The count, and the way into the set, which are one thing (syncExpander).
    // Built as an icon button so the chevron can be swapped by name, with the
    // number pushed in ahead of it.
    S.expandBtn = icon('caret-up', 'box-sizing:border-box;min-height:30px;display:none;'
      + 'align-items:center;gap:4px;padding:2px 7px;border:1px solid transparent;border-radius:7px;'
      + 'background:transparent;color:#71717a;font:12px ui-sans-serif,system-ui;cursor:pointer;');
    S.expandBtn._icon.style.fontSize = '13px';
    S.countEl = el('span', 'font-size:12px;', '0');
    ...
    // Three readings of one set, and they share the same strip of card: the
    // list, the markdown, the annotate/1 JSON. The strip is the capture
    // group's idiom (one segmented control, hairlines through a 1px gap)
    // because it is the same kind of choice, one of three rather than three
    // decisions, and a card with two segmented groups that looked different
    // would be asking the reader to learn the shape twice.
    // ONE ROW, carrying both choices. Which reading, and what it is a reading
    // OF, were a strip and a bar on two lines: 58px of card spent on two
    // questions with three and two answers between them, and the second line
    // appeared and vanished as the reading changed, which moved everything
    // under it. The format group leads, the scope and its Copy ride the right
    // end, and the row holds one line at the card's 360px.
    S.readBar = el('div', 'display:none;gap:6px;align-items:stretch;margin:0 8px 8px;');
    S.readGroup = el('div', 'display:flex;gap:1px;align-items:stretch;'
      + 'background:#e4e4e7;border:1px solid #e4e4e7;border-radius:7px;overflow:hidden;');
    const readChip = (key, label, title) => {
      const b = el('button', 'flex:1;box-sizing:border-box;min-height:26px;border:0;background:#fff;'
        + 'color:#3f3f46;padding:3px 9px;font:600 11px ui-sans-serif,system-ui;cursor:pointer;'
    ...
    S.readChips = {
      notes: readChip('notes', 'Notes', 'The notes as a working list'),
      md: readChip('md', 'Markdown', 'The set as markdown, exactly as Copy hands it over'),
      json: readChip('json', 'JSON', 'The set as annotate/1 JSON, exactly as Copy hands it over'),
    };
    S.readBar.appendChild(S.readGroup);

    // THE SCOPE IS ONE CHIP, not two. `Set` and `Note 2` were a segmented pair
    // in which exactly one was ever live, which is a toggle drawn as a choice
    // and twice the width of one. It carries the scope it IS and swaps on a
    // tap, lit while narrowed, which is how every other state control on this
    // card reads (the mode chips, the eye). It appears only where it means
    // something: a serialization, with a note selected to narrow to.
    S.scopeBtn = el('button', 'display:none;margin-left:auto;box-sizing:border-box;min-height:26px;'
      + 'align-items:center;padding:3px 9px;border:1px solid #d4d4d8;border-radius:7px;'
      + 'background:#fff;color:#3f3f46;font:600 11px ui-sans-serif,system-ui;cursor:pointer;'
      + 'white-space:nowrap;');
    S.scopeBtn.addEventListener('click', () => setScope());
    S.readBar.appendChild(S.scopeBtn);
    p.appendChild(S.readBar);

    // WHERE NOTES COME FROM, said once, to the reader who opened an empty set.
    // It names the three chips sitting a row above it and the gesture that has
    // no chip, which is the one nobody guesses: selecting text on the page.
    ...
    S.empty = el('div', 'display:none;margin:0 8px 10px;padding:10px 4px;text-align:center;'
    ...
    // COPY WHAT IS SHOWN, and the word needs no qualifier. It sat in the footer
    // as a pair, "Copy markdown" and "Copy JSON", which is a format named twice
    // over: once by the strip and again on the button. On the row beside the
    // format chips, they are the qualifier, so the button is one word and the
    // reader never has to check which of two they are about to press.
    S.serialCopy = icon('copy', 'display:none;box-sizing:border-box;min-height:26px;'
      + 'padding:0 9px 0 7px;gap:5px;border:1px solid #d4d4d8;border-radius:7px;'
      + 'background:#f4f4f5;color:#27272a;cursor:pointer;align-items:center;'
      + 'font:600 11px ui-sans-serif,system-ui;');
    S.serialCopy._icon.style.fontSize = '14px';
    S.serialCopy.appendChild(el('span', '', 'Copy'));
    S.serialCopy.addEventListener('click', () => copyShown());
    S.readBar.appendChild(S.serialCopy);

    // The pane is now nothing but the bytes: its bar went up onto the row.
    ...
    setAct('Save jot', 'Save the set as one jot in the estate registry', async () => {
      setStatus('Saving jot…');
      try { await saveJot(); setStatus('Saved as a jot'); }
      catch (e) { setStatus((e && e.message) || 'Jot failed', true); }
    });
  ...
  const disable = () => {
    ...
    S.reviewBtn = S.pageChip = S.placeBtn = S.expandBtn = null;
    S.readBar = S.readGroup = S.readChips = S.empty = null;
    S.serial = S.serialPre = S.serialCopy = S.scopeBtn = S.setActs = null;
  ...
  window.Annotate = {
    ...
    // The card's own reading of the set: expand it, pick which of the three
    // readings shows, and pick whether a serialization is of the set or of the
    // one selected note.
    expand, setReading, setScope, noteMarkdown, noteJSON,
