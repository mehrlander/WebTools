...
(() => {
  ...
  const mountUI = () => {
    ...
    // The pencil is the WAY IN, and it no longer has to be the way out. It was
    // a two-faced toggle: in edit mode it wore a check and the word Done, which
    // took three attempts to make legible (a microphone read as recording, then
    // an amber fill read as live, then a green check finally said the one thing
    // it did) and was still a button whose face had to be repainted in four
    // places and was left wrong in one of them. The keyboard's own dismiss ends
    // edit mode now, so the pencil stays a pencil and says only "type".
  ...
  const paintDraft = () => {
    ...
    const row = [S.compMic, S.compFill, S.compEdit, S.compBack, S.compSave, S.compPad];
  ...
  // finger (or drops a caret, on whitespace). A DOUBLE TAP opens the keyboard
  // with the caret where it landed. Tapping a HANDLE arms it. The next tap in
  // the text places the armed edge there.
  //
  ...
  function bindComposeGestures(surface, host, layer) {
    ...
    on(layer || surface, 'pointerup', (e) => {
      ...
      // KEYBOARD, with the caret where it landed. It took the word until
      // 2026-08-13, which the long press already does and does better: the
      // press is the sure gesture where the double is the quick one, and the
      // quick one is better spent on the mode switch, since reaching the
      // pencil to fix one letter is the trip this surface asks for most. The
      ...
      if (taps === 2) { taps = 0; openEditor(i == null ? S.dict.text.length : i); return; }
  ...
  const openEditor = (at) => {
    ...
    // so a reader who had stopped listening, tapped the pencil, and put the
    // keyboard away came back to a live microphone they never asked for.
    ...
    S.compTa.style.height = frameH > 40 ? (frameH - 2) + 'px' : '';
    // Nothing to repaint on the pencil: paintDraft takes the whole control row
    // away, so there is no button left wearing a face that has to be put back.
    paintDraft();
    S.compTa.focus();
    if (typeof at === 'number') {
      const i = Math.max(0, Math.min(S.compTa.value.length, at));
      try { S.compTa.setSelectionRange(i, i); } catch { }
    }
  };
  const closeEditor = () => {
    if (!S.editing) return;
    ...
    // pencil do not, and a textarea that is hidden while focused leaves the
  ...
  const cancelDraft = () => {
    ...
    if (S.compose) {
      ...
      // button reading "Back to dictation" (found headless, 2026-08-09). A
      // control with one face cannot be left wearing the wrong one.
      for (const b of [S.compMic, S.compFill, S.compEdit, S.compBack, S.compSave, S.compPad]) {
  ...
  const disable = () => {
    ...
    S.compEdit = S.compFill = S.compBack = S.compShiftBtn = S.compPad = null;
    S.padScrim = null;
