...
(() => {
  ...
  function outline(record) {
    ...
    // `exchange` is the SECTION a card belongs to: an ask and everything the
    // session did before the next ask. It is the one grouping a conversation
    // actually has, and it is what the deck's pager clusters on, so a run of
    // six dots reads as "that question took six cards to answer."
    //
    // Grouping by `kind` was the obvious alternative and it is the wrong one.
    // Since a card is either saying something or doing something, and the two
    // alternate, a kind boundary falls between nearly every card and its
    // neighbour: that fragments the pager into single dots rather than
    // clustering it, which is the opposite of what a group is for.
    //
    ...
    return sd().open({
      ...
      index: (i) => {
        ...
        return {
          ...
          title: row.title,
          // What the card DID, under what it said. On a work card this is the
          // tool summary the outline already built ("3× Bash, Read"), which is
          // the line that tells a reader whether the card they are looking for
          // is this one. An ask has no calls and gets its timestamp instead,
          // since the other thing a reader navigates a session by is when.
