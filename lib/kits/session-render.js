...
//   sessionRender.blocks(card)    -> [{turn}|{tools,label,intro?,lead?}]  card layout
...
//
// It only works because the machinery folds, and the machinery includes the
// sentences that introduce it. A run of consecutive tool calls renders as ONE
// collapsed line, built lazily on first open, and the short assistant turn
// that announced the run ("Now let me render it to check the pixels") folds in
// as that line's label. What is left standing on the card is the question and
// the reply that answered it. Measured over the 154 schema-4 records in the
// store on 2026-08-26: 2,458 exchanges, median 10 calls and 3 prose turns
// each, and 94% of the narrating turns fold, so a median card is about four
// lines. The tail is real and stays honest rather than being split: p90 is 44
// calls, and the largest single exchange in the store is 339.
//
...
(() => {
  ...
  // A card: prose turns as messages, runs of tool calls as folds.
  function renderCard(card, host, cr, mo) {
    for (const b of blocks(card)) host.append(b.tools ? toolFold(b, cr, mo) : cr.message(b.turn, mo));
  }
