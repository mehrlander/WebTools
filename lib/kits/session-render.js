...
//   sessionRender.blocks(card)    -> [{turn}|{tools:[turn]}]  a card's layout
...
//
// It only works because the machinery folds. A run of consecutive tool calls
// renders as ONE collapsed line naming what ran ("6 calls · 4× Bash, Read"),
// built lazily on first open, so an exchange reads as prose with the work
// tucked under it. Measured over the 154 schema-4 records in the store on
// 2026-08-26: 2,458 exchanges, median 10 calls and 3 prose turns each, so a
// median card is about seven lines of which three are folds. The tail is real
// and stays honest rather than being split: p90 is 44 calls, and the largest
// single exchange in the store is 339.
//
...
(() => {
  ...
  function blocks(card) {
    ...
    for (const t of card || []) {
      ...
      if (t.role === 'tool' && last && last.tools) { last.tools.push(t); continue; }
      out.push(t.role === 'tool' ? { tools: [t] } : { turn: t });
    }
    for (const b of out) if (b.tools) b.label = runLabel(b.tools);
    return out;
  }
  ...
  function outline(record) {
    ...
    return cards.map((card, i) => {
      ...
      const say = card.find(t => t.role === 'assistant');
  ...
  function toolFold({ tools, label }, cr, mo) {
    const d = el('details', 'group border-l-2 border-info/40 pl-3.5 py-0.5');
    const cap = el('span', 'font-mono text-[9.5px] tracking-widest uppercase opacity-50');
    cap.textContent = label;
    const caret = el('i', 'ph ph-caret-right text-[12px] opacity-50 transition-transform group-open:rotate-90');
