...
(() => {
  ...
  function outline(record) {
    ...
    return cards.map((card, i) => {
      ...
      // its own, and the record already holds the text to find them in. Order
      // and duplicates are kept, since an exchange that closed twice on one
      // state did close twice; a consumer that wants the distinct set takes it.
      //
      ...
      const states = window.ClosingState
        ? card.filter(t => t.role === 'assistant')
              .flatMap(t => window.ClosingState.marks(t.md))
        : [];
