...
(() => {
  ...
  async function open(record, o = {}) {
    ...
    if (window.sessionExport && o.export !== false) actions.push({
      // `ph-list-checks`, which is the picker's OWN mark, so the button and the
      // thing it opens agree. It was `ph-export` and read as the iOS share
      // glyph, promising a share sheet and delivering a checklist; reported
      // 2026-09-01 from the phone, where that square-and-arrow means one thing
      // and it is not this.
      icon: 'ph-list-checks',
