...
//
// It only works because the machinery folds, in two levels. A run of tool
// calls is one fold, and the short assistant turn that announced it ("Now let
// me render it to check the pixels") folds in as that fold's label: one STEP.
// A run of adjacent steps folds again into one SEQUENCE. So a card is the
// question, one line for everything done to answer it, and the reply, and
// opening that line gives the steps back as labelled lines rather than as a
// wall of tool output. Measured over the 154 schema-4 records in the store on
// 2026-08-26: 2,458 exchanges, median 10 calls and 3 prose turns each, and 94%
// of the narrating turns fold. The tail is real and stays honest rather than
// being split: p90 is 44 calls, and the largest single exchange is 339, which
// is now one line until someone opens it.
//
...
(() => {
  ...
  // become its contents.
  //
  // Two levels rather than one flat merge, because the step boundaries are the
  // reader's way back in: opening "7 steps · 45 calls" gives seven labelled
  // lines, not forty-five tool entries, and the sentence on each is what says
  // which one to open. Flattening would trade one slab for a smaller slab.
  //
  // A lone step is left alone. Wrapping one fold in another names nothing the
  // inner fold does not already say and costs a tap to get past.
  ...
  // ── The fold ──────────────────────────────────────────────────────────────
  // One run of consecutive tool calls, as a summary line that opens. This is
  // what lets a whole exchange be a card: the median exchange in the store
  // carries 10 calls, and expanded they bury the three sentences around them.
  //
  ...
  function toolFold({ tools, label, intro, lead }, cr, mo) {
    ...
    // With a lead the sentence goes on top and the counts under it, because
    // the sentence is what the reader is scanning for and the counts are the
    // fixed-width fact that reads fine small. Without one the counts are the
    // whole label and stay on the single line they always were.
    let sum;
    if (lead) {
      ...
      sum = el('summary',
        ...
        el('span', 'flex items-center gap-1.5 pt-0.5', caret, el('i', 'ph ph-wrench text-[12px] opacity-50')),
      ...
      sum = el('summary',
        ...
        caret, el('i', 'ph ph-wrench text-[12px] opacity-50'), cap);
    }
    ...
    d.addEventListener('toggle', () => {
      ...
      built = true;
      // The intro in full, since the summary showed only its first sentence.
      if (intro) body.append(cr.message(intro, mo));
      for (const t of tools) body.append(cr.message(t, mo));
    });
    return d;
  }

  // The sequence fold: one line for a run of steps, holding the step folds.
  // Its contents are the folds themselves, built on first open like theirs, so
  // a card with three hundred calls builds three hundred nothings until asked.
  function seqFold({ steps, label }, cr, mo) {
    const d = el('details', 'group border-l-2 border-info/40 pl-3.5 py-0.5');
    const cap = el('span', 'font-mono text-[9.5px] tracking-widest uppercase opacity-50 truncate');
    cap.textContent = label;
    const sum = el('summary',
      'flex cursor-pointer select-none items-center gap-1.5 list-none [&::-webkit-details-marker]:hidden',
      el('i', 'ph ph-caret-right text-[12px] shrink-0 opacity-50 transition-transform group-open:rotate-90'),
      el('i', 'ph ph-wrench text-[12px] shrink-0 opacity-50'), cap);
    const body = el('div', 'space-y-2 pt-2');
    d.append(sum, body);
    let built = false;
    d.addEventListener('toggle', () => {
      if (built || !d.open) return;
      built = true;
      for (const st of steps) body.append(toolFold(st, cr, mo));
    });
    return d;
  }
  ...
  function renderCard(card, host, cr, mo) {
    for (const b of blocks(card)) {
      host.append(b.steps ? seqFold(b, cr, mo) : b.tools ? toolFold(b, cr, mo) : cr.message(b.turn, mo));
    }
  }
  ...
  window.sessionRender = { ready, turns, groups, blocks, outline, describe, open, deck, bytes };
})();
