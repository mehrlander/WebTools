...
//                      optionally scoped to a folder (`under`). An empty query
//                      under a scope is a LISTING, which is what lets one call
//                      serve both browsing and searching.
...
//                      the DERIVED session name, which is not something a
//                      record quotes; see nameSegs below for why it rides the
//                      same corpus here and its own flag at the terminal.
//
...
(() => {
  ...
  // The derived session name, as searchable segments.
  //
  ...
  // conversation. The `session name:` prefix buys that back, since the matched
  // segment becomes the hit's note line and so says which of the two matched.
  //
  // Both forms are carried so the title as remembered ("fab naming") finds the
  // slug as stored ("fab-naming").
  function nameSegs(row) {
    const name = window.RepoSessionsCache?.nameOf?.(row) || '';
    if (!name) return [];
    const segs = ['session name: ' + name];
  ...
  window.EstateSearch = { clip, tree, names, code, sessions, reset, stats, TREE_RETRY_MS };
})();
