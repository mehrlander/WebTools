...
(() => {
  ...
  // The line splitter lives in kits/csv.js, which every caller loads first
  // (pages/branch.html, branch-brief.js's lazy path, and the pre-build's boot
  // list). It was a private copy here until 2026-08-16, when the registry index
  // needed a third one and the estate's third-recurrence rule came due.
  const parseCsvLine = (line) => window.Csv.parseLine(line);
