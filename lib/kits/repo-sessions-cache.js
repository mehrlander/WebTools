...
(() => {
  ...
  //
  // WHICH TURNS. The same ones the swipe deck leaves expanded, and not one
  // more. A short assistant turn immediately followed by tool calls is not an
  // answer, it is the sentence introducing the work ("Now let me render it to
  // check the pixels"), and the deck folds it into the run it announces.
  // Carrying those here would fill the card with narration the deck itself
  // hides. session-render.js sets the rule at STEP_INTRO and its blocks() is
  // the authority; this is a second reading of the same rule over a different
  // shape, held to it by tools/test/session-main-turns.test.mjs, which runs
  // both over a record and asserts the same turns come back.
  //
  // The cut is at 300 characters. Measured over the store 2026-08-26: 9,739
  // assistant turns are followed by calls and run a median of 97 characters,
  // while the 2,100 that are not run a median of 3,499. Two populations that
  // barely overlap, so the number sits in the gap and 94% of the narrating
  // turns fold.
  const STEP_INTRO = 300;

  // The three caps, and each answers a different question.
  //
  // TURN_HEAD is the opening of a turn, not the turn. A main turn runs a
  // median of 2,995 characters, so 120 buys the first sentence and nothing
  // else. That is the point: this pane is for finding the turn you want, and
  // the deck is one tap away for reading it.
  //
  // PROMPT_HEAD is shorter because an ask here is STRUCTURE, not content. It
  // separates one exchange from the next so twenty reply openings read as a
  // conversation rather than as a wall. 80 characters names a question; the
  // full opening ask is already on the row at ASK_CHARS.
  //
  // TURNS_KEPT bounds the field, and it is the NEWEST that survive: a reader
  // scrolling back goes backwards from the end. Measured 2026-08-27, 20 covers
  // 108 of 225 records whole and truncates the other 117, which is the trade
  // taken deliberately. Uncapped, the median row costs 2,074 bytes and the
  // worst 17,687, and one 166-entry session would have paid for itself out of
  // every other row's budget. Truncation is REPORTED (`turnsCut`), because the
  // store's own schema-5 lesson is that a bound is fine and a silent one is
  // the damage.
  const TURN_HEAD = 120;
  const PROMPT_HEAD = 80;
  const TURNS_KEPT = 20;
  ...
  function priorTurns(r) {
    ...
    for (let i = 0; i < seq.length; i++) {
      ...
      if (t.role === 'user') {
        ...
        out.push(['u', t.md, clock(t.at)]);
        continue;
      }
      if (t.role !== 'assistant') continue;
      if ((t.md || '').length <= STEP_INTRO && seq[i + 1]?.role === 'tool') continue;
      out.push(['a', t.md, clock(t.at)]);
    }
    for (let i = out.length - 1; i >= 0; i--) {
      if (out[i][0] === 'a') { out.splice(i, 1); break; }
    }
    return out
      .slice(-TURNS_KEPT)
      .map(([k, t, ts]) => [k, String(t || '').trim().slice(0, k === 'u' ? PROMPT_HEAD : TURN_HEAD), ts])
      .filter(([, t]) => t);
  }
  ...
  function turnsPartial(r) {
    const seq = mergedTurns(r);
    let n = 0, firstPrompt = true;
    for (let i = 0; i < seq.length; i++) {
      const t = seq[i];
      if (t.role === 'user') { if (firstPrompt) { firstPrompt = false; continue; } n++; continue; }
      if (t.role !== 'assistant') continue;
      if ((t.md || '').length <= STEP_INTRO && seq[i + 1]?.role === 'tool') continue;
      n++;
    }
  ...
  window.RepoSessionsCache = {
    ...
    STEP_INTRO, TURN_HEAD, PROMPT_HEAD, TURNS_KEPT,
    ...
    priorTurns, turnsPartial, closingAt,
