...
(() => {
  ...
  const STATE_RE = /^[ \t>]*(🟢|❇️?|🟡|🆚|✴️?|🟠|⚪️?|🟣|🔴|🔵|⚫)[ \t]*\*\*/gm;
  // Glyph to key, because the row is data: a filter, a test and a tooltip all
  // want a word, and the variation-selector pairs (⚪/⚪️, ✴️/✴, ❇️/❇) are two
  // spellings of one state that only a table can settle. The renderer owns the
  // glyph it draws back, so the vocabulary is named in one place on each side.
  const STATE_MARK = {
  ...
  //
  // CAPPED LIKE THE REPLY AND THE TURNS ARE, and for the same reason, since
  // this is the same split one level down: the NEWEST is what the row's glyph
  // stands for and what the card opens on, so it is carried at 1500 (whole for
  // 174 of the 183); the priors are the scroll back and take TURN_HEAD, the
  // length every other turn on that card is cut to. Newest STATES_KEPT
  // survive. The whole field costs about 431 KB across the store, half of what
  // `turns` already spends, and `statesCut` says when the front was dropped.
  const STATES_KEPT = 12;
  const STATE_LAST = 1500;
  function closingStates(r) {
    ...
    const all = [];
    for (const x of src) {
      const hits = [...String(x.text).matchAll(STATE_RE)];
      if (!hits.length) continue;
      const m = hits[hits.length - 1];
      const key = STATE_MARK[m[1]];
      if (!key) continue;
      all.push({ key, text: String(x.text).slice(m.index).trim(), at: x.at });
    }
    return all.slice(-STATES_KEPT).map((e, i, a) => {
      const last = i === a.length - 1;
      const h = last && e.text.length <= STATE_LAST
        ? { text: e.text, dropped: 0 }
        : head(e.text, last ? STATE_LAST : TURN_HEAD);
      return h.dropped ? [e.key, h.text, clock(e.at), h.dropped]
                       : [e.key, h.text, clock(e.at)];
    });
  }
  ...
  function statesPartial(r) {
    const n = (r?.replies || []).filter(x => x && x.text)
      .reduce((c, x) => c + ([...String(x.text).matchAll(STATE_RE)].length ? 1 : 0), 0);
