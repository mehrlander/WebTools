...
(() => {
  ...
  function closingState(r) {
    ...
    const texts = kept.length ? kept.map(x => String(x.text))
                              : [String(r?.last_message || '')];
    for (let i = texts.length - 1; i >= 0; i--) {
      const hits = [...texts[i].matchAll(STATE_RE)];
      if (hits.length) return STATE_MARK[hits[hits.length - 1][1]] || '';
    }
    return '';
  }
  ...
  function summarize(rec, sha) {
    ...
    return {
      ...
      // as a key. See closingState above for why it scans back through the
      // replies rather than reading the last one.
  ...
  window.RepoSessionsCache = {
    CACHE_PATH, ROW_CAP, TOOLS_KEPT, FILES_KEPT, ROW_V, ASK_CHARS,
    ...
    summarize, durationMins, fileAttention, closingReply, replyPartial, closingState,
