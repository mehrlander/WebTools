...
(() => {
  ...
  //
  // 13 widens `startup` from `[path, basis]` to `[path, basis, via, delivered]`,
  // which is what lets `injectionAcross` below answer whether the injector
  // delivers what it was built to deliver. The unhealed case is visible and
  // honest rather than wrong: a row at 12 carries no `via`, so it counts toward
  // the sessions scanned and contributes no document, and the fold reports both
  // numbers.
  const ROW_V = 13;
  ...
  function injectionAcross(rows) {
    ...
    const docs = new Map();
    let silent = 0, both = 0, unhealed = 0, from = '', to = '';
    for (const row of list) {
      ...
      const vias = new Map();
      for (const [path, , via, delivered] of entries) {
        if (!via) continue;
        const key = via + '\n' + path;
        let d = docs.get(key);
        if (!d) docs.set(key, (d = { path, via, sessions: 0, delivered: {} }));
        d.sessions += 1;
        const verdict = delivered || 'n/a';
        d.delivered[verdict] = (d.delivered[verdict] || 0) + 1;
  ...
  window.RepoSessionsCache = {
    ...
    startupOf, startupAttention, injectionAcross,
