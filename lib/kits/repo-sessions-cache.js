...
(() => {
  ...
  // scrolling back goes backwards from the end. Uncapped, one 166-entry session
  // would have paid for itself out of every other row's budget. Truncation is
  // REPORTED (`turnsCut`), because the store's own schema-5 lesson is that a
  // bound is fine and a silent one is the damage.
  const TURN_HEAD = 240;
  ...
  function priorTurns(r) {
    ...
    return out
      .slice(-TURNS_KEPT)
      .map(([k, t, ts]) => [k, head(t, k === 'u' ? PROMPT_HEAD : TURN_HEAD), ts])
      .filter(([, t]) => t);
  }
  ...
  window.RepoSessionsCache = {
    ...
    priorTurns, turnsPartial, closingAt, head,
