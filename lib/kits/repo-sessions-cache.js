...
(() => {
  ...
  // Through the same head as every other turn. It was a raw slice at
  // REPLY_CHARS, so the one turn a reader opens this card FOR was the only one
  // still cut mid-word and unmarked: "which joins three artifacts of one source
  // pac". Reported 2026-08-27.
  function closingReply(r) { return head(closingText(r), REPLY_CHARS).text; }
  // How much of it is not here. See head: an ellipsis says there is more and
  // nothing about how much.
  function replyDropped(r) { return head(closingText(r), REPLY_CHARS).dropped; }
  ...
  function replyPartial(r) {
    const kept = (r?.replies || []).filter(x => x && x.text);
    if (kept.length) {
      // Off the head's own answer, not a second measurement: a reply that ends
      // on a sentence just under the cap has still left text behind, and the
      // raw length test called that whole.
      return replyDropped(r) ? 'cut' : '';
    }
  ...
  function summarize(rec, sha) {
    ...
    return {
      ...
      // Characters of the closing reply not carried here. The card says the
      // number rather than only that there was one.
      replyDropped: replyDropped(r),
  ...
  window.RepoSessionsCache = {
    ...
    priorTurns, turnsPartial, closingAt, head, replyDropped, IMAGE_ONLY,
