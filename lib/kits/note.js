...
//   data-note-look   a token stamped on the panel as `data-look`, so the page's
//                    own stylesheet can restyle it for these notes only. The
//                    kit ships one look and no variants: a page reproducing
//                    something else owns what that looks like. The sheet render
//                    in alpineComponents/viewer.js asks for "excel" and draws
//                    Excel's comment box, on a page already drawing Excel.
//
...
`;
  ...
  function open(elOrSel) {
    ...
    body.textContent = t;
    panel.append(body);
    // A PER-NOTE LOOK, for a page reproducing something. The kit ships one
    // look and stamps whatever token the note asks for, so the page's own
    // stylesheet can answer `#wt-note[data-look="…"]`; the sheet render uses it
    // to draw Excel's comment box on a page that is already drawing Excel.
    const look = attr(el, 'data-note-look');
