...
(() => {
  ...
  const TITLE_CLS = {
    ...
    'tool-calls': 'text-base-content/55 italic',
    'lead-short': 'text-base-content/55 italic',
    none: 'text-base-content/40 italic',
  };

  // Role, said rather than colour-coded. A blue dot against a purple dot asked
  // the reader to learn a legend that appeared nowhere, and the two are a
  // hard pair to tell apart on a phone in daylight. The word costs four
  // characters and needs no key.
  //
  // The tint is doing the other half, and it is the half that matters when
  // reorienting: an ask is where the session changed direction, so the asks
  // are the spine you scan for. Tinting them makes that spine visible down the
  // whole list without reading a word.
  const ROLE_WORD = { user: 'You', assistant: 'Claude', tool: 'Tool', meta: 'Note' };
  const ROLE_CLS = {
    user: 'text-primary', assistant: 'text-base-content/45',
    tool: 'text-base-content/45', meta: 'text-warning',
  };
  const ROW_CLS = {
    user: 'border-primary bg-primary/[0.055]',
    assistant: 'border-transparent',
    tool: 'border-transparent',
    meta: 'border-warning/40',
  };
  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      const cbox = h('input', { type: 'checkbox', class: 'checkbox checkbox-sm mt-0.5 shrink-0' });
      ...
      const title = h(opts.onOpen ? 'button' : 'div', {
        class: 'block w-full text-left text-[15px] leading-snug sm:text-sm ' + (TITLE_CLS[card.source] || '')
      ...
      const expandIcon = h('i', { class: 'ph ph-caret-down text-[11px] opacity-60' });
      ...
      const expand = h('button', {
        class: 'inline-flex shrink-0 items-center gap-0.5 rounded font-mono text-[11px] '
      ...
      expand.addEventListener('click', () => {
        ...
        expandIcon.className = 'ph ' + (open ? 'ph-caret-down' : 'ph-caret-up') + ' text-[11px] opacity-60';
        if (!open) sync();
      });

      const go = opts.onOpen ? h('button', {
        class: 'btn btn-ghost btn-sm btn-circle shrink-0 self-start -mr-1 text-base-content/35 hover:text-primary',
        'aria-label': 'Read card ' + (ci + 1) + ' as a card',
      }, h('i', { class: 'ph ph-caret-right text-base' })) : '';
      if (opts.onOpen) go.addEventListener('click', () => opts.onOpen(ci));

      list.append(h('div', {
        class: 'border-b border-l-[3px] border-b-base-200 px-2.5 py-3 transition-colors '
             + (ROW_CLS[card.role] || ROW_CLS.assistant),
      },
        h('div', { class: 'flex items-start gap-2.5' },
          cbox,
          h('span', { class: 'font-mono text-[11px] tabular-nums text-base-content/30 mt-0.5 w-4 shrink-0 text-right' },
            String(ci + 1)),
          h('div', { class: 'min-w-0 flex-1' },
            title,
            h('div', { class: 'mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-1' },
              h('span', { class: 'text-[11px] font-medium ' + (ROLE_CLS[card.role] || ROLE_CLS.assistant) },
                ROLE_WORD[card.role] || ROLE_WORD.meta),
              card.ts ? h('span', { class: 'font-mono text-[11px] text-base-content/35' }, card.ts) : '',
              expand,
              card.ran ? h('span', { class: 'font-mono text-[11px] text-base-content/35 truncate' }, card.ran) : '')),
          go),
        turnBox));
    });
    ...
    function turnRow(t, i) {
      ...
      const body = h('div', { class: 'min-w-0 flex-1' },
        h('div', { class: 'flex items-baseline gap-2' },
          h('span', { class: 'text-xs font-medium ' + (ROLE_CLS[t.role] || ROLE_CLS.assistant) }, heading(t)),
          h('span', { class: 'font-mono text-[10px] text-base-content/30' }, t.ts || '')),
        h('div', { class: 'truncate text-xs leading-relaxed text-base-content/55' }, preview(t)));
    ...
    const stat = h('span', { class: 'pb-1.5 font-mono text-[11px] tabular-nums text-base-content/55' }, '');
