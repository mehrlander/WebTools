...
// copy it. Collapsing is the deselect. Inside an open card a long turn still
// previews with a "+N chars" chip, but that second level only reads; it does
// not select.
//
...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      const setOpen = (want) => {
        if (want && !built) {
          built = true;
          turns.forEach((t, k) => turnBox.append(turnRow(t, m.cardStart[ci] + k)));
        }
    ...
    // A turn inside an open card. It carries no control at all now: the card
    // above it decided, and the Options row decides which roles come with it.
    // What is left is the reading, which is the thing this list never showed.
    //
    // TWO LEVELS, and only the outer one selects. A card open or closed is the
    // selection; a turn previewed or whole is just how much of it you want on
    // screen. The chip says how much is left, in characters, because "there is
    // more" is what an ellipsis already says and the reader is asking how much.
    // Same reading chat-render gives a capped turn in the Activity popover,
    // which is where this shape was asked for.
    const PREVIEW = 320;
    function turnRow(t, i) {
      const full = t.role === 'tool' ? (t.md || preview(t)) : (t.md || '');
      const short = t.role === 'tool' ? preview(t) : full.slice(0, PREVIEW).trimEnd();
      const long = full.length > short.length;

      const text = h('div', {
        class: 'text-[13px] leading-relaxed text-base-content/70 '
             + (t.role === 'tool' ? 'truncate' : 'whitespace-pre-wrap'),
      }, long ? short : full);
      const chip = long ? h('span', {
        class: 'ml-1 inline whitespace-nowrap rounded px-1 bg-base-content/10'
             + ' font-mono text-[9.5px] text-base-content/50',
      }, '+' + (full.length - short.length).toLocaleString() + ' chars') : '';
      if (long) text.append(document.createTextNode('\u00A0'), chip);
      ...
      if (long) {
        ...
        body.addEventListener('click', () => {
          open = !open;
          text.classList.toggle('truncate', !open && t.role === 'tool');
          text.textContent = open ? full : short;
          if (!open) text.append(document.createTextNode('\u00A0'), chip);
        });
      }
      return h('div', { class: 'rounded-md px-1.5 py-1.5 hover:bg-base-200/70'
                             + (long ? ' cursor-pointer' : '') }, body);
    }
  ...
  function open(record, opts = {}) {
    ...
    const header = h('div', { class: 'flex items-center gap-3 border-b border-base-300 bg-base-100/90 px-4 py-3 backdrop-blur sm:px-6' },
      ...
      h('div', { class: 'min-w-0 flex-1' },
        // NOT "Session outline". The deck's index sheet is also an outline of
        // the same cards, so a reader arriving here from the header read it as
        // having landed back where they started rather than in a tool (reported
        // 2026-09-01). What this surface is for is picking turns and copying
        // them, and the title should say the job rather than the shape.
        h('h1', { class: 'truncate text-sm font-semibold sm:text-base' }, 'Copy turns'),
