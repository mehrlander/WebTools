...
(() => {
  ...
  // ── The guideline, which now carries where each exchange ARRIVED ──────────
  //
  // It keyed on `kind` until 2026-09-02: blue down the asks, clay down the
  // replies, nothing beside the work, on the argument that this marked the
  // conversation and left the machinery unmarked. Three things had made that
  // wrong. The row states it already, since every row now shows a blue ask and
  // a clay reply, so the rule marked a SUBSET of them in the same two colours
  // for a different distinction. The fact it encoded was `calls === 0`, which
  // the row prints in words as the tool tally, present or absent. And its blue
  // was a lie: `ask` does not mean "you asked", it means the record kept a
  // prompt with no reply beside it, which is a capture gap.
  //
  // What it carries instead is the conventions' CLOSING STATE, the marker a
  // reply ends on. That is a code the reader has already learnt from the chat
  // rather than a legend to acquire here, which is the same argument estate.js
  // makes for drawing the glyph on a session row. Where an exchange closed on
  // more than one, the rail stacks them in order, which is 4.8% of exchanges
  // and reliably the ones worth finding: 🆚 then 🟢 is a decision put and then
  // acted on inside one turn.
  //
  // `note` survives as the one non-state rule, because the capture card is the
  // renderer's own rather than the session's and no closing state can say so.
  // Its colour is written literally at the rail below, for the Tailwind reason
  // stated there.
  const ROLE_CLS = {
  ...
  function index(record, opts = {}) {
    ...
    const o = { ...DEFAULTS, ...opts };
    // Which closing states the rail treats as the ordinary rhythm and leaves
    // undrawn. `opts.routine` names a set; passing [] draws every state. The
    // default is the kit's, which is `ready` and `clean`; see the rail below.
    const routine = opts.routine || (window.ClosingState?.ROUTINE || []);
    // How loudly a routine state is drawn. 0 drops it, 1 is no distinction at
    // all; the default keeps the spine and lets an event out-shout it.
    const fade = opts.routineFade === undefined ? 0.4 : opts.routineFade;
    ...
    line.forEach((card, ci) => {
      ...
      const answer = last && (window.readAloud?.speechText?.(last.md) || last.md || '')
        .replace(/\s+/g, ' ').trim();
      // NO PAD AT ALL, ON EITHER SIDE. The turns inside carry chat-render's own
      // 17px lead indent, which is the row's gutter above, so a left pad would
      // push the open body off the edge the closed row establishes; the right
      // edge is settled by the grid below rather than by a guess here. It also
      // stops carrying `flex-col gap-0.5`: this is a block, the messages
      // renderCard appends bring their own margins, and neither class was
      // doing anything.
      const turnBox = h('div', { class: 'hidden pt-1' });
      const turnsWord = card.turns + (card.turns === 1 ? ' turn' : ' turns');
      const expandIcon = h('i', { class: 'ph ph-caret-down text-xs opacity-50' });
      ...
      const expand = h('span', {
        class: 'inline-flex shrink-0 items-center gap-1 font-mono text-xs text-base-content/50',
      ...
      const replyText = answer ? h('span', {
        class: 'mt-1.5 line-clamp-1 text-[13px] leading-snug text-base-content/70' }, answer) : null;
      ...
      const showReply = (on) => {
        for (const el of [replyMark, replyText]) if (el) el.style.display = on ? '' : 'none';
      };
      ...
      const setOpen = (want) => {
        // ONE RENDERER. `sessionRender.card` is the deck's own slide body, at
        // chat-render's panel density: markdown, the Claude mark, code spans.
        // This file drew its own version of a turn until 2026-09-01 and the
        // reader met both within two taps.
        //
        ...
        if (want && !built && replies.length) {
          built = true;
          // NO CLAMP ON A REPLY. The row's own two lines are the surface
          // level and opening it is the second; clamping each reply as well
          // made a third, and chat-render's fade then dropped a "Show full
          // message" button across the middle of a paragraph. The deck shows
          // replies whole for the same reason, so this matches it.
          SR().card(replies, turnBox, { dense: true, collapse: 0 });
        }
        want ? openCards.add(ci) : openCards.delete(ci);
        turnBox.classList.toggle('hidden', !want || !replies.length);
        title.classList.toggle('line-clamp-2', !want);
        showReply(!want);
      ...
      row.addEventListener('click', () => { setOpen(!openCards.has(ci)); sync(); });
      ...
      // ONE MEASURE FOR BOTH HALVES, AND THE GRID IS WHY. The open body used to
      // be a sibling of the head, so it ran the card's full width while the ask
      // above it stopped short by whatever the deck glyph and its gap took: at
      // 430 that was 366 against 400, two different wrapping measures for one
      // exchange, which reads as the answer being cramped or the ask cut short
      // (reported 2026-09-01). Putting the body in the head's own first column
      // makes the two identical BY CONSTRUCTION, at any width and whether or
      // not there is a glyph: the `auto` track collapses to nothing when the
      // list has no deck to open. A reserved pixel count would have needed the
      // button's size, its gap and its pull kept in agreement here.
      //
      // `col-start-1` is not decoration: with no glyph, `go` contributes no
      // element and auto-placement would drop the body into the second column.
      // ── The rail, stacked ─────────────────────────────────────────────────
      // One segment per DISTINCT state, in the order the exchange closed on
      // them, sharing the card's height. Distinct rather than every hit: an
      // exchange that closed twice on the same state is still one line, and
      // drawing it as two says something about the row that is not true.
      //
      // A positioned strip rather than a border, because a border takes one
      // colour and this takes several. The 2px and the left edge are the border
      // it replaces, so a card with one state is pixel-identical to the rule
      // that stood here before.
      //
      // THE ROUTINE STATES ARE MUTED, NOT DROPPED, and that was decided by
      // looking rather than by the numbers. 73% of exchanges across the store
      // carry a state and `ready` alone is 57% of those, which argued for
      // drawing only the rest. Rendered both ways at 430 on 2026-09-02, the
      // opposite is true: with every row ruled, the two exceptions read AGAINST
      // a baseline, and with only the exceptions ruled they float beside
      // nothing and the rail stops being a column at all. The spine also earns
      // its own keep, since it says which cards are conversation, which is the
      // honest half of what the `kind` rule was reaching for.
      //
      // So the hierarchy comes from weight and not from presence: the code
      // stays true (green is green) and an event is simply louder than the
      // rhythm. `routineFade: 0` from a caller drops them after all, which is
      // the reading this replaced, kept because a denser host may want it.
      const seen = [];
      for (const k of (card.states || [])) if (!seen.includes(k)) seen.push(k);
      const CS = window.ClosingState;
      const rail = h('div', { class: 'absolute inset-y-0 left-0 flex w-[2px] flex-col overflow-hidden' });
      if (card.kind === 'note') {
        // A LITERAL, not built from a constant. Tailwind compiles the classes
        // it can see in the source, and a string assembled at runtime is not
        // one of them; the browser build happens to rescan the DOM and resolve
        // it, which is a reprieve rather than a contract.
        rail.append(h('div', { class: 'grow bg-warning/40' }));
      } else {
        for (const k of seen) {
          const dim = routine.includes(k);
          if (dim && !fade) continue;
          const seg = h('div', { class: 'grow' });
          seg.style.background = CS?.RAIL?.[k] || 'transparent';
          if (dim) seg.style.opacity = String(fade);
          seg.setAttribute('data-note', (CS?.GLYPH?.[k] || '') + ' ' + (CS?.GLOSS?.[k] || k));
          seg.setAttribute('data-note-bare', '');
          rail.append(seg);
        }
      }

      turnBox.classList.add('col-start-1');
      const head = h('div', {
        class: 'grid items-start gap-2.5',
        style: 'grid-template-columns:minmax(0,1fr) auto',
      }, row, go, turnBox);
      const box = h('div', {
        class: 'relative mb-0.5 py-1 pl-3 pr-2.5',
      }, rail, head);
      paint.push(() => {
