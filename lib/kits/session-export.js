  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      const title = h('div', {
        ...
        class: 'w-full text-left leading-snug line-clamp-2 '
      ...
      // Hidden when the card is open, where the replies render in full below and
      // a one-line preview of the last of them would be the same words twice.
      ...
      const reply = answer ? h('div', { class: 'mt-1 flex items-start gap-1.5' },
        h('i', { class: 'ph ph-sparkle shrink-0 text-[11px] mt-[3px]', style: 'color:' + CLAY }),
        h('span', { class: 'line-clamp-1 text-[13px] leading-snug text-base-content/60' }, answer)) : '';
