  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      const title = h(opts.onOpen ? 'button' : 'div', {
        class: 'block w-full text-left text-[15px] leading-snug sm:text-sm '
          + (card.role === 'user' ? 'font-medium ' : '') + (TITLE_CLS[card.source] || '')
      ...
      list.append(h('div', { class: 'border-b border-base-200 px-2.5 py-3' },
