...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      // doing anything.
      //
      // AND IT IS BOUNDED. A long exchange opened to nine screens of reply, so
      // the row that was tapped scrolled off the top and the list under it was
      // unreachable without dragging past all of it. The cap is `60vh` or 28rem,
      // whichever is smaller, so the open card stays a panel inside the list
      // rather than becoming the page; the ring below draws its edge.
      const turnBox = h('div', { class: 'hidden max-h-[min(60vh,28rem)] overflow-y-auto scrollbar-thin pt-1' });
      ...
      paint.push(() => {
        ...
        // ── WHICH ROW IS OPEN, drawn as an edge and never as a fill ──────────
        // Reported 2026-09-02: with several cards on screen the open one had no
        // scope, since the only marks were its height and the clay replies
        // inside it, and both read as more list.
        //
        ...
        // it, so the third answer cannot be the first one in another colour.
        //
        ...
        // exists for, and the body deliberately carries no pad of its own. Only
        // the vertical padding grows, which shifts nothing sideways.
