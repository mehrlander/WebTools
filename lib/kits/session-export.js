    ...
    line.forEach((card, ci) => {
      ...
      const title = h('div', {
        ...
        class: (ask ? 'w-fit max-w-full ' : 'w-full ') + 'text-left leading-snug line-clamp-2 '
          + (ask ? 'text-[14px] font-normal text-base-content/70 '
      ...
      for (const c of closings) {
        ...
        //
        // `mt-[11px]` is measured against the X-HEIGHT, not the line box, and
        // the difference is the whole of it. At `mt-[6px]` the disc sat 3.5px
        // high, which was simply wrong; at `mt-[9px]` it was centred on the
        // line box to within half a pixel and still read high, which is not a
        // contradiction. The box centre and the cap-to-baseline centre agree
        // here to 0.2px, but the ink of a lowercase line sits in the x-band,
        // which for 13px system text runs baseline minus 6.8px and centres
        // 1.6px lower. A disc has no ascender to balance it, so it goes where
        // the ink is: probes at `vertical-align: baseline` and `text-top`
        // measured the band on the rendered page rather than assuming it.
        const mark = h('span', {
          class: 'mt-[11px] block size-[6px] shrink-0 justify-self-center rounded-full',
          'aria-hidden': 'true' });
        ...
        //
        // Through speechText for the same reason the reply preview is: a state
        // block is where the surfacing links live, so raw it previews as a run
        // of URLs.
        const words = (window.readAloud?.speechText?.(c.text) || c.text || '')
          .replace(/\s+/g, ' ').trim();
        const line = h('span', {
          class: 'mt-1 line-clamp-1 text-[13px] leading-snug text-base-content/70' }, words);
        stateEls.push(mark, line);
      }

      const showReply = (on) => {
        for (const el of [replyMark, replyText, ...stateEls]) if (el) el.style.display = on ? '' : 'none';
      };
