...
(() => {
  ...
  //
  // The edge carries the conventions' CLOSING STATE instead, as a cluster of
  // dots; the argument and the two readings tried before it are at the marks
  // block down in the card loop.
  const ROLE_CLS = {
  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      // ── The marks, as a cluster of dots ───────────────────────────────────
      // One dot per DISTINCT state, in the order the exchange closed on them,
      // stacked in the box's own left pad beside the lead glyph. Distinct
      // rather than every hit: an exchange that closed twice on one state is
      // still one dot, and two would say something about the row that is not
      // true.
      //
      // IT WAS A RAIL FOR ONE DAY, a 2px strip running each card's full height,
      // and the phone reported it before the day was out: "is this one all
      // green? I can't quite tell on a glance." Two faults, and they compound.
      // A full-height strip on every row draws ONE line down the list, since
      // 2px of margin between cards is not a break, so the colour changes
      // partway down with nothing saying which row it changed for. And the
      // rhythm was drawn at 0.4 opacity to keep it quiet, which is also the
      // opacity at which 2px of green stops reading as green.
      //
      // A dot answers both without a knob. It is bounded, so one row's mark
      // cannot run into the next row's; and at 6px it is quiet enough that the
      // 57% of exchanges closing on 🟢 need no muting to sit as a background,
      // which is what let the fade go with nothing to replace it. Where an
      // exchange closed on more than one state the dots stack into a short
      // column, 4.8% of them and reliably the ones worth finding: 🆚 then 🟢 is
      // a decision put and then acted on inside one turn.
      //
      // THE MARKER ITSELF WAS THE RUNNER-UP and is the one thing that would
      // need no legend at all, 🟢 and 🆚 drawn as emoji. It costs 10px off the
      // title's measure, since an emoji does not fit the pad a dot does, and it
      // outshouts the conversation it is annotating: rendered side by side at
      // 430 on 2026-09-02, the 🆚 badge was the loudest thing on the screen.
      const seen = [];
      for (const k of (card.states || [])) if (!seen.includes(k)) seen.push(k);
      const CS = window.ClosingState;
      // `top-[13px]` centres a single dot on the lead glyph, which hangs at the
      // box's 4px of pad plus its own 7px; a stack then grows downward from
      // that line rather than straddling it.
      const marks = h('div', { class: 'absolute left-[2px] top-[13px] flex flex-col gap-[3px]' });
      const dot = (note) => {
        const d = h('span', { class: 'block size-[6px] shrink-0 rounded-full' });
        d.setAttribute('data-note', note);
        // A 6px disc has no text to underline, so the note kit draws it bare.
        d.setAttribute('data-note-bare', '');
        return d;
      };
      if (card.kind === 'note') {
        // A RING, NOT A DISC, and the shape is doing the work. This is the one
        // card that is the RENDERER's rather than the session's, so it has no
        // closing state and never will; a filled dot in a twelfth hue would
        // read as a twelfth state, and the theme's `warning` at 88% lightness
        // is invisible at this size anyway. Hollow says "not one of these"
        // without spending a colour.
        const ring = dot('The renderer\'s own card, not a turn: what the session touched');
        ring.style.boxShadow = 'inset 0 0 0 1.5px var(--color-base-content, #1c1e21)';
        ring.style.opacity = '0.35';
        marks.append(ring);
      } else for (const k of seen) {
        const d = dot((CS?.GLYPH?.[k] || '') + ' ' + (CS?.GLOSS?.[k] || k));
        d.style.background = CS?.HUE?.[k] || 'transparent';
        marks.append(d);
      }
