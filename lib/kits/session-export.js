...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      //
      // Inline, not `bg-primary/10`, and not only for the theme tracking:
      // bg-primary/10 is already this list's OPEN-row tint, and the two
      // signals stack on an open row. Composited they read 10% then 20%, so
      // the ask stays the deeper block inside the selected row rather than
      // dissolving into it. A shared class would also have made the tests'
      // open-row count find every ask.
      if (ask) {
      ...
      const answer = last && (window.readAloud?.speechText?.(last.md) || last.md || '')
        .replace(/\s+/g, ' ').trim();
      const turnBox = h('div', { class: 'hidden flex-col gap-0.5 pt-1.5 pl-4 pr-1' });
      const turnsWord = card.turns + (card.turns === 1 ? ' turn' : ' turns');
      const expandIcon = h('i', { class: 'ph ph-caret-down text-xs opacity-70' });
      // A READOUT, not a control. The row itself is the control now, so a
      // second button inside it offering the same thing was the duplication
      // this list has already been through once with the title and the deck.
      const expand = h('span', {
      ...
      // THE ANSWER GETS A HEADER, AND AIR ABOVE IT. The mark rode inline at the
      // head of the reply line, which put the two halves on adjacent lines with
      // one glyph between them and read as a continuation of the ask rather
      // than as the other side of it. Now the ask closes on its fill, `mt-2`
      // opens the exchange, and the reply announces itself before speaking.
      //
      // The WORD is here where the deck drops it. chat-render's dense mode
      // omits the role word for both chat roles, because an alternating
      // transcript has four other carriers saying which is which: the icon, the
      // rail, the indent, and mono against prose. A row has none of that. It
      // holds exactly one ask and one line of one reply, never alternating, so
      // the glyph alone was carrying a distinction the reader had no rhythm to
      // learn. Clay, at the size the strip's own labels take.
      //
      // THE CLAMP GOES ON THE TEXT, not on a flex row. `flex` and
      // `line-clamp-1` both set `display`, and one of them wins: the same
      // collision that made the ask's clamp inert earlier today, made again
      // three hours later on the line beneath it.
      const reply = answer ? h('div', { class: 'mt-2' },
        h('div', { class: 'flex items-center gap-1 text-[11px] font-medium mb-0.5', style: 'color:' + CLAY },
          h('i', { class: 'ph ph-sparkle text-[11px]' }), h('span', {}, 'Claude')),
        h('div', { class: 'line-clamp-1 text-[13px] leading-snug text-base-content/70' }, answer)) : '';
      ...
      const setOpen = (want) => {
        ...
        if (reply) reply.classList.toggle('hidden', want);
      ...
      const row = h('button', {
        class: 'flex min-w-0 grow items-start gap-2.5 text-left',
        'aria-label': 'Open card ' + (ci + 1),
      },
        h('span', { class: 'font-mono text-[11px] tabular-nums text-base-content/30 w-4 shrink-0 pt-0.5 text-right' },
          String(ci + 1)),
        h('div', { class: 'min-w-0 flex-1' },
          ...
          h('div', { class: 'mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-1' },
      ...
      //
      // WHAT IS IN, SHOWN BY THE ROW rather than by a control on it. The tint
      // rides the head, not the whole card: over an expanded card it filled the
      // screen and read as a selected block of text rather than as a mark on a
      // list, which is the opposite of what it is for.
      const head = h('div', { class: 'flex items-start gap-2.5 rounded-md px-1.5 -mx-1.5 py-1 transition-colors' },
        row, go);
      const box = h('div', {
