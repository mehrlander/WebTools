...
(() => {
  ...
  function roleTag(role, size = 'text-[11px]') {
  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      const title = h(opts.onOpen ? 'button' : 'div', {
        ...
        class: 'block w-full text-left leading-snug '
      ...
      // all. The vertical rhythm separates a run of work rows on its own.
      list.append(h('div', {
        class: 'border-l-2 py-3.5 pr-2.5 '
             + (RULE[card.kind] || RULE.work)
             // Work rows step in behind the conversation they belong to, so an
             // ask reads as a heading and the doing reads as its body.
             + (card.kind === 'work' ? ' pl-5' : ' pl-2.5'),
      },
    ...
    function turnRow(t, i) {
      ...
      const body = h('div', { class: 'min-w-0 flex-1' },
        h('div', { class: 'flex items-baseline gap-2' },
          t.role === 'tool'
            ? h('span', { class: 'text-xs font-medium text-base-content/55' }, heading(t))
            : roleTag(t.role, 'text-xs'),
