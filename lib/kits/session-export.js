...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      //
      // THE CLAMP GOES ON THE TEXT, not on the flex row. `flex` and
      ...
      const reply = answer ? h('div', { class: 'mt-2 flex items-start gap-1.5' },
        h('i', { class: 'ph ph-sparkle shrink-0 text-[11px] mt-[3px]', style: 'color:' + CLAY }),
        h('span', { class: 'line-clamp-1 text-[13px] leading-snug text-base-content/70' }, answer)) : '';
      ...
      //
      // NEUTRAL, because primary is spoken for. The tint was bg-primary/10 and
      // the ask's fill is primary too, so an open row was a blue block sitting
      // in a blue field: one colour making two claims, and the weaker of them
      // winning. Selection is a state and takes the theme's own step up from
      // the page; authorship is a meaning and keeps the accent (rule 10).
      ...
      paint.push(() => {
        ...
        head.classList.toggle('bg-base-300/60', on);
        expand.classList.toggle('border-base-content/30', on);
