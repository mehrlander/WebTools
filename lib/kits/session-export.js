// session-export.js — read a session as a list of exchanges, and carry one out.
//
...
// phone. This is that route, and it is one button on the row.
//
// ── The picker that was here, and why one button replaced it ───────────────
//
// Until 2026-09-06 this file ran a selection MODE over the same list: a "Pick
// all / Clear" bar pinned above the rows, a pick circle on every row, and an
// export bar carrying seven render toggles, an Options panel, a Preview panel,
// a size readout, Download and Copy. It went through three designs in a week
// (checkbox per card and per turn, then open-is-selected, then a pick ring) and
// the reported objection never moved: it was hard to make sense of and it cost
// the top of the screen for something nobody reached for.
//
// The capability was real and the mode was the problem. A whole session runs
// to about 98,000 tokens, so there is no useful "copy it all", which is what
// the picker existed to work around; a single exchange runs about 11,000
// characters, which pastes. So the unit is the card, the control is one copy
// button in the row's own rail, and there is no state to explain: nothing is
// selected, nothing is remembered, and the bar that used to appear is gone.
// `markdown()` below is unchanged and still takes an arbitrary selection, so
// the capability the picker offered is intact for any caller that wants it.
//
...
// record's own capture gaps. A caller may turn it off; the row's copy button
// does not, because a fragment that does not say what it is a fragment of is
...
// result bodies were 78% of the bytes. The first three flags say which roles
// come along and the rest govern how much of each renders, which is why bodies
// default off and arguments default on: a Bash turn with no command in it says
// nothing at all. The row's button spends the defaults and offers no dial,
// since seven of them on screen is what this file just removed.
(() => {
  const SR = () => {
  ...
  //
  // Carrying one out is an affordance ON the row rather than a mode. Nothing is
  // selected, so there is no state to draw, no bar to appear and nothing to
  // clear: the list reads as an outline and the copy is a button on the row it
  // copies.
  ...
  function index(record, opts = {}) {
    ...
    // ── One card, out ─────────────────────────────────────────────────────
    // The whole of what the picker did, reduced to the call it was building.
    // `markdown` already takes an arbitrary set of turn indices, so one card is
    // its own contiguous run and nothing here has to know the shape of a turn.
    //
    // EXCEPT FOR `wanted`, WHICH HAS TO STAY ON THIS SIDE. The first three
    // flags say which ROLES come along and the rest say how much of each
    // renders, and that split is not cosmetic: emit() gives a tool turn a
    // heading whatever the render flags say, falling back to "(call only)", so
    // a card holding 34 Bash calls cannot be reduced to its ask by rendering
    // alone. Something has to drop the turn. That was the selection's job and
    // it is this function's now; without it, `index(rec, { args: false })`
    // silently changed from dropping tool turns to printing a stub per call.
    //
    // THE HEADER RIDES ALONG and is not optional here, whatever `o.head` says
    // for a caller. What lands on the clipboard is a fragment of a bounded
    // capture, and headBlock is the only thing that says so: which record, how
    // much of it, and what the recorder dropped. A fragment that arrives in
    // another session without that is the failure the block was written for.
    ...
    const cardMd = (ci) => {
      const take = [];
      (m.cards[ci] || []).forEach((t, k) => { if (wanted(t)) take.push(m.cardStart[ci] + k); });
      return markdown(record, take, { ...o, head: true, model: m });
    };
    ...
    // gone every row owns everything it draws, and the only thing that changes
    // after first paint is a copy button's own glyph for a second and a half.
    ...
    line.forEach((card, ci) => {
      ...
      // ── CARRYING ONE OUT, in the slot the pick circle held ────────────────
      // Same rail, same size, one verb instead of a mode. The circle marked a
      // card for a bar that then copied it; this copies it. There is no state
      // between the tap and the clipboard, so there is nothing to show as
      // selected and nothing to clear.
      //
      // ITS ONLY STATE IS THE ACKNOWLEDGEMENT, and it needs one: a clipboard
      // write is silent, and on a phone the pasteboard is somewhere else
      // entirely. The glyph answers for 1.4s, which is chat-render's own timing
      // for the same job on a code block, so the two agree.
      //
      // NOT AWAITED BEFORE THE WRITE. `copyText` delegates to kits/io.js, which
      // was fetched when this kit loaded precisely so a finger arriving later
      // spends no activation on an import; building the markdown is synchronous
      // and stays inside the gesture.
      // A CLIPBOARD, NOT ph-copy, and this is the one place in the estate that
      // parts from that glyph. ph-copy is two overlapping rounded squares and
      // ph-cards-three is three stacked ones, and they sit 4px apart in this
      // rail: at 18px the pair is one silhouette twice, which was visible in
      // the first render of this button. Everywhere else the estate copies,
      // chat-render's code block and long-paste footers, the button stands
      // alone and ph-copy is right there.
      const copyIcon = h('i', { class: 'ph ph-clipboard-text text-lg' });
      const copy = h('button', {
        class: 'btn btn-ghost btn-sm btn-circle shrink-0 text-base-content/30 hover:text-primary',
        title: 'Copy card ' + (ci + 1) + ' as markdown',
        'aria-label': 'Copy card ' + (ci + 1) + ' as markdown',
      }, copyIcon);
      let copyT = null;
      copy.addEventListener('click', async () => {
        ...
        copyIcon.className = 'ph text-lg ' + (ok ? 'ph-check' : 'ph-warning');
        copy.classList.toggle('text-base-content/30', false);
        copy.classList.toggle(ok ? 'text-primary' : 'text-warning', true);
        clearTimeout(copyT);
        copyT = setTimeout(() => {
          copyIcon.className = 'ph ph-clipboard-text text-lg';
          copy.classList.remove('text-primary', 'text-warning');
          copy.classList.add('text-base-content/30');
        }, 1400);
      });
      const rail = h('div', { class: 'flex shrink-0 items-start' }, copy, go);
    ...
    // getting too big before pasting. That question does not arise for one
    // card, which is why the unit changed rather than the readout moving.
    //
    ...
    // emits. A host that wants them passes them to `index`.
  ...
  // from, which is where each row's copy button is. The deck keeps its own
  // index sheet on the header mark, and that is not a third copy: it is a
  // dropdown that jumps between slides.
