  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      const answer = last && (window.readAloud?.speechText?.(last.md) || last.md || '')
        .replace(/\s+/g, ' ').trim();
      // NO LEFT PAD. The turns inside carry chat-render's own 17px lead indent,
      // which is the row's gutter above; a pad here would push the open body
      // off the edge the closed row establishes.
      const turnBox = h('div', { class: 'hidden flex-col gap-0.5 pt-1 pr-1' });
      ...
      const head = h('div', { class: 'flex items-start gap-2.5' }, row, go);
      const box = h('div', {
