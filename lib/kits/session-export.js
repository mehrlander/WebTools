...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      const replyMark = answer ? h('i', { class: 'ph ph-sparkle mt-[11px] text-[11px] leading-none',
                                          style: 'color:' + CLAY }) : null;
      const replyText = answer ? h('span', {
        class: 'mt-2 line-clamp-1 text-[13px] leading-snug text-base-content/70' }, answer) : null;
      ...
      const row = h('button', {
        ...
        style: 'grid-template-columns:14px minmax(0,1fr)',
        ...
        h('div', { class: 'col-start-2 mt-1 flex flex-wrap items-center gap-x-2.5 gap-y-1' },
