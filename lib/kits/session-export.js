  ...
  function index(record, opts = {}) {
    ...
    line.forEach((card, ci) => {
      ...
      for (const c of closings) {
        ...
        //
        // `mt-[9px]` is measured, not guessed: the line beside it opens with
        // `mt-1`, so a 6px disc centres on a 15px line box at 4 + (15-6)/2, and
        // at 6px it sat 3.5px high against the text it labels.
        const mark = h('span', {
          class: 'mt-[9px] block size-[6px] shrink-0 justify-self-center rounded-full',
          'aria-hidden': 'true' });
