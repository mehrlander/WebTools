...
(() => {
  ...
  function index(record, opts = {}) {
    ...
    const peek = h('div', {
      class: 'hidden absolute left-2 right-2 z-30 max-h-[min(60vh,26rem)] overflow-y-auto '
    ...
    const shutPeek = () => {
      ...
      peek.remove();
      if (peekAt) peekAt.trigger.setAttribute('aria-expanded', 'false');
      peekAt = null;
    };
    ...
    line.forEach((card, ci) => {
      ...
      // ── The two triggers, wired now that the box exists ──────────────────
      // The ask and the reply are separate things to want, so each opens its
      // own panel and a second tap on the same one closes it. The state line
      // opens the REPLY's, because that is what it is: a passage inside the
      // reply, which is also why it is indented under it.
      //
      ...
      // ask reads under the reply it is context for. In a panel titled YOU with
      // its own fill there is nothing to be under, and the reply's own panel is
      // 13px prose, so the shrink is a subordination with no superior. And the
      // verbatim whitespace shows every double space after a sentence, which
      // the row directly above collapses: the same words, twice, differently.
      //
      ...
      const askPanel = async (node) => {
        ...
        await SR().card([ask], node, { dense: true, collapse: 0 });
      ...
      if (ask) asTrigger(title, ci + ':ask', box, 'You', askPanel);
      if (replyText) {
        const fill = (node) => { SR().card(replies, node, { dense: true, collapse: 0 }); };
        asTrigger(replyText, ci + ':reply', box, 'Claude', fill);
        for (const el of stateEls) asTrigger(el, ci + ':reply', box, 'Claude', fill);
      }
