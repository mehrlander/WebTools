...
(() => {
  ...
  // THE KIND THIS RENDER ANNOUNCES, carried on the declaration so a reader of
  // the DOM gets the vocabulary without fetching a registry. kits/annotate.js
  // is that reader: its Section row is labelled from `aimLabel` and `aimHint`
  // rather than from strings of its own, so the aim says what it is about
  // instead of saying "Section" on a page that has four other structures in it.
  //
  // docs/routes-kinds.csv is the owner and tools/test/routes-manifest.test.mjs
  // holds the two together, the same arrangement docs/routes-routes.csv has
  // with toss-render's inlined TOSS_ROUTES: the registry declares, the code
  // inlines so no render path takes a fetch, and adding to one alone fails.
  ...
  // Declaring ANNOUNCES itself, because the surfaces that care are built before
  // the render happens. kits/annotate.js's card is mounted when the annotator is
  // enabled, and its Section chip has no business existing on a page with no
  // markdown render; a chip decided once at mount would be wrong on every deck,
  // where the slide is rendered later and again on every swipe. One event is
  // cheaper than a poll and honest in both directions.
  function declare(box, state) {
    if (!box) return box;
    box.setAttribute(DOC_ATTR, '');
    box.__mdDoc = { kind: KIND, ...state };
    try {
      const d = box.ownerDocument;
      if (d && d.defaultView && d.defaultView.CustomEvent) {
        d.dispatchEvent(new d.defaultView.CustomEvent('md-doc:declared', { detail: { box } }));
      }
    } catch { /* a detached or exotic document: the declaration still stands */ }
    return box;
  }

  // WHAT A DOCUMENT DECLARES, for a caller with no node in hand. `locate` reads
  // from a node up; this reads from the document down, which is the question a
  // CONTROL asks before anything has been aimed at: kits/annotate.js deciding
  // whether to offer its section aim, and the fab's launcher menu deciding
  // whether to offer that aim a gesture earlier still. Both had their own copy
  // of this query; the second was about to be written when it moved here, to
  // the kit that writes the declaration in the first place.
  //
  // SECTIONS ARE REQUIRED, not just the attribute. A heading-less markdown file
  // declares with none, and an aim that cannot hit anything is worse than an
  // absent one. Takes the document so a shell can ask about a framed subject,
  // which is a plain DOM read and needs no kit inside the frame.
  function declaredIn(doc) {
    try {
      for (const box of (doc || document).querySelectorAll('[' + DOC_ATTR + ']')) {
        const st = box.__mdDoc;
        if (st && (st.sections || []).length) {
          return { box, kind: st.kind || KIND, sections: st.sections, addr: st.addr || {} };
        }
      }
