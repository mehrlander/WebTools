...
`;
  ...
  // sees the artifact rather than a promise about it. Fixed height rather
  // than the panel's flex, because a frame with no intrinsic size collapses
  // to nothing inside a scrolling column.
  const renderFrame = (el) => {
    ...
    return `<div style="color:#71717a;font-size:11px;margin:0 0 6px">${line}</div>`
      + `<iframe data-peek-frame title="Rendered on its own" srcdoc="${esc(w.html)}" `
      + `style="width:100%;height:24vh;min-height:160px;border:1px solid #e4e4e7;border-radius:6px;background:#fff"></iframe>`;
  };
