...
(() => {
  ...
  const count = (sel) => { try { return S.doc.querySelectorAll(sel).length; } catch { return 0; } };
  ...
  const selectorFor = (el) => {
    if (el.id) { const s = '#' + cssEsc(el.id); return { sel: s, n: count(s) }; }
    let sel = segment(el);
    if (count(sel) === 1) return { sel, n: 1 };
    for (let p = el.parentElement; p && p !== S.doc.documentElement; p = p.parentElement) {
      sel = segment(p) + ' > ' + sel;
      if (count(sel) === 1) return { sel, n: 1 };
      if (p.id) break;   // an id prefix is as anchored as a selector gets
    }
    return { sel, n: count(sel) };
  };
  ...
  // Only the properties that change how a box behaves, plus the parent's
  // display when it is what positions this node. A full computed style is 340
  // entries and answers nothing.
  const layout = (el) => {
    const w = win();
    ...
    const p = el.parentElement && w.getComputedStyle(el.parentElement).display;
    if (p && /flex|grid/.test(p)) out.parent = p;
    return out;
  };
  ...
  const json = () => {
    const el = current();
    if (!el) return null;
    return { format: 'peek/1', at: new Date().toISOString(), url: S.doc.location?.href || '',
  ...
  const build = (el) => {
    const c = [];
    for (let n = el; n && n.tagName && n !== S.doc.documentElement; n = n.parentElement) c.push(n);
    return c;
  };
