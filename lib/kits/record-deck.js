...
(() => {
  ...
  // The headline: which field names the thing. First non-numeric column, which
  // in practice is the name (`vendor`, `agency`, `path`), falling back to the
  // first column of any kind, because a card with no headline makes the reader
  // work out which record they are on from the body every single swipe.
  const pickTitleField = (columns) =>
    (columns.find(c => !c.num) || columns[0] || {}).field;
  ...
  function open(o = {}) {
    ...
    const opts = { columns, titleField: o.titleField || pickTitleField(columns), describe: o.describe };
