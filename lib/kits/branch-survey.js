...
(() => {
  ...
  //            survey does not make. So it reports the fact and not a cause.
  ...
  // and the survey compares for its unique-path set, and both responses carry
  ...
  function fileStats(files) {
    ...
    for (const f of files || []) {
      ...
      if (st === 'added' || st === 'copied') s.added++;
      else if (st === 'removed') s.removed++;
      else { s.changed++; if (st === 'renamed') s.renamed++; }
    }
    return s;
  }
  ...
  // ── WHEN A BRANCH HAS TO BE SURVEYED AGAIN ────────────────────────────────
  ...
  // calls re-surveying branches whose tips had not moved in weeks.
  //
  ...
  function needsSurvey(b, prior, mainMoved){
    const row = prior.get(b.name);
    if (!row || !row.sha || !b.sha) return true;      // never surveyed, or no tip to compare
  ...
  // once. A branch whose survey failed will fail again for the same reason: a
  ...
  // branch review always surveys live and is the way to force the issue.
  ...
  async function surveyOlder(gh, opts = {}){
    ...
    // moved, so the honest default is to survey.
    const mainMoved = !(opts.mainSha && opts.priorMainSha && opts.mainSha === opts.priorMainSha);
    const queue = older.slice(0, cap);
    const live = queue.filter(b => needsSurvey(b, prior, mainMoved));
    live.push(...pickRetries(queue, prior, live, opts.errorRetry ?? 0));
    // The default tree is one call, and it is the survey's shared input; skip it
    ...
    return { truncated, rows: rows.filter(Boolean), surveyed: live.length, carried: queue.length - live.length };
  }
  ...
  window.BranchSurvey = {
    ...
    treeSets, PATH_STATES, pathStates, countStates, landedSignal, fileStats,
    classify, daysAgo, firstCommitDate, compareFields, needsSurvey,
