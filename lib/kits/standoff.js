...
(() => {
  ...
  //
  // Four of the six preserve the invariants by construction. A split's halves
  // meet exactly where the parent was cut; a merge's survivor spans both; a note
  // touches no span. Only relabel, verdict and the uid space can break one, so
  // those are checked here. Anything authored elsewhere goes through apply().
  const ops = {
