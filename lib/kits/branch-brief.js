...
(() => {
  ...
  async function fetchBrief(gh, { repo, branch, base } = {}) {
    ...
    try {
      ...
      const list = await gh.req(`pulls?state=all&head=${encodeURIComponent(owner + ':' + branch)}&per_page=10`);
      pulls = (list || []).slice().sort((a, b) => (b.number || 0) - (a.number || 0));
    } catch { pulls = []; }
    return { compare, pull: pulls[0] || null, pulls, noBase };
  }
  ...
  window.BranchBrief = { KIND, readAuthored, state, assemble, fetchBrief, fetchPullTarget };
})();
