...
(() => {
  ...
  function assemble({ repo, branch, base, compare, pull, authored } = {}) {
    ...
    return {
      ...
      files,
      pr: pull ? { number: pull.number, title: pull.title || '', draft: !!pull.draft,
                   state: pull.state || 'open', body: pull.body || '' } : null,
  ...
  async function fetchBrief(gh, { repo, branch, base } = {}) {
    let compare = null, noBase = false;
    try {
      compare = await gh.compare(base, branch);
    } catch (e) {
    ...
    try {
      ...
      const list = await gh.req(`pulls?state=all&head=${encodeURIComponent(owner + ':' + branch)}&per_page=5`);
      pull = (list || [])[0] || null;
    } catch { pull = null; }
    return { compare, pull, noBase };
  }

  window.BranchBrief = { KIND, readAuthored, state, assemble, fetchBrief };
})();
