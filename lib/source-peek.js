...
//
// Attaches to window.SourcePeek, loaded via gh.load('source-peek.js').
...
(() => {
  const LINES = 20;      // excerpt height, in lines
  ...
  const CARD_CLS = 'fixed z-[70] w-80 max-w-[92vw] rounded-lg border border-base-300 ' +
  ...
  const HEAD_CLS = 'px-2.5 py-1.5 bg-base-200/60 border-b border-base-300 font-mono text-xs';
  const BODY_CODE = 'm-0 px-2.5 py-2 font-mono text-xs leading-snug max-h-64 ' +
                    'overflow-hidden whitespace-pre-wrap break-words';
  const BODY_MD = 'px-2.5 py-2 prose prose-sm max-w-none max-h-64 overflow-hidden';
  const BODY_NOTE = 'px-2.5 py-2 text-sm italic';
  const FOOT_CLS = 'px-2.5 py-1 border-t border-base-300 bg-base-200/40 ' +
                   'font-mono text-xs text-base-content/50';
  ...
  async function show(el) {
    ...
    paint(el, frame(addr, html + '<div class="' + FOOT_CLS + '">' + esc(b.note) + '</div>'));
  }
  ...
  function install() {
    ...
    // A card positioned from a rect goes wrong the moment anything moves.
    addEventListener('scroll', () => { if (target) hide(); }, true);
  ...
  window.SourcePeek = {
    ...
    kindOf, excerpt, shape, jsonText, fenceFrontmatter, body,
    _cache: cache,
  };

  // Self-installing on load: a page that loads this file wants the behavior,
  // and nothing about it fires until a peek-bearing icon is hovered.
