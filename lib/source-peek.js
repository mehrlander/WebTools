...
(() => {
  ...
  const BODY_CODE = 'm-0 px-3 py-2 font-mono text-[13px] leading-snug max-h-[24rem] ' +
  ...
  const BODY_MD = 'px-3 py-2 prose prose-sm max-w-none max-h-[24rem] overflow-auto';
  const BODY_NOTE = 'px-3 py-2 text-sm italic';
  const FOOT_CLS = 'px-3 py-1 border-t border-base-300 bg-base-200/40 ' +
