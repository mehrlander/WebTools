...
// does not (it is the state of a process). The same test excludes the Diff
// lens's per-side ref override, which says "show me this file at another ref
// right now" and describes a moment of looking rather than the set.
//
// Attaches to window.Surface, loaded via gh.load('surface.js').
(() => {
  ...
  // WHICH two. The per-side ref overrides are not recorded: they describe a
  // moment of looking, not the set.
