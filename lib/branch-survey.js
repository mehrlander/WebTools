...
(() => {
  ...
  async function surveyBranchLive(gh, branch, main, opts = {}){
    ...
    try {
      ...
      firstDate = firstCommitDate(d);
      // Compare returns exactly the commits this branch has and the default does
      // not, so the sessions read off them are the branch's own: no ancestor
      // walk, no bleed from the default branch, and no extra call. Measured over
      // 55 merged branches, 11% were worked across more than one session, so the
      // full list is worth carrying rather than just the newest.
      //
      // Past GitHub's 250-commit compare cap the list is a tail, so the sessions
      // are whatever authored the newest 250 and older ones are invisible. The
      // newest is still right (it is the tip end that survives the cap), so the
      // list stands but stops claiming to be complete. Same cap firstCommitDate
      // refuses to answer past, read from the same total_commits.
      sessions = sessionsIn(commits);
      sessionsExact = (d.total_commits ?? commits.length) <= commits.length;
    } catch (e) {
  ...
  window.BranchSurvey = {
    ...
    treeSets, landedSignal, classify, daysAgo, firstCommitDate,
