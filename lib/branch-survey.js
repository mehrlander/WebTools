...
(() => {
  ...
  function dropFileName(inbox, now) {
    let dir = typeof inbox === 'string' ? inbox.replace(/\/+$/, '') : '';
    if (!dir || dir.includes(':') || dir.includes('@')) dir = 'dump';
    const d = now || new Date(), p = (n) => String(n).padStart(2, '0');
    const stamp = d.getFullYear() + '-' + p(d.getMonth() + 1) + '-' + p(d.getDate())
      + '-' + p(d.getHours()) + p(d.getMinutes());
    return dir + '/' + stamp + '-drop.md';
  }
  ...
  window.BranchSurvey = {
    ...
    lifespanStart, lifespanTitle, dropFileName, dropFileUrl,
  };
})();
