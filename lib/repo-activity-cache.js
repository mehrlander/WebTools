...
(() => {
  ...
  function material(entry) {
    ...
    return {
      ...
      // The session rides the hash, not just the stored row. Without it, the
      // first crawl after per-branch sessions shipped would hash identically to
      // the session-less cache it replaces (same names, shas, and groups), skip
      // the commit, and leave the feature dark until a branch happened to move.
      survey: (entry.survey?.branches || []).map(b => [b.name, b.sha, b.group, b.aheadBy ?? '', (b.sessions || []).join(',') || b.session || '']),
    };
  }
