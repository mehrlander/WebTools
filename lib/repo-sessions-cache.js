...
// view can read that on open. A summary row is ~350 bytes, so the whole cache is
// smaller than one record, and the full record is fetched only when a reader
// opens one.
//
...
// to window.RepoSessionsCache, loaded via gh.load('repo-sessions-cache.js').
(() => {
  ...
  // carried only the transcript uuid, so the two could not be matched. It is
  // empty for a session that never committed, and for every schema-1 and
  // schema-2 record, which is why `branches` below stays the fallback join.
  function summarize(rec, sha) {
    ...
    return {
      ...
      // count and `files` the busiest few; a consumer wanting the whole set
      // (the docs registry's readership column) reads the records, not this.
  ...
  function fileAttention(rows, cap = 200) {
    const by = new Map();
    for (const row of rows || []) {
      for (const [path, n] of row.files || []) {
        let e = by.get(path);
  ...
  function stalePaths(prev, listing) {
    ...
    return (listing || [])
      .filter(e => e && e.path && have[e.path]?.sha !== e.sha)
      .map(e => e.path);
  }
  ...
  window.RepoSessionsCache = {
    CACHE_PATH, ROW_CAP, TOOLS_KEPT, FILES_KEPT,
    hash, topN, fileWeight, summarize, durationMins, fileAttention,
