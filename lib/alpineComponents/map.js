...
<!-- Say which axis this is. The badge on every row below is a KIND,
          ...
          <section x-show="mapTab==='injection'">
            ...
            <template x-if="injection">
              <div class="flex flex-col gap-8 max-w-4xl">
                ...
                <div x-show="!injObserved && injObservedErr"
                     class="text-base text-base-content/60 border border-base-300 rounded-lg px-3 py-2 bg-base-100"
                     x-text="injObservedErr"></div>
      ...
      // on this tab.
      injObserved: null,
      // A WHOLE SENTENCE, not a clause. These were fragments spliced onto a
      // shared trailing phrase, which made each one grammatical and none of them
      // legible: "the sessions cache has not been re-crawled since this reading
      // was added, so this container's reading is the only evidence here" uses
      // three internal words (re-crawled, reading, container) and tells a reader
      // nothing about what to do. A message a reader has to already understand
      // the system to parse is not a message.
      injObservedErr: '',
      async loadObserved(){
        if (this.injObserved) return;
        if (!this.hasToken()) {
          this.injObservedErr = 'Reading what sessions actually received needs a GitHub token, '
            + 'since the session records are in a private repository. Everything above is '
            + 'measured from this checkout and needs no token.';
          return;
        }
        try {
          ...
          // Not an error and not an empty result: the records hold the answer and
          // the summary in front of them does not carry it yet. Say which, and
          // say what closes it, or a reader reasonably concludes no session ever
          // received anything.
          if (!obs.sessions || obs.sessions === obs.unhealed) {
            const n = obs.unhealed || 0;
            this.injObservedErr = 'The session records hold this, but the cache summarising them '
              + 'does not carry it yet: these rows were written before the summary tracked which '
              + 'channel each document came down. The app rebuilds that cache as it crawls the '
              + 'store, so this panel fills itself in on the next pass'
              + (n ? `. ${n} recorded sessions are waiting on it.` : '.');
            return;
          }
          ...
          this.injObservedErr = 'The sessions cache could not be read, so what sessions actually '
            + 'received is unknown here: ' + (e?.message || e) + '. Everything above is measured '
            + 'from this checkout and is unaffected.';
        }
      },
