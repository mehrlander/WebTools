  ...
  Alpine.data('map', function() {
    ...
    return {
        ...
        <div class="w-full">
          ...
          <section x-show="mapTab==='transport'">
            <div class="flex items-center gap-2 mb-5 flex-wrap">
              <code class="text-sm text-base-content/50" x-text="routes ? routes.renderer : 'pages/toss-render.html'"></code>
              <a :href="hubUrl('docs/routes.json')" target="_blank" rel="noopener"
                 class="text-base-content/40 hover:text-primary" title="Curate the manifest (docs/routes.json)">
                <i class="ph ph-github-logo"></i></a>
            </div>
                  ...
                  <div class="flex flex-col gap-1">
                    <template x-for="r in routes.routes" :key="r.key">
                      <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60 group">
                        ...
                        <a :href="routeGh(r)" target="_blank" rel="noopener" title="Open the renderer on GitHub"
                           class="opacity-0 group-hover:opacity-100 text-base-content/30 hover:text-primary transition-opacity shrink-0 mt-1">
                          <i class="ph ph-github-logo"></i></a>
                      </div>
                    </template>
      ...
      async loadRoutes(){
        ...
        try {
          const gh = new window.GH({ token: window.TOKEN, repo: this.hub(), ref: 'main' });
          const parsed = JSON.parse((await gh.get('docs/routes.json')).text);
