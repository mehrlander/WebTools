document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    //               absence of one). Reach is the one field here that is
    ...
    return {
      description: 'Map view: the coordination layer made inspectable, in five tabs. The portable set (docs/portable.json, each row openable in the shell viewer); Surfacing (docs/surfacing.json, the gated index of SURFACING.md\'s primitives, the prose staying authoritative); Showing (docs/routes.json): which link reaches which kind of change and what each one misses, then the address grammar, toss-render\'s delivery modes and their trust postures, and the toss routes; and Docs (docs/docs.json): the documentation registry, every doc\'s subject, status (living, record, or measured), reach (injected, in context, by a skill, by the app, or orphan; derived from the repo and filterable from the strip at the top), size in words with its share of the folder, readership (distinct sessions that opened the file, from the private registry\'s sessions cache; token-gated, and an injected doc says so rather than reading the zero no file tool can avoid giving it), and maintenance, plus the shared-claims table with per-repetition checks. Reach counts files and words weighs them, and the strip shows both because on this folder they disagree: the orphans are the larger count and the smaller mass; readership is the third of that set, since reach says who can get to a doc and readership says who did. And Tests (docs/tests.json): the same census pointed at the suite, every check\'s kind (gate, lockstep, tool, kit, behavior, component, guard) and what breaks if it is deleted, with assertions, method, runner and boot-smoke count derived from the files and gated against the registry. The strip cuts the total by kind, since a pass count cannot tell a boot check from an adversarial gate; a browser check reports no assertion count rather than zero, because test() is not its unit. Per-repo scope and adoption live on the Repos cards. The operational face of the constellation doctrine.',
      ...
      template: `
        <div class="w-full">
          ...
          <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 mb-6 w-fit flex-wrap" role="tablist">
            ...
            <button role="tab" @click="mapTab='surfacing'; loadSurf()"
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors"
            ...
            <button role="tab" @click="mapTab='showing'; loadRoutes()"
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors"
            ...
            <button role="tab" @click="mapTab='docs'; loadDocsReg()"
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors"
            ...
            <button role="tab" @click="mapTab='tests'; loadTestsReg()"
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors"
          ...
          <!-- Two tables from docs/docs.json. Documents: what each file under
               ...
               construction (the census test). Claims: statements living in
               more than one place, each with one authoritative carrier and
               typed repetitions; an absent check renders as visibly absent,
               because an unchecked copy should look unchecked every time this
               tab is opened. -->
            ...
            <template x-if="docsReg">
              <div class="flex flex-col gap-8">
                ...
                <div>
                  ...
                  <!-- The readership caveats, stated once beside the column
                       rather than per row. Without them the numbers invert the
                       truth on exactly the files that matter most: the two
                       injected docs are the most-read in the estate and are the
                       two this can never count. Coverage is partial by
                       construction (only sessions the recorder captured, only
                       schema-3 records, only the four file tools), so the line
                       says what the eye is counting rather than implying a
                       census. -->
                  <p x-show="docReads" class="text-sm text-base-content/45 mt-2 max-w-4xl">
                    <i class="ph ph-eye"></i>
                    <span x-text="'Distinct sessions that opened each file, across ' + docReadsSessions + ' recorded sessions'"></span><span x-show="docReadsAt" x-text="' (as of ' + (docReadsAt || '').slice(0, 10) + ')'"></span>.
                    Counts Read, Edit, Write and NotebookEdit only: a file opened through a shell
                    command, read by a subagent, or injected at session start does not appear.
                    An injected doc therefore says so rather than reading zero.
                  </p>
                </div>

                <div class="grid gap-x-8 gap-y-6 lg:grid-cols-2">
                  <template x-for="grp in docGroups" :key="grp.dir">
                    <div>
                      <h3 class="text-sm font-semibold uppercase tracking-wide text-base-content/40 mb-2" x-text="grp.dir"></h3>
                      <div class="flex flex-col gap-1">
                        <template x-for="d in grp.docs" :key="d.path">
                          <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60 group">
                            ...
                            <div class="min-w-0 flex-1">
                              <div class="flex items-center gap-2 flex-wrap">
                                <button type="button" class="text-base font-medium hover:text-primary text-left"
                                        @click="openHubFile(d.path)" x-text="docTitle(d)"></button>
                                ...
                                <!-- Readership: distinct sessions that opened
                                     this file. Absent entirely without a token,
                                     since the count lives in the private
                                     registry and an empty column would read as
                                     "nobody opened it". -->
                                <span x-show="docReads" class="text-sm tabular-nums flex items-center gap-1"
                                      :class="docReadTone(d)" :title="docReadHint(d)">
                                  <i class="ph ph-eye text-sm"></i><span x-text="docReadLabel(d)"></span>
                                </span>
                              </div>
                              <p class="text-base text-base-content/60" x-text="d.subject"></p>
                              <p class="text-sm text-base-content/40" x-text="d.maintenance"></p>
                            </div>
      ...
      async loadDocReads(){
        ...
        try {
          ...
          this.docReads = by;
          this.docReadsAt = cache.generatedAt || '';
      ...
      // What the column says, and the honest empty. An injected doc reads zero
      // by construction: it arrives in a session's context without a file tool
      // ever opening it, so a bare "0" here would rank the two most-read files
      // in the estate last. Reach already carries that fact, so the cell defers
      // to it rather than reporting a number it knows is wrong.
      docReadLabel(d){
      ...
      docReadHint(d){
        ...
        const a = this.docRead(d);
        if (!a) return 'No recorded session opened this file with a file tool.';
        return 'Opened in ' + a.sessions + ' of ' + this.docReadsSessions + ' recorded sessions ('
          + a.count + ' accesses), last ' + (a.last || '').slice(0, 10);
      },
      docReadTone(d){
        if (d.reach === 'injected') return 'text-base-content/30 italic';
        return this.docRead(d) ? 'text-base-content/50' : 'text-base-content/20';
      },
      // Grouped by directory, root first, so the census reads the way the
      // folder does; row order inside a group follows the registry.
      get docGroups(){
        const groups = new Map();
        for (const d of (this.docsReg?.documents || [])) {
          if (this.docReach && d.reach !== this.docReach) continue;
