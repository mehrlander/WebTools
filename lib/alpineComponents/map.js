...
<!-- Say which axis this is. The badge on every row below is a KIND,
                          ...
                          <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60 group">
                            ...
                            <div class="min-w-0 flex-1">
                              ...
                              <div x-show="testExplained && explainOf(t).length"
                                ...
                                <template x-for="(c, i) in explainOf(t)" :key="i">
                                  <div class="flex flex-col gap-0.5 text-base">
                                    <div class="flex items-baseline gap-2 flex-wrap">
                                      <span class="font-medium" x-text="c.kind"></span>
                                      <span class="text-sm text-base-content/50" x-text="c.term"></span>
                                    </div>
      ...
      async loadTestsReg(){
        ...
        try {
          ...
          try {
            const rows = window.Csv.rows((await gh.get(TESTS_EXPLAIN)).text)
              .filter(r => r.check && r.kind);
            const byPath = new Map();
            for (const r of rows) {
              const file = r.check.split(/[\s(:]/)[0];
              if (!byPath.has(file)) byPath.set(file, []);
              byPath.get(file).push(r);
            }
            this.testExplain = byPath.size ? byPath : null;
      ...
      get testExplainCount(){
        let n = 0; for (const rows of (this.testExplain || new Map()).values()) n += rows.length; return n;
      },
