document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    return {
      description: 'Map view: the coordination layer made inspectable, in eight tabs. Portable (docs/portable.csv, the to-go set, each row openable in the shell viewer); Surfacing (docs/surfacing.csv, the gated index of SURFACING.md\'s primitives, the prose staying authoritative); Showing (docs/routes.json): which link reaches which kind of change and what each one misses, then the address grammar, toss-render\'s delivery modes and their trust postures, and the toss routes; and Docs (docs/docs.csv): the documentation registry, every doc\'s subject, status (living, record, or measured), reach (injected, in context, by a skill, by the app, or orphan; derived from the repo and filterable from the strip at the top), size in words with its share of the folder, readership (distinct sessions that opened the file, from the private registry\'s sessions cache; token-gated, and an injected doc says so rather than reading the zero no file tool can avoid giving it), and maintenance behind one details toggle for the whole census, all navigated from a folder rail whose rows roll up counts and words and carry their own GitHub links; a row\'s title opens the document in a fullscreen swipe deck paging the folder\'s files, while its GitHub icon, inline with the badges, carries the source peek. Owners (docs/owners.json, ?tab=claims): for a statement the coordination layer repeats, its one authoritative carrier, typed repetitions, and per-repetition checks, with an absent check rendered in the warning tone rather than omitted, under the registry\'s own written scope. Named Claims until 2026-08-09, when the registry reconciliation moved it out of docs/docs.csv and off a word the estate spends three other ways. Reach counts files and words weighs them, and the strip shows both because on this folder they disagree: the orphans are the larger count and the smaller mass; readership is the third of that set, since reach says who can get to a doc and readership says who did. And Tests (docs/tests.csv): the same census pointed at the suite, every check\'s kind (gate, lockstep, tool, kit, behavior, component, guard) and what breaks if it is deleted, with assertions, method, runner and boot-smoke count derived from the files and gated against the registry. The strip cuts the total by kind, since a pass count cannot tell a boot check from an adversarial gate; a browser check reports no assertion count rather than zero, because test() is not its unit. And Harness (docs/harness.csv): the harness census, one row per code file under tools/ and scripts/ (tools/test/ excluded; the Tests census owns it), each row an authored role plus derived layer, lines, invocation route (npm script, scenario driver, imported helper, argv, or none found, the warning state), whether it writes files, whether prose names it and a test exercises it; navigated from the same folder rail as Docs (the tree as it exists on disk, counts and blank-role figures rolled up to ancestors), with the invocation pills as a separate filter layer that re-weights the rail. And Registries (docs/properties.json): the declaration table the other seven tabs hang off, one card per registry (title, gloss, kind, carrier, gate, target grain, scope, per-declaration chips), grouped by area (files or names), with derived totals rather than prose-carried counts, and per row the derived renders_in list: the app files that name the carrier in code, each a peekable link, with a warning badge and a strip figure for carriers no app surface reads, since an unread claim is where the registry audits keep finding rot. Per-repo scope and adoption live on the Repos cards. The operational face of the constellation doctrine.',
      ...
      template: `
        <div class="w-full">
          ...
          <!-- ── Registries: the table the other seven tabs hang off ───────
               ...
               that says what a manifest IS: carrier, target grain, scope,
          ...
          <section x-show="mapTab==='registries'">
            ...
            <p class="text-sm text-base-content/50 mb-5 max-w-4xl">
              ...
              is one committed file holding a row per thing it describes; its gate is the test that
              fails when the file and the repo disagree, and the word none says plainly that nothing does.
              A row asserts properties, and one property about one thing answers to exactly one
              registry, which is checked rather than only written down. Both files below are in the
              list: since 2026-08-16 the index governs itself, so kind carries a declared value
              domain rather than a list held in a test. Where each registry renders is derived from
              the code and shown per row; a file no app surface reads wears the warning, since an
              unread claim is where the audits keep finding rot.
            </p>
            ...
            <template x-if="propsReg">
              <div class="max-w-4xl">
                <div class="flex items-center gap-4 flex-wrap mb-5 text-sm">
                  ...
                  <span class="text-base-content/50"><span x-text="registryTotals.census"></span> census / <span x-text="registryTotals.catalog"></span> catalog</span>
                  ...
                  <span x-show="registryTotals.unrendered" class="text-warning"
                        title="Carriers no file under lib/ or pages/ names in code: committed and gated, read by nobody. The number this tab exists to make impossible to ignore.">
                      ...
                      <template x-for="r in area.rows" :key="r.id">
                        <div class="border border-base-300 rounded-lg p-3 bg-base-100">
                          <div class="flex items-baseline gap-2 flex-wrap">
                            ...
                            <span class="badge badge-sm" :class="r.kind === 'census' ? 'badge-success' : 'badge-info'" x-text="r.kind"></span>
                            <span class="grow"></span>
                            <span x-show="!(r.renders_in || []).length" class="badge badge-warning badge-sm"
                                  title="No file under lib/ or pages/ names this carrier in code, so nothing in the app reads or shows it. A GitHub-rendered projection or a runtime-configured read does not count, and may be the honest answer; the badge asks the question rather than settling it.">no app surface</span>
                            <a x-show="r.gate" :href="hubUrl(r.gate)" :data-peek="peek(r.gate)" target="_blank" rel="noopener"
                               class="text-base-content/30 hover:text-primary" :title="'Gated by ' + r.gate">
                              <i class="ph ph-shield-check text-lg"></i></a>
                            <span x-show="!r.gate" class="badge badge-warning badge-sm" title="No gate holds this carrier">no gate</span>
                          </div>
                          ...
                          <div class="flex items-baseline gap-2 flex-wrap mt-2">
                            ...
                            <a :href="hubUrl(r.carrier)" :data-peek="peek(r.carrier)" target="_blank" rel="noopener"
                               class="font-mono text-sm text-base-content/60 hover:text-primary" x-text="r.carrier"></a>
                          </div>
                          ...
                          <div class="flex flex-wrap gap-1 mt-2">
                            <template x-for="d in r.decls" :key="d.property">
                              <span class="badge badge-ghost badge-sm font-mono"
                                    :title="d.gloss + '  [' + d.mode + ', required: ' + d.required + (d.values ? ', domain: ' + d.values.join(' | ') : '') + ']'"
                                    x-text="d.property + (d.values ? ' \u25cf' : '')"></span>
                            </template>
                          </div>
                        </div>
                      </template>
          ...
          <section x-show="mapTab==='tests'">
            ...
            <template x-if="testsReg">
              <div class="flex flex-col gap-8">
                ...
                <div class="flex flex-col gap-2">
                  <!-- The header line: what is true of the whole census. The
                  ...
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <!-- One toggle for the whole census, the same call the docs
                         census makes about maintenance: the assertion list is
                  ...
                  <template x-for="d in testDimensions" :key="d.key">
                    <div class="flex items-baseline gap-2 flex-wrap">
                      <span class="text-sm uppercase tracking-wide text-base-content/35 w-16 shrink-0"
                  ...
                  <template x-for="d in testPicked" :key="d.key">
                    <p class="text-sm text-base-content/50 max-w-4xl">
                      <span class="text-base-content/35" x-text="d.label + ' ' + testPick[d.key] + ': '"></span><span
...
<!-- Say which axis this is. The badge on every row below is a KIND,
                              ...
                              <ol x-show="testNames && t.assertion_names"
                                ...
                                <template x-for="(n, i) in (t.assertion_names || [])" :key="i">
                                  <li class="flex gap-2 text-sm"
                                    ...
                                    <span class="tabular-nums text-base-content/25 shrink-0" x-text="i + 1"></span>
                                    <span x-text="n"></span>
      ...
      async loadManifest(){
        ...
        try {
          ...
          // The set is a crosswalk: it curates WHICH files travel and how a
          // consumer takes each one, and leaves the description of a file to
          // whichever census owns it. Nine scripts are described by the harness
          // census, so their rows here are blank and the role is joined below.
          ...
          const [set, harness] = await Promise.all([
            gh.get(SET_MANIFEST),
            gh.get(TOOLS_MANIFEST).catch(() => null),   // the harness census, for the joined role
          ]);
      ...
      // A row's own role where it has one, the owning census's where it does
      ...
      async loadPropsReg(){
        ...
        try {
          const gh = new window.GH({ token: window.TOKEN, repo: this.hub(), ref: useRef() });
          const [rawReg, rawProp] = await Promise.all([
            gh.get(PROPS_MANIFEST).then(r => r.text),
            gh.get(PROPS_DECLS).then(r => r.text),
          ]);
          const registries = window.Csv.rows(rawReg).map(r => ({
            ...r,
            // The fragment half of `path`, for the three registries that still
            // share docs/routes.json. It goes when each is its own file.
            file: r.path.split('#')[0],
          ...
          this.propsReg = { registries, properties };
        } catch (e) {
      ...
      get registryTotals(){
        ...
        return {
          ...
          census: rows.filter(r => r.kind === 'census').length,
          // A crosswalk IS a catalog (one that curates which targets belong and
          // leaves their description to the census that owns them), so a filter
          // for catalogs has to match it. It is counted separately too, because
          // until 2026-08-16 the word was defined in registries.md and held by
          // no row, which is how a vocabulary entry goes unused for months.
          catalog: rows.filter(r => r.kind === 'catalog' || r.kind === 'crosswalk').length,
          crosswalk: rows.filter(r => r.kind === 'crosswalk').length,
          ...
          gated: rows.filter(r => r.gate && r.gate !== 'none').length,
      ...
      // shape as the harness census's invocation pills over its layer rail.
