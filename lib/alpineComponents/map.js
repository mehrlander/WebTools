document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // restated here.
    //
    // No tone in the strip, on any dimension. Kind chips carried the kind badge
    // colour until 2026-08-10 on the argument that it let a chip be matched to
    // the rows it selects; rendered against the alternatives, that argument does
    // not survive. The chip and the row already share the WORD, which matches
    // unambiguously, so the colour was redundant with the label beside it, and
    // buying redundancy at the price of one row looking unlike the other two is
    // a bad trade. Tinting method and runner instead was the other way out and
    // is worse: their colours would decode to nothing, since no row wears them.
    // Colour still means kind, on the per-row badge, where it is the only
    // carrier and not a duplicate.
    const DIMENSIONS = [
      { key: 'kind', label: 'kind', question: 'what genre of check is this',
                      ...
                      <template x-for="c in d.chips" :key="c.value">
                        <button type="button" @click="toggleDim(d.key, c.value)" :title="c.hint"
                          ...
                          <span class="flex items-baseline gap-0.5">
                            ...
                            <span class="text-sm text-base-content/40 px-0.5">|</span>
