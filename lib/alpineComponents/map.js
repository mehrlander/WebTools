document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    async function renderDoc(host, path, text, addr){
      ...
      host.innerHTML = '<pre class="text-sm font-mono whitespace-pre-wrap m-0">' + escHtml(body) + '</pre>';
    }
      ...
      openDeckMenu(btn){
        ...
        for (const r of rows) {
          ...
          b.innerHTML = '<i class="ph ' + r.icon + ' mt-0.5 shrink-0 text-base text-base-content/50"></i>'
            + '<span class="min-w-0 flex-1"><span class="block text-sm">' + escHtml(r.label) + '</span>'
            + '<span class="block truncate text-xs text-base-content/50">' + escHtml(r.hint || '') + '</span></span>';
