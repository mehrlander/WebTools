document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    //               moves when the estate improves, and because it made the
    //               app's own channel visible for the first time: ten docs are
    //               reached only by lib/ or pages/ naming them, which is what
    //               this view has been building since 2026-08.
    ...
    const REACH = {
      injected: { label: 'injected',   tone: 'badge-success', hint: 'in every session\'s context, unasked' },
      skill:    { label: 'by a skill', tone: 'badge-info',    hint: 'a skill names it, so invoking that skill pulls it' },
      app:      { label: 'by the app', tone: 'badge-primary', hint: 'a page or component names it' },
      orphan:   { label: 'orphan',     tone: 'badge-warning', hint: 'nothing points here; reachable only by browsing docs/' },
    };
    const REACH_ORDER = ['injected', 'skill', 'app', 'orphan'];

    return {
      description: 'Map view: the coordination layer made inspectable, in four tabs. The portable set (docs/portable.json, each row openable in the shell viewer); Surfacing (docs/surfacing.json, the gated index of SURFACING.md\'s primitives, the prose staying authoritative); Showing (docs/routes.json): which link reaches which kind of change and what each one misses, then the address grammar, toss-render\'s delivery modes and their trust postures, and the toss routes; and Docs (docs/docs.json): the documentation registry, every doc\'s subject, status (living, record, or measured), reach (injected, by a skill, by the app, or orphan; derived from the repo and filterable from the strip at the top), and maintenance, plus the shared-claims table with per-repetition checks. Per-repo scope and adoption live on the Repos cards. The operational face of the constellation doctrine.',
          ...
          <section x-show="mapTab==='docs'">
            ...
            <template x-if="docsReg">
              <div class="flex flex-col gap-8">
                ...
                <!-- Reach strip: the four channels with their counts, each a
                ...
                <div>
                  ...
                  <p class="text-sm text-base-content/40 mt-2 max-w-4xl">
                    How a reader reaches each file, derived from the repo by
                    <code>tools/build/docs-reach.mjs</code> and held to this registry by test.
                    An orphan is not dead: it is reachable from the generated index in the
                    docs folder, and that index is the only thing reaching it.
                  </p>
                </div>
      ...
      docReach: '',
      get docReachCounts(){
        const out = REACH_ORDER.map(key => ({ key, ...REACH[key], n: 0 }));
        for (const d of (this.docsReg?.documents || [])) {
