document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    //   Growth      the same corpus as a moving picture: pages/doc-growth.html
    //               framed, every markdown file a bubble over the repo's
    //               history. Docs answers "is this document growing", one row
    //               at a time; this answers "what is the whole corpus doing".
          ...
          <section x-show="mapTab==='showing'">
            ...
            <div class="flex items-center gap-2 mb-5 flex-wrap">
              ...
              <a :href="hubUrl(rendererPath)" :data-peek="peek(rendererPath)" target="_blank" rel="noopener"
                ...
                <i class="ph ph-github-logo"></i></a>
${regChip('SHOWING_MECHANISMS')}${regChip('ROUTES_MODES')}${regChip('ROUTES_ROUTES')}
          ...
          <!-- ── Growth: the same corpus as a moving picture ─────────────────
          ...
          <section x-show="mapTab==='growth'" class="flex flex-col gap-3">
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <a :href="hubUrl(GROWTH_PAYLOAD)" target="_blank" rel="noopener"
                ...
                <span x-text="GROWTH_PAYLOAD"></span><i class="ph ph-github-logo"></i></a>
              <div class="grow"></div>
            ...
            <!-- x-if, not x-show: opening the Map on another tab should never
                 fetch a payload nobody asked to see. -->
            <template x-if="growthSeen">
              <iframe :src="growthUrl" loading="lazy"
          ...
          <section x-show="mapTab==='skills'">
            <div class="flex items-center gap-2 mb-1 flex-wrap">
              ${regChip('SKILLS_MANIFEST')}
            </div>
            <p class="text-sm text-base-content/50 mb-3 max-w-4xl">
              Two sets, one search. Above, the library a session loads on demand, one row per
              directory under skills/, each carrying the trigger description its own SKILL.md owns,
              so searching here searches what a session matches on. Below, what the rest of the
              estate has committed in its own .claude/skills/, read from each repo's own manifest.
              Neither is the plugin's sixteen, which install in every session and are on the
              Portable tab: anything under .claude/skills/ registers as auto-fire, and the library
            ...
            <template x-if="skillsReg">
              <div class="max-w-4xl">
                ...
                <!-- The group strip. Counts re-weight under the query, so a
                     search shows WHERE its matches live before you read one,
                     and a group at nought is dimmed rather than dropped: a
                     disappearing strip is a moving target to click at. -->
                ...
                <!-- Two empty states, because the library going quiet stopped
                     meaning what it used to. A miss here is only a gap if the
                     estate half missed too: searching "fiscal" returns nothing
                     from the library and two from home, which is the case this
                     tab was extended to stop misreporting. -->
                <div x-show="!skillRows.length && !estateSkillGroups.length"
                     class="text-base text-base-content/40 py-6">
                  Nothing matches, in the library or the estate. The trigger text is what a session
                  reads, so a miss here is a real gap rather than a naming problem.
                </div>
                <div x-show="!skillRows.length && estateSkillGroups.length"
                     class="text-base text-base-content/40 py-6">
                  Nothing in the library. The estate answers below.
                </div>
                ...
                <!-- The estate half. No description column, and not an
                     oversight: a trigger description lives in each SKILL.md,
                     and fetching fifteen files across nine repos to fill a
                     column is the cost the manifest declaration exists to
                     avoid. The name links the file, which is one tap. -->
                <!-- The empty estate half is three different facts, and
                     rendering nothing said none of them: the tab then read
                     exactly as it did before the estate half existed, which is
                     how a reader concludes a change did not land. -->
                <div x-show="!hasToken()" class="mt-8 text-sm text-base-content/40">
                  <span class="font-semibold uppercase tracking-wide text-base-content/40">In the estate</span>
                  <p class="mt-1">Sign in to read what other repos have committed. The library above is public;
                  the estate half reads the private registry's crawl.</p>
                </div>
                <div x-show="hasToken() && estateSkills && !estateSkills.length"
                  ...
                  crawl has nothing to collect. Fifteen exist across home, chat-histories and wa-bills;
                  each repo declares its own, and the aggregate appears here once the crawl next runs.</p>
                </div>

                <template x-if="estateSkills && estateSkills.length">
                  <div class="mt-8">
                    <div class="flex items-center gap-3 flex-wrap mb-1">
                      ...
                      <span class="text-sm text-base-content/50">
                        ...
                        <span x-text="' across ' + estateSkillTotals.repos + ' repos'"></span></span>
                    ...
                    <p class="text-sm text-base-content/50 mb-3">
                      Declared by each repo in its own .web-tools.json and collected by the crawl,
                      never inspected from here: the hub aggregates, it does not go reading trees.
                    </p>
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      loadTab(tab){
        ...
        else if (tab === 'growth') this.growthSeen = true;
        ...
        else if (tab === 'skills') { this.loadSkillsReg(); this.loadEstateSkills(); }
      },
...
// Under ?use= Pages still serves the page file from main, so a ref has to go
      // the long way round through the toss renderer. Without one the direct path
      // is cheaper and needs no token.
      get growthUrl(){
        const ref = useRef();
        return ref && ref !== 'main'
          ? '../pages/toss-render.html#gh=' + this.hub() + '@' + ref + ':pages/doc-growth.html'
          : '../pages/doc-growth.html';
      },
      ...
      // Until 2026-08-27 this rendered the difference as the literal word
      // "injected" on the two rows tagged that way in the registry, because a
      // count would have ranked the estate's two most-read files last. The
      // record now carries startup context as data, so the word is a fallback
      // for a cache that predates it rather than the only answer available.
      docReadLabel(d){
      ...
      docReadHint(d){
        const p = this.docPresent(d), a = this.docRead(d);
        if (!p && !a) {
          return d.reach === 'injected'
            ? 'Arrives in every session\'s context by injection, which no file tool records. Not measurable here, and not zero.'
            : '';
        }
        const out = [];
        if (p) {
          ...
          const how = p.receipt && p.reconstructed ? 'by receipt and reconstruction'
            : p.receipt ? 'by receipt from the injecting hook'
            : 'by reconstruction, not observed';
        ...
        if (a) {
          out.push('Opened in ' + a.sessions + ' of ' + this.docReadsSessions + ' recorded sessions ('
            ...
            + '. Counts the file tools only (Read, Edit, Write, NotebookEdit).');
        }
