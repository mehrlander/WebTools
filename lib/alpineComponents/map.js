                      ...
                      <template x-for="c in d.chips" :key="c.value">
                        <button type="button" @click="toggleDim(d.key, c.value)" :title="c.hint"
                          ...
                          <!-- The two counts are different units, so they read as
                               ...
                               naming both, once, could sit beside neither. -->
                          <span class="flex items-baseline gap-0.5">
                            <span x-show="d.tone" class="badge badge-sm" :class="c.tone"
                                  :title="c.files + ' files'" x-text="c.files"></span>
                            <span x-show="!d.tone" class="text-sm tabular-nums text-base-content/50"
                                  :title="c.files + ' files'" x-text="c.files"></span>
                            <span class="text-sm text-base-content/15 px-0.5">|</span>
                            <!-- n/a in the same gray as the count it stands in for.
                                 A browser check asserts in its own harness, so
                                 test() is not its unit and it HAS no count; an
                                 empty slot reads as a load failure and a zero
                                 would claim it checks nothing. -->
