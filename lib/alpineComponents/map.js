document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    //               from docs/owners.json. Its own carrier since 2026-08-09;
    ...
    // second `claims` block inside docs/docs.json, which the registry model
    ...
    const OWNERS_MANIFEST = 'docs/owners.json';
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      async loadManifest(){
        ...
        try {
          ...
          const [set, harness] = await Promise.all([
            gh.get(SET_MANIFEST),
            gh.get(TOOLS_MANIFEST).catch(() => null),   // docs/harness.json
          ]);
          ...
          this.manifest = JSON.parse(set.text);
          this.harnessRoles = harness ? Object.fromEntries(
            (JSON.parse(harness.text).tools || []).map(t => [t.path, t.role])) : {};
        } catch (e) {
      ...
      async loadOwnersReg(){
        ...
        try {
          const gh = new window.GH({ token: window.TOKEN, repo: this.hub(), ref: useRef() });
          const raw = (await gh.get(OWNERS_MANIFEST)).text;
          const parsed = JSON.parse(raw);
          if (!parsed || !Array.isArray(parsed.owners)) throw new Error('no owners table');
          window.SourcePeek?.seed(this.peek(OWNERS_MANIFEST), raw);
          this.ownersReg = parsed;
        } catch (e) {
