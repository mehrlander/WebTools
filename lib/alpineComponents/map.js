document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // reconciliation had made it worth reading: fifteen registries, each with a
    // target grain, a scope, a gate, and two enforcement layers behind it.
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      // The three areas, each with the question that decides membership. The
      // rule is the point: without one, every added registry re-litigates the
      // grouping. `area` is a declared field on the registry row and gated, so
      // this reads the data rather than holding a list of its own.
      get registryAreas(){
        const AREAS = [
          ['contents',     'Repository contents', 'Is the target an artifact in this tree?'],
          ['conventions',  'Conventions',         'Is the target a name the estate settled on?'],
          ['presentation', 'Delivery',            'Is the target a way of getting something in front of a reader?'],
        ];
