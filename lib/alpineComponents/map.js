      ...
      template: `
        <div class="w-full">
          ...
          <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 mb-6 w-fit flex-wrap" role="tablist">
            <button role="tab" @click="setTab('aims')"
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors"
                    :class="mapTab==='aims' ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60 hover:text-base-content'">
              <i class="ph ph-target text-lg"></i>Aims</button>
            ...
            <button role="tab" @click="setTab('claims')"
              ...
              <i class="ph ph-graph text-lg"></i>Themes</button>
            ...
            <button role="tab" @click="setTab('injection')"
              ...
              <i class="ph ph-syringe text-lg"></i>Injection</button>
          </div>
          <!-- ── Aims ───────────────────────────────────── -->
          <section x-show="mapTab==='aims'">
            <div class="flex items-center gap-3 flex-wrap mb-8">
              <a :href="hubUrl(AIMS_MANIFEST)" :data-peek="peek(AIMS_MANIFEST)" target="_blank" rel="noopener"
                 class="text-[11px] font-mono opacity-40 hover:opacity-80 inline-flex items-center gap-1"
                 :title="'What this tab renders, not a registry (' + AIMS_MANIFEST + ')'">
                <span x-text="AIMS_MANIFEST"></span><i class="ph ph-github-logo"></i></a>
              <div class="grow"></div>
...
${regChip('SHOWING_MECHANISMS')}${regChip('ROUTES_MODES')}${regChip('ROUTES_ROUTES')}${regChip('ROUTES_PASTE')}
                    ...
                    <template x-for="k in routes.kinds" :key="k.kind">
                      <div class="border border-base-300 rounded-lg p-2.5 bg-base-100 flex flex-col gap-1.5">
                        ...
                        <div x-show="k.kit" class="text-base">
                          <a :href="hubUrl(k.kit)" :data-peek="peek(k.kit)" target="_blank" rel="noopener"
              ...
              <div class="@container flex flex-col gap-8">
                ...
                <div>
                  ...
                  <template x-if="docPhantoms.length">
                    <div class="mt-3 text-sm text-base-content/60 border border-warning/40 bg-warning/10 rounded-lg px-3 py-2 max-w-4xl">
                      <span class="font-semibold text-warning/90" x-text="docPhantoms.length + ' unresolved '
                        + (docPhantoms.length === 1 ? 'read' : 'reads')"></span>
                      <span>: a session opened these under docs/ and the registry carries no such file, so each is
                      either a doc since deleted or a path someone guessed wrong. A misspelling here is a document
                      that could not be found by the name a reader reached for.</span>
          ...
          <section x-show="mapTab==='claims'">
            <div class="flex items-center gap-2 mb-4 flex-wrap">
              ...
              <template x-if="lens !== 'owners'">
                <a :href="hubUrl(THEMES_GRAPH)" :data-peek="peek(THEMES_GRAPH)" target="_blank" rel="noopener"
                   class="text-[11px] font-mono opacity-40 hover:opacity-80 inline-flex items-center gap-1"
                   :title="'A measurement, not a registry (' + THEMES_GRAPH + ')'">
                  <span x-text="THEMES_GRAPH"></span><i class="ph ph-github-logo"></i></a>
              </template>
            </div>
            ...
            <div class="flex items-center gap-3 mb-4 max-w-4xl min-w-0 flex-wrap">
              ...
              <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 shrink-0">
                <button @click="themeOnly = ''" class="px-2.5 py-1 rounded-md text-base transition-colors tabular-nums"
                        :class="!themeOnly ? 'bg-base-100 shadow-sm' : 'text-base-content/60'">
                  <span class="font-semibold" x-text="themeTally.all"></span> pairs</button>
                <button @click="themeOnly = 'unlisted'" class="px-2.5 py-1 rounded-md text-base transition-colors tabular-nums"
                        :class="themeOnly ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60'">
            ...
            <section x-show="lens === 'clusters' && themeGraph" class="flex flex-col gap-2 max-w-4xl">
                  ...
                  <template x-for="e in c.edges" :key="e.a + e.b">
                    <div class="flex items-baseline gap-2 py-0.5">
                      <span class="tabular-nums text-sm shrink-0 w-7 text-right"
                            :class="themeListedEdge(e) ? 'text-base-content/30' : 'text-primary font-semibold'" x-text="e.w"></span>
            ...
            <section x-show="lens === 'arcs' && themeGraph" class="max-w-4xl">
                  ...
                  <template x-for="f in themeOrdered" :key="f">
                    <div class="flex items-center gap-2" style="height:30px">
                      <span class="w-1.5 h-1.5 rounded-full shrink-0"
                            :class="themeListed.has(f) ? 'bg-base-content/20' : 'bg-primary'"></span>
                      <a :href="hubUrl(f)" :data-peek="peek(f)" target="_blank" rel="noopener"
            ...
            <section x-show="lens === 'atoms' && themeGraph" class="flex flex-col gap-3 max-w-4xl">
              <template x-for="e in themeEdges" :key="e.a + e.b">
                <div class="border-l-2 pl-3" :class="themeListedEdge(e) ? 'border-base-300' : 'border-primary'">
                  <div class="flex items-baseline gap-2 flex-wrap">
            ...
            <section x-show="lens === 'matrix' && themeGraph" class="max-w-4xl">
              <div class="border-l-2 pl-3 mb-4 min-h-14"
                   :class="themeShown && !themeListedEdge(themeShown) ? 'border-primary' : 'border-base-300'">
                <template x-if="themeShown">
                  <div>
                    <div class="flex items-baseline gap-2 flex-wrap">
                      <span class="tabular-nums text-sm shrink-0"
                            :class="themeListedEdge(themeShown) ? 'text-base-content/30' : 'text-primary font-semibold'"
                  ...
                  <div class="inline-grid gap-px shrink-0"
                    ...
                    <template x-for="(cell, ci) in themeCells" :key="ci">
                      <button type="button" @click="themePick = cell.e"
                              ...
                              :class="cell.e ? (themeListedEdge(cell.e) ? 'bg-base-content' : 'bg-primary')
            ...
            <section x-show="lens === 'owners'" class="max-w-4xl">
            ...
            <div x-show="lens !== 'owners' && themeGraph"
              ...
              <span class="flex items-center gap-1.5">
                ...
                <span>in neither <a :href="hubUrl(OWNERS_MANIFEST)" target="_blank" rel="noopener"
              ...
              <span class="flex items-center gap-1.5">
                ...
                <span>both files listed there</span>
              </span>
...
<!-- Say which axis this is. The badge on every row below is a KIND,
          ...
          <section x-show="mapTab==='injection'">
            ...
            <div x-show="note.cell" x-cloak
              ...
              <div x-show="note.cell?.e && !themeListedEdge(note.cell.e)" class="text-base text-primary mt-1">
      ...
      loadTab(tab){
        ...
        else if (tab === 'claims') { this.loadOwnersReg(); this.loadThemes(); }
      ...
      get themeEdges(){
        const all = (this.themeGraph?.edges || []).filter(e => e.w >= this.themeTh);
        return (this.themeOnly ? all.filter(e => !this.themeListedEdge(e)) : all)
          .slice().sort((x, y) => y.w - x.w);
      },
      // Both counts, so the filter's two segments carry their own definition
      // and no legend has to explain what the second one selects.
      get themeTally(){
        const all = (this.themeGraph?.edges || []).filter(e => e.w >= this.themeTh);
        return { all: all.length, unlisted: all.filter(e => !this.themeListedEdge(e)).length };
      },
      ...
      themeArcs(){
        ...
        const paths = this.themeEdges.map(e => {
          ...
          const hot = !this.themeListedEdge(e);
      ...
      get docPhantoms(){
        ...
        const have = new Set((this.docsReg.documents || []).map(d => this.docReadKey(d.path)));
