document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // files that name its carrier, derived by registries-reach.mjs. A carrier
    ...
    const PROPS_MANIFEST = 'docs/properties.json';
        ...
        <div class="w-full">
          ...
          <section x-show="mapTab==='registries'">
            <div class="flex items-center gap-2 mb-1 flex-wrap">
              <span class="text-sm font-semibold uppercase tracking-wide text-base-content/40">Declarations</span>
              <a :href="hubUrl(PROPS_MANIFEST)" :data-peek="peek(PROPS_MANIFEST)" target="_blank" rel="noopener"
                 class="inline-flex items-center gap-1.5 font-mono text-sm text-base-content/60 hover:text-primary"
                 :title="'The declaration table this tab renders (' + PROPS_MANIFEST + '); edit it there'">
            ...
            <p class="text-sm text-base-content/50 mb-5 max-w-4xl">
              ...
              is one committed file, its carrier, holding a row per thing it describes; its gate is
              the test that fails when the file and the repo disagree. A row asserts properties, and
              one property about one thing answers to exactly one registry, which is checked rather
              than only written down. Where each registry renders is derived from the code and shown
              per row; a carrier no app surface reads wears the warning, since an unread claim is
              where the audits keep finding rot.
            </p>
            ...
            <template x-if="propsReg">
              <div class="max-w-4xl">
                <div class="flex items-center gap-4 flex-wrap mb-5 text-sm">
                  ...
                  <span class="text-base-content/50"><span x-text="registryTotals.decls"></span> declarations, <span x-text="registryTotals.closed"></span> with a closed domain</span>
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      async loadPropsReg(){
        ...
        try {
          const gh = new window.GH({ token: window.TOKEN, repo: this.hub(), ref: useRef() });
          const raw = (await gh.get(PROPS_MANIFEST)).text;
          const parsed = JSON.parse(raw);
          if (!parsed || !Array.isArray(parsed.registries)) throw new Error('no registries table');
          window.SourcePeek?.seed(this.peek(PROPS_MANIFEST), raw);
          this.propsReg = parsed;
        } catch (e) {
          this.propsErr = 'Registries load failed: ' + (e?.message || e);
        } finally { this.propsLoading = false; }
      },

      // Declarations grouped under the registry they govern, so the page reads
      ...
      get registryTotals(){
        ...
        return {
          ...
          catalog: rows.filter(r => r.kind === 'catalog').length,
