...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      get surfDeckFiles(){
        return [
          { path: SURF_DOC, title: 'SURFACING.md', subtitle: 'the primitives, in full' },
      ...
      // The mark. A tint and a rule rather than a ring: the target is a whole
      // list item several lines tall, and a ring around a block that height
      // reads as a box somebody drew rather than as "this one".
      MARK_CLS: ['bg-warning/10', 'border-l-2', 'border-warning', 'pl-3', '-ml-3', 'rounded-r'],
      _markTimer: 0,
      markLead(el){
        clearTimeout(this._markTimer);
        const box = el.closest('[data-deck-content]');
        for (const old of box?.querySelectorAll('.bg-warning\\/10') || [])
          old.classList.remove(...this.MARK_CLS);
        el.classList.add(...this.MARK_CLS);
        this.scrollWithin(el);
        // It fades on its own. A mark that stays turns into a false claim the
        // moment the reader swipes to another slide and back.
        this._markTimer = setTimeout(() => el.classList.remove(...this.MARK_CLS), 4000);
      },

      // SCROLL THE SLIDE, NOT THE PAGE. scrollIntoView walks every scrollable
      // ancestor, and docked the page is deliberately still scrollable (the
      // deck only locks page scroll when it takes the whole pane), so the first
      // build centred the bullet by scrolling the card list out from under the
      // reader: the card they had just tapped was off screen beside its own
      // paragraph. Finding the nearest scroller inside the deck and moving only
      // that one is what keeps the pair a pair.
      //
      // Centred when the target fits, topped when it does not, since half of a
      // bullet taller than the pane is worse than its beginning.
      // `ifNeeded` is for the CARD rather than the bullet: the bullet is the
      // target and always moves to the middle, while a card the reader can
      // already see should not jump under the finger that tapped it.
      // scrollIntoView's own 'nearest' was tried for that and does nothing for
      // an element whose top edge is in view and whose bottom is not, which is
      // most cards once docking narrows the column.
      scrollWithin(el, o = {}){
        let box = null;
        ...
        if (o.ifNeeded && r.top >= b.top && r.bottom <= b.bottom) return;
      ...
      async showPrimitive(p, card){
        ...
        const el = this.findLead(box, p.lead);
        ...
        // nothing.
        if (card) { await this.settled(card); this.scrollWithin(card, { ifNeeded: true }); }
      },
