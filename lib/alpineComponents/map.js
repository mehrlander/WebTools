document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    return {
          ...
          <section x-show="mapTab==='docs'">
            ...
            <template x-if="docsReg">
              <div class="flex flex-col gap-8">
                ...
                <!-- Collapsed by default: the claims table keys on CLAIMS, not
                     files, so trailing the census open it read as an appendix
                     (2026-08-07). The count keeps it discoverable; folding it
                     is the interim answer while a real incorporation (a
                     Claims segment, or per-row claim marks) is decided. -->
                <div class="max-w-4xl">
                  <button type="button" @click="docClaims = !docClaims"
                          class="flex items-center gap-2 mb-2 text-base-content/60 hover:text-base-content">
                    <i class="ph" :class="docClaims ? 'ph-caret-down' : 'ph-caret-right'"></i>
                    <h3 class="text-sm font-semibold uppercase tracking-wide text-base-content/40">Shared claims</h3>
                    <span class="badge badge-ghost badge-sm" x-text="(docsReg.claims || []).length"></span>
                  </button>
                  <p x-show="docClaims" class="text-base text-base-content/60 mb-3">Statements that live in more than one place. One authoritative carrier each; every repetition says how it relates and what holds it, and an absent check says so.</p>
                  <div x-show="docClaims" class="flex flex-col gap-2">
      ...
      // The rail's selection, and the claims disclosure (collapsed by default).
      docDir: 'docs',
      docClaims: false,
