        ...
        <div class="w-full">
          ...
          <section x-show="mapTab==='skills'">
            ...
            <p class="text-sm text-base-content/50 mb-3 max-w-4xl">
              ...
              estate has committed in its own .claude/skills/. Neither is the plugin's sixteen,
              which install in every session and are on the Portable tab: anything under
              .claude/skills/ registers as auto-fire, and the library exists to avoid exactly that.
              The estate half is read from each repo's own manifest, so a repo that declares nothing
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      // pager already wrong.
      async openFileDeck(files, start, title, icon){
