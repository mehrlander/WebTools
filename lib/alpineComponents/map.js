document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // restated here.
    const DIMENSIONS = [
      { key: 'kind', label: 'kind', tone: true, question: 'what genre of check is this',
        values: KIND_ORDER, of: t => t.kind, hint: (v, reg) => reg?.kinds?.[v] || '' },
      { key: 'method', label: 'method', tone: false, question: 'how does it reach its subject',
        values: METHOD_ORDER, of: t => t.method, hint: v => METHOD_HINT[v] || '' },
      { key: 'runner', label: 'runner', tone: false, question: 'what runs the file',
              ...
              <div class="flex flex-col gap-8">
                ...
                <div class="flex flex-col gap-2">
                  ...
                  <!-- One labeled row per dimension. The label is the fix for
                       ...
                       The legend is the fix for the chips themselves: each
                       carries two bare numbers in different units, and a
                       reader had nothing telling them which was which. It sits
                       once above the block rather than on every chip, and the
                       header line above uses the same order for the same
                       reason. Not a slash: 13/69 reads as "13 of 69", a ratio
                       between like things, and these are files and assertions. -->
                  <div class="flex items-baseline gap-2">
                    <span class="w-16 shrink-0"></span>
                    <span class="text-sm text-base-content/30">files · assertions</span>
                  </div>
                    ...
                    <div class="flex items-baseline gap-2 flex-wrap">
                      ...
                      <template x-for="c in d.chips" :key="c.value">
                        <button type="button" @click="toggleDim(d.key, c.value)" :title="c.hint"
                          ...
                          <span x-show="d.tone" class="badge badge-sm" :class="c.tone" x-text="c.files"></span>
                          <span x-show="!d.tone" class="text-sm tabular-nums text-base-content/50" x-text="c.files"></span>
                          <span class="text-sm text-base-content/20">·</span>
                          <!-- n/a rather than an omission or a zero. A browser
                               check asserts in its own harness, so test() is not
                               its unit and it HAS no count; leaving the slot
                               blank reads as data the page failed to load, and a
                               zero would claim it checks nothing. -->
                          <span class="text-sm tabular-nums"
                                :class="c.counted ? 'text-base-content/40' : 'text-base-content/25 italic'"
          ...
          chips: d.values
            .map(v => {
              ...
              return {
                ...
                tone: d.tone ? (KIND_TONE[v] || 'badge-ghost') : '',
