    ...
    return {
      ...
      template: `
        <div class="w-full">
          ...
          <p class="text-base text-base-content/60 mb-6 leading-7 text-balance" x-text="tabGloss"></p>
          <!-- ── Aims ───────────────────────────────────── -->
          ...
          <section x-show="mapTab==='surfacing'">
            <div class="flex items-center gap-2 mb-5 flex-wrap">
              ...
              <button type="button" @click="openSurfDeck()"
                      class="flex items-center gap-1.5 text-base text-base-content/60 hover:text-primary px-2 py-1 rounded-lg hover:bg-base-200 transition-colors"
                      title="Read the doc beside these cards, in the house deck">
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      //
      // ONE SLIDE, and the manifest is deliberately not the second. The two
      // files do stand in a fixed relation (SURFACING.md is authoritative,
      // surfacing.csv is its gated index, held both ways by
      // surfacing-manifest.test.mjs), so a swipe between them reads well in
      // principle. It does not read well in fact: renderDoc decides its
      // rendition by extension and has no CSV branch, so the index arrives as
      // wrapped raw text, which is a worse neighbour for the prose than nothing.
      // The cards below already ARE that file rendered. Give renderDoc a table
      // rendition and the second slide is worth adding; the Docs tab's deck
      // pages a folder holding a dozen CSVs and shows every one of them the
      // same way, so the gap is not this tab's.
      openSurfDeck(){
        const files = [{ path: SURF_DOC }];
        return this.openFileDeck(files, 0, {
          icon: 'ph-megaphone', key: 'surfacing', context: 'Surfacing',
          label: () => ({ title: 'SURFACING.md', subtitle: 'the primitives, in full',
                          icon: 'ph-file-text' }),
        });
      },
      async loadSurf(){
