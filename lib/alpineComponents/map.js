...
<!-- Say which axis this is. The badge on every row below is a KIND,
                          ...
                          <div class="flex flex-col gap-4">
                            <p class="text-sm text-base-content/40"
                               x-text="checkingView(g).comparisons + ' comparisons under ' + checkingView(g).explained.length + ' files'
                                       ...
                                       + ' · unit: ' + (checkingView(g).units.join(', ') || 'unstated')"></p>
                            <div class="flex flex-col gap-1">
      ...
      checkingSources(cache, query){
        ...
        for (const [repo, e] of Object.entries(cache?.repos || {})) {
          ...
          out.push({ repo, ref: 'main', files: c.files || '', comparisons: c.comparisons, from: 'crawl' });
        }
      ...
      async loadEstateChecking(){
        ...
        let query = '';
        try { query = new URLSearchParams(location.search).get(CHECKING_KEY) || ''; } catch {}
        ...
        for (const src of this.checkingSources(cache, query)) {
          const g = { repo: src.repo, short: src.repo.split('/').pop(), ref: src.ref, from: src.from,
                      filesPath: src.files, comparisonsPath: src.comparisons, files: [], byPath: null, err: '' };
          try {
