document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    return {
      description: 'Map view: the coordination layer made inspectable, in three tabs. The portable set (docs/portable.json, each row openable in the shell viewer); Showing (docs/routes.json): which link reaches which kind of change and what each one misses, then the address grammar, toss-render\'s delivery modes and their trust postures, and the toss routes; and Docs (docs/docs.json): the documentation registry, every doc\'s subject, status, and maintenance, plus the shared-claims table with per-repetition checks. Per-repo scope and adoption live on the Repos cards. The operational face of the constellation doctrine.',
