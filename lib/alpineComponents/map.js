...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      openDeckMenu(){
        ...
        if (!host || !path) return;
        const rows = [
          { icon: 'ph-at', label: 'Copy address', hint: this.deckAddress(path),
          ...
          { icon: 'ph-link-simple', label: 'Copy GitHub link', hint: 'the blob at ' + useRef(),
            run: () => this.deckCopy(this.hubUrl(path), 'Link copied') },
          { icon: 'ph-clipboard-text', label: 'Copy contents', hint: 'the file as it reads here',
            run: async () => {
              const key = useRef() + ':' + path;
              if (!docCache.has(key)) await this.docDeckRead(path);
              await this.deckCopy(docCache.get(key) || '', 'Contents copied');
            } },
        ...
        // Measured off the kit's own header hook rather than a number written
        // down here, which would be a second copy of its padding.
        menu.style.top = ((host.querySelector('.sd-header')?.offsetHeight || 56) + 4) + 'px';
        for (const r of rows) {
