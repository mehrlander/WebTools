document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // The Tools tab. The census the lib-kits migration argued for:
    ...
    return {
      ...
      template: `
        <div class="w-full">
          ...
          <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 mb-6 w-fit flex-wrap" role="tablist">
            ...
            <button role="tab" @click="setTab('tools')"
                    class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors"
                    :class="mapTab==='tools' ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60 hover:text-base-content'">
              <i class="ph ph-wrench text-lg"></i>Tools</button>
          </div>
          <!-- ── Portable ────────────────────────────────────────────────── -->
          ...
          <section x-show="mapTab==='tools'">
            ...
            <template x-if="toolsReg">
              <div class="flex flex-col gap-8">
                ...
                <div>
                  <div class="flex items-center gap-2 flex-wrap">
                    <template x-for="r in toolLayerCounts" :key="r.key">
                      <button type="button" @click="toggleToolLayer(r.key)" :title="r.gloss"
                              class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg border transition-colors"
                              :class="toolLayer === r.key ? 'border-primary bg-primary/10' : 'border-base-300 hover:bg-base-200'">
                        <span class="badge badge-sm badge-ghost" x-text="r.files"></span>
                        <span class="text-base" x-text="r.label"></span>
                        <span x-show="r.blank" class="text-sm text-warning/70 tabular-nums"
                              :title="'rows with no authored role'" x-text="r.blank"></span>
                      </button>
                    </template>
                    <button type="button" x-show="toolLayer" @click="toolLayer = ''"
                            class="text-sm text-base-content/50 hover:text-primary px-2 py-1">show all</button>
                    <div class="grow"></div>
                    <span class="text-sm text-base-content/40 tabular-nums"
                          x-text="toolTotals.files + ' files · ' + toolTotals.named + ' named · ' + toolTotals.tested + ' tested'"></span>
                  </div>
                  <p x-show="toolLayer" class="text-sm text-base-content/50 mt-2 max-w-4xl"
                     x-text="toolsReg.layers[toolLayer]"></p>
                  <p x-show="!toolLayer" class="text-sm text-base-content/50 mt-2 max-w-4xl">
                    <span x-text="toolTotals.blank"></span> of <span x-text="toolTotals.files"></span> rows do not
                    yet say what they are for; the census counts them rather than hiding them. A
                    <span class="text-info">driver</span> is run by path (npm run shot --script), so being named
                    nowhere is its normal state; <span class="text-warning">none found</span> is a file with no
                    visible way to run at all.
                  </p>
                </div>

                <div class="grid gap-x-8 gap-y-6 lg:grid-cols-2">
                  <template x-for="grp in toolGroups" :key="grp.layer">
                    <div>
                      <h3 class="text-sm font-semibold uppercase tracking-wide text-base-content/40 mb-2">
                        <span x-text="grp.layer"></span>
                        <span class="font-normal normal-case text-base-content/30" x-text="'· ' + grp.files.length"></span>
                      </h3>
                      <div class="flex flex-col gap-1">
                        <template x-for="t in grp.files" :key="t.path">
                          <div class="flex items-start gap-2.5 px-2 py-1.5 rounded-lg hover:bg-base-200/60 group">
      ...
      loadTab(tab){
        ...
        else if (tab === 'tools') this.loadToolsReg();
      },
      ...
      get toolLayerCounts(){
        ...
        const layers = [...new Set(rows.map(t => t.layer))].sort();
        return layers.map(key => {
          const fs = rows.filter(t => t.layer === key);
          return {
            key,
            // The path prefix is the group header below; the strip wants the
            // short name (scenarios, build, scripts) a reader scans by.
            label: key.split('/').pop() || key,
            gloss: this.toolsReg?.layers?.[key] || '',
            files: fs.length,
            blank: fs.filter(t => !t.role).length,
          };
        });
      },
      get toolTotals(){
      ...
      get toolGroups(){
        const rows = (this.toolsReg?.tools || [])
          .filter(t => !this.toolLayer || t.layer === this.toolLayer);
        const groups = new Map();
        for (const t of rows) {
          if (!groups.has(t.layer)) groups.set(t.layer, []);
          groups.get(t.layer).push(t);
        }
        // Within a layer, the accounted files lead and the blank tail is
        // where the reader's eye should land last, not first.
        for (const fs of groups.values())
          fs.sort((a, b) => (b.role ? 1 : 0) - (a.role ? 1 : 0) || a.path.localeCompare(b.path));
        return [...groups.keys()].sort().map(layer => ({ layer, files: groups.get(layer) }));
      },
