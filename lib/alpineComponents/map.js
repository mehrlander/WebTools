document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    // Qualifications, not genres: each is a predicate over rows of any kind,
    // which is why they filter on their own axis and compose with the kind
    // strip rather than sitting in it. Both were prose under the totals until
    // 2026-08-10; the rows already carried the facts (a smoke badge, a browser
    // icon) and the sentence only added weight, giving 19 of 1,446 assertions
    // the same billing as the headline count.
    const TRAITS = [
      { key: 'smoke', icon: 'ph-seal-question', match: t => (t.boot_smoke || 0) > 0,
        hint: 'The assertion only checks that the component mounts without warnings. Cheap and worth having; also the least a passing test can claim.' },
      { key: 'browser', icon: 'ph-browser', match: t => t.assertions === null,
        hint: 'Driven by a real browser, so it is named without .test. and node --test never globs it. It asserts in its own harness, which is why it reports no assertion count and why the suite\'s pass total does not speak for it.' },
    ];
                ...
                <div>
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <!-- One toggle for the whole census, the same call the docs
                         ...
                         per-row disclosure would mean tapping 119 times to ask
                         it. Pair it with a kind filter to scope the expansion;
                         that is what the strip above is for. -->
                    <div class="grow"></div>
                    <span class="text-sm text-base-content/40 tabular-nums"
                  ...
                  <!-- The second filter axis. A trait is true of rows of any
                       kind, so it composes with the strip above rather than
                       joining it, and the separate line is the whole signal
                       that the two are different questions. This replaced a
                       sentence that said the same two numbers in prose while
                       the rows below were already rendering both facts. -->
                  <div class="flex items-center gap-2 flex-wrap mt-2">
                    <template x-for="r in testTraitCounts" :key="r.key">
                      <button type="button" @click="toggleTrait(r.key)" :title="r.hint"
                              class="flex items-center gap-1.5 px-2.5 py-1 rounded-lg border transition-colors text-base"
                              :class="testTrait === r.key ? 'border-primary bg-primary/10' : 'border-base-300 hover:bg-base-200'">
                        <i class="ph" :class="r.icon"></i>
                        <span class="tabular-nums" x-text="r.files"></span>
                        <span x-text="r.key"></span>
                      </button>
                    </template>
                    <button type="button" x-show="testTrait" @click="testTrait = ''"
                            class="text-sm text-base-content/50 hover:text-primary px-2 py-1">any</button>
                    <div class="grow"></div>
                    <!-- One toggle for the whole census, the same call the docs
                         census makes about maintenance: the assertion list is
                         either the question you are asking or it is noise, and a
                         per-row disclosure would mean tapping 119 times to ask
                         it. It sits right of the grow because it is a display
                         control, not a filter; each line here reads filters on
                         the left, readouts and controls on the right. -->
                    <button type="button" @click="testNames = !testNames"
                            ...
                            title="Show what each file's assertions are named: the file's own account of its coverage, read from the test() calls">
                  ...
                  <p x-show="testTrait" class="text-sm text-base-content/50 mt-2 max-w-4xl"
                     x-text="traitMeta(testTrait).hint"></p>
                </div>
...
<!-- Say which axis this is. The badge on every row below is a KIND,
      ...
      get testTraitCounts(){
        const rows = this.testsReg?.tests || [];
        return TRAITS
          .map(t => ({ ...t, files: rows.filter(t.match).length }))
          .filter(t => t.files);
      },
      toggleTrait(key){ this.testTrait = this.testTrait === key ? '' : key; },
      traitMeta(key){ return TRAITS.find(t => t.key === key) || { hint: '' }; },
      get testGroups(){
        const groups = new Map();
        const trait = TRAITS.find(t => t.key === this.testTrait);
        for (const t of (this.testsReg?.tests || [])) {
          if (this.testKind && t.kind !== this.testKind) continue;
          if (trait && !trait.match(t)) continue;
