document.addEventListener('alpine:init', function() {
  Alpine.data('map', function() {
    ...
    //               opens the document rendered in place, in a sheetModal
    //               over the Map, rather than navigating to the files view;
    //               its GitHub icon carries the source peek for the desktop
    //               glance, and the rendition helpers are SourcePeek's own
    //               exports so sheet and peek cannot drift. Reach is the one field here that is
    ...
    const sheetCache = new Map();  // ref:path -> raw text
    ...
    return {
      ...
      template: `
        <div class="w-full">
          ...
          <section x-show="mapTab==='docs'">
                          ...
                          <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2 flex-wrap">
                              <button type="button" class="text-base font-medium hover:text-primary text-left"
                                      ...
                                      @click="openDocSheet(d)" x-text="docTitle(d)"></button>
                              ...
                              <button type="button" @click="docInfo = docInfo === d.path ? '' : d.path"
                                      :title="'Maintenance: who regenerates or edits this file'"
                                      class="text-base-content/30 hover:text-primary"
                                      :class="docInfo === d.path && 'text-primary'">
                                <i class="ph ph-info"></i></button>
                            </div>
                            <p class="text-base text-base-content/60" x-text="d.subject"></p>
                            <p x-show="docInfo === d.path" class="text-sm text-base-content/40" x-text="d.maintenance"></p>
                          </div>
                          <!-- Always visible, not hover-revealed: this icon
                               carries the source peek, and an affordance
                               behind an invisible target is no affordance. -->
                          <a :href="hubUrl(d.path)" :data-peek="peek(d.path)"
                             target="_blank" rel="noopener" title="Open on GitHub"
                             class="text-base-content/30 hover:text-primary shrink-0 mt-1">
                            <i class="ph ph-github-logo"></i></a>
                        </div>
                      </template>
                    </div>
            ...
            <!-- The doc sheet: read a row in place. A title tap opens the
                 document rendered over the Map (bottom sheet on a phone,
                 centered modal on desktop) instead of navigating to the files
                 view; the peek stays the desktop glance, this is the read,
                 and the files view remains the place to work on a file.
                 sheetModal is auto-booted by the pre-build, so nothing loads
                 for it.
                 THE SLOT CARRIES NO ALPINE BINDINGS, deliberately. sheetModal
                 lifts its slot nodes into its own chrome at init, and in a
                 real browser the mutation observer destroys a moved node's
                 effects after the re-attach, so slot bindings freeze at their
                 initial values (the same wiring passes under jsdom, which is
                 why only the pixels caught it). openDocSheet paints these
                 elements imperatively and writes the title through the sheet
                 component's own data, whose chrome bindings are never moved
                 and stay live. -->
            <div data-doc-sheet-host x-data="sheetModal({ openOn: 'open-doc-sheet', maxWidth: 'max-w-3xl' })">
              <div class="px-5 pb-6">
                <div class="flex items-center gap-2 flex-wrap pt-3 pb-2">
                  <code data-doc-sheet-path class="text-sm text-base-content/50"></code>
                  <a data-doc-sheet-gh href="#" target="_blank" rel="noopener" title="Open on GitHub"
                     class="text-base-content/40 hover:text-primary"><i class="ph ph-github-logo"></i></a>
                  <div class="grow"></div>
                  <button type="button" data-doc-sheet-files
                          title="Open in the files view, for history and editing"
                ...
                <div data-doc-sheet-content></div>
              </div>
            </div>
          </section>
      ...
      // The rail's selection and the one-open maintenance disclosure.
      docDir: 'docs',
      docInfo: '',
      ...
      // The doc sheet: fetched full rather than excerpted (the peek is the
      // glance, this is the read), cached per ref:path so re-opening a row
      // costs nothing. docSheet is the state of record (and what the tests
      // assert on); the DOM is painted from it imperatively because the
      // sheet's slot cannot hold live bindings (see the template note).
      docSheet: { path: '', name: '', html: '', err: '', loading: false },
      docSheetEls(){
        // $root, not $el: in a method invoked from a row's @click, $el is the
        // clicked button (so every querySelector misses), while $root is the
        // component's element in both that path and a direct method call.
        const root = this.$root || this.$el;
        return {
          host: root.querySelector('[data-doc-sheet-host]'),
          path: root.querySelector('[data-doc-sheet-path]'),
          gh: root.querySelector('[data-doc-sheet-gh]'),
          files: root.querySelector('[data-doc-sheet-files]'),
          content: root.querySelector('[data-doc-sheet-content]'),
        };
      },
      paintDocSheet(){
        const el = this.docSheetEls();
        if (!el.content) return;
        const s = this.docSheet;
        if (el.path) el.path.textContent = s.path;
        if (el.gh) el.gh.href = this.hubUrl(s.path);
        el.content.innerHTML = s.loading
          ? '<div class="flex justify-center py-10"><span class="loading loading-dots loading-md opacity-30"></span></div>'
          : s.err
            ? '<div class="text-base text-error font-mono py-4">' + s.err.replace(/&/g, '&amp;').replace(/</g, '&lt;') + '</div>'
            : s.html;
      },
      async openDocSheet(d){
        this.docSheet = { path: d.path, name: this.docTitle(d), html: '', err: '', loading: true };
        const el = this.docSheetEls();
        if (el.host && window.Alpine?.$data) window.Alpine.$data(el.host).title = this.docSheet.name;
        if (el.files && !el.files._wired) {
          el.files._wired = true;
          el.files.addEventListener('click', () => {
            if (el.host && window.Alpine?.$data) window.Alpine.$data(el.host).close();
            this.openHubFile(this.docSheet.path);
          });
        }
        this.paintDocSheet();
        window.dispatchEvent(new CustomEvent('open-doc-sheet'));
        try {
          const key = useRef() + ':' + d.path;
          if (!sheetCache.has(key)) {
            const gh = new window.GH({ token: window.TOKEN, repo: this.hub(), ref: useRef() });
            sheetCache.set(key, (await gh.get(d.path)).text);
          }
          this.docSheet.html = await renderDoc(d.path, sheetCache.get(key));
        } catch (e) {
          this.docSheet.err = 'Load failed: ' + (e?.message || e);
        } finally {
          this.docSheet.loading = false;
          this.paintDocSheet();
        }
      },
