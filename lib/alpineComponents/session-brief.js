...
(function () {
  ...
  // Markdown reduced to the prose inside it, through the one pass this estate
  // has for that: kits/read-aloud.js `speechText`. A preview line wants exactly
  // what a spoken line wants, the link's label and not its URL, no fences, no
  // code ticks, no emoji. Where the kit has not landed the raw text still shows,
  // markers and all, which is worse to read and never wrong.
  const plain = (md) => {
    const s = String(md || '');
    return window.readAloud?.speechText?.(s) || s;
  };
  ...
  Alpine.data('sessionBrief', function (opts) {
    ...
    return {
      ...
      loading: false,
      // Whether the kit chain has landed (see `ready`). Read by `closing`.
      kitsIn: false,
      err: '',
      ...
      get closing() {
        // `kitsIn` is a real dependency, not a guard. This getter reduces its
        // text through a kit that arrives AFTER the first paint, and an Alpine
        // getter re-runs only when something reactive it read has changed. A
        // mount handed its record outright (#gz=) changes nothing else after
        // init, so without the flag it would paint the raw markdown once and
        // keep it.
        return this.kitsIn ? plain(this.closingRaw) : this.closingRaw;
      },
      get closingRaw() {
        if (this.record) {
      ...
      async ready() {
        if (window.sessionRender && window.sessionExport && window.readAloud) { this.kitsIn = true; return; }
