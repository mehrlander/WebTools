...
(function () {
  ...
  Alpine.data('sessionBrief', function (opts) {
    ...
    return {
      ...
      loading: false,
      // Which fact of the strip is showing its definition, by key, or '' for
      // none. The id above the strip joins in under the key 'id'.
      fact: '',
      ...
      template: `
        ...
        <div class="w-full flex flex-col gap-2.5 px-4 pt-3"
          ...
          <div x-show="!framed" class="shrink-0 text-lg font-medium leading-snug">
            ...
            <button type="button" class="ml-1.5 font-mono text-sm font-normal opacity-45"
                    :class="fact === 'id' ? 'text-primary opacity-100' : ''"
                    :aria-expanded="fact === 'id' ? 'true' : 'false'"
                    @click="fact = fact === 'id' ? '' : 'id'"
                    x-text="'(' + (shortId || '') + ')'"></button>
          </div>
          ...
          <!-- The facts strip, labelled rather than run together, and drawn
               ...
               full one. -->
          <!-- EACH FACT IS ITS OWN DEFINITION, ONE TAP AWAY. Half of these are
               exact about something the plain word is not, so the definitions
               are load-bearing rather than decorative; they rode in a title
               with cursor-help, which is the shape the house style names
               outright and which a phone cannot perform at all. Tapping a fact
               names it under the strip and tapping it again puts it away, so
               the page carries no standing prose and the reader can still ask.
               Same control as the closing reply beneath it, and it took the
               same trip through a tooltip to get here. -->
          <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm shrink-0">
            <template x-for="f in strip" :key="f.k">
              <button type="button" class="whitespace-nowrap"
                      :aria-expanded="fact === f.k ? 'true' : 'false'"
                      @click="fact = fact === f.k ? '' : f.k">
                <span :class="fact === f.k ? 'text-primary' : 'opacity-55'" x-text="f.k"></span>
                <span class="font-mono font-medium ml-1"
                      :class="f.k === 'failures' && f.v ? 'text-warning' : ''" x-text="f.v"></span>
              </button>
            </template>
          </div>
          <div x-show="factNote" x-cloak
               class="shrink-0 -mt-1.5 text-xs leading-snug text-base-content/60" x-text="factNote"></div>
      ...
      // What the tapped fact means. The strip already carries a definition per
      // row, so this reads that rather than keeping a second copy; the id is
      // the one key that is not a strip row and states its own.
      get factNote() {
        if (this.fact === 'id') {
          return "The record's filename stem, and how every other surface names this session.";
        }
        return this.strip.find(f => f.k === this.fact)?.t || '';
      },
