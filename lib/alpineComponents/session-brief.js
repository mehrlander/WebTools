      ...
      template: `
        ...
        <div class="w-full flex flex-col gap-2.5 px-4 pt-3"
          ...
          <!-- HOW IT ENDED, which is the one thing on this page that is prose
               ...
               the screen. -->
          <div x-show="closingRaw" class="shrink-0 rounded-lg bg-base-200/50">
            <button type="button" class="w-full px-3 py-2 text-left"
              ...
              <!-- A HEIGHT, NOT A LINE CLAMP. That utility counts LINE BOXES,
                   which needs inline content; this holds the paragraphs a
                   markdown render returns. Chrome tolerates that and clamps
                   anyway. iOS Safari does not: it keeps the box at the full
                   height of all five paragraphs and merely hides what spills,
                   so the collapsed block was three lines of text over an inch
                   of empty card (reported 2026-09-01 from the phone). A
                   max-height cuts the box itself and behaves the same
                   everywhere; the mask is what keeps the cut from reading as a
                   rendering fault. -->
              <div x-ref="closingBody" class="opacity-80 overflow-hidden"
                   :style="closingOpen ? '' : CLAMP"></div>
            </button>
          </div>
