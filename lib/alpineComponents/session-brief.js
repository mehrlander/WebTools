...
(function () {
  ...
  Alpine.data('sessionBrief', function (opts) {
    ...
    return {
      ...
      // The speech player, and the mirror of its state. Both null until the
      // first tap: the kit loads on demand and nothing here speaks unbidden.
      player: null,
      speech: null,

      template: `
        <!-- Framed, this is a slide and scrolls inside itself: the title, the
             facts strip and the tab row hold their place while the outline
             under them moves. Standalone it is a page and scrolls as one, for
             the reason branch-brief states: a page that pins its own header
             costs a phone its URL-bar collapse and buys nothing. -->
        <div class="mx-auto w-full max-w-2xl flex flex-col gap-2.5 px-4 pt-3"
          ...
          <div x-show="!framed" class="shrink-0 text-lg font-medium leading-snug">
            ...
            <span class="ml-1.5 font-mono text-sm font-normal opacity-45"
                  title="The record's filename stem, and how every other surface names this session"
                  x-text="'(' + (shortId || '') + ')'"></span>
          </div>
          ...
          <!-- HOW IT ENDED, which is the one thing on this page that is prose
               ...
               the screen. -->
          <div x-show="closing" class="shrink-0 rounded-lg bg-base-200/50 px-3 py-2">
            <div class="flex items-center gap-1.5 text-xs opacity-50 mb-0.5">
              <i class="ph ph-quotes"></i><span x-text="closingLabel"></span>
            </div>
            <p class="text-sm leading-snug line-clamp-3 opacity-80"
               :title="closing" x-text="closing"></p>
          </div>
          ...
          <div class="flex items-center gap-2 shrink-0">
            <div role="tablist" class="tabs tabs-box tabs-sm flex-nowrap bg-base-200 p-0.5 min-w-0
              ...
              <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap"
                 ...
                 @click="pane = 'outline'; $nextTick(() => mountOutline())">
                Outline<span class="font-mono opacity-50" x-text="cards || ''"></span></a>
              <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap"
                 :class="pane === 'files' && 'tab-active'" @click="pane = 'files'">
                Files<span class="font-mono opacity-50" x-text="fileRows.length || ''"></span></a>
              <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap"
                 :class="pane === 'raw' && 'tab-active'"
                 @click="pane = 'raw'; $nextTick(() => mountRaw())">
                Raw<span class="font-mono opacity-50" x-text="rawSize"></span></a>
            </div>
            <div class="grow"></div>
            <!-- LISTEN. Ghost beside the deck's primary, because the deck is
                 what most readers came to do and rule 10 spends the accent
                 once; this one marks itself while it is speaking instead. -->
            <button @click="listen()" :disabled="!record"
                    class="btn btn-sm btn-square btn-ghost max-sm:h-11 max-sm:w-11"
                    :class="speech && speech.state === 'speaking' && 'text-primary'"
                    aria-label="Read this session aloud"
                    :title="speech ? 'Pause' : 'Read the asks and replies aloud'">
              <i class="ph text-lg max-sm:text-xl"
                 :class="speech && speech.state === 'speaking' ? 'ph-pause' : 'ph-speaker-high'"></i></button>
          ...
          <!-- THE TRANSPORT, which appears only once something is speaking and
               is a strip rather than a panel: it sits between the tab row and
               the pane for the whole of a listen, so it must cost the outline
               as little height as a row of buttons can. The turn being read is
               named, not quoted; the outline behind it is where the words are,
               and a second copy of them here would be the pane twice. -->
          <div x-show="speech" x-cloak
               class="shrink-0 flex items-center gap-1 rounded-lg bg-base-200 px-1 py-1">
            <button @click="player.prev()" class="btn btn-xs btn-square btn-ghost max-sm:h-9 max-sm:w-9"
                    aria-label="Previous turn"><i class="ph ph-skip-back"></i></button>
            <button @click="player.toggle()" class="btn btn-xs btn-square btn-ghost max-sm:h-9 max-sm:w-9"
                    aria-label="Play or pause">
              <i class="ph" :class="speech && speech.state === 'speaking' ? 'ph-pause' : 'ph-play'"></i></button>
            <button @click="player.next()" class="btn btn-xs btn-square btn-ghost max-sm:h-9 max-sm:w-9"
                    aria-label="Next turn"><i class="ph ph-skip-forward"></i></button>
            <span class="font-mono text-xs opacity-60 truncate ml-1" x-text="speechWhere"></span>
            <div class="grow"></div>
            <!-- Rate is the one setting a listener changes, and it is kept
                 because a reader who wants 1.5x wants it on every session. -->
            <select class="select select-xs select-ghost w-16 font-mono"
                    :value="speech && speech.rate" @change="setRate($event.target.value)"
                    aria-label="Speed">
              <template x-for="r in [0.8, 1, 1.25, 1.5, 1.75, 2]" :key="r">
                <option :value="r" x-text="r + 'x'"></option>
              </template>
            </select>
            <button @click="player.stop(); player = null; speech = null"
                    class="btn btn-xs btn-square btn-ghost max-sm:h-9 max-sm:w-9"
                    aria-label="Stop reading"><i class="ph ph-x"></i></button>
          </div>
          ...
          <!-- FILES: what the session opened, which the strip counts and had no
               ...
               ways. -->
          <div x-show="pane === 'files'" x-cloak class="min-h-0 grow overflow-y-auto pb-3">
          ...
          <div x-ref="raw" x-show="pane === 'raw'" x-cloak class="min-h-0 grow flex flex-col pb-3"></div>
        </div>
      `,
      ...
      async load() {
        ...
        try {
          ...
          if (!this.record) {
            ...
            if (!path) throw new Error(`No record for ${this.id} in ${this.repo}. Ids are the filename stem, like 2bf8fcae.`);
      ...
      async ready() {
        if (window.sessionRender && window.sessionExport && window.readAloud) return;
      ...
      // ── Listening ─────────────────────────────────────────────────────────
      //
      // The third way through a record, beside the outline and the deck, and
      // the only one that does not need the screen. A session arrives here
      // already read once; hearing it back is what a walk or a commute allows
      // and a deck does not.
      //
      // WHAT IS SPOKEN IS THE CONVERSATION, not the record. Tool calls are the
      // bulk of every session and are unlistenable by construction (a fenced
      // diff, a path, a JSON body), so the queue is the asks and the prose
      // replies, which is the same pair the outline titles a card with. The
      // deck still holds everything.
      //
      // The player is the kit's; this owns the queue, the position readout, and
      // the one thing an Alpine host must do for it, which is re-render on a
      // state it cannot observe. `speech` is that mirror and nothing writes it
      // but onState.
      get speakItems() {
        if (!this.record || !window.sessionRender) return [];
        return window.sessionRender.turns(this.record)
          .filter(t => t.role === 'user' || t.role === 'assistant')
          .map((t, i) => ({
            key: i,
            label: (t.role === 'user' ? 'You' : 'Claude') + (t.ts ? ' · ' + t.ts : ''),
            role: t.role,
            md: t.md || '',
          }));
      },

      // One gesture does the whole thing: load the kit, build the queue, speak.
      // It has to, on Safari, where the synth is unusable until a user gesture
      // has reached it and an await before the first speak() does not break the
      // chain but a second tap would be needed if this only opened the bar.
      async listen() {
        if (this.player) { this.player.toggle(); return; }
        try {
          await this.ready();
          if (!window.readAloud?.supported) throw new Error('this browser does not speak text');
          const items = this.speakItems;
          if (!items.length) throw new Error('no asks or replies in this record to read');
          this.player = window.readAloud.player({
            rate: Number(localStorage.getItem('wt:readRate')) || 1,
            onState: (s) => { this.speech = s; },
          });
          this.player.load(items);
          this.player.play();
        } catch (e) {
      ...
      setRate(r) {
        localStorage.setItem('wt:readRate', String(r));
        this.player?.setRate(r);
      },

      // The bar's own text: who is speaking and where in the record they are.
      get speechWhere() {
        const s = this.speech;
        return s?.item ? `${s.item.label}  ${s.i + 1}/${s.n}` : '';
      },

      // Speech outlives a component the host swaps out (a deck slide stepping
      // on, the page opening a second session), and a synth queue nothing holds
      // a handle to cannot be stopped. Alpine calls this on teardown.
      destroy() { this.player?.stop(); this.player = null; },

      // A short id is what a reader actually has: it is the record's filename
      // stem, and it is what search.py --show and the Sessions pane both print.
      // The day narrows it to one directory where the host knows one; without a
      // day, one recursive tree read names every record and the match is on the
      // suffix. Either way the listing is cached per store, so a deck of
      // sessions reads it once.
      guessPath() {
      ...
      async resolvePath() {
        ...
        return tree.find(p => p.endsWith(`-${this.id}.json`)) || '';
      },
      ...
      async resolveBranch() {
        const want = String(this.branch || '').trim().replace(/^origin\//, '');
        if (!want) return '';
        const tree = await listing(this.repo);
      ...
      mountRaw() {
        ...
        try {
          ...
          // empty under a "Show all 983 lines" button.
      ...
      async openDeck(at) {
        ...
        try {
          ...
          await window.sessionRender.open(this.record, {
