    ...
    return {
        ...
        <div class="w-full flex flex-col gap-2.5 px-4 pt-3"
          ...
          <div x-show="!framed" class="shrink-0 text-lg font-medium leading-snug">
            ...
            <span class="ml-1.5 font-mono text-sm font-normal opacity-45"
                  data-note="The record's filename stem, and how every other surface names this session."
                  x-text="'(' + (shortId || '') + ')'"></span>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm shrink-0">
            <template x-for="f in strip" :key="f.k">
              <!-- Nowrap holds a label and its number together, which is what
                   every fact here is except one: repos carries a list that
                   grew from two names to four when it started reading the
                   container's own scope, and a nowrap list overruns the
                   measure rather than wrapping. It is the only value on the
                   strip that is prose-shaped, so it is the only one that
                   wraps. -->
      ...
      get strip() {
        ...
        return [
          ...
          // container's own list, and `repos` is where the shell actually
          // stood: a checkout worked entirely through absolute paths appears in
          // one and not the other. Showing the narrower field here while the
          // row below acted on the wider one put two different answers to one
          // question a line apart, which a shot caught before this comment
          // existed. An older record carries no `attached` and reads as it
          // always did.
