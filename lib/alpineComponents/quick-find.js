...
//                   Two DEEPER passes ride the same tap gate: "Search file
//                   contents" calls the code-search API (searchCode below:
//                   snippets via text-match, default branches only), and
//                   "Search sessions" greps the captured session records
//                   client-side (searchSessions below: search.py's --grep in
//                   the browser, hits opening the Sessions pane's reader via
//                   web-tools:open-session). A ran search's rows replace the
//                   lanes while the query stays put; editing it falls back.
//
...
document.addEventListener('alpine:init', function () {
  Alpine.data('quickFind', function () {
    ...
    return {
      ...
      _treeLoading: {},
      deepLoading: false,
      code_: null,         // { q, hits, loading, error }: the last content search
      sess_: null,         // { q, hits, loading, error }: the last session search
      _sessRows: null,     // state/sessions.json rows, read once
      _sessRecords: {},    // session id -> { day, ask, segs }: fetched records, kept for the session
      ...
      // One recursive tree call per repo, cached for the session — the same
      // primitive (and the same economy) as the stage's Browse/Search and the
      // mention picker. HEAD resolves the default branch server-side.
      async ensureTree(repo) {
        ...
        try {
          ...
          const t = await gh.req('git/trees/HEAD?recursive=1');
          this.trees_[repo] = {
            paths: (t.tree || []).filter(e => e.type === 'blob').map(e => e.path),
            truncated: !!t.truncated,
          };
        } catch { this.trees_[repo] = { paths: [], truncated: false }; }
      ...
      // ~one line of context around the first case-insensitive hit, for the
      // note under a content or session row.
      clipAround(text, q) {
      ...
      // ── Content search: the GitHub code-search API, tap-gated ────────────
      // One call per tap (the API allows 10/min authenticated, which a tap
      // respects and a keystroke would not). text-match returns the fragments,
      // so rows carry a snippet. Scoped to the account, so the whole estate is
      // one qualifier. Honest limits, and why the file-NAME lanes coexist:
      // DEFAULT BRANCHES ONLY, indexing can lag a push by minutes, and files
      // over ~384 KB are not indexed.
      async searchCode(q) {
        if (this.code_?.loading) return;
        this.code_ = { q, hits: [], loading: true, error: '' };
        try {
          const S = window.__shell;
          const owner = (S.estateRepos?.[0]?.repo || S.REGISTRY_REPO || '').split('/')[0];
          const gh = new window.GH({ token: window.TOKEN });
          const res = await gh.req('/search/code?q=' + encodeURIComponent(q + ' user:' + owner) + '&per_page=20',
            { headers: { ...gh.headers, Accept: 'application/vnd.github.text-match+json' } });
          const hits = (res.items || []).map(it => ({
            repo: it.repository?.full_name || '', path: it.path,
            frag: this.clipAround(it.text_matches?.[0]?.fragment || '', q),
          }));
          this.code_ = { q, hits, loading: false, error: '' };
        } catch (e) { this.code_ = { q, hits: [], loading: false, error: String(e?.message || e) }; }
      },

      // ── Session search: grep the captured records client-side ────────────
      // The corpus is small (tens of records, single-digit MB), the reader
      // already exists (the Sessions pane's paged conversation), and the
      // code-search API is at its worst on prose inside JSON, so this lane
      // fetches and greps directly: candidate list from state/sessions.json,
      // one contents read per record, cached for the session so the first
      // search pays and the rest are instant. Matches look at what a record
      // quotes: the opening ask, every stored prompt, every stored reply, and
      // the closing message (search.py's --grep, in the browser).
      async searchSessions(q) {
        ...
        try {
          ...
          if (!this._sessRows) {
            ...
            this._sessRows = cache.rows || [];
          }
          await Promise.all(this._sessRows.map(async row => {
            if (this._sessRecords[row.id]) return;
            try {
              ...
              this._sessRecords[row.id] = { day: rec.day || row.day || '', ask: rec.opening_ask || '', segs };
            } catch { this._sessRecords[row.id] = { day: row.day || '', ask: row.ask || '', segs: [] }; }
          }));
          const ql = q.toLowerCase();
          const hits = [];
          for (const [id, r] of Object.entries(this._sessRecords)) {
            const seg = r.segs.find(s => String(s).toLowerCase().includes(ql));
            if (seg) hits.push({ id, day: r.day, ask: r.ask, frag: this.clipAround(seg, q) });
          }
      ...
      get rows() {
        ...
        const fileRow = (repo, path, sub, label) => ({
          key: 'f:' + repo + ':' + path, icon: 'ph-file', mono: true,
        ...
        } else if ((this.code_ && this.code_.q === q) || (this.sess_ && this.sess_.q === q)) {
          // ── Results mode: a ran search for THIS query replaces the lanes
          // until the query changes; editing a character falls back to them.
          for (const [slot, icon, what] of [[this.code_, 'ph-file-magnifying-glass', 'content'],
                                            [this.sess_, 'ph-chat-circle-text', 'session']]) {
            if (!slot || slot.q !== q) continue;
            if (slot.loading) out.push({ key: what + ':load', icon: 'ph-circle-notch', kind: 'noop', spin: true,
                                         label: 'Searching ' + what + '…' });
            else if (slot.error) out.push({ key: what + ':err', icon: 'ph-warning', kind: 'noop',
                                            label: what + ' search failed: ' + slot.error });
            else if (!slot.hits.length) out.push({ key: what + ':none', icon, kind: 'noop',
                                                   label: 'No ' + what + ' matches' });
            else if (what === 'content') {
              out.push(...slot.hits.map(h => ({
                key: 'c:' + h.repo + ':' + h.path, icon, mono: true,
                label: h.path, sub: short(h.repo), note: h.frag,
                kind: 'file', repo: h.repo, path: h.path,
              })));
            } else {
              out.push(...slot.hits.map(h => ({
                key: 's:' + h.id, icon,
          ...
          if (ql.length >= 2) {
            ...
            if (S.hasToken?.() && this.treesMissing) {
              out.push({ key: 'deep', icon: this.deepLoading ? 'ph-circle-notch' : 'ph-binoculars',
                         ...
                         label: this.deepLoading ? 'Loading estate trees…' : 'Search every repo for "' + q + '"',
                         sub: 'file names' });
            }
          }
        ...
        // is never an idea, so offering to jot it mid-walk is noise.
      ...
      act(r) {
        ...
        if (r.kind === 'code-gate') { this.searchCode(this.q.trim()); this.$refs.box?.focus(); return; }
