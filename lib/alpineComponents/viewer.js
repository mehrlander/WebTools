    ...
    },
    {
      ...
      after: async (f, ctx) => {
        ...
        try {
          ...
          // THE HOST'S THREE, and each is a fact the viewer cannot know.
          // `start` is which page this file should open on, which a host that
          // navigated here from a citation knows and the file does not carry.
          // `onPaint` fires per page as the column draws it lazily, so a mark
          // on page 60 is drawn when page 60 is, not when the document opened.
          // `onMount` hands back the flow so a host can scroll it, since
          // centring a mark needs the scroller and the mark together.
          const host = ctx.opts || {};
          const start = typeof host.start === 'function' ? host.start(f) : host.start;

          mount = await window.pdf.flow(look, stage, {
            ...(Number.isInteger(start) && start > 0 ? { start } : {}),
            ...(host.onPaint ? { onPaint: host.onPaint } : {}),
            alive: () => !stale(),
            ...
            // The host observes rather than replaces: the pager's own sync runs
            // first either way, so a host cannot leave the arrows disagreeing
            // with the page on screen.
            onPage: (i) => { at = i; sync(i); host.onPage?.(i, f); },
...
document.addEventListener('alpine:init', function() {
  Alpine.data('viewer', function(opts) {
    ...
    //
    // It exists because a host can have a reason to draw on a document that
    // the viewer could not have: home's submittal page marks where a
    // crosswalk row lands on the page it lands on. Without a slot, that page
    // had to keep its own copy of the reader to keep the mark, which is how it
    // came to hold a second mammoth, a second marked and a second SheetJS
