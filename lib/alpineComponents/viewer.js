const ViewRegistry = {
    ...
    },
    {
      ...
      render: () => `<div class="flex flex-col h-full w-full">
        <div class="flex items-center gap-4 px-3 py-1.5 border-b border-base-300 bg-base-200/50 text-base shrink-0">
          ...
          <span id="tab-deck" class="contents"></span>
        </div>
      ...
      after: (f) => {
        requestAnimationFrame(() => {
          ...
          try {
            ...
            ViewRegistry.mountRecordDeck(document.getElementById('tab-deck'), table, f);
          } catch (e) {
  ...
  // The deck entry for the table mode.
  //
  // READ THE ROWS, as against scanning them. A grid answers "which row" well
  // and "what does this row say" not at all, and every table this viewer opens
  // is one a reader might want the second question answered about. The button
  // is swipeDeck.entry(), the estate's one glyph for "this collection has a
  // reader", the same one Branch detail wears over the file deck.
  //
  ...
  // count that decides whether to mount at all would be 0 for every table.
  //
  // The record kit is pulled on first TAP; swipe-deck has to arrive before
  // that, since it mints the button. Both are cheap and both ride the
  // pre-build, so a page carrying the bundle pays no network for either.
  mountRecordDeck(slot, table, f) {
    if (!slot) return;
    const label = () => { try { return table.getRows('active').length; } catch { return 0; } };
    const mount = async () => {
      const n = label();
      if (!n) return;                       // an empty or fully filtered-out table offers nothing to read
      if (!window.swipeDeck) {
