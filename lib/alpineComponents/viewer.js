const ViewRegistry = {
      ...
      after: async (f, host) => {
        ...
        try {
          ...
          await painter.renderAsync(prepared.bytes, body, styles, {
            className: uid,
            inWrapper: true,
            // Word's own pagination, as last saved: the file records where
            // each page broke, and honouring it is what puts "Page 1 of 2"
            // under the content it was under. Ignored, the painter's default,
            // a section is one tall page.
          ...
          // MAXZ, the PDF column's bounds and its gesture code (kits/pdf.js,
          // `flow`), so the two document readers answer the same fingers the
          // same way. The natural width is measured once, at zoom 1; every
          // later measurement is of the pane, whose width is the only thing
          // that moves.
          const MINZ = 0.5, MAXZ = 4;
          const natural = Math.max(0, ...[...body.querySelectorAll(`section.${uid}`)]
            .map(s => s.getBoundingClientRect().width));
          let fitScale = 1, z = 1;
          const scale = () => fitScale * z;
          const apply = () => { body.style.zoom = String(scale()); };
          const refit = () => {
            if (!pane.isConnected) return;
            const avail = pane.clientWidth - 16;
            fitScale = natural > 0 && avail > 0 && natural > avail ? avail / natural : 1;
            apply();
          };
          refit();
          ...
          // The browser's own pinch is switched off so a two-finger gesture
          // reaches the handlers below; single-finger pans stay native.
          ...
          // the content coordinate under the anchor is taken at the old scale
          // and the scroll is set to put it back under the anchor at the new.
          // Exact in both axes, since CSS zoom is a pure scale of the layout
          // and the wrapper aligns `safe center`, so an overflowing page starts
          // at the left edge rather than losing its left half off screen.
          //
          // PREVIEW, THEN COMMIT. A CSS zoom re-lays out the whole document,
          // every page, table and run, and on a phone that is longer than a
          // frame, so a pinch that committed on every move landed only on the
          // frames that survived and read as jumping between sizes. While the
          // fingers are down the scale is drawn as a transform on `shell`,
          // which the compositor draws with no layout at all, and the zoom is
          // written once when they lift; the wheel commits once it goes quiet.
          // The transform's translate keeps the anchor fixed by the same
          // arithmetic the commit uses, so the two agree to the pixel and the
          // hand-off is invisible. The PDF column skips this because its
          // pages are canvases and a relayout there is cheap.
          const clamp = (nz) => Math.max(MINZ, Math.min(MAXZ, nz));
          const anchorOf = (ax, ay) => {
            const r = pane.getBoundingClientRect();
            return { px: ax == null ? r.width / 2 : ax - r.left,
                     py: ay == null ? r.height / 2 : ay - r.top };
          };
          let live = null, frame = 0;
          const preview = (nz, ax, ay) => {
            live = { z: clamp(nz), ax, ay };
            if (frame) return;
            frame = requestAnimationFrame(() => {
              frame = 0;
              if (!live) return;
              const k = live.z / z;
              const { px, py } = anchorOf(live.ax, live.ay);
              const tx = (pane.scrollLeft + px) * (1 - k), ty = (pane.scrollTop + py) * (1 - k);
              shell.style.willChange = 'transform';
              shell.style.transformOrigin = '0 0';
              shell.style.transform = `translate(${tx}px, ${ty}px) scale(${k})`;
              zoomPill.classList.remove('hidden');
              zoomLevel.textContent = Math.round(live.z * 100) + '%';
            });
          };
          const applyZoom = (nz, ax, ay) => {
            ...
            const want = clamp(nz);
            if (!(want > 0) || Math.abs(want - z) < 0.001) { showZoom(); return; }
            const { px, py } = anchorOf(ax, ay);
            const was = scale();
            const cx = (pane.scrollLeft + px) / was, cy = (pane.scrollTop + py) / was;
            z = want;
            apply();
            const now = scale();
            pane.scrollLeft = Math.max(0, cx * now - px);
            pane.scrollTop = Math.max(0, cy * now - py);
            showZoom();
          };
          const commit = () => { if (live) applyZoom(live.z, live.ax, live.ay); };
          ...
          // reports lines where a trackpad reports pixels. Previewed per
          // event and committed 120ms after the last one.
          let settle = 0;
          pane.addEventListener('wheel', (e) => {
            if (!e.ctrlKey && !e.metaKey) return;
            e.preventDefault();
            const dy = e.deltaMode === 1 ? e.deltaY * 16 : e.deltaMode === 2 ? e.deltaY * 400 : e.deltaY;
            preview((live ? live.z : z) * Math.exp(-dy * 0.01), e.clientX, e.clientY);
            clearTimeout(settle);
            settle = setTimeout(commit, 120);
          }, { passive: false });

          // Touch: two pointers, the zoom following the ratio of their distance
          // to what it was when the second landed, anchored on their midpoint.
          // The browser keeps panning under the gesture, and the anchor reads
          // the live scroll each frame, so a pan corrects itself.
          const pts = new Map();
          let pinch = null;
          const spread = () => {
          ...
          pane.addEventListener('pointerdown', (e) => {
            ...
            if (pts.size === 2) pinch = { d: spread().d, z };
          });
          pane.addEventListener('pointermove', (e) => {
            ...
            const s = spread();
            preview(pinch.z * (s.d / pinch.d), s.x, s.y);
          });
          for (const ev of ['pointerup', 'pointercancel', 'pointerleave']) {
            pane.addEventListener(ev, (e) => {
              if (!pts.delete(e.pointerId) || pts.size >= 2) return;
              if (pinch) { pinch = null; commit(); }
            });
          }
  ...
  pageCss(uid) {
    return `.${uid}-wrapper{background:transparent;padding:12px 8px 0;align-items:safe center}` +
