  ...
  modules: [
    ...
    },
    {
      ...
      render: (f) => {
        ...
        // The one place `max-w-none` is the wrong fix. Tailwind v4 emits
        // utilities into `@layer utilities` while the typography stylesheet is
        // unlayered, so the `.prose` measure wins whatever the utility says.
        // Structure carries it instead: the pane scrolls, the column is its
        // child. tools/test/viewer.test.mjs holds it, and the marker below is
        // an HTML comment because a line inside a template literal can carry
        // no other kind.
