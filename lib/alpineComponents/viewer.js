    ...
    },
    {
      ...
      render: () => `<div data-pdf="root" class="h-full w-full flex flex-col bg-base-200">
        <div data-pdf="bar" class="hidden shrink-0 items-center justify-between gap-2 px-2 py-1.5 border-b border-base-300 bg-base-100">
          ...
          <div class="flex items-center gap-1 shrink-0">
            <!-- The flow switch. ICON-ONLY, and it names the mode you get on
                 tap rather than the one you are in, which is the same thing
                 pages/pdf-inspect.html's Page/Stack swap does at narrow
                 widths. A label here would be a fourth text run in a bar that
                 already holds three, and the 390px geometry check is what
                 decided it: the pager, the byte size and "Inspect" leave no
                 room for a fifth word. -->
            <button data-pdf="flow" class="btn btn-xs btn-ghost px-1.5 hidden" type="button">
              <i data-pdf="flow-icon" class="ph ph-file"></i>
            </button>
      ...
      after: async (f, ctx) => {
        ...
        try {
          ...
          const flowBtn = el('flow'), flowIcon = el('flow-icon');
          const mountFlow = (kind, start) => {
            ...
            sync(at);
            // The button names what a TAP GIVES YOU, not where you are.
            if (flowIcon) flowIcon.className = 'ph ' + (kind === 'page' ? 'ph-scroll' : 'ph-file');
            if (flowBtn) {
          ...
          if (flowBtn && pages > 1) {
            flowBtn.classList.remove('hidden');
          ...
          // The buttons DRIVE the mounted flow rather than keeping a page
          // number of their own. That is the whole reason the pager and the
          // gesture cannot disagree: there is one position, the scroll offset
          // of whatever is mounted, and the arrows and a thumb are two ways of
          // moving it.
