  ...
  modules: [
    ...
    },
    {
      ...
      after: async (f) => {
        ...
        if (!img || !msg) return;
        const show = (src) => {
          img.onload = () => { msg.remove(); img.classList.remove('hidden'); };
        ...
        try {
          ...
          show('data:' + mime + ';base64,' + b64.replace(/\s/g, ''));
        } catch (e) {
...
document.addEventListener('alpine:init', function() {
  Alpine.data('viewer', function(opts) {
    ...
    return {
      ...
      get stats() {
        // Both halves are lies for an image: the "lines" are however many
        // newline bytes happen to fall in the binary, and the KB is the size of
        // the mangled text decode, not of the file. Better silent than wrong;
        // the real bytes are only known after the module fetches them.
