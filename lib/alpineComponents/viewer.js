    ...
    },
    {
      ...
      after: async (f, ctx) => {
        ...
        try {
          ...
          //
          // WHERE IT GOES, since a magnifier alone does not say: the workbench
          // at pages/pdf-inspect.html, which takes a document APART rather than
          // reading it. Text containers, characters and vector rules drawn as
          // layers over the page, detected columns and lattice cells, and the
          // table read two independent ways so agreement is visible
          // (docs/pdf-structure.md). Its main mode is PAGE-SCOPED: the overlays
          // describe one page at a time. So the link carries the page the
          // reader is on, kept current by `sync`, and the title says which page
          // it will open. Before that it always landed on page 1 and the
          // reader's position was silently thrown away at the door, which is
          // the sort of loss a link cannot report.
          const addr = f.repo && f.name
            ? f.repo + (f.ref ? '@' + f.ref : '') + ':' + f.name
            : '';
          let inspect = null;
          if (addr && ctx?.controls && !stale()) {
          ...
          const aimInspect = (a) => {
            if (!inspect) return;
            inspect.href = 'https://mehrlander.github.io/web-tools/pages/pdf-inspect.html'
              + (use ? '?use=' + encodeURIComponent(use) : '')
              + '#gh=' + addr + (pages > 1 ? '&page=' + (a + 1) : '');
            inspect.title = pages > 1
              ? `Take page ${a + 1} apart in the PDF workbench`
              : 'Take this page apart in the PDF workbench';
