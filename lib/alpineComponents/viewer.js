  ...
  drawSheet(sheet, xl, key) {
    ...
    const body = layout.rows.map(row => {
      ...
      const cells = row.cells.map(c => {
        ...
        // The stored value where the drawn one is a rounding of it: a format
        // that shows 13% for 0.125 is what Excel draws and is not the number.
        const title = c.raw ? ` title="${esc(c.raw)}"` : '';
        ...
        return `<td${span}${classes.length ? ` class="${classes.join(' ')}"` : ''}` +
               `${cellStyle ? ` style="${cellStyle}"` : ''}${title}>${text}</td>`;
      }).join('');
