    ...
    },
    {
      ...
      after: async (f, host) => {
        ...
        try {
          ...
          // FIT THE PAGE TO THE PANE. A page is drawn at its own width (8.5in
          // is 816px) and a docked pane or a phone is narrower, so the render
          // is zoomed down to fit rather than scrolled sideways; a pane wider
          // than the page leaves it at size. Measured at zoom 1 each time,
          // since a measurement taken under the previous zoom compounds.
          const fit = () => {
            const sec = body.querySelector(`section.${uid}`);
            if (!sec || !pane.isConnected) return;
            body.style.zoom = '1';
            const w = sec.getBoundingClientRect().width;
            const avail = pane.clientWidth - 16;
            if (w > 0 && avail > 0 && w > avail) body.style.zoom = String(avail / w);
          };
          fit();
          if (typeof ResizeObserver !== 'undefined') {
            const ro = new ResizeObserver(() => { if (pane.isConnected) fit(); else ro.disconnect(); });
            ro.observe(pane);
          }
          ...
          host.root.__doc = { pages, survey, locate };
        } catch (e) {
