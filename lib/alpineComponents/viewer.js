      ...
      render: () => `<div id="viewer-pdf" class="h-full w-full flex flex-col bg-base-200">
        <div id="viewer-pdf-bar" class="hidden shrink-0 items-center justify-between gap-2 px-2 py-1.5 border-b border-base-300 bg-base-100">
          <div class="flex items-center gap-2 min-w-0">
            ...
            <span id="viewer-pdf-size" class="font-mono text-xs opacity-50 truncate"></span>
          </div>
        ...
        <div id="viewer-pdf-stage" class="flex-1 min-h-0 relative">
          <div id="viewer-pdf-msg" class="absolute inset-0 grid place-items-center text-sm text-base-content/50">
          ...
          const render = (i, slide) => {
            ...
            (async () => {
              try {
                ...
                // subtract: this held a 28px allowance for padding that no
                // longer exists, which kept the page 14px narrower than its
                // pane on each side with nothing visible in the gap.
                const avail = Math.max(160, stage.clientWidth || root.clientWidth);
                const unit = pg.getViewport({ scale: 1 });
                const vp = pg.getViewport({ scale: Math.min(2, avail / unit.width) * dpr });
