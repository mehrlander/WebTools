...
document.addEventListener('alpine:init', function() {
  Alpine.data('viewer', function(opts) {
    ...
    return {
      // The registry above is the list, so name it rather than a plausible
      // one: this line advertised an `image` mode that has never been
      // registered, which is the kind of claim a drawer repeats to a reader
      // who then goes looking. A pasted image is rendered by toss-render's own
      // shell, not here.
      description: 'Multi-mode file viewer (raw, code, preview, table, tree) with pluggable render modules',
