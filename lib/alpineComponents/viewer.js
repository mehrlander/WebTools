const ViewRegistry = {
      ...
      after: async (f, host) => {
        ...
        try {
          ...
          const show = (i) => {
            ...
            pane.replaceChildren(ViewRegistry.drawSheet(s, xl, key));
          };
          ViewRegistry.mountSheetTabs(tabs, sheets, show);
          ViewRegistry.publishSheets(host, sheets, show, stale);
          show(0);
        } catch (e) {
  ...
  // written rather than behind "Could not read the workbook".
  ...
  async openWorkbook(f, host) {
    ...
    if (!window.xlsxKit) {
      ...
      else await this.loadAsset('https://cdn.jsdelivr.net/gh/mehrlander/web-tools@main/lib/kits/xlsx.js');
    }
    let result;
    try {
  ...
  publishSheets(host, sheets, show, stale) {
    if (!host?.root) return;
    host.root.__sheets = {
  ...
  drawSheet(sheet, xl, key) {
    ...
    const body = layout.rows.map(row => {
      ...
      const cells = row.cells.map(c => {
        ...
        if (c.note) classes.push(c.note.options ? 'dv' : 'note');
        ...
        // EVERYTHING THE CELL KNOWS AND DOES NOT DRAW, in one tooltip: the
        // stored value where the drawn one is a rounding of it (a format
        // showing 13% for 0.125 is what Excel draws and is not the number),
        // the form's input message, and the choices a list allows.
        const lines = [];
        if (c.raw) lines.push(c.raw);
        if (c.note) {
          const head = [c.note.title, c.note.prompt].filter(Boolean).join(': ');
        ...
        return `<td${span}${classes.length ? ` class="${classes.join(' ')}"` : ''}` +
