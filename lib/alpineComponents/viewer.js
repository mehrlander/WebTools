const ViewRegistry = {
  ...
  // a stage item with no repo behind it. Wider than IMAGE_MIME by exactly one
  // entry, and kept as its own question rather than merged into that map,
  // because `isImage` decides whether to render an <img> and a PDF is not one.
  // The stage reads this, so naming a dropped file `.pdf` is enough to open it.
  mimeFor(ext) {
    return this.IMAGE_MIME[ext] || (this.isPdf(ext) ? 'application/pdf' : '');
  },
  ...
  modules: [
    ...
    },
    {
      ...
      render: () => `<div id="viewer-pdf" class="h-full w-full flex flex-col bg-base-200">
        <div id="viewer-pdf-bar" class="hidden shrink-0 items-center justify-between gap-2 px-2 py-1.5 border-b border-base-300 bg-base-100">
          <div class="join">
            <button id="viewer-pdf-prev" class="btn btn-xs join-item" title="Previous page"><i class="ph ph-caret-left"></i></button>
            <span id="viewer-pdf-page" class="btn btn-xs join-item no-animation pointer-events-none font-mono tabular-nums">1 / 1</span>
            <button id="viewer-pdf-next" class="btn btn-xs join-item" title="Next page"><i class="ph ph-caret-right"></i></button>
          </div>
          <a id="viewer-pdf-open" class="btn btn-xs btn-ghost gap-1 hidden" target="_blank"
             title="Open this PDF in the inspector">
            <i class="ph ph-magnifying-glass-plus"></i> Inspect
          </a>
        </div>
        <div class="flex-1 overflow-auto grid place-items-center p-3">
          <canvas id="viewer-pdf-canvas" class="hidden max-w-full shadow-sm bg-base-100"></canvas>
          <div id="viewer-pdf-msg" class="text-sm text-base-content/50 flex items-center gap-2">
            <span class="loading loading-spinner loading-sm"></span> Reading the PDF…
          </div>
        </div>
      </div>`,
      after: async (f, host) => {
        ...
        if (!root) return;
        const el = (id) => root.querySelector('#viewer-pdf-' + id);
        const canvas = el('canvas'), msg = el('msg'), bar = el('bar');
        if (!canvas || !msg) return;

        // Every await below can outlive the file it was started for: switching
        // rows in a list is faster than fetching a document. One counter on the
        // registry, read after each await, so a superseded render stops instead
        // of painting the previous file's page over the current one.
        const token = ViewRegistry._pdfToken = (ViewRegistry._pdfToken || 0) + 1;
        const stale = () => ViewRegistry._pdfToken !== token;
        ...
        try {
          ...
          if (!window.pdf) {
            // gh.load honours ?use=, so a branch preview reads the branch's
            // kit. The CDN copy is the fallback for a page with no gh at all.
            if (window.gh?.load) await gh.load('kits/pdf.js');
            else await ViewRegistry.loadAsset('https://cdn.jsdelivr.net/gh/mehrlander/web-tools@main/lib/kits/pdf.js');
          }
          if (stale()) return;
          await window.pdf.loadPdfjs();
          if (stale()) return;

          // slice() because pdf.js transfers the buffer to its worker, and the
          // pager needs the bytes again on every page change.
          const doc = await window.pdfjsLib.getDocument({ data: bytes.slice(0) }).promise;
          if (stale()) return;

          const pages = doc.numPages;
          host?.report(`${pages} page${pages === 1 ? '' : 's'} · ${(size / 1024).toFixed(1)} KB`);

          let page = 1, drawing = false;
          const label = el('page'), prev = el('prev'), next = el('next');
          const paint = async () => {
            if (drawing || stale()) return;
            drawing = true;
            try {
              const pg = await doc.getPage(page);
              if (stale()) return;
              // Fit the width the pane actually has, then oversample by the
              // device pixel ratio so the text is not a blur on a phone. The
              // 2x ceiling keeps a poster-sized page from allocating a canvas
              // measured in tens of megapixels.
              const dpr = window.devicePixelRatio || 1;
              const avail = Math.max(160, root.clientWidth - 24);
              const unit = pg.getViewport({ scale: 1 });
              const fit = Math.min(2, avail / unit.width);
...
document.addEventListener('alpine:init', function() {
  Alpine.data('viewer', function(opts) {
    ...
    return {
      ...
      // it exists now, and it is the only module that fetches its own bytes,
      // because an image cannot be rebuilt from the text the viewer was handed.
      description: 'Multi-mode file viewer (raw, code, preview, table, tree, image) with pluggable ' +
        'render modules. The image module re-fetches the file as base64 through window.gh, since a ' +
        "binary's bytes do not survive the text pipeline, so it works in a private repo too; it is " +
        'also the one module marked exclusive, which outranks a host\'s blanket defaultMode.',
      ...
      //
      // An EXCLUSIVE available mode outranks all of that, and only one module
      // claims it: images. A host that sets `defaultMode: 'raw'` for its file
      // view is saying how to open a text file, since that is the only kind it
      // had; it cannot distinguish that from asking for a PNG's mangled bytes
      // as text, which nobody has ever wanted. So a module that alone can
      // display the file wins, and the mode strip still offers raw one tap
      // away. A caller naming the exclusive mode explicitly gets the same
      // answer, so the rule only ever redirects a default.
