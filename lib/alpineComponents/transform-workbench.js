        ...
        after: (f) => {
          requestAnimationFrame(() => {
            ...
            try {
              ...
              // The grid is a window, not the dataset: Tabulator builds an
              // internal model per row it is handed, so past the cap that
              // model alone can exhaust memory. Copy and download carry all
              // rows regardless.
              const CAP = 20000;
              const ref = document.getElementById("tf-workbench")?.__viewerData;
              let data = Array.isArray(ref) ? ref : JSON.parse(f.content);
              if (data.length > CAP) {
                const note = document.getElementById("tf-tab-note");
                if (note) note.textContent = "showing the first " + CAP.toLocaleString() + " of " + data.length.toLocaleString() + " rows (filters see the shown window; copy and download carry all)";
  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("tfViewer", function (opts) {
      ...
      return {
        ...
        // round-trip through a full serialization. Stored on the root element,
        // outside Alpine's reactivity.
        async show(file, content, data) {
          this.$root.__viewerData = data ?? null;
          this.file = file;
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        blankTab(name = "", fnSrc = "rows => rows") {
          return { name, fnSrc, fnGz: "", err: null, ok: false, rows: [], jGz: "", dirty: false, fixed: false, outs: {} };
        },
              ...
              if (fnKeys.length) {
                ...
                const newTabs = (await Promise.all(fnKeys.map(async (fk, fi) => {
                  ...
                  if (typeof o[dk] === "string") {
                    const rows = rowsFromCol(JSON.parse(await this.unzip(o[dk])));
                    return Object.assign(tab, { rows, jGz: o[dk], ok: true });
                  }
        ...
        mirrorOuts() {
          ...
          for (const t of this.tabs) {
            const o = (t.outs || {})[ds];
            if (o && !o.err) Object.assign(t, { rows: o.rows, jGz: o.jGz || "", ok: true, err: null });
            else Object.assign(t, { rows: [], jGz: "", ok: false, err: (o && o.err) || null });
          }
        },
        dsSwitched() { this.mirrorOuts(); this.syncSurfaces(); },

        // ---- run (async worker) ----
        runWorker(payload, onResult) {
        ...
        async run() {
          ...
          const needs = (t) => t.dirty || (combined ? !t.ok : !Object.keys(t.outs || {}).length);
          ...
          const ranIdx = new Set([...tasks.map(x => x.i), ...skipped]);
          if (!ranIdx.size) {
            // Nothing dirty: any built bundle is still valid, and a missing
            // one stays lazy (ensureBundle owns it).
            this.ran = true;
            return this.$nextTick(() => this.syncSurfaces());
          }
          const seedAlias = skipped.map(i => this.tabs[i].name).filter(Boolean);
          const batches = this.datasets.map(d => ({ name: d.name, rows: Alpine.raw(d.rows) }));

          if (combined) {
            for (const i of skipped)
              Object.assign(this.tabs[i], { rows: this.tagCol ? this.datasets.flatMap(d => this.rawFor(d)) : this.raw, ok: true, jGz: "", fnGz: "", err: null, dirty: false });
            if (tasks.length) {
              const seed = {};
              ...
              await this.runWorker({ batches, tagCol: this.tagCol, tasks, seed, seedAlias }, (res) => {
                ...
                if (res.err) Object.assign(t, this.blankTab(t.name, t.fnSrc), { err: res.err, fixed: t.fixed });
                else Object.assign(t, { rows: res.rows, ok: true, jGz: "", fnGz: "", err: null, dirty: false });
              });
            }
          } else {
            // Separate mode: the task set runs once per batch (a fresh worker
            // each, so each batch's bulk is reclaimed before the next). $refs
            // resolve within a batch's own run; the seed for each run is the
            // clean tabs' out for that batch.
            for (const idx of ranIdx) this.tabs[idx].outs = {};
            for (const d of this.datasets) {
              for (const i of skipped) this.tabs[i].outs[d.name] = { rows: this.rawFor(d) };
              if (!tasks.length) continue;
              const seed = {};
              this.tabs.forEach((t, ti) => {
                const o = !ranIdx.has(ti) && t.name && (t.outs || {})[d.name];
                if (o && o.rows && o.rows.length) seed[t.name] = Alpine.raw(o.rows);
              });
              await this.runWorker({ batches: [{ name: d.name, rows: Alpine.raw(d.rows) }], tagCol: this.tagCol, tasks, seed, seedAlias }, (res) => {
                const t = this.tabs[res.i];
                t.outs[d.name] = res.err ? { err: res.err } : { rows: res.rows };
              });
            }
            for (const idx of ranIdx) Object.assign(this.tabs[idx], { fnGz: "", dirty: false });
            this.mirrorOuts();
          }

          // The bundle (fn/data gzips, src capture) is stale now and rebuilds
          // lazily: on the Bundle view, a copy, or a download. Runs stay cheap.
          this.invalidateBundle();
          this.ran = true;
        ...
        async ensureBundle() {
          ...
          for (const t of this.tabs) {
            const has = combined ? (t.ok && !t.err) : Object.keys(t.outs || {}).some(k => t.outs[k].rows);
            if (!has) continue;
            if (!t.fnGz) t.fnGz = await this.zip(t.fnSrc);
            if (combined) {
              if (!t.jGz) t.jGz = await this.zip(JSON.stringify(colFromRows(Alpine.raw(t.rows))));
            } else {
              for (const k of Object.keys(t.outs)) {
                const o = t.outs[k];
                if (o.rows && !o.jGz) o.jGz = await this.zip(JSON.stringify(colFromRows(Alpine.raw(o.rows))));
              }
            }
          }
          this.rebuildBundle();
        ...
        // ---- views ----
        // The export text is produced on demand (a click), never held in state
        // or a DOM attribute: with large batches the eager CSV/JSON strings
        // were most of the run cost and all of the attribute bloat.
        async exportText(k) {
          if (k === "bundle") { await this.ensureBundle(); return this.bundle; }
          if (k === "profile") return this.profileData?.csv ?? "";
          return Papa.unparse(Alpine.raw(this.tabs[this.active]?.rows || []));
        },
        async copyResult() {
          const text = await this.exportText(this.curV);
        ...
        sz(k) {
          if (k === "bundle") return this.bundleSz || (this.ran ? "…" : "");
          if (k === "profile") {
        ...
        sendToViewer(filename, payloadStr, targetMode = "table", data = null) {
          ...
          if (v && v.__viewer) {
            v.__viewer.show(filename, payloadStr, data).then(() => {
        ...
        syncSurfaces() {
          if (this.curV === "csv") {
            ...
            const send = () => this.sendToViewer("manifest.json", JSON.stringify(this.bundleObj || {}, null, 2), "tree");
            if (!this.bundle && this.ran) this.ensureBundle().then(send);
            else send();
          }
        },
        profile(rows) {
        ...
        async dl() {
          const v = this.views.find(x => x.k === this.curV);
          const u = URL.createObjectURL(new Blob([await this.exportText(this.curV)], { type: v.m }));
            ...
            <div class="flex flex-wrap items-center gap-y-1.5 mb-2 shrink-0">
              ...
              <div class="flex items-center gap-2 ml-auto">
                ...
                <div class="flex gap-1">
                  <!-- The export text (full CSV or bundle) is produced at click
                       time, never bound into the attribute: with large batches
                       that binding held the whole dataset in the DOM. -->
