  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        // Everything the given key set determines, over the scan window.
        closureOf(keys) {
          const CAP = this.wizCapRows();
          const all = Alpine.raw(this.wizSrcTab()?.rows || []);
          const rows = all.length > CAP ? all.slice(0, CAP) : all;
          if (!rows.length || !keys.length) return { det: [], nKeys: 0 };
          const first = new Map(), live = new Set(this.wizCols.filter(c => !keys.includes(c) && (this.wizColN[c] || 0) > 1));
          for (const r of rows) {
            if (!live.size && first.size) break;
          ...
          return { det: this.wizCols.filter(c => live.has(c)), nKeys: first.size };
        },
        // Promote the current pick to a menu entry: compute what its key set
        // delineates, fold the free columns in, and paint the result back so
        // the sidebar shows what came along.
        addToMenu() {
          const keys = this.pickKeys();
          if (!keys.length) return;
          const picked = this.wizCols.filter(c => this.wizPick[c] === 1 && !keys.includes(c));
          const { det, nKeys } = this.closureOf(keys);
          const entry = {
            key: keys.length === 1 ? keys[0] : keys,
            cols: [...keys, ...this.wizCols.filter(c => picked.includes(c) || det.includes(c))],
            nKeys, depth: 0, custom: true,
          };
          this.wizCands = this.wizCands.filter(c => !(c.custom && this.kLabel(c) === this.kLabel(entry)));
          this.wizCands.push(entry);
          this.selectCand(entry);
        },
        // Preview the pick as the table it would produce, in the viewer, over
        // the FULL rows (the honest table, not the scan window); no tab is
        // created.
        previewPick() {
          const keys = this.pickKeys();
          const cols = this.wizCols.filter(c => this.wizPick[c]);
          if (!keys.length) return;
          const all = Alpine.raw(this.wizSrcTab()?.rows || []);
          const m = new Map();
          for (const r of all) {
            const k = keys.length === 1 ? r[keys[0]] : JSON.stringify(keys.map(c => r[c]));
            if (!m.has(k)) m.set(k, Object.fromEntries(cols.map(c => [c, r[c]])));
          }
          const rows = [...m.values()];
          // .json so the viewer's Table mode is available for it
          this.sendToViewer("dim-preview.json", this.previewJson(rows), "table", rows);
        },
        wizRefsLine() {
        ...
        scanDims() {
          ...
          // Candidates render as a rollup tree: the most detailed key is the
          // parent and each key it determines nests under it (Program under
          // Program Index), depth marking the delineation. A mutual (1:1)
          // pair keeps the wider or earlier column on top; the seen set stops
          // the cycle a mutual pair would otherwise recurse into.
          ...
          const node = key => ({ key, cols: [key, ...cols.filter(c => dets[key].has(c))], nKeys: distinct[key] });
          const childrenOf = key => {
            const abs = keys.filter(b => b !== key && dets[key].has(b));
            return abs.filter(b => !abs.some(m => m !== b && dets[m].has(b)));
          };
          const flatten = (key, depth, out, seen) => {
            if (seen.has(key)) return out;
            seen.add(key);
            out.push({ ...node(key), depth });
            for (const c of childrenOf(key).sort((x, y) => dets[y].size - dets[x].size)) flatten(c, depth + 1, out, seen);
            return out;
          };
          const seen = new Set();
          this.wizCands = keys.filter(k => !absorbed.has(k))
            .sort((x, y) => dets[y].size - dets[x].size)
            .flatMap(k => flatten(k, 0, [], seen));
        },
        dimName(seed) {
        ...
        // ---- misc ----
        clear() {
          Object.assign(this, { raw: [], datasets: [], scope: "__all", editingDs: -1, addOpen: false, bndOpen: false, combine: true, tagCol: "", ran: false, bundle: "", bundleSz: "", bundleObj: {}, bndSel: {}, srcGz: {}, wizOpen: false, wizCands: [], wizCols: [], wizColN: {}, wizScanned: 0, wizSel: "", wizPick: {}, tabs: [this.identityTab()], active: 0, profileData: {}, loaded: "" });
          this.saveTabs();
    ...
    <div class="lg:flex-1 flex flex-col lg:flex-row gap-2 lg:gap-0 min-h-0" :class="(drag || bndDrag || wizDrag) && 'pointer-events-none'">
      ...
      <div class="card bg-base-100 border border-base-300 flex flex-col overflow-hidden" :class="wizOpen ? 'flex-1 min-h-0' : 'shrink-0'">
        ...
        <div x-show="wizOpen" class="flex-1 min-h-0 flex max-h-[18rem] lg:max-h-none">
          ...
          <div class="w-44 shrink-0 border-r border-base-200 overflow-y-auto p-1 flex flex-col gap-0.5">
            <template x-for="c in wizCols" :key="c">
              <button class="btn btn-xs h-5 min-h-0 px-1.5 normal-case font-mono justify-start gap-1 border transition-none w-full flex-nowrap"
            ...
            <!-- Acting on the pick: preview it in the viewer, add it straight
                 as a tab, promote it to a menu entry (which computes what its
                 key delineates), or start over. -->
            <div x-show="wizCols.length" class="grid grid-cols-2 gap-1 mt-1">
          ...
          <div class="flex-1 overflow-y-auto p-2 text-[11px]">
            ...
            <template x-for="c in wizCands" :key="(c.custom ? '*' : '') + kLabel(c)">
              <div class="flex items-center gap-2 py-1 pl-1 border-b border-base-200 last:border-0 cursor-pointer hover:bg-base-200/40 rounded"
