...
(() => {
  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        // meta: { label, seed: [recipe names to open as tabs], collapse }.
        // Loading is a viewing gesture, so the editor collapses by default and
        // identity auto-runs; a paste is the intake gesture and keeps it open.
        ...
        blankTab(name = "", fnSrc = "rows => rows") {
          return { name, fnSrc, fnGz: "", err: null, obj: null, rows: [], json: "", csv: "", jGz: "", sz: {}, dirty: false, fixed: false };
        },
        ...
        async ensureSrcGz() {
          if (this.preserveSrc && this.raw.length && !this.srcGz) this.srcGz = await this.zip(JSON.stringify(colFromRows(this.raw)));
        },
        ...
        async processText(t) {
          ...
          try {
            if (t.startsWith("{")) {
              ...
              const fnKeys = Object.keys(o).filter(k => k === "fn" || k.startsWith("fn_"));

              if (fnKeys.length) {
                const newTabs = (await Promise.all(fnKeys.map(async fk => {
                  const name = fk === "fn" ? "" : fk.slice(3), dk = name ? "data_" + name : "data";
                  if (typeof o[dk] !== "string") return null;

                  const [fnSrc, dataStr] = await Promise.all([this.unzip(o[fk]), this.unzip(o[dk])]);
                  const col = JSON.parse(dataStr), rows = rowsFromCol(col);
                  const json = JSON.stringify(col, null, 2), csv = Papa.unparse(rows);

                  return { ...this.blankTab(name, fnSrc), fnGz: o[fk], obj: col, rows, json, csv, jGz: o[dk], sz: { json: sizeOf(json), jGz: sizeOf(o[dk]), csv: sizeOf(csv) } };
                }))).filter(Boolean);
                ...
                if (newTabs.length) {
                  ...
                  this.active = 0;
                  this.srcGz = o.src || "";
                  this.preserveSrc = !!o.src;
                  this.raw = o.src ? rowsFromCol(JSON.parse(await this.unzip(o.src))) : [];
        ...
        // Take rows (from a paste, a drop, or a host's loadRows) into the
        // workbench: set raw, reset every tab to unrun, show raw in the viewer,
        // and tell the host.
        ingest(rows) {
          this.raw = (rows || []).filter(r => Object.values(r).some(v => v !== ""));
        ...
        async run() {
          ...
          while (changed) {
            ...
            for (const t of this.tabs) {
              if (t.dirty || !t.obj) continue;
              if (refsOf(t.fnSrc).some(n => {
                const dep = this.tabs.find(x => x.name === n);
                return dep && (dep.dirty || !dep.obj);
              })) { t.dirty = true; changed = true; }
            }
          }

          const tasks = this.tabs.flatMap((t, i) => (t.dirty || !t.obj) ? [{ i, name: t.name, fnSrc: t.fnSrc }] : []);
          if (!tasks.length) {
          ...
          const seed = {};
          for (const t of this.tabs) if (!t.dirty && t.obj && t.name && t.rows.length) seed[t.name] = Alpine.raw(t.rows);

          const workerUrl = URL.createObjectURL(new Blob([TF_WORKER_SRC], { type: "application/javascript" }));
          const worker = new Worker(workerUrl);
          worker.postMessage({ raw: Alpine.raw(this.raw), tasks, seed });

          worker.onmessage = async (e) => {
            await Promise.all(e.data.results.map(async res => {
        ...
        rebuildBundle() {
          const obj = this.preserveSrc && this.srcGz ? { src: this.srcGz } : {};
          const used = new Set();
          for (const t of this.tabs) {
            if (!t.fnGz) continue;
        ...
        // ---- misc ----
        clear() {
          Object.assign(this, { raw: [], ran: false, bundle: "", bundleSz: "", bundleObj: {}, srcGz: "", tabs: [this.identityTab()], active: 0, rawProfile: false, profileData: {}, loaded: "" });
          this.saveTabs();
  ...
  <div class="tf-root flex flex-col lg:h-full min-h-0 gap-2 select-none" x-data="transformWorkbench(${dataExpr || ""})"
    ...
    <div class="flex flex-wrap items-center px-1 shrink-0 gap-x-3 gap-y-1.5">
      ...
      <span x-show="loaded" class="text-[10px] opacity-50 font-mono truncate max-w-[16rem]" x-text="loaded"></span>
    ...
    <div class="card bg-base-100 border border-base-300 shrink-0" x-show="!raw.length">
      <div class="p-1">
        <div class="border border-dashed rounded-md py-3 text-center cursor-pointer hover:bg-base-200 transition-colors"
             :class="hot ? 'border-primary bg-primary/5' : ran ? 'border-warning/40 bg-warning/5' : 'border-base-300'"
             @paste.prevent="handlePaste($event)"
             @dragenter.prevent="hot = true" @dragover.prevent
             @dragleave.prevent="hot = false" @drop.prevent="handleDrop($event)"
             tabindex="0">
          <div x-show="!ran" class="text-base-content/40 text-xs flex items-center justify-center gap-2 pointer-events-none">
            <i class="ph-bold text-lg" :class="hot ? 'ph-file-arrow-down text-primary' : 'ph-clipboard-text'"></i>
            <span x-text="hot ? 'Drop to load' : 'Paste or drop TSV, CSV, JSON, or a bundle'"></span>
          </div>
          <div x-show="ran" class="flex items-center justify-center gap-2 text-warning text-xs font-bold pointer-events-none">
