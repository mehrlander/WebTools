          ...
          <div x-show="ran" class="flex flex-col h-full">
            <div class="flex flex-wrap items-center gap-y-1.5 mb-2 shrink-0">
              <div class="flex gap-1.5 items-center">
                ...
                <!-- The inspection scope: Data and Profile speak about the
                     active tab at this scope (the combined stack or a single
                     batch). A scope the last Run did not target computes on
                     demand. -->
                <select x-show="scopeOptions().length > 1" :value="scope" @change="setScope($event.target.value)"
                        title="What Data and Profile speak about: the combined stack, or one batch (computed on demand if needed)"
                        class="select select-xs select-bordered h-7 min-h-0 max-w-[9rem] font-mono ml-1">
                  <template x-for="n in scopeOptions()" :key="n">
                    <option :value="n" x-text="scopeLabel(n)"></option>
                  </template>
                </select>
              </div>
