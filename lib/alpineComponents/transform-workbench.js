  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        previewOn: "",   // banner label while a dim preview borrows the viewer
        loaded: "",
        ...
        // Preview the pick as a relationship assessment, not a silent table
        // swap: for each tested column, does the key set delineate it, and
        // where do the multiplicities sit (how many keys carry more than one
        // value, and the worst example). Runs over the FULL rows; no tab is
        // created; a banner marks the viewer as borrowed until the next
        // normal sync reclaims it.
        previewPick() {
          const keys = this.pickKeys();
          if (!keys.length) return;
          ...
          const seen = new Map();   // key -> { col -> Set of values, capped }
          for (const r of all) {
          ...
          const report = attrs.map(c => {
            ...
            return {
              ...
              Delineated: varying === 0 ? "yes" : "no: " + varying.toLocaleString() + " of " + nKeys.toLocaleString() + " keys vary",
              "Worst key": worstK == null ? "" : String(worstK) + " → " + [...seen.get(worstK)[c]].slice(0, 4).join(" | ") + (worstN > 4 ? " …" : ""),
            };
          }).sort((a, b) => (a.Delineated === "yes" ? 0 : 1) - (b.Delineated === "yes" ? 0 : 1));
          this.sendToViewer("dim-assess.json", this.previewJson(report), "table", report);
          this.previewOn = keys.join(" + ") + " · " + nKeys.toLocaleString() + " keys · testing " + attrs.length + " column" + (attrs.length > 1 ? "s" : "");
        },
        wizRefsLine() {
        ...
        syncSurfaces() {
          this.previewOn = "";   // any normal sync reclaims the viewer
          if (this.curV === "csv") {
      ...
      <div class="card bg-base-100 border border-base-300 flex flex-col overflow-hidden"
        ...
        <div class="p-2 flex flex-col h-full">
          ...
          <div x-show="ran" class="flex flex-col h-full">
            ...
            <!-- While a dim preview borrows the viewer, say so; closing (or
                 any tab/view/scope change) reclaims it. -->
            <div x-show="previewOn" class="flex items-center gap-2 px-2 py-1 mb-1 rounded bg-primary/10 text-[11px] shrink-0">
              <i class="ph-bold ph-eye"></i><span class="font-medium">dim preview:</span>
              <span class="font-mono text-[10px] opacity-70 truncate" x-text="previewOn"></span>
              <button class="btn btn-xs btn-ghost h-5 min-h-0 px-1 ml-auto" @click="syncSurfaces()" title="Back to the tab's output"><i class="ph ph-x"></i></button>
            </div>
