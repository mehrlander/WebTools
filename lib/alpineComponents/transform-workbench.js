  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        pvCols: [], pvIsNum: {}, pvN: {}, pvDec: {}, pvDims: [], pvMeasure: "", pvAgg: "sum", pvDrag: "",
        pvSrc: "", pvOut: null,
        ...
        pvDropAt(i) {
          ...
          dims.splice(j, 0, c);
          if (dims.join("\u0001") === this.pvDims.join("\u0001")) return;
          this.pvDims = dims;
          this.pvDraw();
        },
        pvDropOut() {
          const c = this.pvDrag; this.pvDrag = "";
          if (!c || !this.pvDims.includes(c)) return;
        ...
        pvTree(rows) {
          const dims = this.pvDims, agg = this.pvAgg, mk = this.pvMeasure;
          const root = { label: "Total", value: 0, n: 0, kids: new Map() };
          const bump = (node, v) => {
            node.n++;
          ...
          for (const r of rows) {
            ...
            for (const d of dims) {
              ...
              if (!kid) { kid = { label: k, value: 0, n: 0, kids: new Map() }; node.kids.set(k, kid); }
              bump(kid, v);
              node = kid;
            }
          }
        ...
        clear() {
          Object.assign(this, { raw: [], datasets: [], scope: "__all", editingDs: -1, addOpen: false, bndOpen: false, combine: true, tagCol: "", ran: false, bundle: "", bundleSz: "", bundleObj: {}, bndSel: {}, srcGz: {}, wizOpen: false, wizCands: [], wizCols: [], wizColN: {}, wizScanned: 0, wizCustom: [], wizSel: "", wizPick: {}, previewOn: "", tabs: [this.identityTab()], active: 0, profileData: {}, loaded: "",
            pvCols: [], pvIsNum: {}, pvN: {}, pvDec: {}, pvDims: [], pvMeasure: "", pvAgg: "sum", pvSrc: "", pvOut: null });
          this.saveTabs();
          ...
          <div x-show="ran" class="flex flex-col h-full">
            ...
            <div x-show="curV === 'pivot' && !previewOn && tabs[active]?.rows?.length" class="flex flex-col gap-1.5 mb-2 shrink-0 text-[11px]">
              ...
              <div class="flex flex-wrap items-center gap-1 min-h-7 rounded-[var(--radius-field)] border border-dashed px-1 py-0.5"
                   :class="pvDrag && !pvDims.includes(pvDrag) ? 'border-primary/60 bg-primary/5' : 'border-base-300'"
                   @dragover.prevent @drop.prevent="pvDropAt(pvDims.length)">
              ...
              <div class="flex flex-wrap items-center gap-1 px-1"
                   :class="pvDrag && pvDims.includes(pvDrag) ? 'rounded-[var(--radius-field)] border border-dashed border-error/50 bg-error/5' : ''"
                   @dragover.prevent @drop.prevent="pvDropOut()">
                <template x-for="c in pvCols.filter(x => !pvDims.includes(x))" :key="'pvp-' + c">
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                <button class="btn btn-xs h-6 min-h-0 px-2 btn-ghost font-normal normal-case ml-auto opacity-50 hover:opacity-100"
                        @click="pvDims = []; pvMeasure = ''; pvAgg = 'sum'; pvSrc = ''; syncSurfaces()"
