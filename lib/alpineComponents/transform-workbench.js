  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        wizPick: {}, wizFact: true, wizSplit: 55, wizDrag: false,
        loaded: "",
        ...
        cyclePick(c) { this.wizPick[c] = ((this.wizPick[c] || 0) + 1) % 3; },
        scanDims() {
          ...
          Object.assign(this, { wizScanned: rows.length, wizCands: [], wizCols: [], wizPick: {} });
        ...
        // ---- misc ----
        clear() {
          Object.assign(this, { raw: [], datasets: [], scope: "__all", editingDs: -1, addOpen: false, bndOpen: false, combine: true, tagCol: "", ran: false, bundle: "", bundleSz: "", bundleObj: {}, bndSel: {}, srcGz: {}, wizOpen: false, wizCands: [], wizCols: [], wizScanned: 0, wizPick: {}, tabs: [this.identityTab()], active: 0, profileData: {}, loaded: "" });
          this.saveTabs();
    ...
    <div class="lg:flex-1 flex flex-col lg:flex-row gap-2 lg:gap-0 min-h-0" :class="(drag || bndDrag || wizDrag) && 'pointer-events-none'">
      ...
      <div class="card bg-base-100 border border-base-300 flex flex-col overflow-hidden" :class="wizOpen ? 'flex-1 min-h-0' : 'shrink-0'">
        <div class="flex items-center gap-2 px-2 py-1 shrink-0" :class="wizOpen && 'border-b border-base-300'">
          ...
          <span x-show="wizOpen" class="font-mono text-[10px] opacity-50"
                x-text="wizScanned ? wizScanned.toLocaleString() + ' rows scanned' : 'no output to scan yet'"></span>
        ...
        <div x-show="wizOpen" class="flex-1 min-h-0 overflow-y-auto p-2 text-[11px] space-y-3 max-h-[18rem] lg:max-h-none">
          <!-- The active fn in one line: shape and both directions of $refs. -->
          <div class="font-mono text-[10px] opacity-60 flex flex-wrap gap-x-3 gap-y-0.5">
            <span x-text="wizInfo().name + ': ' + wizInfo().rows.toLocaleString() + ' rows × ' + wizInfo().cols + ' cols'"></span>
            <span x-show="wizInfo().reads.length" x-text="'reads ' + wizInfo().reads.map(n => '$' + n).join(', ')"></span>
            <span x-show="wizInfo().readBy.length" x-text="'read by ' + wizInfo().readBy.join(', ')"></span>
          </div>
          <div x-show="wizCands.length">
            <div class="opacity-50 mb-1">Candidates found here (a key column and every column it fully determines):</div>
          ...
          <div x-show="wizCols.length">
            <div class="opacity-50 mb-1">Or build one by hand: tap a column once to include it, twice to make it a key. Several keys form a compound key; none makes a plain distinct table.</div>
            <div class="flex flex-wrap gap-1 mb-1.5">
              <template x-for="c in wizCols" :key="c">
                <button class="btn btn-xs h-6 min-h-0 px-2 normal-case font-mono transition-none border gap-1"
                        ...
                        @click="cyclePick(c)">
                  <i x-show="(wizPick[c] || 0) === 2" class="ph-bold ph-key text-[9px]"></i>
                  <span x-text="c"></span>
                </button>
              </template>
            </div>
            <button class="btn btn-xs btn-outline h-6 min-h-0 px-2 normal-case" :disabled="!Object.values(wizPick).some(v => v)" @click="addManualDim()">+ dim from picked</button>
          </div>
        </div>
      </div>
      </div>
