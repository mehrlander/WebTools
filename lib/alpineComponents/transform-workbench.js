  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        pvCols: [], pvIsNum: {}, pvN: {}, pvDec: {}, pvDims: [], pvMeasure: "", pvAgg: "sum",
        pvSrc: "", pvOut: null,
          ...
          <div x-show="ran" class="flex flex-col h-full">
            ...
            <div x-show="curV === 'pivot' && !previewOn && tabs[active]?.rows?.length" class="flex flex-col gap-1.5 mb-2 shrink-0 text-[11px]">
              <div class="flex flex-wrap items-center gap-1">
                ...
                <template x-for="c in pvCols" :key="'pvd-' + c">
                  <button class="btn btn-xs h-6 min-h-0 px-2 font-normal normal-case gap-1 border border-base-300 transition-none"
                          :class="pvDims.includes(c) ? 'btn-primary btn-soft' : 'btn-ghost bg-base-100'"
                          :disabled="!pvDims.includes(c) && pvDims.length >= 4"
                          :title="(pvN[c] > 1000 ? '1000+' : pvN[c]) + ' distinct'"
                          @click="pvToggleDim(c)">
                    <span x-show="pvDims.includes(c)" class="font-mono opacity-60" x-text="pvDims.indexOf(c) + 1"></span>
                    <span x-text="c"></span>
                  </button>
                </template>
              </div>
