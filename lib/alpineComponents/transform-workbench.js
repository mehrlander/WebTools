...
(() => {
  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        bndSel: {}, srcGz: {},
        // The dimension wizard's working state.
        wizOpen: false, wizSrc: "", wizCands: [], wizCols: [], wizScanned: 0,
        wizManKey: "", wizManCols: {}, wizFact: true,
        loaded: "",
        ...
        // Mechanical star-schema help: scan a tab's rows for functional
        // dependencies (key -> attribute), fold into each candidate every
        // column its key can express (Program Index absorbing Program), and
        // generate ordinary editable dim/fact tabs from the finding or from a
        // hand-picked column set. The generated dim fn re-checks the
        // dependency on every run and reports conflicts, so dirty reference
        // data surfaces instead of silently keeping the first spelling.
        openWizard() {
          this.wizOpen = !this.wizOpen;
          if (!this.wizOpen) return;
          const t = this.tabs[this.active] || this.tabs[0];
          this.wizSrc = t.name || "identity";
          this.scanDims();
        },
        wizSrcTab() { return this.tabs.find(t => (t.name || "identity") === this.wizSrc) || this.tabs[0]; },
        scanDims() {
          const CAP = 50000;   // dependency scan window; the generated fn re-checks everything at run
          const all = Alpine.raw(this.wizSrcTab()?.rows || []);
          const rows = all.length > CAP ? all.slice(0, CAP) : all;
          Object.assign(this, { wizScanned: rows.length, wizCands: [], wizCols: [], wizManKey: "", wizManCols: {} });
          ...
          // A candidate absorbs another whose key it determines; a mutual
          // (1:1) pair keeps the wider candidate, then the earlier column.
          // Only maximal candidates are presented.
          const keys = Object.keys(dets), absorbed = new Set();
          for (const a of keys) for (const b of keys) {
            if (a === b || !dets[a].has(b)) continue;
            if (dets[b].has(a)) {
              if (dets[b].size > dets[a].size) continue;
              if (dets[b].size === dets[a].size && cols.indexOf(b) < cols.indexOf(a)) continue;
            }
            absorbed.add(b);
          }
          this.wizCands = keys.filter(k => !absorbed.has(k)).map(key => ({
            key, cols: [key, ...cols.filter(c => dets[key].has(c))],
            nKeys: distinct[key],
            absorbs: keys.filter(b => b !== key && dets[key].has(b)),
          })).sort((x, y) => y.cols.length - x.cols.length);
        },
        dimName(key) {
          const base = "dim_" + String(key).toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
          let n = base, i = 2;
        ...
        genDimSrc(name, key, attrs) {
          const q = JSON.stringify;
          const pick = "{ " + [key, ...attrs].map(c => q(c) + ": r[" + q(c) + "]").join(", ") + " }";
          const checks = attrs.map(c => "seen[" + q(c) + "] !== r[" + q(c) + "]").join(" || ");
          return [
            "rows => {",
            "  const m = new Map();",
            "  let conflicts = 0;",
            "  for (const r of " + this.wizRef() + ") {",
            "    const seen = m.get(r[" + q(key) + "]);",
            "    if (!seen) m.set(r[" + q(key) + "], " + pick + ");",
            attrs.length ? "    else if (" + checks + ") conflicts++;" : null,
            "  }",
            "  if (conflicts) console.warn(" + q(name) + " + \": \" + conflicts + \" rows disagree with their key's first-seen attributes\");",
            "  return [...m.values()];",
            "}",
          ].filter(Boolean).join("\n");
        },
        addDim(cand) {
          const name = this.dimName(cand.key);
          const attrs = cand.cols.filter(c => c !== cand.key);
          const tab = Object.assign(this.blankTab(name, this.genDimSrc(name, cand.key, attrs)),
            { dirty: true, dim: { key: cand.key, cols: cand.cols, src: this.wizSrc } });
          this.tabs.push(tab);
        ...
        addManualDim() {
          if (!this.wizManKey) return;
          const picked = this.wizCols.filter(c => c !== this.wizManKey && this.wizManCols[c]);
          this.addDim({ key: this.wizManKey, cols: [this.wizManKey, ...picked] });
          this.wizManCols = {};
        },
        ...
        updateFactTab() {
          const drop = [...new Set(this.tabs.flatMap(t => t.dim ? t.dim.cols.filter(c => c !== t.dim.key) : []))];
          const q = JSON.stringify;
        ...
        // ---- misc ----
        clear() {
          Object.assign(this, { raw: [], datasets: [], scope: "__all", editingDs: -1, addOpen: false, bndOpen: false, combine: true, tagCol: "", ran: false, bundle: "", bundleSz: "", bundleObj: {}, bndSel: {}, srcGz: {}, wizOpen: false, wizCands: [], wizCols: [], wizScanned: 0, tabs: [this.identityTab()], active: 0, profileData: {}, loaded: "" });
          this.saveTabs();
  ...
  <div class="tf-root flex flex-col lg:h-full min-h-0 gap-2 select-none" x-data="transformWorkbench(${dataExpr || ""})"
       @mousemove.window="if(drag){const r=$el.getBoundingClientRect(); split=Math.min(80,Math.max(20,(($event.clientX-r.left)/r.width)*100))} if(bndDrag){const r=$refs.rightcol.getBoundingClientRect(); bndSplit=Math.min(85,Math.max(20,(($event.clientY-r.top)/r.height)*100))}"
       @mouseup.window="drag=false; bndDrag=false">
    ...
    <div class="lg:flex-1 flex flex-col lg:flex-row gap-2 lg:gap-0 min-h-0" :class="(drag || bndDrag) && 'pointer-events-none'">
        ...
        <div class="p-2 flex flex-col h-full">
          <div class="flex justify-between items-center mb-2 shrink-0 gap-1.5">
            <div class="flex gap-1.5 items-center flex-1 min-w-0 p-1 overflow-x-auto [&::-webkit-scrollbar]:hidden">
              ...
              <button @click="openWizard()" class="btn btn-xs btn-ghost px-2 shrink-0" :class="wizOpen ? 'text-primary opacity-100' : 'opacity-40 hover:opacity-100'"
                      title="Dimension wizard: extract repeating columns into dimension tabs"><i class="ph-bold ph-magic-wand"></i></button>
          ...
          <textarea x-show="!wizOpen" class="textarea textarea-bordered font-mono text-[11px] w-full flex-1 leading-tight resize-none focus:outline-none p-2 border-base-200"
          ...
          <div x-show="tabs[active]?.err && !wizOpen" class="text-error text-[10px] font-mono mt-1" x-text="tabs[active]?.err"></div>

          <!-- The dimension wizard: replaces the code editor while open. -->
          <div x-show="wizOpen" class="flex-1 min-h-0 overflow-y-auto border border-base-200 rounded p-2 text-[11px] space-y-3">
            <div class="flex items-center gap-2 flex-wrap">
              <i class="ph-bold ph-magic-wand text-primary"></i><span class="font-medium">Dimension wizard</span>
              <span class="opacity-50">source:</span>
              <select class="select select-xs select-bordered h-6 min-h-0 font-mono" x-model="wizSrc" @change="scanDims()">
                <template x-for="t in tabs" :key="t.name || 'identity'">
                  <option :value="t.name || 'identity'" x-text="t.name || 'identity'"></option>
                </template>
              </select>
              <span class="opacity-40 font-mono text-[10px]"
                    x-text="wizScanned ? wizScanned.toLocaleString() + ' rows scanned' : 'no rows here yet: paste data and Run first'"></span>
              <label class="flex items-center gap-1.5 cursor-pointer ml-auto opacity-70 hover:opacity-100 select-none" title="Keep one generated fact tab that drops every column a dim expresses, keys kept">
                <input type="checkbox" class="checkbox checkbox-xs" x-model="wizFact"> <span>maintain fact tab</span>
              </label>
            </div>
            <div>
              <div class="opacity-50 mb-1">Found candidates (a key column and every column it fully determines):</div>
              <template x-for="c in wizCands" :key="c.key">
                <div class="flex items-start gap-2 py-1.5 border-b border-base-200 last:border-0">
                  <button class="btn btn-xs btn-outline h-6 min-h-0 px-2 shrink-0 normal-case font-mono" @click="addDim(c)">+ <span x-text="dimName(c.key)"></span></button>
                  <div class="min-w-0">
                    <div><span class="font-mono font-medium" x-text="c.key"></span> <span class="opacity-40" x-text="'(' + c.nKeys.toLocaleString() + ' distinct)'"></span></div>
                    <div class="opacity-60 font-mono text-[10px]" x-text="c.cols.slice(1).join(', ')"></div>
                    <div x-show="c.absorbs.length" class="opacity-40 text-[10px]" x-text="'also expresses the ' + c.absorbs.join(', ') + ' level' + (c.absorbs.length > 1 ? 's' : '')"></div>
                  </div>
                </div>
              </template>
              <div x-show="wizScanned && !wizCands.length" class="opacity-40 italic">no functional dependencies found in the scanned rows</div>
            </div>
            <div>
              <div class="opacity-50 mb-1">Or build one by hand (pick the key, check the columns it should carry):</div>
              <div class="flex items-center gap-2 flex-wrap mb-1.5">
                <span class="opacity-50">key:</span>
                <select class="select select-xs select-bordered h-6 min-h-0 font-mono" x-model="wizManKey">
                  <option value="">choose…</option>
                  <template x-for="c in wizCols" :key="c"><option :value="c" x-text="c"></option></template>
                </select>
                <button class="btn btn-xs btn-outline h-6 min-h-0 px-2 normal-case" :disabled="!wizManKey" @click="addManualDim()">+ dim</button>
              </div>
              <div class="flex flex-wrap gap-x-3 gap-y-1">
                <template x-for="c in wizCols.filter(c => c !== wizManKey)" :key="c">
                  <label class="flex items-center gap-1 cursor-pointer font-mono text-[10px] select-none">
                    <input type="checkbox" class="checkbox checkbox-xs" :checked="!!wizManCols[c]" @change="wizManCols[c] = $event.target.checked">
                    <span x-text="c"></span>
                  </label>
                </template>
              </div>
