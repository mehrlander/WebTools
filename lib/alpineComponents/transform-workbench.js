  ...
  document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        wizCapped: false, wizSel: "", wizPick: {}, wizFact: true, wizSplit: 55, wizDrag: false,
        loaded: "",
        ...
        // Selecting a candidate paints its columns onto the sidebar pills:
        // key blue, carried columns dark. The pills stay editable from there.
        selectCand(c) {
          this.wizSel = c.key;
          const pick = {};
          pick[c.key] = 2;
          for (const col of c.cols.slice(1)) pick[col] = 1;
          this.wizPick = pick;
        },
        wizRefsLine() {
        ...
        scanDims() {
          ...
          Object.assign(this, { wizScanned: rows.length, wizCapped: all.length > CAP, wizCands: [], wizCols: [], wizColN: {}, wizSel: "", wizPick: {} });
        ...
        // ---- misc ----
        clear() {
          Object.assign(this, { raw: [], datasets: [], scope: "__all", editingDs: -1, addOpen: false, bndOpen: false, combine: true, tagCol: "", ran: false, bundle: "", bundleSz: "", bundleObj: {}, bndSel: {}, srcGz: {}, wizOpen: false, wizCands: [], wizCols: [], wizColN: {}, wizScanned: 0, wizCapped: false, wizSel: "", wizPick: {}, tabs: [this.identityTab()], active: 0, profileData: {}, loaded: "" });
          this.saveTabs();
      ...
      <div class="card bg-base-100 border border-base-300 flex flex-col overflow-hidden" :class="wizOpen ? 'flex-1 min-h-0' : 'shrink-0'">
        ...
        <div x-show="wizOpen" class="flex-1 min-h-0 flex max-h-[18rem] lg:max-h-none">
          ...
          <div class="w-44 shrink-0 border-r border-base-200 overflow-y-auto p-1 flex flex-col gap-0.5">
            ...
            <button x-show="wizCols.length" class="btn btn-xs btn-outline h-6 min-h-0 mt-1 normal-case"
                    :disabled="!Object.values(wizPick).some(v => v)" @click="addManualDim()">+ dim from picked</button>
          </div>
          <div class="flex-1 overflow-y-auto p-2 text-[11px]">
            <div x-show="wizRefsLine()" class="font-mono text-[10px] opacity-50 mb-1.5" x-text="wizRefsLine()"></div>
            <!-- The rollup tree: a child is a coarser level its parent's key
                 already delineates; tap a row to paint it onto the sidebar. -->
            <template x-for="c in wizCands" :key="c.key">
              <div class="flex items-center gap-2 py-1 pl-1 border-b border-base-200 last:border-0 cursor-pointer hover:bg-base-200/40 rounded"
                   ...
                   :class="wizSel === c.key && 'bg-base-200/60'"
                   @click="selectCand(c)">
                <i x-show="c.depth" class="ph ph-arrow-elbow-down-right opacity-30 text-[10px] shrink-0"></i>
                <span class="font-mono font-medium shrink-0" x-text="c.key"></span>
                <span class="opacity-40 shrink-0 text-[10px]" x-text="'(' + c.nKeys.toLocaleString() + ')'"></span>
                <span class="opacity-50 font-mono text-[10px] truncate" x-text="c.cols.slice(1).join(', ')"></span>
            ...
            <div x-show="wizCapped" class="opacity-40 text-[10px] mt-1.5" x-text="'dependencies checked on the first ' + wizScanned.toLocaleString() + ' rows; the generated fn re-checks everything at run'"></div>
          </div>
        </div>
      </div>
      </div>
