    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        async togglePreserveSrc() {
          this.invalidateBundle();
          if (this.ran && this.bndOpen) await this.ensureBundle();
        },
  ...
  <div class="tf-root flex flex-col lg:h-full min-h-0 gap-2 select-none" x-data="transformWorkbench(${dataExpr || ""})"
    ...
    <div class="flex flex-wrap items-center px-1 shrink-0 gap-x-3 gap-y-1.5">
      ...
      <template x-for="(d, i) in datasets" :key="i">
        <!-- overflow-hidden clips both segments to the join's radius, and the
             remove segment follows the selection color, so a selected chip is
             one solid unit rather than a dark half against a white x. -->
        <div class="join border shrink-0 overflow-hidden" :class="(datasets.length === 1 || scopeActive() === d.name) ? 'border-neutral' : 'border-base-300'">
    ...
    <div class="lg:flex-1 flex flex-col lg:flex-row gap-2 lg:gap-0 min-h-0" :class="(drag || bndDrag) && 'pointer-events-none'">
      ...
      <div class="card bg-base-100 border border-base-300 flex flex-col overflow-hidden" :class="bndOpen ? 'flex-1 min-h-0' : 'shrink-0'">
        <div class="flex items-center gap-2 px-2 py-1 shrink-0" :class="bndOpen && 'border-b border-base-300'">
          ...
          <label class="flex items-center gap-1.5 cursor-pointer text-[10px] opacity-60 hover:opacity-100 select-none ml-2">
