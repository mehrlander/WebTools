      ...
      return {
        ...
        async runInner() {
          ...
          if (combined) {
            ...
            if (tasks.length) {
              let done = 0;
              this.progress = "0/" + tasks.length + " fns";
              const seed = {};
              ...
              await this.runWorker({ batches, tagCol: this.tagCol, tasks, seed, seedAlias }, (res) => {
                ...
                this.progress = (++done) + "/" + tasks.length + " fns";
              }, (p) => { this.progress = p.name + " (" + (done + 1) + "/" + tasks.length + ")"; });
            }
          } else {
            ...
            let bi = 0;
            for (const d of this.datasets) {
              const base = "batch " + d.name + " (" + (++bi) + "/" + this.datasets.length + ")";
              this.progress = base;
        ...
        async copyResult() {
          const text = (await this.exportPayload(this.curV)).text;
