          ...
          requestAnimationFrame(() => {
            ...
            try {
              ...
              const tbl = new Tabulator(target, {
                data: Array.isArray(ref) ? ref : JSON.parse(f.content),
                autoColumns: true,
    ...
    Alpine.data("transformWorkbench", function (opts) {
      ...
      return {
        ...
        blankTab(name = "", fnSrc = "rows => rows") {
          return { name, fnSrc, fnGz: "", err: null, obj: null, rows: [], jGz: "", dirty: false, fixed: false, outs: {} };
        },
              ...
              if (fnKeys.length) {
                ...
                const newTabs = (await Promise.all(fnKeys.map(async (fk, fi) => {
                  ...
                  if (typeof o[dk] === "string") {
                    ...
                    return Object.assign(tab, { obj: col, rows, jGz: o[dk] });
                  }
                  ...
                  for (const key of dsKeys) {
                    ...
                    tab.outs[key.slice(pre.length)] = { obj: col, rows, jGz: o[key] };
                  }
                  return tab;
                }))).filter(Boolean);
        ...
        mirrorOuts() {
          ...
          for (const t of this.tabs) {
            const o = (t.outs || {})[ds];
            if (o && !o.err) Object.assign(t, { obj: o.obj, rows: o.rows, jGz: o.jGz || "", err: null });
            else Object.assign(t, { obj: null, rows: [], jGz: "", err: (o && o.err) || null });
          }
        },
        ...
        async run() {
          ...
          if (combined) {
            ...
            for (const res of results) {
              ...
              else Object.assign(t, { rows: res.rows, obj: colFromRows(res.rows), jGz: "", fnGz: "", err: null, dirty: false });
            }
          } else {
            ...
            for (const d of this.datasets) {
              ...
              for (const res of results) {
                const t = this.tabs[res.i];
                if (res.err) t.outs[d.name] = { err: res.err };
                else t.outs[d.name] = { rows: res.rows, obj: colFromRows(res.rows) };
              }
            }
        ...
        async ensureBundle() {
          ...
          const combined = this.isCombined();
          for (const t of this.tabs) {
            const has = combined ? !!t.obj : Object.keys(t.outs || {}).some(k => t.outs[k].obj);
            if (!has) continue;
            if (!t.fnGz) t.fnGz = await this.zip(t.fnSrc);
            if (combined) {
              if (!t.jGz) t.jGz = await this.zip(JSON.stringify(Alpine.raw(t.obj)));
            } else {
              for (const k of Object.keys(t.outs)) {
                const o = t.outs[k];
                if (o.obj && !o.jGz) o.jGz = await this.zip(JSON.stringify(Alpine.raw(o.obj)));
              }
            }
          }
          this.rebuildBundle();
