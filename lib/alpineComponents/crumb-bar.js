          ...
          <template x-for="(c, i) in list" :key="c.key + '|' + (c.label || '')">
            <div class="flex items-center gap-0.5 min-w-0"
              ...
              <button type="button" @click="tap(c)"
                      ...
                      class="group/crumb flex h-11 min-w-0 items-center gap-1 rounded-lg px-2 transition-colors hover:bg-base-200 active:bg-base-300"
                ...
                <i x-show="c.menu && !open" class="ph ph-caret-down shrink-0 text-sm text-base-content/40"></i>
                <i x-show="c.menu && open" class="ph ph-caret-up shrink-0 text-sm text-base-content/60"></i>
              </button>
            </div>
          </template>
        </nav>`,
