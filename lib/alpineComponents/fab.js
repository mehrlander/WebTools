...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
        ...
        <div class="absolute inset-y-0 right-0 transition-transform duration-300 ease-out pointer-events-none"
          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto overscroll-contain">
            <header class="px-2 py-1.5 border-b border-base-300 flex items-center justify-between gap-2 shrink-0">
              <!-- min-w-0 + overflow-x-auto: four tabs plus the refresh
                   controls exceed a narrow drawer, and a flex row without
                   min-w-0 refuses to shrink below its content, so the overflow
                   was CLIPPED with no way to reach it. Now it scrolls. -->
              <div class="flex items-center gap-0.5 min-w-0 overflow-x-auto">
                ...
                <button @click="activeTab = 'render'; loadPageBranches()"
                        class="flex items-center gap-1 px-1.5 py-1 rounded text-[13px] font-semibold transition-colors min-w-0"
                ...
                <!-- Notes is the fourth tab, and the only one that operates ON
                     the page rather than describing it. It earns drawer space
                     because the annotator's own 360px card cannot hold the
                     whole set, the two serializations, and an editor with room
                     in it: that view had been a bespoke modal, which is one
                     dialog convention too many for a repo that already has a
                     sidebar. The count rides the label so a set left open on
                     another page announces itself without a tap. -->
                <button @click="activeTab = 'notes'; annOpen()"
                        class="flex items-center gap-1 px-1.5 py-1 rounded text-[13px] font-semibold transition-colors min-w-0"
                        :class="activeTab === 'notes' ? 'bg-primary/10 text-primary' : 'text-base-content/60 hover:bg-base-200'">
            ...
            <!-- Notes is the one tab the readout sits out. Every other tab
                 describes the page load, so the strip is context for all of
                 them; Notes is about the reader's own marks on the document,
                 where a byte count is noise, and the tab needs the height for
                 its own head. -->
            <button x-show="trafficLine && activeTab !== 'notes'" @click="activeTab = 'traffic'; refreshTraffic()"
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div x-show="widthReachable" class="shrink-0 border-b"
                ...
                <div class="flex items-center gap-0.5 px-1.5 py-1">
                  <template x-for="p in WIDTHS" :key="p.w">
                    <button @click="setWidth(p.w)"
                            :title="p.w ? 'Render this page in a ' + p.w + 'px viewport' : 'Render at the width this device actually has'"
                            ...
                            :class="p.w === frameWidth
                              ? (p.w ? 'bg-warning/25 text-warning' : 'bg-primary/10 text-primary')
                              : 'text-base-content/60 hover:bg-base-200'">
                      <i class="ph text-sm shrink-0" :class="p.icon"></i>
              ...
              <!-- PAGE MODE BARS: whatever else the page says it is being
                   presented at, under the two bars this tab owns. The ref bar
                   asks which code is running, the width bar what shape it is
                   laid out at, and a page contributing here is answering the
                   same class of question about itself: show-repo's is how much
                   of its own header and sidebar surrounds the view, which is
                   what makes an unframed link to one of its views possible.
                   Same idiom as the width bar deliberately, since a reader who
                   has used one has used all three.
                   The FAB reads and calls; it holds no opinion about what a
                   mode MEANS, which is why this is a template over a
                   contributed list rather than a third hard-coded bar. -->
              <template x-for="m in pageModes" :key="m.from + ':' + m.key">
                <div class="shrink-0 border-b border-base-300 bg-base-200/50">
                  <div class="flex items-center gap-0.5 px-1.5 py-1">
                    <span class="shrink-0 px-1 text-[11px] font-semibold uppercase tracking-wide text-base-content/40"
                          x-text="m.label"></span>
                    <template x-for="o in m.options" :key="o.value">
                      <button @click="setPageMode(m, o.value)" :title="o.title || o.label"
                              class="flex items-center gap-1 px-1.5 py-0.5 rounded text-[12px] font-semibold transition-colors min-w-0"
                              :class="o.value === m.value ? 'bg-primary/10 text-primary' : 'text-base-content/60 hover:bg-base-200'">
                        <i class="ph text-sm shrink-0" :class="o.icon || 'ph-circle'"></i>
                        <span class="truncate" x-text="o.label"></span>
                      </button>
                    </template>
                  </div>
                  <div x-show="m.hint" class="px-2.5 pb-1 text-[11px] leading-snug text-base-content/50" x-text="m.hint"></div>
                </div>
              </template>
              </div>
      ...
      annMsg: '', annErr: false,
      groups: [],
      _modeTick: 0,   // forces pageModes to repaint after a set (see pageModes)
      consoleLogs: [],
      loadedScripts: [],
      highlighted: null,
      ...
      detect() {
        ...
        const scan = (doc, A, shell) => {
          ...
          doc.querySelectorAll('[x-data]').forEach(el => {
            ...
            // and a `modes` array of segmented bars for the Render tab.
            //
            // `actions` is copied out, `modes` is not: an action is a verb, so
            // a snapshot of closures is the whole of it, while a mode is STATE
            ...
            // getter re-read through pageModes.
            //
            ...
            // contract as their own. It showed up the day `modes` arrived (one
            ...
            if (!groups[key].description || !groups[key].actions.length || !groups[key].modeSrc) {
              try {
                ...
                if (data && !groups[key].modeSrc && Array.isArray(data.modes) && data.modes.length) groups[key].modeSrc = data;
              } catch (err) {}
            }
          });
      ...
      // Page-contributed STATE, the counterpart to pageActions and the thing
      // the Render tab's own two bars are: which ref, what width, and now
      // whatever else the page says it is being presented at. A row is
      // { key, label, hint, value, options: [{ value, label, icon, title }], set }.
      //
      // Re-read from the component on every paint (modeSrc), not copied at
      // scan time, so the lit segment is the page's live answer. In this window
      ...
      // observe, so setPageMode bumps _modeTick to force the repaint the
      // subject cannot ask for.
      get pageModes() {
        this._modeTick;
        return this.groups.flatMap(g => {
          let list = [];
          try { list = Array.isArray(g.modeSrc?.modes) ? g.modeSrc.modes : []; } catch (err) {}
          return list.map(m => ({ ...m, from: g.name, side: g.shell ? 'shell' : 'subject' }));
        });
      },

      setPageMode(m, value) {
        if (!m || typeof m.set !== 'function' || value === m.value) return;
        this.outError = '';
        try { m.set(value); }
