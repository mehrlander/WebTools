          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto">
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              <div x-show="activeTab === 'render'" class="p-2 border-b border-base-300/60 shrink-0">
                ...
                <div x-show="path" class="px-1 font-mono text-[12px] text-base-content/60 truncate" x-text="path"></div>
                ...
                <div x-show="actionMsg" class="text-[12px] text-success font-mono mt-0.5 px-1" x-text="actionMsg"></div>
              </div>
                  ...
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                        ...
                        <template x-for="b in visibleBranches" :key="b.name">
                          ...
                          <div class="flex flex-col gap-0.5 px-1.5 py-1 rounded transition-colors"
                            ...
                            <button @click="pickFrameRef(b.name)"
                                    class="flex items-center gap-1.5 text-[13px] font-mono w-full text-left cursor-pointer">
                    ...
                    <div class="shrink-0">
                      <div class="flex items-center gap-1.5 mb-1">
                        <div class="text-[12px] uppercase tracking-wider opacity-50 font-semibold">Any ref</div>
                        <div class="text-[12px] opacity-50 ml-auto">selected <span class="font-mono text-base-content/80" x-text="frameRef || 'main'"></span></div>
                      </div>
                      <div class="flex gap-1">
                        <input x-model="frameRefInput" placeholder="branch / tag / sha"
                               @keyup.enter="applyFrameRef()"
                               class="input input-xs input-bordered flex-1 font-mono text-[13px]">
                    ...
                    <div x-show="!viaToss && !tossUrl" class="text-[12px] text-base-content/40 leading-snug shrink-0">Toss renders through toss-render, which serves owner repos only.</div>
                    <div x-show="frameError" class="text-[12px] text-error font-mono break-all shrink-0" x-text="frameError"></div>

                    <!-- Take-away menu. One trigger, five named outputs, each
                         ...
                         Shown in every context the drawer is, toss included:
                         takeTarget aims each action at the page on screen, and a
                         row that cannot deliver in the current mode says so in
                         its own label rather than vanishing. It used to be hidden
                         inside a toss, which removed it from exactly the case it
                         was most wanted in: an un-deployed page you can only
                         reach through one. -->
                    <div class="border-t border-base-300/60 pt-2 flex flex-col gap-1 shrink-0">
                      <div class="dropdown dropdown-top w-full">
                        ...
                        <!-- Grouped by what the action DOES to you: one copies
                             to the clipboard, one navigates away, two land a
                             file. The group heading carries that distinction,
                             so each item can be a bare noun with no sentence
                             explaining it; the long form survives as the row's
                             tooltip for anyone who wants it. -->
                        <ul tabindex="0" class="dropdown-content z-50 menu menu-sm bg-base-100 rounded-box shadow-lg border border-base-300 w-full mb-1 p-1 flex-nowrap max-h-96 overflow-y-auto">
                          <li class="menu-title text-[13px] px-2 pt-1 pb-0.5 leading-snug" x-text="takeSummary"></li>
                          <template x-for="g in takeGroups" :key="g.kind">
                            <li>
                              <h2 class="menu-title text-[13px] px-2 pt-1.5 pb-1" x-text="g.kind"></h2>
                              <!-- daisyUI indents a nested menu and draws a
                                   guide line before it; both are styled through
                                   :where(), which carries no specificity, so
                                   plain utilities override them. -->
                                ...
                                <template x-for="a in g.items" :key="a.key">
                                  <li>
                                    <button @click="runTake(a.key); $el.closest('.dropdown')?.querySelector('[tabindex]')?.blur()"
                                            :title="a.desc" class="py-1.5 text-[13px] gap-2">
                                      <i class="ph shrink-0" :class="a.icon"></i><span x-text="a.label"></span>
                                    </button>
                                  </li>
                                </template>
                              </ul>
        ...
        return [
          ...
          { kind: 'Open', items: [
            { key: 'stage', icon: 'ph-stack', label: 'Stage',
              desc: 'Same fileset on show-repo: pick, trim, diff, copy, download.' + caveat },
          ] },
          { kind: 'Download', items: [
