...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
      ...
      //
      // BY ROLE, NOT BY REPO. The estate declares a mark per repo and the app
      // caches them, so a row could wear its repo's icon; that reads better in
      // the one context where the cache exists and reaches for show-repo's
      // internals from a component that mounts on any page. The role is
      // already derived here, it is the same answer everywhere, and a spatial
      // control wants one rule rather than the better of two. ph-tree-structure
      // is this file's own mark for show-repo (see linksFor) and the one
      // pages/index.html sets beside the words "Web Tools"; ph-frame-corners is
      // the estate's render mark. Change one, change all three, so the strip
      // keeps saying the same words in every context.
      layerIcon(L) {
        ...
        if (L.role === 'app') return 'ph-tree-structure opacity-50';
