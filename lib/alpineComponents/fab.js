...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
        ...
        <div class="absolute inset-y-0 right-0 transition-transform duration-300 ease-out pointer-events-none"
          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto overscroll-contain">
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <!-- One row, two kinds of control, and the page's ride WITH the
                   presets rather than under them. A contributed toggle answers
                   the same question the presets do (how is this view being
                   presented), and the first cut gave it a labelled row of its
                   own: that was a second heading, a second hint line, and most
                   of the drawer's height spent on one binary. So the row wraps
                   instead, past a hairline. -->
              <div x-show="widthReachable || pageToggles.length" class="shrink-0 border-b"
                ...
                <div class="flex flex-wrap items-center gap-0.5 px-1.5 py-1">
                  <template x-for="p in WIDTHS" :key="p.w">
                    <button x-show="widthReachable" @click="setWidth(p.w)"
                ...
                <!-- A contributed hint, when the page has something to say about
                     the state it is in. Empty is the normal case, so the row
                     stays one line high until something is off default. -->
                <template x-for="t in pageToggles.filter(t => t.hint)" :key="'hint:' + t.from + ':' + t.key">
                  <div class="px-2.5 pb-1 text-[11px] leading-snug text-base-content/50" x-text="t.hint"></div>
                </template>
              </div>
              </div>
      ...
      // A row is { key, label, icon, on, title, hint, set }.
      //
