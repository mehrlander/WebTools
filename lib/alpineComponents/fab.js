...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
                  ...
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                    ...
                    <div class="min-h-0 flex-1 flex flex-col">
                      <div class="flex items-center justify-between mb-1 shrink-0">
                        ...
                        <a x-show="currentPr" :href="prUrl(currentPr && currentPr.number)" target="_blank"
                           class="text-[12px] link link-hover font-mono"
                           x-text="'#' + (currentPr && currentPr.number) + (currentPr && currentPr.draft ? ' draft' : '')"></a>
                      </div>
                      ...
                      <div x-show="!pageBranchesLoading" class="min-h-0 flex-1 overflow-y-auto">
                        <template x-if="currentPr">
                          <div class="flex flex-col gap-2">
                            <div class="text-[13px] font-semibold leading-snug" x-text="currentPr.title"></div>
                            ...
                            <div x-show="!prBodyHtml && currentPr.body" class="text-[12px] opacity-50">Rendering…</div>
                            <div x-show="!currentPr.body" class="text-[12px] opacity-50 italic">This PR has an empty body.</div>
                          </div>
                        </template>

                        <!-- No PR. Two different nothings, and conflating them
                             would misreport: the default branch is not missing a
                             guide, it is the thing guides are written against. -->
                        <div x-show="!currentPr" class="flex flex-col gap-2 text-[13px]">
                          <template x-if="viewingRef === (defaultBranch || 'main')">
                            <p class="opacity-60 leading-snug">
                              <span class="font-mono" x-text="viewingRef"></span> is the default branch, so it has no
                              guide of its own. Pick a branch above to read its guide and render this page from it.
                            </p>
                          </template>
                          <template x-if="viewingRef !== (defaultBranch || 'main')">
                            <div class="flex flex-col gap-2">
                              <p class="opacity-60 leading-snug">
                                No open PR for <span class="font-mono" x-text="viewingRef"></span>, so there is no guide
                                to show. The branch page reads its state from the API instead.
                              </p>
                              <a :href="branchPageUrl" target="_blank" rel="noopener"
                                 class="inline-flex items-center gap-1.5 self-start px-2 py-1 rounded border border-base-300 hover:border-primary hover:text-primary transition-colors text-[12px]">
                                <i class="ph ph-git-branch"></i><span>Open the branch page</span>
                              </a>
                            </div>
                          </template>
                        </div>
      ...
      // The PR for the ref ON DISPLAY, not for a pending selection: the
      // dropdown navigates, so what you are reading is always the guide of what
      // you are looking at.
      get currentPr() {
      ...
      openTarget(href) {
        ...
        const addr = owner + '/' + name + '@' + ref + ':' + path;
        const base = 'https://mehrlander.github.io/web-tools/pages/toss-render.html';
        const ext = (path.split('.').pop() || '').toLowerCase();
        const label = path.split('/').pop();
        if (ext === 'html' || ext === 'htm') {
          return { kind: 'render', url: base + '#gh=' + addr, label, icon: 'ph-disc',
                   title: 'Render ' + path + ' at ' + ref };
        }
        if (['md', 'markdown', 'csv', 'tsv', 'json', 'txt', 'log', 'yml', 'yaml'].includes(ext)) {
          return { kind: 'read', url: base + '#data=' + addr, label, icon: 'ph-file-text',
