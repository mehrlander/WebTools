...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
      ...
      template: `
        <div :style="'transform:translate(' + x + 'px,' + y + 'px)'"
          ...
          <!-- The launcher's second gesture: a long press (or a right-click)
               ...
               start. Both entries land in the annotator; everything else the
               fab can do already has a home inside the drawer, so the menu
               stays two rows rather than becoming a second take grid.
               Its own pointer handlers are stopped: the wrapper is the drag
               surface, and without this a tap on a row would also toggle the
               drawer under it. -->
          <div x-show="fabMenu" x-cloak x-transition.opacity.duration.120ms
               ...
               class="absolute bottom-full right-0 mb-2 w-60 rounded-box border border-base-300 bg-base-100 shadow-2xl overflow-hidden">
            <button @click="fabMenu = false; annNotePage()"
                    class="w-full flex items-start gap-2.5 px-3 py-2 text-left hover:bg-base-200">
              <i class="ph ph-note-pencil text-[15px] mt-0.5 text-primary"></i>
              <span class="min-w-0">
                <span class="block text-sm font-semibold">Note this page</span>
                <span class="block text-[11px] text-base-content/50 leading-snug">Dictate straight into a note; the address rides it</span>
              </span>
            </button>
            <button @click="fabMenu = false; annStart()"
                    class="w-full flex items-start gap-2.5 px-3 py-2 text-left hover:bg-base-200 border-t border-base-300/60">
              <i class="ph ph-selection-plus text-[15px] mt-0.5 text-base-content/60"></i>
              <span class="min-w-0">
                <span class="block text-sm font-semibold">Take notes</span>
                <span class="block text-[11px] text-base-content/50 leading-snug">Annotator on, drawer open on Notes</span>
              </span>
            </button>
          </div>
        </div>
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div x-show="activeTab === 'notes'" class="min-h-0 flex-1 flex flex-col">
                ...
                <div class="shrink-0 flex items-center gap-2 px-2.5 py-1.5 border-b border-base-300 bg-base-200/40">
                  ...
                  <button x-show="annOn" @click="annStop()"
                          class="btn btn-ghost btn-xs shrink-0" title="Turn the annotator off and drop the on-page card">Off</button>
                </div>

                <!-- Not on yet: one button, and an honest word about what the
                     annotator can reach here. -->
                <div x-show="!annOn" class="p-3 flex flex-col gap-2.5 items-start">
                  <div class="text-sm text-base-content/60">
                    Pin notes to this page: select text, or capture an element or a region.
                    Each note anchors by quote and by DOM path, so the set survives a re-render
                    and says which passage it means. Or note the page as a whole, for what is
                    wrong with it rather than with a passage in it.
                  </div>
                  <div x-show="annBlind" class="text-[12px] text-warning flex items-start gap-1.5">
                  ...
                  <button @click="annStart()" class="btn btn-sm btn-primary">
                    <i class="ph ph-note-pencil"></i> Start annotating
                  </button>
                </div>
                ...
                <template x-if="annOn">
                  <div class="min-h-0 flex-1 flex flex-col">
                    ...
                    <div x-show="annTab === 'notes' && annItems.length"
                         class="min-h-0 flex-1 overflow-y-auto p-2 space-y-1.5">
      ...
      // "Note this page": the whole-page note, one tap from the launcher's
      // long-press menu. Turns the annotator on if it is off, then opens a
      // draft with nothing to aim at, which is the point. The drawer gets out
      // of the way (openAnnotate already closes it) because what happens next
      // is dictating into the card over the page being complained about.
      async annNotePage() {
        const A = this._annKit();
        if (!(A && A.enabled)) await this.openAnnotate();
        const K = this._annKit();
        if (K && K.notePage) K.notePage();
        this.open = false;
        this.annSync();
      },
