  ...
  Alpine.data('fab', function() {
    return {
      ...
      get takeTarget() {
        ...
        const via = this.subjectVia;
        const repo = via ? via.repo : this.repo;
        const ref = (via ? via.ref : this.ref) || 'main';
        let gh = window.gh;
        if (window.GH && repo) {
          try { gh = new window.GH({ repo, ref, loadBase: 'lib/' }); } catch (e) {}
        }
        return {
          gh,
      ...
      // The file a take operates on: the frame's own document. Only a routed
      // subject makes this differ from the one the drawer is titled with.
