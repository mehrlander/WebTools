  ...
  Alpine.data('fab', function() {
    return {
      ...
      template: `
        <div :style="'transform:translate(' + x + 'px,' + y + 'px)'"
          ...
          <!-- The launcher's second gesture: a long press (or a right-click)
               ...
               THE THIRD IS THE WAY OUT: "Web Tools home", which leaves for the
          ...
          <div x-show="fabMenu" x-cloak x-transition.opacity.duration.120ms
            ...
            <!-- FOUR ROWS WHERE THERE WAS ONE, and the reason is that the
                 card has four aims and this menu had one destination. Every
                 other aim was two taps further in: open the card, open its aim
                 menu, pick. The gesture each aim then needs is unchanged; what
                 moves is where it is armed from, which is the tap a reader has
                 already made to raise this menu.

                 STILL ONE LINE EACH, per the rule above. The card's aim menu
                 carries a hint under every row and this one carries none: a
                 menu raised by a held finger is read in the half-second before
                 the finger lifts.

                 THE SECTION ROW NAMES ITSELF, and it is the only conditional
                 ...
                 no row at all and the next kind to declare gets its own name
                 here without this file learning it. -->
            <button @click="fabMenu = false; annAim('page')"
              ...
              <span class="text-sm font-semibold">Note the page</span>
            </button>
            <button @click="fabMenu = false; annAim('pick')"
                    class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200">
              <i class="ph ph-crosshair-simple text-[15px] text-primary"></i>
              <span class="text-sm font-semibold">Note an element</span>
            </button>
            <button x-show="annKind" @click="fabMenu = false; annAim('section')"
                    class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200">
              <i class="ph ph-text-align-left text-[15px] text-primary"></i>
              <span class="text-sm font-semibold"
                    x-text="'Note a ' + ((annKind &amp;&amp; annKind.aimLabel) || 'section').toLowerCase()"></span>
            </button>
            <button @click="fabMenu = false; annAim('region')"
                    class="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-base-200">
              <i class="ph ph-frame-corners text-[15px] text-primary"></i>
              <span class="text-sm font-semibold">Note a region</span>
            </button>
            <button @click="fabMenu = false; pasteToStage()"
              ...
              <i class="ph ph-clipboard-text text-[15px] text-primary"></i>
              <span class="text-sm font-semibold">Paste to Stage</span>
            </button>
          ...
          { kind: 'Open', items: [
            ...
            { key: 'annotate', icon: 'ph-note-pencil', label: 'Annotate',
              desc: 'Pin notes to this page: select text, pick elements, or note the page as a whole, then copy the set out or save it as a jot.'
                + (this.annBlind ? ' Subject frame unreadable: notes attach to this renderer shell, not the page.' : '') },
            // The second take that operates ON the view. Annotate is for
            // pinning a note; this is for the question underneath it, what is
            // this element and what contains it, which a reader has as often
            // without wanting to write anything down (kits/peek.js).
            { key: 'peek', icon: 'ph-crosshair', label: 'Peek',
              desc: 'Point at anything to name it: selector, box, layout, subtree and the ancestors containing it. Point again in the same spot to step up the chain.'
                + (this.annBlind ? ' Subject frame unreadable: this reads the renderer shell, not the page.' : '') },
          ] },
          { kind: 'Save', items: [
      ...
      async runTake(key) {
        ...
        else if (key === 'peek') await this.openPeek();
      ...
      // page loads its own kits and this component is the shell's.
      _mdDoc() {
        if (window.mdDoc) return window.mdDoc;
        try {
          const w = window.__tossFrame && window.__tossFrame.contentWindow;
          if (w && w.mdDoc) return w.mdDoc;
        } catch (e) {}
        return null;
      },

      // The kind declared on that document, read at menu-open time. An enabled
      // annotator answers for itself; otherwise md-doc is asked directly, since
      // the menu has to decide BEFORE anything is loaded on its account and
      // loading the annotator to find out would cost a long press a fetch.
      // No md-doc anywhere means nothing has declared, which is the same answer.
      _declaredKind() {
        try {
          const A = this._annKit();
          if (A && A.enabled && A.declaredKind) return A.declaredKind();
          const md = this._mdDoc();
          const d = md && md.declaredIn && md.declaredIn(this._annDoc());
