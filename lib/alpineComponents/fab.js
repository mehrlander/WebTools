...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
      ...
      goTarget(t) {
        if (!t || !t.url) return;
        // CLOSE the drawer, and do not hand it forward. A ref switch keeps it
        // open on purpose: you were comparing refs of one page and the list you
        // were comparing them in should survive the trip. A file pick is the
        // opposite errand. You asked to SEE something, and at phone width this
        // drawer is 352 of 390 pixels, so reopening it puts the drawer on top
        // of the file you just asked for and the render reads as a blank page.
        // Measured that way, not reasoned: the markdown was rendering the whole
        // time, in the 38 px strip beside the drawer.
        this.open = false;
        this._clearDrawerHandoff();
        if (this.viaToss) {
