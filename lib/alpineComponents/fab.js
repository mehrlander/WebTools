...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
      ...
      template: `
        <div :style="'transform:translate(' + x + 'px,' + y + 'px)'"
          ...
          <!-- The launcher's second gesture: a long press (or a right-click)
               ...
               start.
               ONE row, and not for want of candidates. The row starts the
               annotator with a page draft already staged, which is the note
               that needs no aiming; the other three targets are reached by the
               gesture that defines them, selecting text or tapping a chip on
               the card that just appeared. A second row offering those would
               ask the reader to choose in a menu what the page itself answers
               in one gesture. Everything else the fab does already has a home
               inside the drawer.
          ...
          <div x-show="fabMenu" x-cloak x-transition.opacity.duration.120ms
               ...
               class="absolute bottom-full right-0 mb-2 w-64 rounded-box border border-base-300 bg-base-100 shadow-2xl overflow-hidden">
            <button @click="fabMenu = false; annNotePage()"
                    class="w-full flex items-start gap-2.5 px-3 py-2.5 text-left hover:bg-base-200">
              <i class="ph ph-note-pencil text-[15px] mt-0.5 text-primary"></i>
              <span class="min-w-0">
                <span class="block text-sm font-semibold">Take a note</span>
                <span class="block text-[11px] text-base-content/50 leading-snug">Opens on this page, not recording. Tap the mic, or select text for a passage.</span>
              </span>
            </button>
          </div>
        </div>
              ...
              <div x-show="activeTab === 'text'" class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
                ...
                <template x-if="textStats">
                  <div>
                    ...
                    <!-- MATCH. The files this text names, resolved against the
                         repo and glossed by whichever registry owns each one.
                         Behind a tap because everything above it is local and
                         this is not: one tree fetch plus three registry reads,
                         which the drawer's own readout will count. -->
                    <div class="border-t border-base-300/60">
                      <div class="flex items-center justify-between gap-2 px-2.5 pt-2 pb-0.5">
                        <div class="text-[12px] uppercase tracking-wider text-base-content/50 font-semibold">Named files</div>
                        <button x-show="textStats.named.length && textMatchState !== 'loading'"
                                @click="textMatchRun()"
                                class="text-[12px] link link-hover"
                                x-text="textMatchState === 'idle' ? 'resolve (' + textStats.named.length + ')' : 'again'"></button>
                        <span x-show="textMatchState === 'loading'" class="loading loading-dots loading-xs text-primary"></span>
                      </div>

                      <div x-show="!textStats.named.length" class="px-2.5 pb-2 text-[13px] text-base-content/50 italic">
                        This text names no files.
                      </div>
                      <div x-show="textStats.named.length && textMatchState === 'idle'"
                           class="px-2.5 pb-2 text-[12px] text-base-content/50 leading-snug">
                        Costs a tree read and three registry reads. Nothing above this line touched the network.
                      </div>
                      ...
                      <template x-if="textMatchState === 'done' && textMatch">
                        <div>
                          <div x-show="!textMatch.isHub" class="px-2.5 pb-1.5 text-[12px] text-base-content/50 leading-snug">
                            Resolved against <span class="font-mono" x-text="textMatch.repo"></span>, whose files these are.
                            The registries that say what a file <em>is</em> live in the hub repo, so these rows carry
                            links and no descriptions.
                          </div>
                          <div class="pb-1">
                            <template x-for="h in textMatch.hits" :key="h.path">
                              <a :href="h.blob" target="_blank"
                                 class="block px-2.5 py-1.5 border-t border-base-300/40 hover:bg-base-200/60">
                                <div class="flex items-baseline gap-1.5 min-w-0">
                                  <i class="ph ph-check text-success text-[13px] shrink-0"></i>
                                  <span class="font-mono text-[13px] truncate min-w-0" x-text="h.path"></span>
                                  <span x-show="h.tag" class="text-[11px] uppercase tracking-wide text-base-content/40 shrink-0" x-text="h.tag"></span>
                                </div>
                                <div x-show="h.what" class="pl-[1.1rem] text-[12px] text-base-content/60 leading-snug" x-text="h.what"></div>
                              </a>
                            </template>
                          </div>
                          <!-- A candidate that does not resolve is not an error.
                               It is usually another repo's path, a URL fragment,
                               or a filename that reads like one. Listed, not
                               flagged, and never counted as a finding. -->
                          <div x-show="textMatch.missed.length" class="px-2.5 py-1.5 border-t border-base-300/40">
                            <div class="text-[12px] text-base-content/50 leading-snug">
                              <span x-text="textMatch.missed.length"></span> not in this repo at
                              <span class="font-mono" x-text="textMatch.ref"></span>:
                              <span class="font-mono text-base-content/40" x-text="textMatch.missed.join(', ')"></span>
                            </div>
                          </div>
                          <div x-show="textMatch.truncated" class="px-2.5 pb-1.5 text-[12px] text-warning">
                            The repo tree came back truncated, so a miss here may be the tree's fault.
                          </div>
                        </div>
                      </template>
                    </div>
                    ...
                    <div class="px-2.5 py-2 border-t border-base-300/60 text-[12px] text-base-content/50 leading-snug">
                      No model, and no glossary: the estate keeps no committed vocabulary keyed by
                      the words prose actually uses, so terms of art are not matched here.
                    </div>
                  </div>
                </template>
              </div>
                    ...
                    <div x-show="annTab === 'notes' && annItems.length"
                      ...
                      <template x-for="(it, i) in annItems" :key="it.id">
                        <div class="rounded border overflow-hidden transition-colors"
                             :class="it.id === annSel ? 'border-warning bg-warning/5' : 'border-base-300/60 bg-base-200/40'">
      ...
      // annotator rather than a second copy of the drawer.
      fabMenu: false,
      consoleOpen: false,
      ...
      _textRead(input) {
        ...
        while ((n = walker.nextNode())) {
          ...
          if (isChrome) chrome += this._words(n.data);
          else { body += n.data + '\n'; runs++; }
        }
        const words = this._words(body);
        const sentences = body.split(/(?<=[.!?])\s+/).map(s => s.trim()).filter(s => this._words(s) > 2);
      ...
      // ── Match ─────────────────────────────────────────────────────────────
      // The files this page NAMES, resolved against the repo, and what the
      // estate's registries say each one is. This is the operation the tab was
      // conceived for: terms on a page, connected to data that is registered.
      //
      // THE JOIN IS THE PATH, and that is the finding rather than a shortcut.
      // The obvious design was a glossary: match the estate's terms of art
      // ("toss", "stage", "guide PR") against the prose and link each to its
      // definition. There is nothing to match against. docs/surfacing.json's
      // rows are keyed by sentence-shaped titles ("Reference is a link"), which
      // do not occur in prose; docs/text-fields.csv and docs/properties.json
      // are keyed by ordinary English words (note, open, scope, role) whose
      // false-positive rate would swamp anything true. The estate has no
      // committed vocabulary keyed by surface form, deliberately: vocab.py
      // builds one in 1.7 seconds and the concept-index skill declines to
      // commit the result, on the same rule that retired the merge guide.
      // Inventing one here would be inventing a registry the estate has
      // decided not to keep.
      //
      // What it has instead is better for this purpose. Three registries are
      // keyed BY PATH (docs.json, tests.json, harness.json), and a path is a
      // token that really does appear in this prose. So path matching IS
      // registry matching here, with no glossary in the middle and no guess in
      // any step. Between them they cover docs/, tools/test/, and the rest of
      // tools/ with scripts/; a path outside those resolves in the tree and
      // gets a link with no gloss, which is the honest result rather than a
      // gap to apologize for.
      //
      // COSTS CALLS, SO IT IS A TAP. Everything else in this tab is local, and
      // the pane says so; this is a tree fetch plus up to four registry reads,
      // which the drawer's own readout will count. Never automatic on tab open.
      textMatch: null, textMatchState: 'idle', textMatchError: '',

      // The registries keyed by path, and the field in each that says what a
      // row IS. Kept as data so adding one is a row rather than a branch.
      MATCH_REGISTRIES: [
        { path: 'docs/docs.json',     rows: 'documents', key: 'path', what: 'subject', extra: 'status' },
        { path: 'docs/tests.json',    rows: 'tests',     key: 'path', what: 'protects', extra: 'kind' },
        { path: 'docs/harness.json',  rows: 'tools',     key: 'path', what: 'role',    extra: 'layer' },
      ],
      ...
      async textMatchRun() {
        ...
        this.textMatchError = '';
        try {
          const named = (this.textStats && this.textStats.named) || [];
          if (!named.length) { this.textMatch = { hits: [], missed: [] }; this.textMatchState = 'done'; return; }

          // No repo, no resolution, and say which is missing. The fab runs on
          // pages it cannot place (a bare host, a #gz= sandbox), and the
          // failure to name that is how "0 of 6 resolved" gets read as six
          // dead links rather than as a page with no repo behind it.
          const repo = this.repo;
          if (!repo) throw new Error('No repo detected for this page, so there is no tree to resolve against.');
          const ref = this.takeRef || 'HEAD';

          // The tree reader is a kit, pulled on first use rather than at boot,
          // since a reader who never taps resolve should never pay for it.
          if (!window.EstateSearch) {
            await window.gh.load('kits/estate-search.js', { by: 'alpineComponents/fab.js' });
          }
          if (!window.EstateSearch) throw new Error('kits/estate-search.js loaded but registered nothing');

          const { paths, truncated } = await window.EstateSearch.tree(repo, ref, window.TOKEN || '');
          const inRepo = new Set(paths);

          // Registry rows only where the registries actually live. On another
          // repo the tree still resolves and the rows simply are not there,
          // which the pane says rather than silently showing bare links.
          const isHub = repo === 'mehrlander/web-tools';
          const rows = isHub ? await this._matchRegistries(repo, ref) : {};

          const hits = [], missed = [];
          for (const p of named) {
            if (!inRepo.has(p)) { missed.push(p); continue; }
            const row = rows[p] || null;
            hits.push({
              path: p,
              blob: `https://github.com/${repo}/blob/${encodeURIComponent(ref)}/${p}`,
              what: row ? row.what : '',
              tag: row ? row.tag : '',
              from: row ? row.from : '',
            });
          }
          this.textMatch = { hits, missed, truncated, isHub, ref, repo };
      ...
      // path -> { what, tag, from }. A registry that fails to read is skipped
      // rather than failing the match: a missing row costs a gloss, and a
      // missing tree costs the answer, so only the tree is allowed to throw.
      async _matchRegistries(repo, ref) {
        const out = {};
        const gh = new window.GH({ token: window.TOKEN || '', repo, ref });
        await Promise.all(this.MATCH_REGISTRIES.map(async (r) => {
          try {
            const doc = JSON.parse((await gh.get(r.path)).text);
            for (const row of (doc[r.rows] || [])) {
              const k = row[r.key];
              if (!k || out[k]) continue;
              out[k] = { what: row[r.what] || '', tag: row[r.extra] || '', from: r.path };
            }
          } catch { /* a registry that will not read costs a gloss, not the answer */ }
        }));
        return out;
      },
      ...
      annTabs: [{ key: 'notes', label: 'Notes' }, { key: 'md', label: 'Markdown' },
                { key: 'json', label: 'JSON' }],
      annItems: [], annSel: null, annOn: false, annOut: '',
      annMsg: '', annErr: false,
      groups: [],
      consoleLogs: [],
      loadedScripts: [],
      ...
      get annBlind() { return !!(this.viaToss && !this.subjectReached); },
