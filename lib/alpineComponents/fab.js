...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
                  ...
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                    ...
                    <div class="border-t border-base-300/60 pt-2 pb-10 flex flex-col gap-1.5 shrink-0">
                      <div class="flex items-baseline gap-1.5 px-0.5">
                        ...
                        <button @click="takeDetailOpen = !takeDetailOpen"
                                class="ml-auto shrink-0 opacity-40 hover:opacity-90 transition-opacity"
                                :class="[outBusy && 'hidden', takeDetailOpen && 'opacity-90']"
                                :title="takeDetailOpen ? 'Hide what each one gives you' : 'Show what each one gives you'"
                                :aria-label="takeDetailOpen ? 'Hide details' : 'Show details'">
                          <i class="ph text-sm" :class="takeDetailOpen ? 'ph-caret-up' : 'ph-info'"></i>
                        </button>
                      </div>
                      ...
                      <!-- Rendered, not hovered: a title attribute is invisible on
                           a touch screen, which is where this panel mostly gets
                           used. Same strings the tooltips carry. Below the rows,
                           not above them, so opening it never shifts a button out
                           from under the finger already reaching for it. -->
                      <div x-show="takeDetailOpen" class="flex flex-col gap-1 px-0.5 pt-0.5">
                        <template x-for="a in takeDetails" :key="'d-' + a.key">
                          <div class="text-[11px] leading-snug">
                            <span class="font-semibold opacity-60" x-text="a.label"></span>
                            <span class="opacity-40"> · </span>
                            <span class="opacity-50 font-mono" x-text="a.desc"></span>
                          </div>
                        </template>
                      </div>
      ...
      subjectReads: [], subjectReached: false, payloadHtml: '', takeDetailOpen: false,
      ...
      // The descriptions above are the honest answer to "what will I get", and on
      // a phone a title attribute never shows them. Same strings, rendered.
      get takeDetails() { return this.takeGrid.flatMap(g => g.items); },
