...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div x-show="activeTab === 'render'" class="shrink-0">
                <div class="relative p-2" @click.outside="ghMenu = false">
                  ...
                  <!-- The github mark is a MENU now, not a link to one file.
                       As a link it answered the narrowest question GitHub can
                       be asked ("show me this blob") and left the rest of the
                       repo's surface, its commits above all, with no route at
                       all. The rows come from lib/github-links.js, the same
                       list show-repo's sidebar uses, plus the two that are only
                       meaningful here: this file, and its commits. -->
                  <!-- THE PATH IS A PICKER. It named the file and did nothing,
                       which was fine while this tab was about one page, and
                       stopped being fine the moment the tab became about
                       rendering: the whole drawer can already toss any file in
                       the repo, and the one control naming a file could not
                       choose a different one. So it opens the repo tree
                       (alpineComponents/path-picker, the same tap-through
                       selector show-repo uses) at the ref on display, and
                       picking a file renders it. Nothing else in the drawer
                       moves: the ref bar still says which ref, the guide still
                       describes the branch. -->
                  <div x-show="path" class="px-1 flex items-center gap-1.5">
                    <button @click="togglePicker()" :title="'Pick another file to render from ' + repo + ' at ' + viewingRef"
                            class="flex min-w-0 items-center gap-1 text-left group/path">
                      <span class="font-mono text-[12px] text-base-content/60 group-hover/path:text-base-content truncate transition-colors"
                            x-text="path"></span>
                      <i class="ph shrink-0 text-[10px] opacity-40" :class="pickerOpen ? 'ph-caret-up' : 'ph-caret-down'"></i>
                    </button>
                    <button x-show="repo" @click="toggleGhMenu()"
                            :title="'GitHub links for ' + repo + ' at ' + viewingRef"
                            aria-label="GitHub links"
                            class="ml-auto shrink-0 flex items-center gap-0.5 opacity-40 hover:opacity-90 transition-opacity">
                  ...
                  <div class="px-1" x-ref="picker" @path-pick="renderPicked($event.detail)">
                    <div x-data="pathPicker({ trigger: false, gh: () => pickerGh(), roots: () => pickerRoots() })"></div>
                  </div>
                      ...
                      <div x-show="!pageBranchesLoading" class="min-h-0 flex-1 overflow-y-auto">
                        <template x-if="guidePr">
                          <div class="flex flex-col gap-2">
                            ...
                            <!-- Styled by arbitrary variants rather than by the
                                 typography plugin's prose class, deliberately.
                                 (No backticks in this comment: the template is
                                 a template literal and one would close it.)
                                 The fab mounts on
                                 every page that boots lib, and the typography
                                 plugin is a separate CDN entry not all of them
                                 carry; a guide that silently loses its bullets
                                 and its link color on those pages would be the
                                 kind of failure nobody reports. These generate
                                 from the live DOM like every other utility here. -->
                            <div x-show="prBodyHtml" x-html="prBodyHtml"
                                 class="text-[13px] leading-relaxed break-words
                                        [&_p]:my-1.5
                                        [&_ul]:my-1.5 [&_ul]:pl-4 [&_ul]:list-disc
                                        [&_ol]:my-1.5 [&_ol]:pl-4 [&_ol]:list-decimal
                                        [&_li]:my-0.5
                                        [&_a]:text-primary [&_a]:underline [&_a]:decoration-primary/30
                                        [&_h1]:text-[13px] [&_h2]:text-[13px] [&_h3]:text-[13px]
                                        [&_h1]:font-bold [&_h2]:font-bold [&_h3]:font-bold
                                        [&_h1]:mt-3 [&_h2]:mt-3 [&_h3]:mt-3
                                        [&_h1]:mb-1 [&_h2]:mb-1 [&_h3]:mb-1
                                        [&_code]:font-mono [&_code]:text-[11px] [&_code]:bg-base-300/50 [&_code]:px-1 [&_code]:rounded
                                        [&_pre]:bg-base-300/40 [&_pre]:p-2 [&_pre]:rounded [&_pre]:overflow-x-auto [&_pre]:text-[11px]
                                        ...
                                        [&_table]:text-[11px] [&_img]:max-w-full"></div>
      ...
      prBodyHtml: '', prBodyFor: null, prTargets: [],
      ...
      // One root: the repo this page came from, at the ref on display. Not the
      // whole estate, because the drawer is about THIS page, and a picker that
      // opened on a list of ninety repos would answer a question nobody asked
      // here. The ref bar moves the ref; this moves the path.
      pickerRoots() {
        const repo = this.repo;
        if (!repo) return [];
        return [{ repo, ref: this.viewingRef, label: repo.split('/').pop() + ' @ ' + this.viewingRef }];
      },
      ...
      renderPicked(detail) {
        ...
        const t = this.renderTarget(detail.repo || this.repo, detail.ref || this.viewingRef, detail.path, true);
        if (!t) return;
        // A page at this repo and ref IS this drawer's own subject: go through
        // the toss the ref bar uses, so the fab rides along and the escape
        // handle still works. Anything else opens beside it.
        if (t.kind === 'render' && (detail.repo || this.repo) === this.repo) {
          this.frameRef = detail.ref || this.viewingRef;
          this._handOffDrawer();
          location.href = t.url;
          return;
        }
      ...
      // marked, lazily and once. Same URL and same shape as source-peek.js and
      // chat-render.js use, so the three share one cached asset.
      _needMarked() {
        return this._markedP ||= window.marked ? Promise.resolve() : new Promise((res, rej) => {
      ...
      // Where a link in a guide body should actually go. A guide names its files
      // as GitHub blob links, which is right for a body read on GitHub and wrong
      // for one read inside a preview drawer: the reader is standing in the page
      // and the link sends them to source. So a blob link is re-aimed at the
      // renderer that can show the thing.
      //
      //   .html            the toss, which is what this drawer is about
      //   data-ish files   the data view (markdown renders as preview there)
      //   anything else    left alone; source is the honest answer for source
      //
      ...
      // session branch is `claude/<slug>`, so the naive first-segment split
      // yields ref `claude` and a path starting with the rest of the branch
      // name. Both halves are then wrong and the resulting address 404s.
      //
      // The branch list is the disambiguator, and it is already loaded: match
      // the LONGEST known ref that prefixes the tail. Falls back to the first
      // segment, which is right for a sha, a tag, and any unslashed branch, and
      // is the best available guess before the survey lands.
      splitBlobRef(rest) {
        if (!rest || !rest.includes('/')) return null;
        const known = [this.defaultBranch || 'main', ...this.pageBranches.map(b => b.name)]
          .filter(Boolean)
      ...
      // Returns null for a link that should pass through untouched, so callers
      // can treat "no opinion" and "re-aim here" as the same branch.
      openTarget(href) {
        const m = /^https?:\/\/github\.com\/([^/]+)\/([^/]+)\/blob\/(.+?)(?:[?#].*)?$/i.exec(href || '');
        if (!m) return null;
        const [, owner, name, rest] = m;
        const split = this.splitBlobRef(decodeURIComponent(rest));
        if (!split) return null;
        return this.renderTarget(owner + '/' + name, split.ref, split.path);
      },

      // Where a repo file opens. The one routing table, read by the guide's
      // links and by the path picker, differing only in `any`:
      //
      //   .html            the toss, which is what this drawer is about
      //   data-ish files   the data view (markdown renders as preview there)
      //   anything else    null, unless `any` — see renderPicked for why the
      //                    picker is allowed to be less careful than a link
      renderTarget(repo, ref, path, any) {
        if (!repo || !ref || !path) return null;
        const addr = repo + '@' + ref + ':' + path;
        const base = 'https://mehrlander.github.io/web-tools/pages/toss-render.html';
        const ext = (path.split('.').pop() || '').toLowerCase();
        const label = path.split('/').pop();
        if (ext === 'html' || ext === 'htm') {
          return { kind: 'render', repo, ref, path, url: base + '#gh=' + addr, label, icon: 'ph-disc',
                   title: 'Render ' + path + ' at ' + ref };
        }
        if (any || ['md', 'markdown', 'csv', 'tsv', 'json', 'txt', 'log', 'yml', 'yaml'].includes(ext)) {
          return { kind: 'read', repo, ref, path, url: base + '#data=' + addr, label, icon: 'ph-file-text',
                   title: 'Read ' + path + ' at ' + ref };
        }
        return null;
      },

      // Render the current PR body once, re-aiming its links on the way through,
      // and collect the renderable ones into the chip strip. Parsing the output
      // rather than string-replacing the markdown keeps one definition of what a
      // link is (the parser's), and means a link written as a reference, an
      // autolink, or inline all get the same treatment.
      async renderPrBody() {
        ...
        try { await this._needMarked(); } catch (e) { this.prBodyHtml = ''; return; }
        if (this.prBodyFor !== key) return;     // a switch landed while loading
        let html = '';
        try { html = window.marked.parse(pr.body); } catch (e) { return; }
        const doc = new DOMParser().parseFromString(html, 'text/html');
        // Dedupe the STRIP by file, not by URL. A guide names each changed file
        // three times by convention, [new] and [main] and [diff], and the first
        // two are the same file at two refs: deduping on the address gave a
        // strip that listed show-repo.html twice and show-repo.md twice, which
        // reads as a bug and is useless as a menu. Keyed by kind and path, the
        // ref on display wins, since that is the version the drawer is about.
        // The PROSE keeps every link re-aimed individually: there both refs are
        // meaningful, because the sentence around them says which is which.
        const byFile = new Map(), targets = [];
        for (const a of doc.querySelectorAll('a[href]')) {
          a.setAttribute('target', '_blank');
          a.setAttribute('rel', 'noopener');
          const t = this.openTarget(a.getAttribute('href'));
          if (!t) continue;
          ...
          if (held.ref !== this.viewingRef && t.ref === this.viewingRef) {
            targets[targets.indexOf(held)] = t;
            byFile.set(key, t);
          }
        }
        this.prTargets = targets;
        this.prBodyHtml = doc.body.innerHTML;
      },
      ...
      get takeSubject() {
        const base = (this.path || '').split('/').pop() || 'this page';
        ...
        return [
          ...
          { kind: 'Open', items: [
            { key: 'stage', icon: 'ph-stack', label: files ? 'Stage (' + files + ' file' + (files === 1 ? '' : 's') + ')' : 'Stage',
              desc: list + ' at ' + (this.ref || 'main') + ', on show-repo.' + caveat },
          ] },
          { kind: 'Save', items: [
