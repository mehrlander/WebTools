...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto">
            <header class="px-2 py-1.5 border-b border-base-300 flex items-center justify-between gap-2 shrink-0">
              <div class="flex items-center gap-0.5">
                ...
                <!-- Traffic. The label hides on a narrow viewport rather than
                     letting three labelled tabs squeeze the header: the drawer
                     is 92vw below 24rem, and on a 360px phone the third label
                     is what pushes the refresh controls off the row. -->
                <button @click="activeTab = 'traffic'; refreshTraffic()"
                  ...
                  <span class="max-[400px]:hidden">Traffic</span>
                </button>
              </div>
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <!-- TRAFFIC. Three bands, three different questions that all get
                   ...
                   rate limit bounds it), RESIDENT is what is held locally (not
                   traffic at all, but the same question with no other home).
              ...
              <div x-show="activeTab === 'traffic'" class="min-h-0 flex-1 overflow-y-auto">
                ...
                <template x-if="trafTotals && trafTotals.calls">
                  <div>
                    ...
                    <!-- Grouped by endpoint shape, which is the only attribution
                         a browser can make honestly: there is no async caller
                         context, and the scripts registry refused stack sniffing
                         for the same reason. Forty calls to the commits
                         endpoint are the activity crawl, whatever function
                         issued them. -->
                ...
                <div class="px-2.5 pt-2 pb-0.5 flex items-baseline justify-between gap-2 border-t border-base-300/60">
                  <div class="text-[12px] uppercase tracking-wider text-base-content/50 font-semibold">Resident</div>
                  <div class="text-[11px] text-base-content/40">held on this device</div>
                </div>
                <div class="px-2.5 pb-3 space-y-0.5 text-[12px]">
                  ...
                  <div x-show="!trafStores.length && !trafEstimate" class="text-base-content/50 italic">
                    Nothing held for this origin.
                  </div>
                  <div class="text-[11px] text-base-content/40 pt-1">
                    Read once when the tab opened. Nothing here is traffic; it is what a
                    reload would not have to fetch again.
                  </div>
                </div>
              </div>
      ...
      // Resident is origin-scoped, so it is the same answer for the shell and
      ...
      renderTarget(repo, ref, path, any) {
        ...
        // No ref is not a missing ref: the grammar reads a bare repo:path as
        // the repo's default branch, which is exactly what a cross-repo pick
        // means. Only the title has to say so, since "at " with nothing after
        ...
        // `addr` rides alongside the url because an in-toss re-address needs the
        // address, not the link: the renderer is already open, and handing it a
        // URL would reload the shell to reach a state it can reach in place.
        if (ext === 'html' || ext === 'htm') {
      ...
      async renderPrBody() {
        ...
        for (const a of doc.querySelectorAll('a[href]')) {
          ...
          if (!t) {
            // Not something this drawer can render: a session link, a compare
            // view, an issue. Those still belong to GitHub and still open away.
            a.setAttribute('target', '_blank');
            a.setAttribute('rel', 'noopener');
            continue;
          }
          // A renderable link renders IN PLACE, like every other way into a
          // file here. It keeps its href so a long-press can still copy the
          // durable address; onGuideClick intercepts the ordinary tap. A new
          // tab was the old behavior and it was the wrong one twice over: it
          // is not "as if we changed the branch", and an in-app browser that
          // will not open one leaves a blank window with nothing to say why.
