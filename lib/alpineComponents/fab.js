    ...
    return {
      ...
      template: `
        ...
        <div class="absolute inset-y-0 right-0 transition-transform duration-300 ease-out pointer-events-none"
          ...
          <!-- shadow-md, down from shadow-2xl, and contain:paint. The panel is
               full-height and appears in one frame over whatever the page is
               showing; a 50px-blur shadow on an element that size is an
               offscreen pass the browser has to run before it can composite,
               and containment tells it the panel's paint cannot escape its
               own box, so nothing outside needs re-deriving. Both are the
               same suspicion as the header's blur above: this drawer dies in
               its first frame on an iPhone, and these are the two things in it
               whose cost is a function of what is behind them. -->
          <div x-init="_crumb('d:body')" style="contain: paint;"
               class="h-full bg-base-100 border-l border-base-300 shadow-md flex flex-col pointer-events-auto overscroll-contain">
      ...
      // AN EXPERIMENT, AND IT SAYS SO. Off unless the address carries
      // ?fabframe=hide, which is why it is safe to ship: it changes nothing
      // for any ordinary reader.
      //
      // Inside a toss the drawer is a fixed layer that appears over an iframe,
      // and that combination is the one thing every failing case on the device
      // shares. Measured 2026-09-08: the drawer opens on a plain deployed page
      // and dies in its first frame on the SAME page delivered through a toss,
      // with 128 elements under it, so neither the page's content nor the
      // drawer's own size can be the cause. Compositing a fixed overlay above
      // an out-of-process frame is where an iPhone is losing the tab.
      //
      // So: hide the frame while the drawer is up. If the drawer then opens,
      // the overlay-above-a-frame interaction is the mechanism and the real fix
      // is to lay the two out side by side rather than stacked. If it still
      // dies, the frame is not what the browser is choking on and this comes
      // straight back out.
      _tossFrameEl() {
        try { return (window.__tossFrame && window.__tossFrame.style) ? window.__tossFrame : null; }
        catch { return null; }
      },
      _hideFrameWhileOpen(open) {
        try {
          if (!/[?&]fabframe=hide\b/.test(location.search)) return;
          const f = this._tossFrameEl();
          if (!f) return;
          f.style.visibility = open ? 'hidden' : '';
          this._crumb(open ? 'frame:hidden' : 'frame:shown');
        } catch { }
      },

      toggle() {
        this._crumb('toggle:enter');
        if (this.open) { this._hideFrameWhileOpen(false); this.close(); this._crumbDone(); return; }
        this._crumb('detect');
        this.detect();
        this._crumb('refreshLayers');
        this.refreshLayers();
        this._crumb('open');
        // Before the state flips, so the frame is already out of the way when
        // the drawer's first frame is composited.
        this._hideFrameWhileOpen(true);
        this.open = true;
      ...
      close() {
        // Restore the frame here too, since the drawer is closed from several
        // places (the header X, escape, a layer pick) and not only via toggle.
        this._hideFrameWhileOpen(false);
        this.open = false;
        this.clearHighlight();
      },
