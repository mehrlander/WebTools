...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
      ...
      // action came from. Page actions are collected from the SHELL only, which
      // is the fact that makes toss-render's own actions un-previewable: in a
      // toss the shell is whatever github.io served, always the default branch.
      get pageActions() {
        return this.groups.flatMap(g => (g.actions || []).map(a => ({ ...a, from: g.name })));
      },
      ...
      get takeGrid() {
        ...
        this.pageActions.forEach((a, i) => {
          ...
          bucket.get(k).push({ key: 'page:' + i, icon: a.icon || 'ph-lightning', label: a.label,
                               desc: (a.desc || a.label) +
                                     ' Contributed by this page (' + (a.from || 'unknown') + '), not by the toolbar.',
                               kind: 'page', run: a.run });
        });
