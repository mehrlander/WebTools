  ...
  Alpine.data('fab', function() {
    return {
      ...
      adoptSubject() {
        ...
        // the lot there re-ran the whole branch survey per swipe
        ...
        // branch survey is the one genuinely per-file answer here ("which
        ...
        if (this.repo !== was.repo) { this._div = {}; this._survey = {}; }
        if (refChanged) {
      ...
      // The survey's reads, behind a sixty-second read-through cache.
      //
      ...
      // files along and four back is eight surveys for four answers. Same TTL
      ...
      _survey: {},
      _surveyRead(key, make) {
        const now = Date.now();
        const hit = this._survey[key];
        if (hit && now - hit.at < 60000) return hit.p;
        const p = make();
        this._survey[key] = { at: now, p };
        p.catch(() => { if (this._survey[key]?.p === p) delete this._survey[key]; });
        return p;
      },

      async loadPageBranches(force) {
        if (force) { this.pageBranchesLoaded = false; this._survey = {}; this._div = {}; }
        ...
        try {
          ...
          if (typeof tmp.branches !== 'function') {
            ...
            if (this.path && typeof tmp.branchesForPath === 'function') {
              try {
                const r = await this._surveyRead('path|' + this.repo + '|' + this.path,
            ...
            if (!rows) {
              ...
              try {
                list = typeof tmp.branchesDated === 'function'
                  ? await this._surveyRead('all|' + this.repo, () => tmp.branchesDated()) : null;
              } catch (e) { list = null; }
              if (!list) list = (await this._surveyRead('bare|' + this.repo, () => tmp.branches()))
      ...
      // Memoized across surveys, because ahead/behind is a property of the
      // BRANCH PAIR and not of the file. The survey itself is genuinely
