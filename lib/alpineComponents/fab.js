...
const registerFab = function() {
  Alpine.data('fab', function() {
    return {
      ...
      _crumb(stage) {
        try { localStorage.setItem('fab:step', stage + ' @' + Date.now()); } catch {}
      },
      _crumbDone() {
      ...
      toggle() {
        ...
        // Synchronous work is done. ensureBrief/loadVersion/loadGuide are async
        // and still in flight, so the trail is dropped a frame later: a crash
        // inside one of them leaves the crumb naming which was last started.
        this._crumb('paint');
        requestAnimationFrame(() => requestAnimationFrame(() => this._crumbDone()));
      },
