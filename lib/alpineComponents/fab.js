  ...
  Alpine.data('fab', function() {
    return {
      ...
      openTarget(href) {
        ...
        if (ext === 'html' || ext === 'htm') {
          return { kind: 'render', ref, path, url: base + '#gh=' + addr, label, icon: 'ph-disc',
                   title: 'Render ' + path + ' at ' + ref };
        }
        if (['md', 'markdown', 'csv', 'tsv', 'json', 'txt', 'log', 'yml', 'yaml'].includes(ext)) {
          return { kind: 'read', ref, path, url: base + '#data=' + addr, label, icon: 'ph-file-text',
