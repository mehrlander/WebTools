...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto">
            <header class="px-2 py-1.5 border-b border-base-300 flex items-center justify-between gap-2 shrink-0">
              ...
              <div class="flex items-center gap-1 shrink-0">
                <span x-show="capMsg" x-cloak class="text-[11px] text-base-content/50" x-text="capMsg"></span>
                <!-- Capture: the drawer's whole diagnostic bundle as one JSON
                     string on the clipboard, so "does this page work in your
                     browser" is answered by a paste instead of a screenshot. -->
                <button x-show="activeTab === 'inspect'" @click="copyCapture()" class="btn btn-ghost btn-xs btn-square"
                        title="Copy capture: scripts, components, console, and reads, as JSON" aria-label="Copy capture">
                  <i class="ph" :class="capMsg && !capMsg.startsWith('Copy failed') ? 'ph-check text-success' : 'ph-copy'"></i>
                </button>
      ...
      // First cut of the capture task: clipboard out, no write path yet.
      ...
      async copyCapture() {
        try {
          ...
          this.capMsg = 'Copied ' + Math.max(1, Math.round(text.length / 1024)) + 'K';
        } catch (e) {
          this.capMsg = 'Copy failed: ' + (e?.message || e);
        }
        clearTimeout(this._capT);
        this._capT = setTimeout(() => { this.capMsg = ''; }, 4000);
      },
