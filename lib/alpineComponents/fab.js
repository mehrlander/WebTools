              ...
              <div x-show="activeTab === 'render'" class="min-h-0 flex-1 flex flex-col">
                ...
                <template x-if="path">
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                    ...
                    <!-- The whole ref block is now one conditional row. It began
                         as three (an "Any ref" heading, a "selected <ref>"
                         readout, a free-text field and Set) plus a button that
                         was always on screen.

                         It appears only when the selection differs from what is
                         rendered, because that is the only state in which it does
                         anything: press it at the current ref and you get the view
                         you already have. Selecting a row is the ask; this is the
                         confirm, and with nothing to confirm it is noise. The loop
                         closes on itself, since re-tossing resets the selection to
                         the newly rendered ref and the button leaves again.

                         The free-text field is gone. The branch list carries every
                         branch in the repo once expanded, so it only ever added
                         tags and shas, which is not how this panel is used. -->
                    <div x-show="refPending" class="shrink-0 flex flex-col gap-1">
                      <button @click="renderAtRef()" :disabled="!viaToss && !tossUrl" class="btn btn-primary gap-1.5 w-full min-w-0"
                    ...
                    <div x-show="refPending && !viaToss && !tossUrl" class="text-[12px] text-base-content/40 leading-snug shrink-0">Toss renders through toss-render, which serves owner repos only.</div>
