  ...
  Alpine.data('fab', function() {
    return {
            ...
            <header class="px-2 py-1.5 border-b border-base-300 flex items-center justify-between gap-2 shrink-0">
              <div class="flex items-center gap-0.5">
                ...
                <button @click="activeTab = 'notes'; annOpen()"
                  ...
                  <span>Notes</span>
                  <span x-show="annItems.length" x-text="annItems.length"
                        class="inline-flex items-center justify-center text-[11px] font-bold leading-none rounded-full bg-warning text-warning-content px-1 min-w-[14px]"></span>
                </button>
              </div>
              ...
              <div x-show="activeTab === 'notes'" class="min-h-0 flex-1 flex flex-col">
                ...
                <div x-show="!annOn" class="p-3 flex flex-col gap-2.5 items-start">
                  <div class="text-sm text-base-content/60">
                    ...
                    and says which passage it means.
                  </div>
                ...
                <template x-if="annOn">
                  <div class="min-h-0 flex-1 flex flex-col">
                    ...
                    <div class="shrink-0 flex gap-0 px-2.5 pt-2">
                      <template x-for="t in annTabs" :key="t.key">
                        <button @click="annSetTab(t.key)"
                                class="flex-1 px-2 py-1 text-[12px] font-semibold border border-base-300 transition-colors first:rounded-l last:rounded-r [&:not(:first-child)]:border-l-0"
                    ...
                    <div x-show="!annItems.length" class="text-sm text-base-content/50 italic px-3 py-6 text-center">
                      No notes yet. Select text on the page, or use Element / Region on the card.
                    </div>
      ...
      get annSubject() {
        ...
        const s = window.Annotate && window.Annotate.subject;
      ...
      async annOpen() {
        if (!window.Annotate) {
          try { await this._selfLoad('kits/annotate.js', () => !!window.Annotate); } catch (e) {}
        }
        this.annSync();
      },
      async annStart() { await this.openAnnotate(); this.open = true; this.activeTab = 'notes'; },
      annStop() {
        if (window.Annotate) window.Annotate.disable();
        this.annSync();
      },
      ...
      annEdit(id, value) { if (window.Annotate) window.Annotate.update(id, value); },
      annRemove(id) { if (window.Annotate) window.Annotate.remove(id); },
      ...
      annSelect(id, scroll = true) {
        if (window.Annotate) window.Annotate.select(id === this.annSel && scroll ? null : id, { scroll });
      },
      annClear() { if (window.Annotate) window.Annotate.clear(); this._annSay('Cleared'); },

      async annCopy(kind) {
        try {
          const t = await window.Annotate.copy(kind);
      ...
      async annJot() {
        try { this._annSay('Saving jot…'); await window.Annotate.saveJot(); this._annSay('Jot saved'); }
