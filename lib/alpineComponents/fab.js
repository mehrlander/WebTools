...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
              ...
              <div x-show="activeTab === 'render'" class="min-h-0 flex-1 flex flex-col">
                ...
                <template x-if="path">
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                    ...
                    <!-- One row where there were three. The "Any ref" heading and
                         the "selected <ref>" readout both said what the button can
                         say itself, so the button names the ref it will render at
                         and the other two rows are gone.

                         The free-text field is now behind the ⌨ toggle, because
                         the list above already carries every branch in the repo
                         once expanded: typing is only for a TAG, a SHA, or a ref
                         the branch survey does not enumerate. That is a real case
                         and worth keeping, but it is not the common one, and it
                         was taking a row and a half of prime space to serve it. -->
                    <div class="shrink-0 flex flex-col gap-1">
                      <div x-show="refInputOpen" class="flex gap-1">
                        <input x-model="frameRefInput" placeholder="tag / sha / unlisted ref"
                               @keyup.enter="applyFrameRef()" x-ref="refInput"
                      ...
                      <div class="flex gap-1">
                        <button @click="renderAtRef()" :disabled="!viaToss && !tossUrl" class="btn btn-primary gap-1.5 flex-1 min-w-0"
                          ...
                          <i class="ph ph-disc shrink-0"></i>
                          <span class="truncate font-mono text-[13px]" x-text="(viaToss ? 'Re-toss at ' : 'Toss at ') + (frameRef || 'main')"></span>
                        </button>
                        <button @click="toggleRefInput()" class="btn btn-square shrink-0"
                                :class="refInputOpen && 'btn-active'"
                                title="Type a tag, a sha, or a ref the branch list does not carry"
                                aria-label="Enter a ref by hand">
                          <i class="ph" :class="refInputOpen ? 'ph-x' : 'ph-keyboard'"></i>
                        </button>
                      </div>
                    </div>
      ...
      frameRef: 'main', frameRefInput: '', refInputOpen: false,
      ...
      toggleRefInput() {
        this.refInputOpen = !this.refInputOpen;
        if (this.refInputOpen) this.$nextTick(() => this.$refs.refInput?.focus());
      },
      ...
      applyFrameRef() {
        ...
        this.frameRef = v;
        this.frameRefInput = '';
        this.refInputOpen = false;   // the button now names it; the field is done
      },
