    ...
    return {
            ...
            <div class="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div x-show="activeTab === 'render'" class="shrink-0">
                <div class="relative p-2" @click.outside="ghMenu = false">
                  <!-- WHICH LAYER, above the identity it governs.

                       A page reaches the screen through a stack of frames, and
                       every pane under this one answers about exactly one of
                       them. Picking silently is what let the app view report
                       show-repo while the reader was looking at a page from
                       another repo, and there is no layer that is the right
                       answer in every context, which is the tell that choosing
                       one was the wrong move. So the stack is named and the
                       reader picks.

                       ONE LAYER IS A LABEL, not a control: a row strip with a
                       single row is a control that cannot do anything, which
                       reads as broken rather than as "there is nothing to
                       choose". That is the ordinary case, a page open on its
                       own, so it has to be the quiet one.

                       Each row carries its own off-ref mark. The launcher is
                       one glyph and follows the SELECTED layer, so without a
                       mark per row a layer sitting on branch code could hide
                       behind a neutral button two layers up. -->
                  <template x-if="layers.length > 1">
                    <div class="mb-2 flex items-stretch gap-0.5 rounded border border-base-300 bg-base-200/40 p-0.5">
                      <template x-for="(L, i) in layers" :key="i">
                        <button @click="selectLayer(i)" :disabled="L.sealed" :title="layerTitle(L)"
                                class="min-w-0 flex-1 rounded px-1.5 py-1 text-left transition-colors"
                                :class="[i === layerIndex ? 'bg-base-100 shadow-sm ring-1 ring-primary/40'
                                                          : 'opacity-60 hover:opacity-100 hover:bg-base-content/10',
                                         L.sealed ? 'opacity-50 cursor-not-allowed' : '']">
                          <span class="flex items-center gap-1">
                            <i class="ph shrink-0 text-[10px]"
                               :class="L.sealed ? 'ph-lock-simple opacity-60'
                                       : layerOffRef(L) ? 'ph-disc text-warning'
                                       : 'ph-circle opacity-25'"></i>
      ...
      readLayers(root) {
        ...
        out.forEach((L, i) => {
          ...
          if (i === out.length - 1) { L.role = out.length > 1 ? 'page' : 'shell'; return; }
      ...
      layerName(L) {
        ...
        return L.label || (L.path || '').split('/').pop() || L.repo || 'unknown';
      },
      layerTitle(L) {
        if (!L) return '';
