          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto">
            <header class="px-2 py-1.5 border-b border-base-300 flex items-center justify-between gap-2 shrink-0">
              <div class="flex items-center gap-0.5">
                <!-- Word-only tabs. The icons were dropped 2026-08-06: three
                     glyphs bought nothing a one-word label does not already
                     say, and they were the reason the row ran out of width.
                     That width was being paid for twice, since Traffic also
                     hid its label below 400px to keep the refresh controls on
                     the row. Removing the icons buys back more than the label
                     costs, so all three tabs now read at every viewport. -->
                ...
                <button @click="activeTab = 'inspect'; detect()"
                        class="px-2 py-1 rounded text-sm font-semibold transition-colors"
