  ...
  Alpine.data('fab', function() {
    return {
                  ...
                  <div class="p-2 flex flex-col gap-2 min-h-0 flex-1">
                    ...
                    <div class="border-t border-base-300/60 pt-2 pb-10 flex flex-col gap-1.5 shrink-0">
                      ...
                      <div class="grid grid-cols-2 gap-1">
                        <template x-for="a in takeItems" :key="a.key">
                          <button @click="runGridItem(a)" :title="a.desc"
                                  :class="outBusy && 'pointer-events-none opacity-60'"
                                  class="btn btn-xs btn-ghost bg-base-200 hover:bg-base-300 justify-start gap-1.5 font-normal">
      ...
      get takeGroups() {
        ...
        return [
          { kind: 'Copy', items: [
            { key: 'render', icon: 'ph-code', label: 'Copy HTML',
              desc: t.payload
            ...
            { key: 'brief', icon: 'ph-clipboard-text', label: 'Copy brief',
          ...
          { kind: 'Open', items: [
            // The count is the label's whole job: "Stage" alone never said what.
            { key: 'stage', icon: 'ph-stack', label: files ? 'Stage ' + files + ' file' + (files === 1 ? '' : 's') : 'Stage files',
          ...
          { kind: 'Download', items: [
            // The pair differs in one thing, so the labels differ in one word.
            // Not "Offline zip": neither is offline, since the third-party CDN
            // tags stay in both. What the second buys is independence from THIS
            // repo, which is what breaks on a private repo or an unpushed branch.
            { key: 'export', icon: 'ph-file-archive', label: 'Zip',
              desc: 'The page and the data it read()s, as files. Its code still loads from ' +
                    'the CDN, so this needs the repo reachable. Smaller.' },
            { key: 'offline', icon: 'ph-hard-drives', label: 'Zip + code',
              desc: 'The same, with the code chain inlined, so it opens without reaching this ' +
                    'repo at all. Third-party CDN tags (Tailwind, Alpine, Phosphor) stay in both.' },
          ] },
        ];
      },
      ...
      // Flattened for rendering. takeGrid keeps the ordering (clipboard first,
      // then the one that navigates, then the ones that land a file), but the
      // headings that used to carry the verb are gone: each label carries its
      // own now, which is three fewer lines and a button that reads on its own
      // wherever it happens to sit.
      get takeItems() { return this.takeGrid.flatMap(g => g.items); },
