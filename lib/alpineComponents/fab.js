...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
              ...
              <div x-show="!subjectLocal && subjectBase" x-cloak
                ...
                <div x-show="compareMenu" x-cloak
                  ...
                  <button @click="compareWith('')" :disabled="!compareOff && !compareRef"
                          ...
                          :class="(!compareOff && !compareRef) ? 'bg-warning/15 cursor-default' : 'hover:bg-base-300/50 cursor-pointer'">
                  ...
                  <template x-for="b in compareTargets" :key="'cmp-' + b.name">
                    <button @click="compareWith(b.name)" :disabled="b.name === compareRef"
                            class="flex items-center gap-1.5 px-1.5 py-1 rounded text-[13px] font-mono text-left transition-colors"
                            :class="[b.name === compareRef ? 'bg-warning/15 cursor-default' : 'hover:bg-base-300/50 cursor-pointer',
                                     (b.status === 'same' || b.status === 'missing') ? 'opacity-50' : '']">
                      <i class="ph text-sm shrink-0"
                         :class="b.status === 'baseline' ? 'ph-house-line opacity-50' : 'ph-git-branch opacity-50'"></i>
                  ...
                  <!-- Two empty states, because they mean different things: the
                       survey has not answered, or it answered and the merge
                        ...
                        <div>
                          ...
                          <template x-for="h in textMatch.hits" :key="h.path">
                            <div class="border-t border-base-300/40">
                              ...
                              <div x-show="!h.what" class="px-2.5 pb-1.5 pl-[1.9rem] text-[12px] text-base-content/35 italic leading-snug">
                                ...
                                <a :href="o.exists === false ? null : o.blob"
                                  ...
                                  <i class="ph text-[13px] shrink-0"
                                     :class="o.exists === true ? 'ph-check text-base-content/40'
                                           : o.exists === false ? 'ph-x text-base-content/30' : 'ph-minus text-base-content/25'"></i>
                                  <span class="font-mono text-[13px] truncate min-w-0"
                                        ...
                                        x-text="o.path"></span>
                                  <span x-show="o.exists === false" class="text-[11px] uppercase tracking-wide text-base-content/35 shrink-0">not in repo</span>
                                </a>
                              </template>
      ...
      // row IS. Data rather than branches, so adding one is a row.
      // `groups` marks the nested shape pages.json uses; `live` gives a
      // deployed URL, which is the one gloss better than a description.
      // data/design/content.csv is deliberately absent: it is a CSV, its
      // locators are often directories rather than files, and its description
      // classifies how content was made rather than saying what the file is.
      MATCH_REGISTRIES: [
        { path: 'docs/docs.json',     rows: 'documents', key: 'path', what: 'subject',  tag: 'status' },
        { path: 'docs/tests.json',    rows: 'tests',     key: 'path', what: 'protects', tag: 'kind' },
        { path: 'docs/harness.json',  rows: 'tools',     key: 'path', what: 'role',     tag: 'layer' },
        { path: 'docs/portable.json', rows: 'items',     key: 'path', what: 'role',     tag: 'kind' },
        { path: 'pages/pages.json',   rows: '',          key: 'href', what: 'note',     tag: '',
          groups: 'items', prefix: 'pages/', alt: 'title',
      ...
      async _registered(repo, ref) {
        ...
        await Promise.all(this.MATCH_REGISTRIES.map(async (r) => {
          try {
            ...
            const rows = r.groups
              ? [].concat(...(Array.isArray(doc) ? doc : []).map(g => g[r.groups] || []))
              : (doc[r.rows] || []);
      ...
      adoptSubject() {
        ...
        // Both surveys fall back to the hub repo when `repo` is empty, which is
      ...
      goTarget(t) {
        if (!t || !t.url) return;
        // A deck slide is not a page to navigate TO. The reader is somewhere
        // in a changeset, and answering "show me this at main" by leaving for
        // a single-file renderer throws away the list, their place in it, and
        // the way back. The deck can re-render the slide where it stands, so
        // it is asked first and its answer is authoritative: false means it
        // cannot show that file, and then this is a real navigation after all.
        if (this.subjectRoute === 'deck' && typeof window.__deckNavigate === 'function') {
