...
document.addEventListener('alpine:init', function() {
  Alpine.data('fab', function() {
    return {
        ...
        <div class="absolute inset-y-0 right-0 transition-transform duration-300 ease-out pointer-events-none"
          ...
          <div class="h-full bg-base-100 border-l border-base-300 shadow-2xl flex flex-col pointer-events-auto overscroll-contain">
            <header class="px-2 py-1.5 border-b border-base-300 flex items-center justify-between gap-2 shrink-0">
              <!-- THE STRIP CARRIES ONE LABEL, THE ACTIVE TAB'S.
                   ...
                   So the label appears on the SELECTED tab only. That is the
                   one place a label is not redundant: an icon says what a tab
                   would do, and the reader has already decided that, while the
                   name of what is in front of them is worth stating. Four icons
                   plus one label costs about 195px of 348px, so a sixth tab
                   fits without another ratchet. Unselected tabs keep their name
                   in title and aria-label, so hover and a screen reader both
                   still say it.
              ...
              <div class="flex items-center gap-0.5 min-w-0 overflow-x-auto">
                <template x-for="t in TABS" :key="t.key">
                  <button @click="setTab(t.key)" :title="t.label" :aria-label="t.label"
                          ...
                          class="flex items-center gap-1 px-1.5 py-1 rounded text-[13px] font-semibold transition-colors shrink-0"
                    ...
                    <span x-show="activeTab === t.key" x-text="t.label"></span>
                  </button>
                </template>
              </div>
                ...
                <template x-if="textStats">
                  <div>
                    ...
                    <div class="px-2.5 py-2 border-t border-base-300/60 text-[12px] text-base-content/50 leading-snug">
                      Read only: no network, no model. Matching terms against the registries
                      is not in this pane.
                    </div>
                  </div>
                </template>
      ...
      textStats: null, textScanned: false,
      ...
      textScan() {
        this.textStats = this._textRead(this.textDoc());
      ...
      // Pure enough to test: takes a document, returns the figures. Kept off
      // the annotate kit because that kit's index exists to anchor quotes and
      // skips nothing this needs to skip differently; if a third reader turns
      // up, the shared walk graduates to a kit rather than growing a flag.
      _textRead(doc) {
        ...
        const barePaths = this._barePaths(doc, root);
        return {
          words, chrome, runs,
      ...
      // A repo-path-shaped token sitting outside any anchor. Deliberately
      // narrow: two or more segments and a file extension, so "and/or" and a
      // bare "docs" never count. It reports a COUNT of candidates, not a
      // verdict that each is a real file; resolving them against the tree is
      // the Match operation and it is not in this pane.
      _barePaths(doc, root) {
        ...
        let n;
        while ((n = walker.nextNode())) {
          if (n.parentElement?.closest('a, code, pre, [x-data^="fab"], [data-annotate-ui]')) continue;
          hits += (n.data.match(RE) || []).length;
        }
        return hits;
      },
