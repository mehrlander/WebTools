  ...
  Alpine.data('stager', function() {
    ...
    return {
                    ...
                    <div class="flex flex-col gap-1 w-12 shrink-0">
                      <template x-for="(m, i) in dictMarks" :key="i">
                        <button @pointerdown.prevent="dictMark(m)"
                                ...
                                :title="m === '¶' ? 'New paragraph' : 'Insert ' + m" x-text="m"></button>
                      </template>
      ...
      dataUri(it) {
        ...
        // IMAGE_MIME until 2026-08-15, which meant a dropped PDF fell through
        // to the text path and showed its own object syntax as mojibake.
      ...
      get dictSel() { return !!(this._dict && this._dict.hasSelection); },
      get dictMarks() {
        if (this.dictSel) return ['AB', 'ab', 'Ab'];
        return this.dictShift ? [';', '!', '¶'] : ['.', ',', '?'];
      },
