...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
      ...
      cmpViews: [
        { key: 'unified', label: 'Unified', icon: 'ph-rows' },
        { key: 'split',   label: 'Split',   icon: 'ph-columns' },
        { key: 'patch',   label: 'Patch',   icon: 'ph-file-text' },
      ],
      setCmpView(key) {
      ...
      async _cmpRender(i, slide) {
        ...
        if (d && this.cmpView === 'patch') {
          const text = window.textDiff.patch(d.ops, d.al, d.bl, {
            context: 3, nameA: this.previewName(a), nameB: this.previewName(b),
          });
          ...
          if (!text) {
            ...
            for (const r of window.textDiff.patchRows(text)) pre.append(H('div', {
              class: 'px-2 ' + (r.t === 'add' ? 'bg-success/10 text-success'
                              ...
                              : r.t === 'meta' ? 'text-base-content/40' : 'text-base-content/55'),
            }, r.line || ' '));
          }
          box.append(pre);
        }
      ...
      _cmpSplit(d) {
        ...
        const TINT = { add: 'bg-success/10 text-success', del: 'bg-error/10 text-error',
                       ctx: 'bg-base-100 text-base-content/55', none: 'bg-base-200/30' };
        const MARK = { del: 'bg-error/25 rounded-sm', add: 'bg-success/25 rounded-sm' };
