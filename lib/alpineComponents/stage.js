...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
                ...
                <div x-show="offers.length" x-cloak
                  ...
                  <div class="flex flex-wrap gap-1" x-effect="seedPeeks()">
                    <template x-for="fl in offers" :key="fl.name">
                      <button @click="toggleFlavor(fl)" :data-peek="peekKey('flavor', fl)"
                    ...
                    <template x-for="t in pasteLinks" :key="t.key">
                      <button @click="extractLinks(t)" :data-peek="peekKey('links', t)"
                              :title="'Read the ' + t.n + ' links in the ' + t.from + ' out into ' + t.dest"
                    ...
                    <!-- THE WHOLE FLAVOR, CONVERTED. The links pill is a
                         ...
                         them. It is the one pill whose preview has to be MADE
                         before it can be shown, so a hover starts the conversion
                         and the card comes with the next one. -->
                    <template x-for="t in pasteMarkdown" :key="t.key">
                      <button @click="stageMd(t)" @mouseenter="primeMd(t)" :data-peek="peekKey('md', t)"
                              :title="'Convert the ' + t.from + ' to markdown as ' + t.dest"
                              class="badge badge-lg h-auto py-1 gap-1.5 cursor-pointer badge-outline border-dashed hover:badge-primary">
      ...
      // A NEW BAR DROPS THE OLD CONVERSIONS: they are readings of the paste on
      // screen, and the paste has changed. The peek cache is keyed by file name
      // and re-seeded by the effect, so it needs no clearing of its own.
      set offers(v) { this._mdText = null; Alpine.store('browser').stageOffers = v || []; },
      offer(flavors) {
      ...
      peekKey(kind, o) {
        if (kind === 'flavor') return o.kind === 'file' ? '' : o.name;
        if (kind === 'links') return o.dest;
        // mdSeq is read, not used: the conversion lands in a Map, which Alpine
        // does not watch, so the counter is what re-evaluates this binding.
        if (kind === 'md') return (this.mdSeq, this._mdText?.has(o.key)) ? o.dest : '';
        return '';
      },
      ...
      seedPeeks() {
        const P = window.SourcePeek, IN = window.StageIntake;
        if (!P) return;
        this.mdSeq;
        for (const fl of this.offers) {
          if (fl.kind !== 'file' && fl.text) P.seed(fl.name, fl.text);
        }
        for (const t of this.pasteLinks) P.seed(t.dest, IN.linksMd(t.links));
        for (const t of this.pasteMarkdown) {
          const md = this._mdText?.get(t.key);
          if (md) P.seed(t.dest, md);
        }
      },
