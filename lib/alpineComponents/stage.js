...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div class="flex items-center gap-1 text-base font-semibold uppercase tracking-wide text-base-content/40">
                  ...
                  <button x-show="dictAvail" @click="dictOpen ? dictCancel() : dictStart()"
                    ...
                    <i class="ph text-base" :class="dictOpen ? 'ph-x' : 'ph-microphone'"></i>
                  </button>
                ...
                <div x-show="dictOpen" x-cloak
                  ...
                  <div x-show="!dictEdit" class="flex gap-1.5 items-stretch">
                    <div class="flex-1 min-w-0 min-h-[3.5rem] max-h-40 overflow-y-auto text-base leading-relaxed px-1">
                      <span class="whitespace-pre-wrap" x-text="dictText"></span><span
                            class="whitespace-pre-wrap opacity-40 italic"
                            x-text="dictInterim ? (dictText ? ' ' : '') + dictInterim : ''"></span>
                      <span x-show="!dictText && !dictInterim" class="opacity-40 italic">Listening…</span>
                    </div>
                    <div class="flex flex-col gap-1 w-11 shrink-0">
                      <template x-for="(m, i) in dictMarks" :key="i">
                        <button @pointerdown.prevent="dictMark(m)"
                                class="btn btn-sm h-9 min-h-0 px-0 text-lg font-semibold"
                      ...
                      <button @pointerdown.prevent="dictShift = !dictShift"
                              class="btn btn-xs h-7 min-h-0 px-0 font-mono"
                              :class="dictShift ? 'btn-warning' : ''"
                              title="The other three marks"
                              x-text="dictShift ? 'abc' : '·!¶'"></button>
                    </div>
                  </div>
                  ...
                  <div class="flex items-center gap-2">
                    <button x-show="!dictEdit" @click="dictToggle()" class="btn btn-sm btn-square"
                            :class="dictOn ? 'btn-error' : 'btn-ghost'"
                            :title="dictOn ? 'Recording. Tap to stop.' : 'Resume'">
                      <i class="ph ph-microphone text-base"></i>
                    </button>
                    <!-- Backspace sits down here rather than in the pad. It
                         does not INSERT anything, so reading as a seventh mark
                         put an undo one slip away from an exclamation point;
                         the annotator's composer separated the two on the same
                         grounds. The bottom row has the room. -->
                    <button x-show="!dictEdit" @pointerdown.prevent="dictBack()"
                            class="btn btn-sm btn-square btn-ghost" title="Delete the last word">
                    ...
                    <button @click="dictEdit ? dictEditClose() : dictEditOpen()"
                            ...
                            :class="dictEdit ? 'btn-warning' : 'btn-ghost'"
                            :title="dictEdit ? 'Back to dictation' : 'Type instead (opens the keyboard)'">
                      <i class="ph text-base" :class="dictEdit ? 'ph-microphone' : 'ph-pencil-simple'"></i>
                    </button>
      ...
      async dictStart() {
        ...
        try {
          ...
          this._dict = window.Dictate.create({
            onText: (t) => { this.dictText = t; },
            onInterim: (t) => { this.dictInterim = t; },
            onState: () => { this.dictOn = !!(this._dict && this._dict.listening); },
            onError: (m) => { this.dictErr = m; },
          });
          this.dictText = ''; this.dictInterim = ''; this.dictOpen = true;
          this._dict.start();
        } catch (e) {
      ...
      get dictMarks() { return this.dictShift ? [';', '!', '¶'] : ['.', ',', '?']; },
      ...
      dictMark(m) {
        if (this._dict) this._dict.punct(m);
