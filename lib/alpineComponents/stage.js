  ...
  Alpine.data('stager', function() {
    ...
    return {
              ...
              <div x-show="items.length" class="flex flex-col gap-2">
                ...
                <div class="flex items-center justify-between gap-2 border-b border-base-300 pb-2">
                  ...
                  <!-- Only the lens's own actions, and only on Out. What acts
                       ...
                       files on the bench answered no question the staged list
                       and the inline file viewer do not answer better, and it
                       was the one control here that produced no output. The
                       size still reads out beside the lens, which is the part
                       of "show me the block" anyone actually wanted. -->
                ...
                <template x-if="outTab === 'diff'">
                  <div class="flex flex-col gap-2">
                    <!-- STAGE TWO THINGS AND IT COMPARES THEM. The lens used
                         to open on two selects and two "ref" boxes, which
                         announced the wrong premise: it read as "type two
                         refs", nobody types a ref, and with one item staged
                         both selects named the same file so the pair looked
                         like something you had to construct. It was never a
                         second way to stage; the selects only ever chose among
                         the staged items, and B already auto-paired to the
                         second one. That was invisible under the machinery.

                         So the machinery appears only when it has a job. Under
                         two staged items there is nothing to compare and the
                         lens says so. At exactly two the pair is not a choice,
                         so it is stated, not offered, and the diff runs on
                         arrival. Past two the pair IS a choice, and only then
                         are there selects.

                         The per-side ref override is gone with them. It was
                         the one control that made this look like a form, it
                         served the version diff (one path, two refs), and the
                         Diff page already does that better: it takes an
                         owner/repo[@ref]:path per side with its own browser.
                         This lens is the quick look over what is staged; that
                         page is where an arbitrary pair belongs, and "Open in
                         Diff" hands this pair over. -->
                    <p x-show="items.length < 2" class="text-base text-base-content/50 py-2">
                      Stage two files to compare them.
                    </p>
                      ...
                      <div class="flex flex-col gap-1">
                        <template x-for="(side, i) in ['A', 'B']" :key="side">
                          <div class="flex items-center gap-1.5 min-w-0">
                            <span class="badge badge-sm badge-ghost font-bold w-6 justify-center shrink-0" x-text="side"></span>
                            <span class="font-mono text-base truncate"
                                  x-text="bundleHeader(items[i === 0 ? diffA : diffB]).replace('// === ', '')"></span>
                          </div>
                        </template>
                      </div>
                    </template>
                    ...
                    <div class="flex items-center gap-1.5" x-show="items.length >= 2">
          ...
          <div x-show="preview" x-cloak
            ...
            <div class="bg-base-100 border-base-300 shadow-xl w-full h-full flex flex-col overflow-hidden sm:h-auto sm:max-w-3xl sm:max-h-[85vh] sm:border sm:rounded-xl">
              <div class="px-3 py-2 bg-base-200/60 flex items-center gap-1 text-base border-b border-base-300 shrink-0">
                <i class="ph ph-eye opacity-60 shrink-0"></i>
                <span class="font-medium opacity-70 shrink-0">Preview</span>
              ...
              <div class="flex flex-col flex-1 min-h-0 overflow-hidden"
                ...
                <div x-show="!preview?.note" class="flex flex-col flex-1 min-h-0 overflow-hidden p-3"
                     id="stage-preview-viewer" x-data="viewer({ bindStore: false, fill: true })"></div>
                <div x-show="preview?.note" class="flex flex-col items-center justify-center flex-1 gap-2 p-8 text-center">
      ...
      init() {
        ...
        autoPair();
        // Stage two things, open the lens, see the diff. Waiting behind a
        // button was defensible while the pair was something you assembled
        // from selects and ref boxes; with the pair simply being what is
        // staged, the button was one tap between a question and its answer.
        // Cheap, because diffSide reads the bundle's content cache.
        const autoRun = () => {
          if (this.outTab !== 'diff' || this.diffRows || this.diffBusy) return;
          if (this.items.length < 2) return;
          this.runDiff();
        };
        this.$watch(() => this.outTab, autoRun);
        this.$watch(() => this.items.length, () => this.$nextTick(autoRun));
      ...
      async previewAt(i) {
        ...
        if (it.local) {
          ...
          else this.preview = { i, name, note: 'Binary (' + this.fmtSize(it.size) + '). Staged for copy, not preview.' };
          return;
        }
        try {
          ...
          this.preview = { i, name, note: 'Could not load it: ' + (e.message || e) };
        }
      },
