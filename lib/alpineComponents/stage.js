...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div x-show="dictOpen" x-cloak
                  ...
                  <div x-show="!dictEdit"
                       class="min-h-[3.5rem] max-h-40 overflow-y-auto text-base leading-relaxed px-1">
                  ...
                  <!-- The mark row belongs to dictation: the keyboard has its
                       own punctuation and its own backspace, so both go away
                       in edit mode rather than sitting there doing nothing. -->
                  <div class="join" x-show="!dictEdit">
                    <!-- Period first, same order as the annotator's row: the
                         most-tapped mark, and the one reached for at the end of
                         a sentence when the reader is looking away. -->
                    <template x-for="m in ['.', ',', ';', '?', '!']" :key="m">
                      <button @click="dictMark(m)" class="btn btn-sm join-item font-mono" x-text="m"></button>
                    </template>
                    <button @click="dictMark('¶')" class="btn btn-sm join-item" title="New paragraph">
                      <i class="ph ph-paragraph"></i>
                    </button>
                    <button @click="dictBack()" class="btn btn-sm join-item" title="Delete the last word">
      ...
      dictMark(m) { this._dict && this._dict.punct(m); },
