...
window.StageIntake = (() => {
  ...
  // WHERE A NEW ITEM LANDS, and why that became a parameter. Appending is right
  // for every intake that is BUILDING a set: a drop, a paste onto the bench, a
  // link seed, a manifest's stage.files. It is wrong for the one intake that
  // arrives to be READ AGAINST something already staged, because the preview's
  // comparison is positional (previewPair: where you are and what is next to
  // it), so a paste landing at the end of a five-item stage cannot be compared
  // with the file that prompted it.
  //
  // `at` is what lets that paste take the position the existing pair rule
  // already reads. The alternative was a pinned pair overriding position, which
  // would be the first thing on the stage to select a pair without BEING a
  // position, and the preview's whole claim is that the position is the
  // selection. One store write either way, so a deck open over the stage
  // rebuilds once rather than twice.
  const put = (fresh, at) => {
    if (!fresh.length) return fresh;
    const cur = staged();
    const i = at == null ? cur.length : Math.max(0, Math.min(cur.length, at));
    Alpine.store('browser').stage = [...cur.slice(0, i), ...fresh, ...cur.slice(i)];
    return fresh;
  };
  ...
  const take = (d, at) => {
    ...
    if (d.file || d.bytes || d.buf) {
      ...
      return put([text === null
        ...
        : { ...textItem(d.name, text), type: d.type || 'text/plain', size: d.size, bytes, buf: d.buf }], at);
    }
    if (d.text != null) {
      ...
      if (lines.length && refs.length === lines.length) {
        ...
        return put(refs.filter(r => !seen.has(window.StageLink.fmtItem(r))), at);
      }
      return put([textItem(d.name || nameForText(d.text), String(d.text))], at);
    }
    return [];
  };
  ...
  const takeFlavor = async (fl, at) => {
    if (!fl) return [];
    const name = fl.name || nameForFlavor(fl);
    if (fl.kind === 'file') return takeFile(fl.file, name, at);
    return take({ text: fl.text, size: fl.size,
                  name: fl.type === 'text/plain' ? '' : name }, at);
  };
  ...
  const takeClipboard = async (opts = {}) => {
    ...
    const primary = flavors.find(f => f.kind === 'file') ||
                    flavors.find(f => f.type === 'text/plain') || flavors[0];
    // `at` rides through for the one caller that is not building a set: the
    // reader's compare-with-the-clipboard, which needs the paste in the
    // position the preview's pair rule already reads. See put().
    const added = await takeFlavor({ ...primary, name: nameForFlavor(primary) }, opts.at);
    const offers = offerable(flavors.filter(f => f !== primary));
    if (opts.offer !== false) Alpine.store('browser').stageOffers = offers;
    return { added, offers };
  };
  ...
  return { take, takeFile, takeDrop, takePaste, takeClipboard, takeFlavor, offerable, focus,
...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
      ...
      // One resolved compare per BUILT slide, keyed by slide index. The deck
      // mounts three slides at once and each one's pair is its own, so this is
      // what keeps them from racing over the fields above. Emptied with the
      // deck; a slide the deck releases drops its entry with it.
      _pDiffAt: {},
      ...
      focusFromStore() {
        ...
        if (!it) return;
        // A BUNDLE OPENS IN THE TOOL, not in the preview. Every other arrival
      ...
      async previewAt(i, mode) {
        ...
        const opts = {
          ...
          // A released slide lets go of its compare with its DOM: the map is
          // per built slide, and a stale entry would republish a pair whose
          // rows are no longer on screen when the reader steps back onto it.
          release: (n, el) => { el.replaceChildren(); delete this._pDiffAt[n]; },
          onClose: () => { this._pDeck = null; this._pSigOpen = null; this._pNotes = {}; this._pDiffAt = {}; this.preview = null; },
        };
      ...
      _pDrop() {
        if (!this._pDeck) return;
        this._pDeck.drop();
        this._pDeck = null; this._pSigOpen = null; this._pNotes = {}; this._pDiffAt = {};
      },
      ...
      _pSyncChrome(n) {
        ...
        // leave the copy and the handoff aimed at the slide before it. This is
        // what makes "stepping with the diff open re-pairs" true of the
        // controls and not only of the rows.
        if (diff) this._pPublishDiff(n);
      ...
      _pActions(n) {
        ...
        // previewPairLabel already assembles exactly that for the header. In
        // diff mode the same button is the way back, so it names the file it
        // returns to rather than describing a toggle.
        if (this.items.length > 1) out.push({
          icon: diff ? 'ph-file-text' : 'ph-git-diff',
          title: diff ? 'Back to ' + this._pName(n)
                      : 'Compare ' + this.previewPairLabel(n),
          onClick: () => this.togglePreviewDiff(),
        });
        // THE OTHER SIDE IS OFTEN NOT ON THE STAGE AT ALL. It is on the
        // clipboard: an older copy of this document, a chunk someone sent you,
        // the version you are pasting from somewhere else. Unconditional,
        // because the case the positional compare above cannot serve is a stage
        // of ONE, which is exactly where a single pasted arrival lands you.
        out.push({
          icon: 'ph-clipboard-text',
          title: 'Compare with the clipboard: stage it next and diff against it',
          onClick: () => this.compareWithPaste(),
        });
      ...
      //
      // The gesture a reader actually has in hand: you are looking at a file
      // and the thing to check it against is on the clipboard rather than on
      // the stage. Nothing below is new machinery. The read and the fold are
      // the intake's (takeClipboard, the same call behind the bench's Paste
      // button and the app header's), and the comparison is the preview's own.
      // The whole of this is putting the paste where the pair rule reads, and
      // turning the mode on.
      //
      // WHY IT LANDS NEXT rather than at the end: see put()'s note. The stage's
      // comparison is a position, so making the paste occupy the next position
      // is how "compare this with that" is said inside the design that is
      // already here, instead of adding a second way to select a pair.
      //
      // NOTHING IS AWAITED BEFORE takeClipboard. Reading the clipboard spends
      // the tap's own user activation, and a lazy load in front of it spends
      // the gesture and then reports the loss as a clipboard failure; io.js is
      // preloaded at boot for that reason, and the intake throws rather than
      // fetching. Same rule the app header's Paste button follows.
      //
      // The two failures are told apart the way pasteIn tells them apart: an
      // empty read is the ordinary outcome of tapping before copying anything
      // and is reported as information, while a read that throws keeps the
      // error colour.
      async compareWithPaste() {
        const toast = Alpine.store('toast');
        const i = this.preview ? this.preview.i : 0;
        try {
          const { added } = await window.StageIntake.takeClipboard({ at: i + 1 });
          if (!added.length) return toast('clipboard-text', 'Nothing came off the clipboard', 'alert-info', 2500);
          await this.previewAt(i, 'diff');
        } catch (e) {
          toast('warning', 'Paste: ' + (e?.message || e), 'alert-error', 5000);
        }
      },
      ...
      async _pRenderDiff(i, slide) {
        ...
        const [a, b] = pr;
        // THIS SLIDE'S COMPARE, held on this slide. See _pDiff for what the
        // shared fields did to three concurrent builders.
        let d = null;
        try {
          d = await this._pDiff(a, b);
        } catch (e) {
      ...
      //
      // Two items hid all three (previewPair clamps, so both slides pair 0,1),
      ...
      async _pDiff(a, b) {
        ...
        const rows = this.diffLines(textA, textB);
      ...
      // the deck goes through _pDiff and publishes per slide instead.
      async runDiff() {
        if (this.diffBusy) return;
        this.diffBusy = true;
        try {
          const d = await this._pDiff(this.diffA, this.diffB);
          this.diffRows = d.rows;
          this.diffStat = d.stat;
          this._diffTextA = d.textA;
          this._diffTextB = d.textB;
