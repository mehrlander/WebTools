...
window.StageIntake = (() => {
  ...
  // WHETHER A TEXT IS BASE64, strictly, because the cost of a false positive is
  // an action offered that produces garbage. Four tests, each earning its place:
  // the alphabet (with the url-safe pair accepted, since that is what half the
  // world emits), a length that is a multiple of 4 (which is what the padding is
  // FOR, and what rules out most prose), a floor of 16 characters (below that a
  // word like `deadbeef` qualifies on arithmetic alone), and an actual decode
  // that yields text. Whitespace is stripped first, since a wrapped payload is
  // still one payload.
  //
  // A `data:...;base64,` URI is deliberately NOT recognized. Its payload is
  // almost always an image, so decoding it means staging BYTES under a sniffed
  // media type rather than text, which is a different intake; recognizing the
  // prefix and then handing back mojibake would be worse than not offering.
  const B64_CHARS = /^[A-Za-z0-9+/\-_]+={0,2}$/;
  const b64Body = (text) => String(text || '').replace(/\s+/g, '');
  const looksBase64 = (text) => {
    const s = b64Body(text);
    if (s.length < 16 || s.length % 4 !== 0 || !B64_CHARS.test(s)) return false;
    return fromBase64(s) !== null;
  };

  // The decode, or null when the bytes are not text. Null is the answer
  // `looksBase64` reads, so the test and the conversion are one implementation
  // and cannot disagree about what "valid" means. A NUL byte is the tell that
  // the payload was a file rather than a document, which this route cannot take.
  const fromBase64 = (text) => {
    try {
      const bin = atob(b64Body(text).replace(/-/g, '+').replace(/_/g, '/'));
  ...
  return { take, takeFile, takeDrop, takePaste, takeClipboard, takeFlavors, takeFlavor, offerable, focus,
           ...
           b64OfText, b64OfBytes, looksBase64, fromBase64 };
})();
    ...
    return {
      ...
      flavorActions(fl) {
        ...
        if (fl.kind === 'file') {
          ...
          out.push({ id: 'base64', icon: 'ph-brackets-curly', label: 'To base64' });
          return out;
        }
        out.push({ id: 'copy', icon: 'ph-copy', label: 'Copy', note: this.fmtSize(fl.size) });
        if (IN.isMarkup(fl.name)) out.push({ id: 'markdown', icon: 'ph-text-align-left', label: 'To markdown' });
        out.push({ id: 'base64', icon: 'ph-brackets-curly', label: 'To base64' });
        // The inverse, offered only where it will work: looksBase64 runs the
        // decode it is deciding about, so an item that appears always produces
        // something.
        if (IN.looksBase64(fl.text)) {
          out.push({ id: 'unbase64', icon: 'ph-brackets-angle', label: 'From base64' });
        }
        return out;
      },
      ...
      async runAction(fl, a) {
        ...
        try {
          ...
          if (a.id === 'base64') {
            ...
            return this.landDerived(IN.derivedName(fl.name, 'base64', 'txt'), b64);
          }
          if (a.id === 'unbase64') {
            const text = IN.fromBase64(fl.text);
            if (text == null) throw new Error('that is not base64 after all');
            return this.landDerived(IN.derivedName(fl.name, 'decoded', IN.extForText(text)), text);
          }
        } catch (e) {
      ...
      // Stage a made file and open the reader on it: the conversion is the thing
      // that was asked for rather than a row to go and find.
      landDerived(name, text) {
        const added = window.StageIntake.take({ text, name });
      ...
      async stageMd(fl) {
        ...
        this.landDerived(window.StageIntake.derivedName(fl.name, 'markdown'), md);
      },
      ...
      _rActions(n) {
        ...
        if (it && it.local && it.isText && IN.isMarkup(it.name)) out.push({
          ...
          onClick: () => this.stageMd(it).catch(e =>
