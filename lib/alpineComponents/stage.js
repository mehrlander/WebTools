...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
                ...
                <div x-show="offers.length" x-cloak
                  ...
                  <div class="flex flex-wrap gap-1">
                    <template x-for="fl in offers" :key="fl.name">
                      <div class="badge badge-lg h-auto gap-0 p-0 overflow-hidden"
                           :class="flavorStaged(fl) ? 'badge-primary' : 'badge-outline'">
                        <button @click="toggleFlavor(fl)"
                                :title="(flavorStaged(fl) ? 'Remove ' : 'Stage ') + fl.name"
                        ...
                        <button @click="peek('flavor', fl)" :title="'Look inside ' + fl.name"
                                class="self-stretch flex items-center px-1.5 border-l cursor-pointer opacity-50 hover:opacity-100"
                                :class="flavorStaged(fl) ? 'border-primary-content/30' : 'border-current/30'">
                          <i class="ph text-sm" :class="peeking('flavor', fl) ? 'ph-caret-up' : 'ph-eye'"></i>
                        </button>
                      </div>
                    </template>
                    ...
                    <template x-for="t in pasteLinks" :key="t.key">
                      <div class="badge badge-lg h-auto gap-0 p-0 overflow-hidden badge-outline border-dashed">
                        <button @click="extractLinks(t)" :title="'Read the ' + t.n + ' links in the ' + t.from + ' out into ' + t.dest"
                                class="flex items-center gap-1.5 px-2.5 py-1 cursor-pointer">
                          <i class="ph ph-link-simple text-sm"></i>
                          <span x-text="t.n + ' links'"></span>
                          <span class="opacity-50" x-text="'in ' + t.from"></span>
                        </button>
                        <button @click="peek('links', t)" title="Look at the links first"
                                class="self-stretch flex items-center px-1.5 border-l border-dashed border-current/30 cursor-pointer opacity-50 hover:opacity-100">
                          <i class="ph text-sm" :class="peeking('links', t) ? 'ph-caret-up' : 'ph-eye'"></i>
                        </button>
                      </div>
                    </template>
                    ...
                    <!-- THE WHOLE FLAVOR, CONVERTED. The links pill is a
                         ...
                         them. No count on the label, since knowing it would mean
                         running the conversion on every paste to fill in a pill
                         nobody may tap. -->
                    <template x-for="t in pasteMarkdown" :key="t.key">
                      <div class="badge badge-lg h-auto gap-0 p-0 overflow-hidden badge-outline border-dashed">
                        <button @click="stageMd(t)" :title="'Convert the ' + t.from + ' to markdown as ' + t.dest"
                                class="flex items-center gap-1.5 px-2.5 py-1 cursor-pointer">
                        ...
                        <button @click="peekMd(t)" title="Look at the markdown first"
                                class="self-stretch flex items-center px-1.5 border-l border-dashed border-current/30 cursor-pointer opacity-50 hover:opacity-100">
                          <i class="ph text-sm" :class="peeking('md', t) ? 'ph-caret-up' : 'ph-eye'"></i>
                        </button>
                      </div>
                    </template>
                  </div>

                  <!-- ONE PANEL, not one per pill. Two open previews is a
                       comparison nobody asked this bar for, and the panel is
                       tall enough that two would push the staged list off a
                       phone screen. -->
                  <div x-show="peeked" x-cloak class="rounded border border-base-300 bg-base-100 overflow-hidden">
                    <div class="px-2 py-1 text-base bg-base-200/60 flex items-center gap-1.5">
                      <span class="font-mono truncate" x-text="peeked?.name"></span>
                      <span class="opacity-50 shrink-0" x-text="peeked?.note"></span>
                    </div>
                    <template x-if="peeked?.img">
                      <img :src="peeked.img" alt="" class="max-h-40 w-auto p-1">
                    </template>
                    <template x-if="peeked && !peeked.img">
                      <pre class="m-0 px-2 py-1.5 text-sm leading-5 font-mono max-h-40 overflow-auto whitespace-pre-wrap break-words" x-text="peeked.text"></pre>
                    </template>
                  </div>
                </div>
      ...
      async peekMd(t) {
        if (this.peeking('md', t)) return this.peek('md', t);
        try { await this.mdFor(t.flavor); } catch (e) { return this.mdFailed(e); }
        this.peek('md', t);
      },
      async stageMd(t) {
        let md;
      ...
      // A NEW BAR CLOSES THE OLD PANEL, and revokes the blob URLs it minted:
      // the preview is a reading of the paste on screen, and a key left standing
      // would point at a flavor from a paste that is gone.
      set offers(v) { this.closePeek(); Alpine.store('browser').stageOffers = v || []; },
      offer(flavors) {
      ...
      //
      // The pills answered "which formats did this copy hold" and could not
      // answer "which one do I want", because the only thing distinguishing
      // `html 4.1 KB` from `txt 192 B` was a title attribute, and a title is
      // nothing at all on a phone. So each pill has an eye, and the panel it
      // opens shows the bytes: an image DRAWN, text as text.
      //
      // The key is a kind plus a name rather than the object, since the offers
      // array is rebuilt by every read of the store and an identity comparison
      // would close the panel underneath the reader.
      peekKey: '',
      _peekUrls: null,
      peeking(kind, o) { return this.peekKey === kind + ':' + (o?.name || o?.key); },
      peek(kind, o) {
        const k = kind + ':' + (o?.name || o?.key);
        this.peekKey = this.peekKey === k ? '' : k;
      },
      closePeek() {
        this.peekKey = '';
        for (const u of (this._peekUrls?.values() || [])) URL.revokeObjectURL(u);
        this._peekUrls = null;
        this._mdText = null;
      },

      // How much of a text flavor the panel shows. Enough to tell an html
      // document from its own plain-text rendering, which is the comparison
      // this exists for, and short enough that the staged list stays on screen.
      PEEK_CHARS: 1200,

      // A BLOB URL RATHER THAN A DATA URI, because a pasted screenshot is
      // megabytes and base64 in an attribute is the slow way to say the same
      // thing. Cached per name so re-opening the same pill does not mint a
      // second URL, and revoked together when the bar goes.
      get peeked() {
        const [kind, ...rest] = String(this.peekKey).split(':');
        const name = rest.join(':');
        if (!name) return null;
        if (kind === 'links') {
          const t = this.pasteLinks.find(x => x.key === name);
          return t && { name: t.dest, note: t.n + ' links', text: window.StageIntake.linksMd(t.links) };
        }
        // The conversion is already done: peekMd awaits it before naming the
        // key, so this getter is never the thing that waits.
        if (kind === 'md') {
          const t = this.pasteMarkdown.find(x => x.key === name);
          const md = t && this._mdText?.get(name);
          return md ? { name: t.dest, note: this.fmtSize(md.length), text: md } : null;
        }
        const fl = this.offers.find(o => o.name === name);
        if (!fl) return null;
        if (fl.kind === 'file') {
          if (!/^image\//.test(fl.type || '')) return { name: fl.name, note: this.fmtSize(fl.size), text: '(not text)' };
          this._peekUrls ??= new Map();
          if (!this._peekUrls.has(fl.name)) this._peekUrls.set(fl.name, URL.createObjectURL(fl.file));
          return { name: fl.name, note: this.fmtSize(fl.size), img: this._peekUrls.get(fl.name) };
        }
        const text = String(fl.text || '');
        return { name: fl.name, note: this.fmtSize(fl.size),
                 text: text.slice(0, this.PEEK_CHARS) + (text.length > this.PEEK_CHARS ? '\n…' : '') };
      },
