...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
      ...
      // Choosing re-pairs every slide, so it rebuilds for the reason a mode
      // change does: render() reads the pair, and nothing else should have to
      // know both shapes. '' returns to the neighbour default.
      compareWith(key) {
        this.compareKey = key || '';
      ...
      // is. Two things ask for this and they are the same act: changing the MODE
      // changes what a slide is, and changing the PICK changes what every slide
      // compares. Both are properties of the deck rather than of one slide, so
      // neither can be a swap inside the slide the reader happens to be on.
      _pRebuild() {
        if (!this._pDeck) return;
        this._pNotes = {};
        this._pDiffAt = {};
