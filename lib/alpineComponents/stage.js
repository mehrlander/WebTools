    ...
    return {
          ...
          <div class="grid gap-4 items-start lg:grid-cols-[minmax(0,1fr)_20rem]">
              ...
              <div class="flex flex-col gap-2">
                ...
                <div class="flex items-center gap-1.5 flex-wrap">
                  ...
                  <div class="grow min-w-48" @path-pick="destSpec = $event.detail.spec">
                    <div x-data="pathPicker({ mode: 'dir', roots: () => pickerRoots(), value: destSpec, placeholder: 'Send to: pick a repo folder' })"></div>
                  </div>
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div x-show="dictOpen" x-cloak data-no-swipe
                     class="border border-base-300 rounded-lg p-2 flex flex-col gap-2">
                  ...
                  <div x-show="!dictEdit" class="flex gap-1.5 items-stretch">
                    ...
                    <!-- py-3, roomier than the text needs: the handles sit
                         above and below the line they mark and would clip
                         against a scrolling box on the first or last line. -->
                    <div class="flex-1 min-w-0 min-h-[3.5rem] max-h-40 overflow-y-auto text-base leading-relaxed px-1 py-3">
                  ...
                  <textarea x-show="dictEdit" x-cloak x-ref="dictTa" x-model="dictDraft"
                            rows="3" class="textarea textarea-bordered w-full text-base leading-relaxed"
                            placeholder="Type the note"></textarea>
                  <div class="flex items-center gap-2">
                    <button x-show="!dictEdit" @click="dictToggle()" class="btn btn-square h-11 min-h-0 w-11"
                            :class="dictOn ? 'btn-error' : 'btn-ghost'"
                            :title="dictOn ? 'Recording. Tap to stop.' : 'Resume'">
                      <i class="ph ph-microphone text-2xl"></i>
                    </button>

                    <!-- The pencil is a MODE SWITCH, not a one-way door, which
                         is the annotator's rule and the reason the glyph flips
                         to a microphone: dictation is the default and typing is
                         the other half of one toggle. -->
                    <!-- In edit mode the button grows the word DONE. The glyph
                         flip alone was not enough: a microphone reads as "start
                         dictating", not as "finish typing and go back", so the
                         way out of the keyboard looked like a way deeper in and
                         readers reported finding no confirm at all. -->
                    <button @click="dictEdit ? dictEditClose() : dictEditOpen()"
                            class="btn h-11 min-h-0 gap-1.5"
                            :class="dictEdit ? 'btn-warning px-3' : 'btn-ghost btn-square w-11'"
                            :title="dictEdit ? 'Done: back to dictation' : 'Type instead (opens the keyboard)'">
                      <i class="ph text-2xl" :class="dictEdit ? 'ph-microphone' : 'ph-pencil-simple'"></i>
                    ...
                    <div class="grow"></div>
                    <!-- With a pin armed, the row's business is the pin.
                         Stage steps aside for two arrows that walk the armed
                         edge one character at a time, because aiming at a
                         character boundary is the one thing a finger cannot
                         do: the tap gets it close and these settle it. Stage
                         is GONE rather than disabled, since the two answer
                         different questions and a Stage button sitting there
                         mid-adjustment invites the tap that ends the note. -->
                    <template x-if="dictArmed && dictSel">
                      <div class="flex gap-1.5">
                        <button @pointerdown.prevent="dictNudge(-1)"
                                class="btn btn-outline btn-error h-11 min-h-0 w-11 btn-square"
                                title="Move the armed pin left">
                          <i class="ph ph-caret-left text-2xl"></i>
                        </button>
                        <button @pointerdown.prevent="dictNudge(1)"
                                class="btn btn-outline btn-error h-11 min-h-0 w-11 btn-square"
                                title="Move the armed pin right">
                          <i class="ph ph-caret-right text-2xl"></i>
                        </button>
                      </div>
                    </template>
                    <button x-show="!(dictArmed && dictSel)" @click="dictStage()"
                ...
                <div x-show="!items.length && !dictOpen"
      ...
      async pasteIn() {
        ...
        try {
          ...
          if (!window.io?.paste) throw new Error('io kit unavailable');
          const text = await window.io.paste();
          if (text == null || text === '') return toast('warning', 'Clipboard is empty or unreadable', 'alert-error', 4000);
      ...
      dictBind(host) {
        this._dictHost = host;
        let timer = null, x = 0, y = 0, long = false, lastTap = 0;
        ...
        host.addEventListener('pointerdown', (e) => {
          ...
          if (edge) {
            e.preventDefault();
            this.dictArmed = this.dictArmed === edge ? null : edge;   // tap again to disarm
            long = true; this.dictPaint();
            return;
          }
          clear();
          timer = setTimeout(() => {
        ...
        host.addEventListener('pointerup', (e) => {
          ...
          if (i == null) return;
          // A DOUBLE TAP takes the word, the same as a long press: the faster
          // of the two when the target is already in view, and offering both
          // costs nothing (one is quick, the other is sure).
          const t = Date.now(), dbl = (t - lastTap) < 300;
          lastTap = t;
          if (dbl) { this._dict.selectWordAt(i); this.dictArmed = null; this.dictPaint(); return; }
      ...
      dictEditClose() {
        ...
        this.dictEdit = false;
        // Returning to dictation resumes listening: the mode IS the intent, so
        // making the reader tap the mic again to get back where they were is a
        // tax on the round trip. The annotator's closeEditor does the same.
        if (this._dict.available()) this._dict.start();
      },
      dictMark(m) {
        if (!this._dict) return;
