  ...
  Alpine.data('stager', function() {
    ...
    const nameForText = (text) => {
      ...
      const ext = /^(<!doctype html|<html)/i.test(s) ? 'html'
        ...
        : /^[{[]/.test(s) ? 'json'
        : isDelimited(text) ? 'tsv'
        : /^#{1,6}\s|^---\s*$/m.test(s) ? 'md'
        : 'txt';
      return stamp + '-paste.' + ext;
    };

    // Tab-separated, which is what a spreadsheet range puts on the clipboard as
    // text/plain. Worth detecting because the viewer picks its mode off the
    // extension: called .txt a pasted range is a wall of text, called .tsv it
    // opens as a TABLE. The test is deliberately strict, since a stray tab in
    // prose must not rename the file: at least two lines, every non-empty line
    // carrying a tab, and every one of them carrying the SAME number, which is
    // what makes it a grid rather than text that happens to contain tabs.
    const isDelimited = (text) => {
      const lines = String(text || '').split(/\r?\n/).filter(l => l.trim());
      if (lines.length < 2 || lines.length > 5000) return false;
      const n = (lines[0].match(/\t/g) || []).length;
      return n > 0 && lines.every(l => (l.match(/\t/g) || []).length === n);
    };
    ...
    return {
      ...
      template: `
        <div class="relative min-h-[55vh]"
          ...
          <div x-show="preview" x-cloak
            ...
            <!-- A FIXED HEIGHT AT EVERY WIDTH, and the two symptoms it fixes
                 were one bug. It was "sm:h-auto sm:max-h-[85vh]" on desktop,
                 so the box sized itself to whatever was in it: the dialog
                 jumped between a two-line paste and a long file, and stepping
                 through the staged set resized the window under the reader's
                 cursor, which is the one place a modal must hold still.
                 Worse, an auto-height box gives its children no definite
                 height to divide, so the viewer's fill body ("flex-1 min-h-0",
                 which grows to the HOST's height) had no host height to grow
                 to: its overflow-auto never became a scroll container, the
                 content pushed the box to the 85vh cap, and the box's own
                 overflow-hidden then clipped the rest with no scrollbar
                 anywhere. So "the height moves" and "it will not scroll" were
                 the same class, and pinning the height fixes both: every
                 descendant now resolves against a known height and scrolls
                 inside it. The phone case was always right ("h-full", a
                 full-screen sheet) and is unchanged.

                 NO BACKTICKS IN HERE. This comment sits inside the component's
                 template, which is a JS template literal, so a backtick ends
                 the string and the rest of the markup parses as code. It cost
                 a boot: the node suite has no parser in the loop for the
                 built file, and the first thing that said so was a browser
                 reporting "Unexpected identifier 'sm'" from inside gh.load. -->

            <div class="bg-base-100 border-base-300 shadow-xl w-full h-full flex flex-col overflow-hidden sm:h-[85vh] sm:max-w-3xl sm:border sm:rounded-xl">
              ...
              <div class="flex flex-col flex-1 min-h-0 overflow-hidden"
                ...
                <div x-show="preview?.mode !== 'diff' && !preview?.note" class="flex flex-col flex-1 min-h-0 overflow-hidden p-3"
                     id="stage-preview-viewer"
                     x-data="viewer({ bindStore: false, fill: true, defaultMode: (f) => window.ViewRegistry.READ_MODE(f) })"></div>
      ...
      init() {
        ...
        try {
          ...
          if (lk.gz) {
            window.StageLink.decodeLocals(lk.gz).then(locals => {
              ...
              const fresh = locals.filter(l => !have.has(l.name)).map(l => ({
                local: true, id: ++seq, name: l.name, path: l.name,
                size: l.text.length, type: 'text/plain', isText: true, text: l.text,
              }));
        ...
        autoDiff();
        // Paste anywhere on the view stages a file or a block of refs. Bound
        // to the window so it works over the whole view, gated to the stage.
        // A form field keeps its own paste; see below, where the flavors are
        // known and there is something better to do than return.
        this._onPaste = (e) => {
          ...
          // A FORM FIELD KEEPS ITS PASTE, and the rest is offered rather than
          // taken. The field can only hold text, so a spreadsheet pasted into
          // the destination box used to mean the image and the HTML table were
          // simply gone: not because the clipboard withheld them, but because
          // this handler returned before reading them. Nothing is
          // preventDefault'ed here, so the field pastes natively; what it
          // cannot hold is put on the offer bar, one tap from the stage.
          const t = e.target;
          if (t && t.tagName && /^(INPUT|TEXTAREA|SELECT)$/.test(t.tagName)) {
            this.offer(flavors.filter(f => f.kind === 'file' || f.type !== 'text/plain'));
            return;
          }
          e.preventDefault();
          // The primary is what this handler already took before it could see
          // the others (a file if there was one, else the plain text), so a
          // paste that used to work is unchanged and the rest is addition.
          const primary = flavors.find(f => f.kind === 'file') ||
                          flavors.find(f => f.type === 'text/plain') || flavors[0];
          this.stageFlavor(primary);
      ...
      // `as` overrides the File's own name, which a clipboard image needs: the
      // platform invents "image.png" for it, carrying no more meaning than a
      // stamp and colliding with the next one pasted.
      async ingestFile(file, as) {
        try {
          const buf = await file.arrayBuffer();
          this.onDropped({ file, name: as || file.name, size: file.size, type: file.type, bytes: new Uint8Array(buf), buf });
        } catch {
      ...
      // A file (or pasted text) arrived from the drop-zone. A file whose bytes
      // read as text becomes a local TEXT item (textFromBytes decides), so a
      // dropped .md previews rendered, diffs, joins the bundle, and rides a
      // link exactly as the same characters pasted would; anything else holds
      // its bytes. Pasted text that reads entirely as stage refs (one per
      // line) stages those refs instead; anything else is held as a local text
      // item.
      onDropped(d) {
        const s = Alpine.store('browser');
        ...
        // drop-zone emits `file` for files and `name: ''` for text, so nothing
        // it sends changes branch.
        if (d.file || d.bytes || d.buf) {
          const bytes = d.bytes || (d.buf ? new Uint8Array(d.buf) : null);
          const text = textFromBytes(d.name, bytes);
          // The bytes ride along on a text item too: nothing downstream reads
          // them once `isText` is true, and keeping them means a rename into a
          // renderable extension is still answerable from the original file.
          s.stage = [...this.items, text === null
            ? { local: true, id: ++seq, name: d.name, path: d.name,
                size: d.size, type: d.type, isText: false, bytes, buf: d.buf }
            : { local: true, id: ++seq, name: d.name, path: d.name,
                size: d.size, type: d.type || 'text/plain', isText: true, text, bytes, buf: d.buf }];
          return;
        }
        if (d.text != null) {
          ...
          const name = d.name || nameForText(d.text);
          s.stage = [...this.items, {
      ...
      offer(flavors) {
        ...
        // cumulative.
        const have = new Set(this.localItems.map(it => it.path || it.name));
        this.offers = (flavors || [])
          .map(fl => ({ ...fl, name: nameForFlavor(fl) }))
      ...
      async stageFlavor(fl) {
        ...
        if (fl.kind === 'file') {
          await this.ingestFile(fl.file, name);
        } else {
          // Ref lines stay ref lines: a plain-text flavor still goes through
          // onDropped's parse, so pasting a block of addresses stages them as
          // refs the way it always did. A named flavor passes its name, which
          // is how text/html arrives as .html rather than being sniffed.
          this.onDropped({ text: fl.text, size: fl.size,
      ...
      async previewAt(i, mode) {
        ...
        if (it.local) {
          if (it.isText) { drive(fmt(it.text || ''), { local: true }); return; }
          ...
          if (src) { drive(src, { local: true }); return; }
          this.preview = { i, name, mode: 'file', note: 'Binary (' + this.fmtSize(it.size) + '). Staged for copy, not preview.' };
          return;
        }
        try {
