...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
      ...
      // The stitch borrows the full stop's cell, which costs nothing in the two
      // states that offer it: the caret is aimed away from the end, or a full
      // stop already sits where a second one would go and adds nothing.
      //
      // NO STEP-BACK KEY HERE. pages/dictate.html pairs the stitch with one
      // that walks the caret to an older break, which its across-the-page row
      // has room for and this three-cell column does not. So this pad repairs
      // the break at the caret, and reaching an older one means tapping the
      // text first.
