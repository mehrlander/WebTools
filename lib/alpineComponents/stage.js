...
window.StageIntake = (() => {
  ...
  // What the extraction is called: the source's own name with its extension
  // swapped for `-links.csv`, so the pair reads as a pair in the staged list.
  const linksName = (name) => {
    const s = String(name || 'paste');
    const dot = s.lastIndexOf('.');
    return (dot > 0 ? s.slice(0, dot) : s) + '-links.csv';
  };

  // RFC-4180 quoting, which is not optional here: link text is prose and prose
  // has commas in it, and an unquoted one turns a two-column table into a
  // ragged three.
  const csvCell = (v) => {
    const s = String(v ?? '');
    return /[",\n\r]/.test(s) ? '"' + s.replace(/"/g, '""') + '"' : s;
  };
  const linksCsv = (links) =>
    ['text,url', ...(links || []).map(l => csvCell(l.text) + ',' + csvCell(l.url))].join('\n');
  ...
  return { take, takeFile, takeDrop, takePaste, takeClipboard, takeFlavors, takeFlavor, offerable, focus,
           ...
           linksOf, linksIn, linksName, linksCsv };
})();
...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
                ...
                <div x-show="offers.length" x-cloak
                  ...
                  <div class="flex flex-wrap gap-1">
                    ...
                    <!-- THE DERIVATIONS, in the same row and never hidden. They
                         ...
                         for, and there is at most one of these. Dashed and
                         un-monospaced so it does not read as a fourth format,
                         since it is a new file made out of one rather than
                         another encoding of it. -->
      ...
      // ── WHAT IS INSIDE ────────────────────────────────────────────────────
      //
      // Reading the links out of a paste is a DERIVATION, not a third format and
      // not another tool, and it shipped on 2026-08-28 as a row of its own for
      // that reason. A row was too much furniture for it: two coloured bands
      // over one paste, the second one loud about a thing you usually do not
      // want. It rides the flavors bar's own menu now, which is where a reader
      // already looks to ask what this paste can give them.
      //
      // Scoped to the paste's flavors rather than the whole stage, since the bar
      // is about the paste. A staged file that arrived some other way keeps its
      // route through the reader's header.
      get pasteLinks() {
        ...
        for (const fl of this.offers) {
          ...
          if (links.length) out.push({ key: fl.name, dest: IN.linksName(fl.name), links,
                                       from: this.flavorLabel(fl), n: links.length });
        }
        return out;
      },

      // The extraction, opening the reader on what it made. A table of links is
      // the thing that was asked for rather than a row in a list to go and
      // find, so this is the one intake besides a single arrival that focuses.
      extractLinks(t) {
        if (!t) return;
        const IN = window.StageIntake;
        const added = IN.take({ text: IN.linksCsv(t.links), name: t.dest });
        this.closePeek();
      ...
      get peeked() {
        ...
        if (kind === 'links') {
          ...
          return t && { name: t.dest, note: t.n + ' rows', text: window.StageIntake.linksCsv(t.links) };
        }
      ...
      _rActions(n) {
        ...
        const inside = it ? window.StageIntake.linksIn(it) : [];
        if (inside.length) out.push({
          icon: 'ph-link-simple',
          title: 'Read the ' + inside.length + ' links in ' + it.name + ' out into a table',
          onClick: () => this.extractLinks({ dest: window.StageIntake.linksName(it.name), links: inside }),
        });
