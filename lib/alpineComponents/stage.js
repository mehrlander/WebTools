...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
        ...
        <div class="relative min-h-[55vh]"
          ...
          <div class="grid gap-4 items-start lg:grid-cols-[minmax(0,1fr)_20rem]">
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <!-- ── Add: ONE box ────────────────────────────────────────────
                   Browse, Recent, and Search were three ways to name a file
                   that shares one corpus (addRoots) and one outcome (a staged
                   ref). They differed only in what you already knew: where it
                   lives, that it changed lately, or part of its name. Three
                   controls for three guesses at your own memory is a choice
                   the reader has to make before the tool does anything, so
                   they are one field and one list now.

                   The list answers both questions at once. WHERE YOU ARE
                   (addScope: root, or inside a repo folder) and WHAT YOU TYPED
                   (addQ) pick the rows:

                     root  + empty  →  the repos, then recent files under them
                     root  + query  →  matching repos and recent files, plus an
                                       offer to search the rest
                     inside+ empty  →  that folder's contents
                     inside+ query  →  every path under it in that repo

                   Containers sort before leaves at equal score, which is
                   path-picker's rule, so descending never hides behind a file.

                   THE ONE COST THAT STAYS EXPLICIT is the deep search. A
                   filename-contains across every root repo needs a recursive
                   tree per repo, and firing that because a character was typed
                   spends four API calls on a keystroke. So typing at root
                   filters what is already in hand (repo names, recent files)
                   and the escalation is a row you tap. It also gets cheaper as
                   you browse: descending loads that repo's tree into the same
                   cache the deep search reads, so the offer names only the
                   repos still unread, and disappears when none are. -->
              <div class="flex flex-col gap-1.5">
                <div class="flex items-center gap-1 text-base font-semibold uppercase tracking-wide text-base-content/40">
                  Add
                  <div class="grow"></div>
                  <!-- The one source that is not a repo file, so it stays a
                       header action rather than a row: local bytes cannot be
                       named by ref, which is why they drop out of a #stage=
                       link and out of a saved surface. -->
                  ...
                  <button x-show="!addScope" @click="loadRecent(true)" class="btn btn-square btn-ghost btn-sm hover:text-primary" title="Refresh recent">
                ...
                <!-- The field. text-lg below sm is not a size preference: iOS
                     zooms the viewport on focus for anything under 16px, and
                     this is the one input on the bench. No autofocus, so a tap
                     down the list never summons the keyboard. -->
                <label class="input input-bordered flex items-center gap-2">
                  <i class="ph opacity-50" :class="addScope ? 'ph-folder-open' : 'ph-magnifying-glass'"></i>
                  <input x-model="addQ" type="text" :placeholder="addPlaceholder"
                         autocomplete="off" autocapitalize="off" spellcheck="false"
                         @keydown.escape.stop="addQ ? addQ = '' : addUp(-1)"
                ...
                <!-- Crumbs, only once descended: where you are, and every step
                     back up. The root crumb is an icon so the row stays short
                     on a phone at three levels deep. -->
                <div x-show="recentOpen && addScope" class="flex items-center gap-0.5 flex-wrap text-base">
                ...
                <p x-show="recentOpen && addHint" class="text-base text-base-content/45 px-2" x-text="addHint"></p>
                ...
                <div x-show="recentOpen" class="flex flex-col max-h-72 overflow-y-auto overscroll-contain rounded-lg">
                  <template x-for="row in addRows()" :key="row.key">
                    <div class="group flex items-center rounded-lg hover:bg-base-200 transition-colors">
                      <!-- One row, three jobs, and the icon is what says which:
                           a caret descends, a plus stages, a check unstages.
                           The primary action is the button; the GitHub jump
                           rides beside it, since an anchor inside a button is
                           not a control. -->
                  ...
                  <!-- The escalation. Named with a count, because "search the
                       rest" should say how much rest there is before it spends
                       a call per repo. -->
                  <button x-show="addOffer" @click="loadAllTrees()" :disabled="deepBusy"
                          class="flex items-center gap-2.5 px-2 py-2 rounded-lg text-left hover:bg-base-200 transition-colors border-t border-base-200 mt-1">
                    <i class="ph text-lg shrink-0 text-primary/70"
                       :class="deepBusy ? 'ph-circle-notch animate-spin' : 'ph-magnifying-glass'"></i>
                    <span class="text-base text-base-content/70" x-text="addOffer"></span>
                  </button>
      ...
      _recentLoaded: false,
      // ── The one Add box ────────────────────────────────────────────────
      // addQ is what you typed, addScope is where you are ('' repo = root).
      // Between them they pick every row; there is no mode flag, because a
      // mode flag is exactly the tab strip this replaced.
      addQ: '',
      addScope: null,      // null | {repo, ref, dir}
      ...
      // The repos with no tree read yet. The escalation names this count, and
      // when it empties the deep search is already paid for and just runs.
      addUnread() {
        return [...new Set(this.addRoots().map(r => r.repo))].filter(r => !this.trees[r]);
      },
      get addBusy() { return !!this.treeBusy || (this.recentLoading && !this.recent.length); },
      get addTruncated() {
      ...
      get addPlaceholder() {
        const sc = this.addScope;
        return sc ? 'Filter ' + this.shortRepo(sc.repo) + (sc.dir ? '/' + sc.dir : '') + '…'
                  : 'Search files, or pick a repo…';
      },
      ...
      // The escalation: read every root repo still unread, so a query can run
      // across all of them. This is the one expensive thing in the box, which
      // is why it is a row you tap rather than something a keystroke triggers.
      async loadAllTrees() {
      ...
      // Descend into a repo (dir '') or a folder. Entering clears the query,
      // since it was aimed at the level you just left; path-picker does the
      ...
      addUp(i) {
        if (i < 0) { this.addScope = null; this.addQ = ''; return; }
        const c = this.addCrumbs()[i];
        if (c) { this.addScope = { ...this.addScope, dir: c.dir }; this.addQ = ''; }
      },
      ...
      // THE LIST. Where you are and what you typed pick the rows; see the
      // template's header comment for the four cases in a table.
      addRows() {
        const q = this.addQuery;
        const sc = this.addScope;

        if (sc) {
          const t = this.trees[sc.repo];
          if (!t) return [];
          if (!q) return this.dirEntries(sc.repo, sc.ref, sc.dir);
          // Inside a repo the tree is already in hand, so a query searches the
          // whole subtree rather than only the level: free, and it is what
          // typing inside a folder plainly means.
          const base = sc.dir ? sc.dir + '/' : '';
          return t.paths
            .filter(p => p.startsWith(base) && p.toLowerCase().includes(q))
            .sort((a, b) => this.addRank(this.baseName(a), q) - this.addRank(this.baseName(b), q)
                          || a.localeCompare(b))
            .slice(0, 100)
            .map(p => this.fileRow(sc.repo, sc.ref, p, ''));
        }

        // Root. Repos and recent files rank together, so a strong file match
        // can outrank a weak repo match; a tie goes to the container.
        //
        // A REPO IS NEVER FILTERED OUT, only demoted. Dropping the unmatched
        // ones was the first cut, and it took the whole navigation route away
        // the moment a character was typed: type anything that is not a repo
        // name and the list went to "No matching files." with no repo left to
        // enter and no way on but clearing the field. The repos are the one
        // route that always works, since they need no query to be right, so an
        // unmatched one falls below every matched leaf (2.9 beats the worst
        // real score of 2) and stays reachable.
        const scored = [];
        for (const r of this.addRoots()) {
          const sr = this.addRank(this.shortRepo(r.repo), q);
          scored.push({ s: sr < 3 ? sr : 2.9, c: 0, row: {
            kind: 'repo', key: 'r:' + r.repo, repo: r.repo, ref: r.ref || '',
            label: this.shortRepo(r.repo), sub: r.repo, title: r.repo } });
        }
        const seen = new Set();
        for (const it of this.recent) {
          const s = Math.min(this.addRank(this.baseName(it.path), q),
                             q && it.path.toLowerCase().includes(q) ? 2 : 3);
          if (s > 2) continue;
          seen.add(it.repo + ':' + it.path);
          scored.push({ s, c: 1, row: this.fileRow(it.repo, it.ref, it.path, it.date) });
        }
        scored.sort((a, b) => a.s - b.s || a.c - b.c);
        const rows = scored.map(x => x.row);

        // Deep hits, only once every root repo has been read: no gate is owed
        // for work already done, so the offer disappears rather than lingering.
        if (q.length >= 2 && !this.addUnread().length) {
          for (const r of [...new Set(this.addRoots().map(x => x.repo))]) {
            for (const p of (this.trees[r]?.paths || [])) {
              if (!p.toLowerCase().includes(q) || seen.has(r + ':' + p)) continue;
              seen.add(r + ':' + p);
              rows.push(this.fileRow(r, '', p, ''));
      ...
      // The escalation row's label, or '' when there is nothing to offer:
      // no query, too short a one, or every repo already read.
      get addOffer() {
        if (this.addScope) return '';
        const q = this.addQuery;
        if (q.length < 2) return '';
        const n = this.addUnread().length;
        if (!n) return '';
        return this.deepBusy ? 'Reading ' + n + (n === 1 ? ' repo…' : ' repos…')
             : 'Search ' + n + (n === 1 ? ' more repo' : ' more repos') + ' for "' + q + '"';
      },
      get addEmpty() {
        if (this.addBusy || this.addRows().length) return '';
        if (this.addScope) return this.addQuery ? 'No matching files here.' : 'This folder is empty.';
        // At root the repos are always listed, so addRows is never empty there
        // and this is the descended case plus the cold, tokenless one.
        return this.recentLoading ? '' : 'Nothing recent. Pick a repo to browse.';
      },
      // Said above the list when a query matched no FILE, because the repos
      // below it are always there and would otherwise read as hits.
      get addHint() {
        if (this.addScope || !this.addQuery) return '';
        return this.addRows().some(r => r.kind === 'file') ? ''
             : 'No file by that name yet. Pick a repo to browse' + (this.addOffer ? ', or search the rest.' : '.');
      },
      addRowIcon(row) {
