...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
      ...
      // TWO OF THE KIT'S THREE AIMS. kits/dictate.js `stitchAim` says which
      // repair the key would make: a caret dropped in the gap ('caret'), a full
      // stop ending the buffer with nothing after it ('end'), or an earlier
      // break reached with no caret at all ('back'). Only the first two are
      // cheap to show HERE, because this pad has three cells and the stitch
      // borrows the full stop's: in those two states a full stop is either
      // aimed away from or inert, while 'back' is true of almost any buffer
      // with two sentences in it and would take the cell for good.
      //
      // pages/dictate.html gives the third a key of its own, which is what its
      // across-the-page row has room for and this three-cell column does not.
      // The tap calls the same widened `stitch()` either way.
      get dictStitch() {
        this.dictBeat;
        const aim = this._dict && this._dict.stitchAim;
