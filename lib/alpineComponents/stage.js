...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
      ...
      destSpec: '',
      // The preview is a POSITION in the stage: { i, name, note }. The deck
      // owns the walking, so this is all the component still needs to know:
      // which item the header is naming. `_pDeck` is the kit's handle, and its
      ...
      // down to be rebuilt around a changed set (_pDrop(true)) is not the
      ...
      init() {
        ...
        this.ensureBundle();
        // The pair is resolved from the preview's position and the reader's
        // pick (previewPair), so nothing here chooses one. This only CLAMPS: an
        ...
        // previewPair would already fall back from; forgetting the key as well
      ...
      previewPairLabel(at) {
        ...
        if (!pr) return '';
        return this.previewName(pr[0]) + ' ↔ ' + this.previewName(pr[1]);
      },
      ...
      async openCompare(at) {
        if (!this._pDeck || this.items.length < 2) return;
      ...
      async _openCompare(at) {
        ...
        const opts = {
          ...
          onClose: () => {
            ...
            // discard the walk.
            if (landed != null && this._pDeck) {
              this._pDeck.deck.go(landed);
              this.preview = { ...this.preview, i: landed, name: this._pName(landed),
                               note: this._pNotes[landed] || '' };
              this._pSyncChrome(landed);
            }
          },
        };
        if (!window.swipeDeck && window.gh?.load) await window.gh.load('kits/swipe-deck.js');
        if (!window.swipeDeck || !this._pDeck) return;
        this._cmpDiffs = {};
        this._cmpDeck = window.swipeDeck.drill(this._pDeck, opts);
      ...
      // itself. Same distinction, and the same reason, as _pDrop's.
      _cmpDrop() {
      ...
      _cmpSyncChrome(n) {
        ...
        this._pPublishDiff(n);
        h.setTitle(this.previewPairLabel(n));
        ...
        // `_pChan` gives: this deck owns that choice.
        this._pSay(n);
      },
      ...
      async previewAt(i, mode) {
        ...
        if (!this.items[i]) { this._cmpDrop(); this._pDrop(); return; }
        ...
        if (this._pDeck) {
          if (this._pSig() === this._pSigOpen) {
            this.preview = { ...this.preview, i, name: this._pName(i), note: this._pNotes[i] || '' };
        ...
        this.preview = { i, name: this._pName(i), note: this._pNotes[i] || '' };
        if (!window.swipeDeck && window.gh?.load) await window.gh.load('kits/swipe-deck.js');
        ...
        const opts = {
          ...
          // fresh read and gets the neighbour default back. `_pReplacing` is
          // the difference: a deck taken down to be rebuilt around a changed
          // set fires this too (see _pDrop), and throwing the pick away there
          ...
          onClose: () => {
            ...
            if (this._pReplacing) return;
            this._cmpDrop();
            if (this._pChan) { this._pChan.release(); this._pChan = null; }
            this.compareKey = ''; this.compareOpen = false; this.preview = null;
          },
        };
        ...
        if (!this._pChan && window.subjectChannel) {
          this._pChan = window.subjectChannel.open({ keep: ['__deckNavigate'] });
          this._pChan.set('__deckNavigate', (spec) => this._pNavigate(spec));
        }
        ...
        if (!this._pWatching) {
          ...
          this.$watch(() => this._pSig(), () => {
            if (!this._pDeck) return;
            const n = this.items.length;
            if (!n) { this._cmpDrop(); this._pDeck.close(); return; }
            const cmp = this._cmpDeck;
            const at = cmp ? cmp.deck.active() : this._pDeck.deck.active();
            this.previewAt(Math.max(0, Math.min(at, n - 1)), cmp ? 'diff' : '');
          });
        }
      },
      ...
      _pDrop(replacing) {
        if (!this._pDeck) return;
        this._pReplacing = !!replacing;
        try { this._pDeck.drop(); } finally { this._pReplacing = false; }
        this._pDeck = null; this._pSigOpen = null; this._pNotes = {};
      },
      ...
      _pSyncChrome(n) {
        const h = this._pDeck;
        if (!h) return;
        h.setTitle(this._pName(n));
        h.setSubtitle(this._pWhere(n));
      ...
      _pSay(n) {
        ...
        if (!it || !this._pChan) return;
        this._pChan.announce(it.local
          ? { local: true, label: this._pName(n), route: 'stage' }
      ...
      _pActions(n) {
        ...
        // previewPairLabel already assembles exactly that for the header.
        //
        ...
        if (this.items.length > 1) out.push({
          icon: 'ph-git-diff',
          title: 'Compare ' + this.previewPairLabel(n),
      ...
      async _cmpRender(i, slide) {
        ...
        const bar = H('div', { class: 'flex items-center gap-1.5 flex-wrap shrink-0' },
          H('span', { class: 'font-mono text-base truncate' }, this.previewName(a)),
        ...
        }, H('span', { class: 'truncate' }, this.previewName(b)),
        ...
        if (d && this.cmpView === 'patch') {
          const text = window.textDiff.patch(d.ops, d.al, d.bl, {
            context: this.cmpContext, nameA: this.previewName(a), nameB: this.previewName(b),
          });
      ...
      get diffPatch() {
        ...
        return textDiff.patch(this._diffOps, this._diffAl, this._diffBl, {
          ...
          nameA: this.previewName(this.diffA),
          nameB: this.previewName(this.diffB),
        });
      },
