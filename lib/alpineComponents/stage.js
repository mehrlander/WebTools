    ...
    return {
      ...
      template: `
        <div class="relative min-h-[55vh]"
          ...
          <!-- No title row: the nav already says where you are, and no other
               view titles itself. The stage-wide actions (add, link, clear,
               save) sit on the thing they act on; the count lives on the
               aside's Staged label; the empty state carries Add file, since
               the tab bar does not exist until something is staged.

               The one row above the work is the mode pill: Working and Saved
               are two modes of one thing, the surface, so they share a nav
               stop the way Activity's three sub-views do. This side is the
               bench (edit); Saved is the shelf (display). -->
          <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 mb-3 w-fit" role="tablist">
            ...
            <button role="tab" @click="$store.browser && window.__shell?.goSurfaces()"
          ...
          <div class="grid gap-4 items-start lg:grid-cols-[minmax(0,1fr)_20rem]">
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <div class="flex flex-col gap-1.5">
                <!-- Always labeled, empty stage included, since the browse
                     button hangs off it and this is the third way in beside
                     the picker and the finder. -->
                <div class="flex items-center gap-1 text-base font-semibold uppercase tracking-wide text-base-content/40">
                  Add
                  <div class="grow"></div>
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <div x-show="recentOpen" class="flex flex-col max-h-72 overflow-y-auto overscroll-contain rounded-lg">
                    <!-- A finder row's primary action is staging, so the row is a
                         button; the jump-over rides beside it rather than inside
                         it (an anchor nested in a button is not a control). -->
                    <template x-for="it in finderRows()" :key="'r:' + it.repo + ':' + it.path">
                      <div class="group flex items-center rounded-lg hover:bg-base-200 transition-colors">
                        ...
                        <a :href="itemGh(it)" :data-peek="itemPeek(it)" target="_blank" rel="noopener"
                           :title="it.repo + ':' + it.path + ' on GitHub'"
          ...
          <dialog x-ref="saveDlg" class="modal" onclick="if(event.target===this)this.close()">
            <div class="modal-box max-w-2xl">
              <h3 class="font-bold text-lg flex items-center gap-2 mb-3">
                <i class="ph ph-push-pin"></i>Save as surface
              </h3>
      ...
      get saveSurface() {
        return window.Surface.fromStage(this.items, {
          ...
          destination: (this.saveDest || '').trim(),
      ...
      get savePath() { return 'surfaces/' + window.Surface.fileName(this.saveSurface.surface); },
      ...
      async saveAsSurface() {
        ...
        try {
          ...
          const path = 'surfaces/' + window.Surface.fileName(surface);
          const reg = this.srcGh(window.__shell?.REGISTRY_REPO || '', '');
          await reg.save(path, window.Surface.write(surface), 'Save stage as surface via show-repo');
