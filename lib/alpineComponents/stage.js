    ...
    return {
      ...
      template: `
        <div class="relative min-h-[55vh]"
          ...
          <!-- THE TRANSFORM WORKBENCH, over the stage rather than beside it.
               The tool wants the viewport (a tab strip, an editor, a table),
               so it takes one, the way the preview and the estate's branch
               takeover do, rather than being folded into a column it would
               have to fight.

               Mounted ONCE and then only shown or hidden, never re-created:
               the workbench holds tab sources, batches and a run's output in
               its own component state, so an x-if that rebuilt it on each open
               would silently discard the work between two visits. It is also
               the reason there can be exactly one of these, since the tool
               addresses its viewer and table by document id rather than
               through its root.

               NO BACKTICKS IN HERE; this comment is inside a template
               literal (see the preview overlay below for what that cost). -->
          <div x-show="tfOpen" x-cloak
               class="fixed inset-0 z-50 flex items-stretch justify-center bg-black/40 sm:items-center sm:p-4"
               @keydown.escape.window="tfOpen = false">
            <div class="bg-base-100 w-full h-full sm:rounded-xl sm:shadow-2xl sm:max-w-6xl sm:h-[85vh]
                        flex flex-col overflow-hidden">
              <div class="flex items-center gap-2 px-3 py-2 border-b border-base-300 shrink-0">
                <i class="ph ph-function text-secondary text-xl"></i>
                <span class="font-mono text-base truncate" x-text="tfName"></span>
                <span class="text-base text-base-content/40" x-text="tfNote"></span>
                <div class="grow"></div>
                <button @click="tfOpen = false" class="btn btn-ghost btn-square opacity-60 hover:opacity-100 shrink-0" title="Close">
                  <i class="ph ph-x text-2xl"></i>
                </button>
              </div>
              <div x-ref="tfHost" class="flex-1 min-h-0 overflow-hidden p-3"></div>
            </div>
          </div>
      ...
      async openTransform(it) {
        ...
        this.tfBusy = true;
        try {
          await this.mountTransform();
          this.tfName = it.name;
          this.tfNote = '';
          this.tfOpen = true;
          const wb = this.$refs.tfHost?.querySelector('.tf-root')?.__workbench;
          ...
          // and delimited text both parse), which is exactly the set this chip
          // recognizes. Handing it the raw text keeps ONE reader of these
      ...
      // Mount once, on the first open. THE WORKBENCH EXPECTS ITS HOST TO HAVE
      // BROUGHT ITS OWN LIBRARIES, and both of them are ones this shell does
      // not otherwise load. PapaParse is the parse path's, called without a
      // guard, so a missing Papa throws rather than degrading. Tabulator is the
      // table's, and its absence is worse than an error: the render hook reads
      // `typeof Tabulator === "undefined"` and RETURNS, so the tool draws its
      // whole chrome around an empty pane and says nothing. Measured here on
      // the first mount, which is the only reason this loads two things rather
      // than one.
      //
      ...
      //
      // The markup is injected rather than written into this template because
      // the tool owns its own markup (panelHTML), and an injected subtree needs
      // an explicit initTree since Alpine has already walked this one.
      async mountTransform() {
        if (this.tfMounted) return;
        ...
        host.innerHTML = window.TransformWorkbench.panelHTML(
