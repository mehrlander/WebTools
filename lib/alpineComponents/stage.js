...
window.StageIntake = (() => {
  ...
  //
  // A copy off a web page puts two readings on the clipboard, and the URLs are
  // in exactly one of them. `text/plain` carries the link TEXT and drops every
  // address; `text/html` carries the addresses and is unreadable as a document.
  // So the reader who wanted the links had both halves on the stage and no way
  // to put them together: read the html and you are looking at markup, read the
  // text and the URLs are simply gone. Reported 2026-08-28.
  //
  // This is the third thing a staged item can offer, and it is not either of
  // the two already here. The flavors bar offers ANOTHER ENCODING the platform
  // already made; the transform chip offers ANOTHER TOOL for what was read.
  // This makes a NEW ARTIFACT by reading structure out of one, which is why it
  // gets its own row rather than a third meaning for "carried".
  //
  // CSV rather than a rendered list, and that is the one choice here worth
  // defending. A `.csv` opens as a table with a per-column filter (READ_MODE),
  // its raw mode is the pasteable `text,url` lines, and `transformKindOf` calls
  // it rows, so the extraction lands ON the machinery that exists instead of
  // beside it. A markdown list would render tappable and be a dead end.
  const LINK_EXT = new Set(['html', 'xml', 'md', 'txt']);
  const MARKUP_EXT = new Set(['html', 'xml']);
  const isMarkup = (name) => MARKUP_EXT.has(extOf(name));
  // Past this the parse is not worth the main thread, and nothing a person
  // pasted to read the links out of is this big.
  const LINK_MAX = 4e6;

  // An href worth carrying OUT of the document it was in. A `javascript:` is
  // not an address, and a bare `#section` is in-page navigation that means
  // nothing once it is on its own in a table; both are noise in the common
  // case, which is a page's worth of markup. Everything else stays, `mailto:`
  // included, since that is a link somebody deliberately wants to grab.
  const usableHref = (href) => {
    const h = String(href || '').trim();
    if (!h || h.startsWith('#')) return '';
    if (/^(javascript|data|about|blob):/i.test(h)) return '';
    return h;
  };

  // A RELATIVE HREF STAYS RELATIVE, and there is nothing to be done about it
  // here: a paste carries the markup and not the page it came off, so there is
  // no base to resolve against. Most sites that matter write absolute hrefs in
  // the markup a copy serializes; where one does not, the column says so
  // plainly rather than inventing an origin.
  const linksFromHtml = (text) => {
    const out = [];
    let doc = null;
    try { doc = new DOMParser().parseFromString(String(text), 'text/html'); } catch { return out; }
    for (const a of doc.querySelectorAll('a[href]')) {
      const url = usableHref(a.getAttribute('href'));
      if (url) out.push({ text: (a.textContent || '').replace(/\s+/g, ' ').trim(), url });
    }
    return out;
  };

  // The same reading for text that is not markup: a markdown link, an angle
  // autolink, or a bare address sitting in prose. Markdown is scanned first and
  // its spans are blanked out, so `[Docs](https://x/y)` yields one row rather
  // than a labelled one and a bare one for the same address.
  const MD_LINK = /\[([^\]\n]*)\]\(\s*<?([^)\s>]+)>?[^)]*\)/g;
  const BARE_URL = /\b(?:https?:\/\/|mailto:)[^\s<>()\[\]"']+[^\s<>()\[\]"'.,;:!?]/g;
  const linksFromText = (text) => {
    const s = String(text);
    const out = [];
    const rest = s.replace(MD_LINK, (m, label, url) => {
      const u = usableHref(url);
      if (u) out.push({ text: String(label).trim(), url: u });
      return ' '.repeat(m.length);
    });
    for (const m of rest.matchAll(BARE_URL)) {
      const u = usableHref(m[0]);
      if (u) out.push({ text: '', url: u });
    }
    return out;
  };

  // ONE ROW PER ADDRESS, first label wins. A page's markup repeats its nav on
  // every copy that includes it, and forty rows of the same masthead is not
  // what anybody meant by "get the links out"; the first occurrence is also the
  // one most likely to carry real link text, since a repeat is often an icon
  // with none.
  const dedupeLinks = (list) => {
    const by = new Map();
    for (const l of list) {
      const prev = by.get(l.url);
      if (!prev) by.set(l.url, l);
      else if (!prev.text && l.text) prev.text = l.text;
    }
    return [...by.values()];
  };
  ...
  // The links in a piece of text, read as markup or as prose by its extension.
  // Keyed on the NAME for the reason transformKindOf is: the intake already
  // decided what this is, and a second sniff here would be a way for the two
  // to drift.
  const linksOf = (text, name) => {
    const s = String(text || '');
    if (!s || s.length > LINK_MAX) return [];
    const ext = extOf(name);
    if (!LINK_EXT.has(ext)) return [];
    return dedupeLinks(MARKUP_EXT.has(ext) ? linksFromHtml(s) : linksFromText(s));
  };

  // The same question asked of a staged item. A ref holds no text until it is
  // fetched, and a binary holds none at all, so both answer empty the way they
  // do for the transform chip.
  const linksIn = (it) =>
    (it && it.local && it.isText && typeof it.text === 'string')
      ? linksOf(it.text, it.name) : [];
  ...
  // one.
  const derivedName = (name, tag) => {
    const s = String(name || 'paste');
    const dot = s.lastIndexOf('.');
    return (dot > 0 ? s.slice(0, dot) : s) + '-' + tag + '.md';
  };
  ...
  //
  // A `]` inside link text closes the label early, and a url carrying a space or
  // a paren closes the target early; both are the same class of silent bug, so
  // each gets its own escape. An angle-bracketed target is the CommonMark way
  // to say "all of this is the address", and it is also the whole line for a
  // link with no text, since `[](url)` renders as nothing to click.
  const mdLink = (text, url) => {
    const label = String(text || '').replace(/([\[\]])/g, '\\$1');
    const target = /[\s()]/.test(url) ? '<' + url + '>' : url;
    return label ? '[' + label + '](' + target + ')' : '<' + url + '>';
  };
  const linksMd = (links) =>
    (links || []).map(l => '- ' + mdLink(l.text, l.url)).join('\n') + '\n';
  ...
  return { take, takeFile, takeDrop, takePaste, takeClipboard, takeFlavors, takeFlavor, offerable, focus,
           ...
           linksOf, linksIn, derivedName, linksMd, mdOf, isMarkup };
})();
...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div x-show="offers.length" x-cloak
                  ...
                  <!-- LEFT-ALIGNED, NOT CENTRED. A daisyUI tooltip centres on
                       its anchor and cannot flip, so a 256px box over the first
                       pill in the row hung off the left edge of a phone. Pinning
                       the box's left edge to the pill's is what fits every
                       position: a pill's left edge is never left of the bar's,
                       and the widest case (the last pill in a row) still lands
                       inside 390px at this width. -->
                  <div class="flex flex-wrap gap-1">
                    <template x-for="fl in offers" :key="fl.name">
                      <div class="tooltip tooltip-bottom">
                        <div class="tooltip-content w-56 max-w-[calc(100vw-3rem)] p-2" style="left:0;translate:none;transform:none" x-html="tipHtml(tipFor('flavor', fl))"></div>
                        <button @click="toggleFlavor(fl)"
                                class="badge badge-lg h-auto py-1 gap-1.5 cursor-pointer"
                                :class="flavorStaged(fl) ? 'badge-primary' : 'badge-outline hover:badge-primary'">
                    ...
                    <!-- THE DERIVATIONS, in the same row and never hidden. They
                         were a ⋯ menu for one afternoon: an option you have to
                         open something to find out about is one nobody reaches
                         for, and there are at most two of these. Dashed and
                         un-monospaced so they do not read as a fourth format,
                         since each is a new file made out of one rather than
                         another encoding of it. Both land as MARKDOWN: the links
                         were a csv for a day, which reads as a table and is not
                         a thing you can paste anywhere, and copying out is the
                         errand. -->
                    <template x-for="t in pasteLinks" :key="t.key">
                      <div class="tooltip tooltip-bottom">
                        <div class="tooltip-content w-56 max-w-[calc(100vw-3rem)] p-2" style="left:0;translate:none;transform:none" x-html="tipHtml(tipFor('links', t))"></div>
                        <button @click="extractLinks(t)"
                                class="badge badge-lg h-auto py-1 gap-1.5 cursor-pointer badge-outline border-dashed hover:badge-primary">
                          <i class="ph ph-link-simple text-sm"></i>
                          <span x-text="t.n + ' links'"></span>
                          <span class="opacity-50" x-text="'in ' + t.from"></span>
                        </button>
                      </div>
                    </template>

                    <!-- THE WHOLE FLAVOR, CONVERTED. The links pill is a
                         reduction of one thing markup carries; this is the
                         general case behind it, and the reason there is a
                         library here at all: headings, lists, code and tables
                         all survive, where text/plain flattens every one of
                         them. The hover starts the conversion the tap would
                         otherwise wait for, which is what makes the preview
                         possible at all: it is the one pill whose content has
                         to be MADE before it can be shown. -->
                    <template x-for="t in pasteMarkdown" :key="t.key">
                      <div class="tooltip tooltip-bottom" @mouseenter="primeMd(t)">
                        <div class="tooltip-content w-56 max-w-[calc(100vw-3rem)] p-2" style="left:0;translate:none;transform:none" x-html="tipHtml(tipFor('md', t))"></div>
                        <button @click="stageMd(t)"
                                class="badge badge-lg h-auto py-1 gap-1.5 cursor-pointer badge-outline border-dashed hover:badge-primary">
                          <i class="ph text-sm" :class="mdBusy ? 'ph-circle-notch animate-spin' : 'ph-text-align-left'"></i>
                          <span>markdown</span>
                          <span class="opacity-50" x-text="'from ' + t.from"></span>
                        </button>
                      </div>
                    </template>
                  </div>
      ...
      // ── WHAT CAN BE READ OUT OF THIS PASTE ────────────────────────────────
      //
      // Two derivations, both landing as markdown, and that is a correction
      // rather than a preference. The links first shipped as a `.csv`, reasoned
      // from the machinery (a csv opens as a filterable table, transformKindOf
      // calls it rows) rather than from the errand, which is copying them out
      // and using them somewhere else. Markdown renders the list tappable, its
      // raw mode pastes anywhere, and it generalizes: the same conversion that
      // makes a link list makes a whole page readable.
      //
      // Scoped to the paste's flavors, since the bar is about the paste. A
      // staged file that arrived some other way keeps its route through the
      // reader's header, which offers the same two.
      get pasteLinks() {
        const IN = window.StageIntake;
        const out = [];
        for (const fl of this.offers) {
          if (fl.kind === 'file' || !fl.text) continue;
          const links = IN.linksOf(fl.text, fl.name);
          if (links.length) out.push({ key: fl.name, dest: IN.derivedName(fl.name, 'links'), links,
                                       from: this.flavorLabel(fl), n: links.length });
        }
        return out;
      },

      // MARKUP ONLY, and no count on the pill. A link count is a fact about the
      // html cheap enough to read at render time; how much markdown it makes is
      // not known until the library is loaded and the conversion is run, and
      // running one per paste to label a pill nobody may tap is the wrong trade.
      get pasteMarkdown() {
        const IN = window.StageIntake;
        return this.offers
          .filter(fl => fl.kind !== 'file' && fl.text && IN.isMarkup(fl.name))
          .map(fl => ({ key: fl.name, flavor: fl, dest: IN.derivedName(fl.name, 'markdown'),
                        from: this.flavorLabel(fl) }));
      },

      // The extraction, opening the reader on what it made. The list is the
      // thing that was asked for rather than a row to go and find, so this is
      // the one intake besides a single arrival that focuses.
      extractLinks(t) {
        if (!t) return;
        const IN = window.StageIntake;
        const added = IN.take({ text: IN.linksMd(t.links), name: t.dest });
        if (added[0]) IN.focus(added[0]);
      },
      ...
      async mdFor(fl) {
        ...
        if (this._mdText.has(fl.name)) return this._mdText.get(fl.name);
        this.mdBusy = true;
        try {
          await this.loadMarkdownDeps();
          const md = window.StageIntake.mdOf(fl.text);
          this._mdText.set(fl.name, md);
          return md;
        } finally { this.mdBusy = false; }
      },
      async stageMd(t) {
        let md;
        try { md = await this.mdFor(t.flavor); } catch (e) { return this.mdFailed(e); }
        const added = window.StageIntake.take({ text: md, name: t.dest });
        if (added[0]) window.StageIntake.focus(added[0]);
      },
      mdFailed(e) {
      ...
      // A NEW BAR DROPS THE OLD PREVIEWS, blob URLs and conversions alike: they
      // are readings of the paste on screen, and the paste has changed.
      set offers(v) { this.resetTips(); Alpine.store('browser').stageOffers = v || []; },
      offer(flavors) {
      ...
      //
      // A tooltip carrying the CONTENT. The pills said which formats a copy
      // held and could not say which one you want, because the only thing
      // separating `html 4.1 KB` from `txt 192 B` was a title attribute reading
      // "Stage 2026-08-28-paste.html", which is the pill's own label again.
      //
      // This was a split pill with an eye and a panel below the row for an
      // afternoon, and the eye was the mistake: two tap targets inside one
      // badge, and the second one had to be learned. One pill, one tap, and the
      // preview costs nothing to ignore because it is a hover. What that trades
      // away is the phone, where nothing hovers; the panel reached there and
      // was confusing everywhere, and a confusing control on every device is
      // the worse half of that trade.
      TIP_CHARS: 400,
      _tipUrls: null,

      // A blob URL rather than a data URI, since a pasted screenshot is
      // megabytes and base64 in an attribute is the slow way to say the same
      // thing. Cached per name, revoked together when the bar goes.
      _imgUrl(fl) {
        this._tipUrls ??= new Map();
        if (!this._tipUrls.has(fl.name)) this._tipUrls.set(fl.name, URL.createObjectURL(fl.file));
        return this._tipUrls.get(fl.name);
      },

      // The preview for one pill: a head line naming what it is, and either the
      // first characters of the text or the image itself.
      tipFor(kind, o) {
        const IN = window.StageIntake;
        const cut = (t) => {
          const s = String(t || '');
          return s.slice(0, this.TIP_CHARS) + (s.length > this.TIP_CHARS ? '\n…' : '');
        };
        if (kind === 'links') return { name: o.dest, note: o.n + ' links', text: cut(IN.linksMd(o.links)) };
        if (kind === 'md') {
          const md = this._mdText?.get(o.key);
          return md ? { name: o.dest, note: this.fmtSize(md.length), text: cut(md) }
                    : { name: o.dest, note: 'converting…', text: '' };
        }
        if (o.kind === 'file') {
          return /^image\//.test(o.type || '')
            ? { name: o.name, note: this.fmtSize(o.size), img: this._imgUrl(o) }
            : { name: o.name, note: this.fmtSize(o.size), text: '(not text)' };
        }
        return { name: o.name, note: this.fmtSize(o.size), text: cut(o.text) };
      },

      // The tooltip's body. Built as a string rather than as bound children
      // because the same box has to hold text or an image, and daisyUI gives it
      // one content element: a `<template x-if>` pair inside a tooltip is two
      // subtrees Alpine keeps in sync for a box that is not on screen. Escaped
      // through the viewer's own escaper, since the text here is pasted markup
      // and this is the one place it is written INTO a document rather than
      // read out of one.
      tipHtml(t) {
        if (!t) return '';
        const esc = window.ViewRegistry.esc;
        const head = '<div class="flex items-baseline gap-1.5 text-left">'
          + '<span class="font-mono truncate">' + esc(t.name) + '</span>'
          + '<span class="opacity-60 shrink-0">' + esc(t.note) + '</span></div>';
        const body = t.img
          ? '<img src="' + esc(t.img) + '" alt="" class="mt-1 max-h-32 w-auto rounded">'
          : (t.text ? '<pre class="mt-1 text-left font-mono text-xs leading-4 whitespace-pre-wrap '
                      + 'break-words max-h-32 overflow-hidden opacity-90">' + esc(t.text) + '</pre>' : '');
        return head + body;
      },

      // The markdown pill is the one preview that has to be MADE, so hovering
      // starts it and the tooltip fills in when it lands. Failure is silent
      // here and loud on the tap: a hover nobody asked anything of should not
      // raise a toast.
      primeMd(t) {
        if (this._mdText?.has(t.key)) return;
        this.mdFor(t.flavor).catch(() => {});
      },

      // Everything a bar's previews minted, dropped with the bar.
      resetTips() {
        for (const u of (this._tipUrls?.values() || [])) URL.revokeObjectURL(u);
        this._tipUrls = null;
        this._mdText = null;
      },
      ...
      _rActions(n) {
        ...
        // THE SAME TWO DERIVATIONS RIDE THE HEADER, for the reason the
        // transform offer does: a single arrival routes to the Stage and opens
        // on itself, so the reader is looking at the markup rather than at the
        // row that carries the pills. Recomputed per slide, since what can be
        // read out is a property of the ITEM. Staging the result replaces the
        // deck rather than stacking one, which readerAt already does for any
        // changed set.
        const IN = window.StageIntake;
        const inside = it ? IN.linksIn(it) : [];
        if (inside.length) out.push({
          icon: 'ph-link-simple',
          title: 'Read the ' + inside.length + ' links in ' + it.name + ' out as markdown',
          onClick: () => this.extractLinks({ dest: IN.derivedName(it.name, 'links'), links: inside }),
        });
        if (it && it.local && it.isText && IN.isMarkup(it.name)) out.push({
          icon: 'ph-text-align-left',
          title: 'Convert ' + it.name + ' to markdown',
          onClick: () => this.stageMd({ flavor: it, dest: IN.derivedName(it.name, 'markdown') }),
        });
      ...
      _rRender(i, slide) {
        ...
        el.setAttribute('x-data', "viewer({ bindStore: false, fill: true, identify: false, defaultMode: (f) => window.ViewRegistry.READ_MODE(f) })");
        slide.append(el);
      ...
      async _rDrive(i, el, slide) {
        ...
        const show = async (content, origin) => {
          ...
          // THE SIZE ARRIVES LATER THAN THE FILE. `show()` resolves once the
          // mode is chosen, but a mode module's `after()` runs in a $nextTick
          // and the byte count is something it reports from there, after
          // fetching the real bytes. So the sync above is too early for the
          // one fact the viewer alone knows, and a subtitle built then says
          // only the repo.
          //
