    ...
    return {
      ...
      template: `
        <div class="relative min-h-[55vh]"
          ...
          <!-- No title row and no mode pill. The bench renders inside the card
               of the surface it is editing, on the one Surfaces shelf, so the
               card's header already says which surface this is and the shelf
               already says where you are. The stage-wide actions sit on the
               thing they act on; the empty state carries Add file, since the
               tab bar does not exist until something is staged. -->
      ...
      init() {
        ...
        // The Recent sweep can just run: the bench mounts only when a card's
        // edit is opened, where it used to mount hidden at page load and had to
        // wait for a view to become current before spending the calls.
        this.loadRecent();
