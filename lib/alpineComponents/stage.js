              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div x-show="offers.length" x-cloak
                  ...
                  <div class="flex items-center gap-1.5 text-base text-base-content/60">
                    ...
                    <div class="grow"></div>
                    <!-- THE DERIVATIONS, behind one discreet control. Reading the
                         links out is not a fourth format, so it does not belong
                         among the chips; it is also not wanted often enough to
                         spend a band of a phone screen on. A menu that appears
                         only when there is something in it costs nothing when
                         there is not. -->
                    <div x-show="pasteLinks.length" class="dropdown dropdown-end">
                      <button tabindex="0" class="opacity-40 hover:opacity-100 hover:text-primary focus:outline-none focus:opacity-100 focus:text-primary"
                              title="Read something out of this paste">
                        <i class="ph ph-dots-three-outline"></i></button>
                      <ul tabindex="0" class="dropdown-content menu menu-sm bg-base-100 rounded-box z-10 mt-1 w-60 p-1 shadow-lg border border-base-300">
                        <template x-for="t in pasteLinks" :key="t.key">
                          <li><a @click="extractLinks(t); $el.blur()" class="flex items-center gap-2">
                            <i class="ph ph-link-simple text-primary"></i>
                            <span x-text="'Links from ' + t.from"></span>
                            <span class="opacity-50" x-text="t.n"></span>
                          </a></li>
                        </template>
                      </ul>
                    </div>
                  ...
                  <div class="flex flex-wrap gap-1">
                    <template x-for="fl in offers" :key="fl.name">
                      <button @click="toggleFlavor(fl)"
                              ...
                              class="badge badge-lg gap-1.5 cursor-pointer h-auto py-1"
