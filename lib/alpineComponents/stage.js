...
window.StageLink = (() => {
  ...
  const parseLink = (hash) => {
    ...
    return { items, prompts: decodePrompts(p.prompts), mode: p.mode === 'diff' ? 'diff' : '', dest };
  };
          ...
          <div class="grid gap-4 items-start lg:grid-cols-[minmax(0,1fr)_20rem]">
            ...
            <div class="flex flex-col gap-2 min-w-0">
              ...
              <div x-show="!items.length"
                ...
                <!-- An AIMED stage says so while it is still empty. The
                     destination field lives with the staged items, which is
                     the right place once there are any, and no place at all
                     for a reader who just arrived from the branch page's
                     add-file plus with a dest= preset: the aim was set and
                     invisible. Naming it here is what makes the link legible
                     ("this is where the next thing I stage will go"). -->
                <template x-if="destSpec.trim()">
                  <p class="text-base text-base-content/60 mb-2">
                    Aimed at <span class="font-mono text-base-content/80" x-text="destSpec"></span>
                  </p>
                </template>
                ...
                <div class="flex items-center justify-center gap-2 mt-4">
                  <button @click="$refs.fileInput.click()" class="btn btn-ghost btn-sm gap-1.5"
                          title="Add a local file (or drop / paste anywhere on this view)">
                    <i class="ph ph-paperclip text-lg"></i>Add file
                  </button>
                  <button @click="pasteIn()" class="btn btn-ghost btn-sm gap-1.5"
                          title="Paste the clipboard as a staged file (or as refs, one per line)">
                    <i class="ph ph-clipboard-text text-lg"></i>Paste
                  </button>
                </div>
              </div>
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <div class="flex flex-col gap-1.5">
                <div class="flex items-center gap-1 text-base font-semibold uppercase tracking-wide text-base-content/40">
                  ...
                  <!-- An EXPLICIT paste, beside the file picker. The
                       window-level paste listener below covers a desktop
                       ⌘V, but iOS Safari fires no paste event without a
                       focused editable, so a phone had no way in at all.
                       kits/io.js owns that fallback. -->
