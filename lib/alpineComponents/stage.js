...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
          ...
          <div class="grid gap-4 items-start @2xl:grid-cols-[minmax(0,1fr)_22rem] @5xl:grid-cols-[minmax(0,1fr)_26rem] @7xl:grid-cols-[minmax(0,1fr)_30rem]">
            ...
            <div class="flex flex-col gap-3 min-w-0">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <!-- WHAT IS INSIDE. A copy off a web page splits in two on the
                     clipboard: text/plain holds every link's label and none of
                     its addresses, text/html holds the addresses inside markup
                     nobody wants to read. Both were stageable and neither
                     answered "just give me the links". This row does, in one
                     tap, and it reads the pending OFFERS too, so the html need
                     not be staged first to be read out of. -->
                <div x-show="extractables.length" x-cloak
                     class="border border-accent/30 bg-accent/10 rounded-lg px-2.5 py-2 flex flex-col gap-1.5">
                  <div class="flex items-center gap-1.5 text-base text-base-content/60">
                    <i class="ph ph-link-simple text-accent"></i>
                    <span>Inside</span>
                  </div>
                  <div class="flex flex-wrap gap-1">
                    <template x-for="t in extractables" :key="t.key">
                      <button @click="extractLinks(t)"
                              :title="t.title"
                              class="badge badge-outline badge-lg gap-1.5 cursor-pointer hover:badge-accent h-auto py-1">
      ...
      // ── WHAT IS INSIDE ────────────────────────────────────────────────────
      //
      // The third row, and a third kind. The flavors bar offers another
      // ENCODING the platform already made of one paste; the transform chip
      // offers another TOOL for what was read; this reads structure OUT of an
      // item and stages the result as its own file.
      //
      // It runs over the OFFERS as well as the staged items, and that is what
      // makes the case it was built for one tap. A web-page copy stages its
      // text/plain, which carries every link's label and not one of its
      // addresses, and offers the text/html beside it. An extraction that only
      // saw the stage would charge a tap on that offer first, to stage a
      // document nobody wanted, before the links could be read out of it.
      get extractables() {
        const IN = window.StageIntake;
        const have = new Set(this.localItems.map(it => it.name));
        const out = [];
        const add = (key, name, links) => {
          if (!links.length) return;
          // Never offer an extraction that is already staged, which is the
          // flavors bar's own dedupe rule and needed for the same reason: the
          // sources survive being extracted (an offer stays on the bar), so
          // without this a second tap would stage a second identical table.
          const dest = IN.linksName(name);
          if (have.has(dest)) return;
          out.push({ key, name, dest, links,
                     label: links.length + (links.length === 1 ? ' link' : ' links'),
                     title: 'Read the ' + links.length + ' links in ' + name + ' out into ' + dest });
        };
        for (const it of this.localItems) add(this.itemKey(it), it.name, IN.linksIn(it));
        for (const fl of this.offers) {
          if (fl.kind !== 'file' && fl.text) add('offer:' + fl.name, fl.name, IN.linksOf(fl.text, fl.name));
        }
        return out;
      },
