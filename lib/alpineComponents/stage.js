...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
      ...
      addRows() {
        const q = this.addQ.trim().toLowerCase();
        const sc = this.addScope;
        ...
        for (const r of this.addRoots()) {
          ...
          if (sr < 3) scored.push({ s: sr, c: 0, row: {
      ...
      get addEmpty() {
        ...
        if (this.addScope) return this.addQ.trim() ? 'No matching files here.' : 'This folder is empty.';
        if (this.addQ.trim()) return this.addOffer ? '' : 'No matching files.';
