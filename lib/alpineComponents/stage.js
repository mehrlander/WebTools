...
window.StageIntake = (() => {
  ...
  return { take, takeFile, takeDrop, takePaste, takeClipboard, takeFlavors, takeFlavor, offerable, focus,
           ...
           derivedName, opensRaw, mdOf, isMarkup };
})();
...
document.addEventListener('alpine:init', function() {
  Alpine.data('stager', function() {
    ...
    return {
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <div x-show="offers.length" x-cloak
                  ...
                  <div class="flex flex-wrap gap-1" x-effect="seedPeeks()">
                    ...
                    <!-- THE DERIVATION, in the same row and never hidden. It was
                         a ⋯ menu for one afternoon: an option you have to open
                         something to find out about is one nobody reaches for.
                         Dashed and un-monospaced so it does not read as a fourth
                         format, since it is a new file made out of one rather
                         than another encoding of it. There were two pills for a
                         day, the second a links extractor; converting the markup
                         carries every link as [text](url) already, so the
                         reduction went and the general case stayed. -->
                    <!-- Headings, lists, code, tables and links all survive,
                         where text/plain flattens every one of them, which is
                         the reason there is a library here at all. Its preview
                         is the one that has to be MADE before it can be shown,
                         so a hover starts the conversion and the card comes with
                         the next one. -->
      ...
      seedPeeks() {
        const P = window.SourcePeek;
        if (!P) return;
        this.mdSeq;
        for (const fl of this.offers) {
          if (fl.kind !== 'file' && fl.text) P.seed(fl.name, fl.text);
        }
        // 'source', not the markdown rendition the card's own extension rule
        // would pick: this file opens raw when staged, and a card that renders
        // what the reader is about to see as source would be previewing a
        // different thing.
        for (const t of this.pasteMarkdown) {
          const md = this._mdText?.get(t.key);
          if (md) P.seed(t.dest, md, 'source');
        }
      },
