    ...
    return {
      ...
      async loadActivity(reg){
        ...
        try {
          ...
          // The rows exist now, so a `&detail=` in the address can resolve.
      ...
      openDetailFromUrl(){
        ...
        const [, repo, name] = m;
        const row = this.openRows.find(r => r.repo === repo && r.name === name) || { repo, name };
