document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    const agePill = (ago, busy, busyLabel, what) => `
      <button @click="window.__shell?.goState()"
              ...
              title="${what} — open State for its age, what builds it, and Refresh">
        ...
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div x-show="tab==='repos'">
            ...
            <div x-show="authed" class="flex items-center justify-end gap-3 mb-3">
              ...
              ${agePill('configsGeneratedAt', "window.__shell?.configRefreshing", 'Syncing…', 'Repo configs')}
            </div>
          ...
          <div x-show="tab==='activity' || tab==='sessions' || tab==='guides'"
            ...
            <template x-if="tab==='sessions' && authed">
              ${agePill('sessionsGeneratedAt', 'sessionsBusy', 'Crawling…', 'Sessions cache')}
            </template>
            <template x-if="tab==='guides' && authed">
              ${agePill('guidesLoadedAt', 'guidesBusy', 'Reading…', 'Guides shelf')}
            </template>
            <template x-if="tab==='activity' && authed">
              <div class="flex items-center gap-2">
                ...
                ${agePill('activityGeneratedAt', 'activityBusy', 'Crawling…', 'Branch activity cache')}
              </div>
            </template>
          </div>
