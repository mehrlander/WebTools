    ...
    return {
                ...
                <div class="flex items-baseline gap-2 px-3 pt-2.5 pb-2 border-b border-base-200 sticky top-0 z-10 bg-base-100">
                  ...
                  <span class="text-sm text-base-content/70"
                        x-text="rowCard.kind === 'prose'
                                  ? (rowCard.turns?.length === 1 ? 'turn' : 'turns')
      ...
      openSessionCard(row, cls, ev){
        ...
        if (cls === 'turns') {
          ...
          // transcript capped at 60vh by CSS, so anchorMenu cannot infer it the
          // way it does for a list of menu rows, and the nine-row estimate it
          // used to get left a fifth of the card below a phone's fold.
          this.rowCardAt = window.__shell?.anchorMenu?.(ev, 9, {
            width: this.REPLY_CARD_W, align: 'left',
