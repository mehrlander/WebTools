    ...
    return {
      ...
      async mountReplyCard(card){
        ...
        if (card.staleV) host.append(this._replyFoot(
          ...
          + ' Press Refresh on this pane.', true));
        if (card.priorCut) host.append(this._replyFoot(
          ...
          + '; the whole conversation is in the session.', true));
        ...
        for (const t of card.turns) {
          const msg = window.chatRender.message(t, { collapse: 0 });
          // INSIDE the turn's own element, not beside it. chat-render gives a
          // message a coloured left rail and a text indent; a note appended
          // here inherits both, so the rail runs past it and it reads as that
          // turn's last line. As a sibling it sat a full gap away with the
          // rail stopping above it, equidistant between two turns and looking
          // like it belonged to neither.
          if (t.dropped) msg.append(this._replyFoot(t.dropped.toLocaleString() + ' more characters'));
          host.append(msg);
        }
      ...
      // A turn's note goes INSIDE its message, where the rail and the indent
      // are already right, so it needs neither. A CARD's note (a stale row, a
      // truncated scroll back) stands on its own and takes the indent that
      // lines it up with the turn text below it.
      //
      ...
      _replyFoot(text, standalone){
        const el = document.createElement('p');
        el.className = 'text-[10px] text-base-content/60 leading-snug mt-0.5'
          + (standalone ? ' pl-4' : '');
        el.textContent = '… ' + text;
        return el;
      },
