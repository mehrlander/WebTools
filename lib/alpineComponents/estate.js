document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    const ROUTES_MANIFEST = 'docs/app-routes.json';
    ...
    // is on the quick pass or the survey, or counting repos rather than session
    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <!-- FILES: the route the row was missing. The branch name
                         ...
                         the same control as the survey's verdict rather than a
                         ...
                         The landed RATIO used to ride here too, so a surveyed
                         row read 28/80 landed 11 missing * while an unsurveyed
                    ...
                    <span class="flex items-center gap-3 font-mono tabular-nums shrink-0">
                      <!-- Two pairs, two TRIGGERS. Each opens a card of its own
                           ...
                           control when the survey found nothing missing, a signal
                           ...
                           stranded. -->
                      <button @click.stop="openFileCard(row, 'changed', $event)"
                              @mouseenter="hoverFileCard(row, 'changed', $event)"
                              @mouseleave="hoverLeaveFileCard()"
                      ...
                      <template x-if="fileParts(row)?.added">
                        <button @click.stop="openFileCard(row, 'added', $event)"
                                @mouseenter="hoverFileCard(row, 'added', $event)"
                                @mouseleave="hoverLeaveFileCard()"
                      ...
                      <button x-show="row.nMissing" @click.stop="openFileCard(row, 'missing', $event)"
                              @mouseenter="hoverFileCard(row, 'missing', $event)"
                              @mouseleave="hoverLeaveFileCard()"
                      ...
                      <span x-show="row.noBase" class="text-warning"
                            title="No shared ancestor with the default branch, so the compare fell back to this branch's recent history and any count here spans more than the branch itself">*</span>
                    </span>
          ...
          <div class="flex-1 min-w-0" :class="tab==='sessions' ? '' : 'hidden'">
            <!-- The sessions crawl is lighter than the branch survey but not
          ...
          <div x-show="fileCard" x-cloak @click.outside="closeFileCard()"
               @mouseenter="cancelFileCardClose()" @mouseleave="hoverLeaveFileCard()"
              ...
              <div class="flex flex-col">
                <div class="flex items-baseline gap-2 px-3 pt-2.5 pb-2 border-b border-base-200 sticky top-0 bg-base-100">
                  <i class="ph text-base self-center"
                     :class="fileCard.cls === 'added' ? 'ph-file-plus text-success'
                           : fileCard.cls === 'missing' ? 'ph-warning-circle text-warning'
                           : 'ph-files text-base-content/50'"></i>
                  <span class="font-mono font-semibold tabular-nums" x-text="fileCardSummary?.count"></span>
                  <span class="text-sm text-base-content/70"
                        x-text="fileCard.cls === 'added' ? 'new'
                              : fileCard.cls === 'missing' ? 'missing' : 'changed'"></span>
                  <span class="grow"></span>
                  <span x-show="fileCardSummary?.lines" class="font-mono text-xs text-base-content/40"
                        x-text="fileCardSummary?.lines"
                ...
                <p x-show="fileCard.cls === 'missing'"
                   class="px-3 py-2 border-b border-base-200 text-xs text-base-content/60">
                  On this branch and not on <span class="font-mono" x-text="fileCard.base"></span>,
                ...
                <div x-show="fileCard.shape.exts.length || fileCard.shape.dirs.length"
                     class="px-3 py-2 flex flex-col gap-1.5 border-b border-base-200 text-xs">
                  <div class="flex flex-wrap gap-x-3 gap-y-1">
                    <template x-for="pair in fileCard.shape.exts" :key="'e' + pair[0]">
                  ...
                  <div class="flex flex-wrap gap-x-3 gap-y-1">
                    <template x-for="pair in fileCard.shape.dirs" :key="'d' + pair[0]">
                ...
                <div class="flex flex-col py-1">
                  <div x-show="fileCardMine?.loading"
                  ...
                  <div x-show="fileCardMine && !fileCardMine.loading && fileCardMine.noBase"
                       class="px-3 py-2 text-xs text-warning">
                    No merge base with <span class="font-mono" x-text="fileCard.base"></span>,
                    so there is no diff to list here.</div>
                  <div x-show="fileCardMine?.error" class="px-3 py-2 text-xs text-warning"
                       x-text="fileCardMine?.error"></div>
                  <!-- A row OPENS, rather than only linking out. The compare
                       ...
                       out to the file. -->
                  <template x-for="f in fileCardList" :key="f.path">
                    <div class="flex flex-col">
                      <div class="flex items-baseline gap-2 px-3 py-1 hover:bg-base-200 transition-colors"
                           :class="fileCardOpen === f.path && 'bg-base-200'">
                        <button @click="toggleFileCardPatch(f.path)" :disabled="!f.patch"
                          ...
                          <i class="ph text-xs shrink-0 self-center text-base-content/30"
                             :class="!f.patch ? 'ph-dot' : fileCardOpen === f.path ? 'ph-caret-down' : 'ph-caret-right'"></i>
                          <span class="min-w-0 truncate">
                            <span class="font-mono text-base-content/40" x-text="fileCardDir(f.path)"></span><span
                              class="font-mono text-base-content/85" x-text="fileCardName(f.path)"></span></span>
                        </button>
                        ...
                        <a :href="fileBlobUrl(fileCard.repo, fileCard.name, f.path)" target="_blank" rel="noopener"
                      ...
                      <template x-if="fileCardOpen === f.path && f.patch">
                ...
                <button @click="openFileCardBranch()"
      ...
      // and show-repo.html's VIEWS table are held in lockstep by a gate, at a
      ...
      // the same reason it does in the branch survey: two dozen commit reads
      ...
      async loadRouteJoin(force){
        ...
        try {
          this.routeManifest = JSON.parse((await gh.get(ROUTES_MANIFEST)).text);
        } catch (e) {
      ...
      // whether it is on the quick pass or the survey true-up, or counting
      ...
      // magnitude (a repo with 30 surveyable branches against one with two), so
      ...
      // the activity refresh ran quick-then-survey; one pass now, so the plain
        ...
        for (const [repo, e] of Object.entries(this.activity)){
          ...
          for (const b of (e.survey?.branches || [])){
            ...
            out.push({ repo, def, name: b.name, date: b.date || '', subject: b.subject || '', pr,
                       ...
                       // survey's own. Both carry a status and a line count per
          ...
          for (const p of (e.openPRs || [])){
            ...
            out.push({ repo, def, name: p.head, date: p.updatedAt || '', subject: p.title || '', pr: p,
                       ...
                       group: 'active',
                       // Same shape as a surveyed row, all zero: the survey
      ...
      BRANCH_SCOPES: [
        ...
        // The one scope that reads the PR index rather than the survey's
        ...
        // survey cannot see (its verdict is landed-or-not, and abandoned work
      ...
      inScope(r, scope){
        ...
        // PR state, not a survey group, and at any age: a branch abandoned in
      ...
      openBranchMenu(row, ev){
        ...
        this.closeFileCard();       // three panels now, and one at a time
        this.menuBranch = row;
      ...
      // survey's touched-path set, which is the same count read another way.
      // Null rather than a zeroed object where none of them does, since the
      // survey's own rule holds here too: "not measured" and "measured zero"
      ...
      // line totals, and the survey's three-way verdict where there is one.
      ...
      FILE_CARD_W: 384,
      fileCard: null,        // { repo, name, base, cls, count, shape, lines }
      fileCardAt: null,      // { x, y } from the shell's shared anchor geometry
      fileCardRead: null,    // { key, loading, error, noBase, files }

      fileCardKey(repo, name, base){ return repo + '@' + name + '...' + (base || ''); },
      get fileCardStyle(){
        return window.__shell?.menuStyle?.(this.fileCardAt) || 'left:-9999px;top:-9999px';
      },
      ...
      openFileCard(row, cls, ev){
        ...
        this.branchMenuAt = null;
        const f = this.fileStats(row) || {};
        // `missing` is the survey's class, not the compare's, so it comes from a
        ...
        this.fileCardOpen = '';
        this.fileCardAt = window.__shell?.anchorMenu?.(ev, 9, { width: this.FILE_CARD_W, align: 'left' }) || null;
        this.loadFileCard(row);
      },
      // The same digest the crawl builds, over a path list rather than a compare.
      // Through BranchSurvey.fileKind, so the missing card's histogram cannot
      // disagree with the other two about what an extension is.
      shapeOfPaths(paths){
        const K = window.BranchSurvey?.fileKind;
      ...
      closeFileCard(){ this.fileCard = null; this.fileCardAt = null; this.cancelFileCardClose(); },
      _fcOpenT: null, _fcCloseT: null,
      hoverFileCard(row, cls, ev){
        const shell = window.__shell;
        if (!shell?.finePointer) return;          // touch has a tap, and it opens this
        const el = ev?.currentTarget;
        this.cancelFileCardClose();
        if (this.fileCard && this.fileCard.name === row.name && this.fileCard.cls === cls) return;
        this._fcOpenT = setTimeout(() => this.openFileCard(row, cls, el), shell.HOVER_OPEN_MS);
      },
      hoverLeaveFileCard(){
        const shell = window.__shell;
        if (!shell?.finePointer) return;
        this.cancelFileCardClose();
        this._fcCloseT = setTimeout(() => this.closeFileCard(), shell.HOVER_CLOSE_MS);
      },
      cancelFileCardClose(){ clearTimeout(this._fcOpenT); clearTimeout(this._fcCloseT); },
      ...
      async loadFileCard(row){
        const key = this.fileCardKey(row.repo, row.name, row.def);
        if (this.fileCardRead?.key === key && !this.fileCardRead.error) return;
        this.fileCardRead = { key, loading: true, error: '', noBase: false, files: [] };
        try {
          ...
          if (this.fileCardRead?.key !== key) return;   // the reader moved on
          const files = (compare?.files || []).map(f => ({
            path: f.filename, prev: f.previous_filename || '',
            cls: window.BranchSurvey.fileClass(f),
          ...
          this.fileCardRead = { key, loading: false, error: '', noBase: !!noBase, files };
          this.absorbCompare(row, compare);
        } catch (e) {
          if (this.fileCardRead?.key === key)
            this.fileCardRead = { key, loading: false, error: e?.message || String(e),
                                  noBase: false, files: [] };
        }
      },
      ...
      absorbCompare(row, compare){
        if (!compare || !window.BranchSurvey) return;
        const e = this.activity?.[row.repo];
        if (!e) return;
        const patch = {
          stats: window.BranchSurvey.fileStats(compare.files),
        ...
        for (const b of (e.survey?.branches || [])) if (b.name === row.name) { Object.assign(b, patch); hit = true; }
      ...
      // read is in flight.
      get fileCardMine(){
        const c = this.fileCard, r = this.fileCardRead;
        return (c && r && r.key === this.fileCardKey(c.repo, c.name, c.base)) ? r : null;
      },
      // The card's files. Two sources, because `missing` is a verdict about
      // paths and the other two are statuses in a diff: the survey's own list
      ...
      get fileCardList(){
        const c = this.fileCard;
        if (!c) return [];
        const r = this.fileCardMine;
      ...
      get fileCardSummary(){
        const c = this.fileCard;
        if (!c) return null;
        const r = this.fileCardMine, list = this.fileCardList;
      ...
      PATCH_CAP: 400,
      fileCardOpen: '',
      toggleFileCardPatch(path){ this.fileCardOpen = this.fileCardOpen === path ? '' : path; },
      patchLines(patch){
      ...
      fileCardDir(path){ const i = path.lastIndexOf('/'); return i < 0 ? '' : path.slice(0, i + 1); },
      fileCardName(path){ const i = path.lastIndexOf('/'); return i < 0 ? path : path.slice(i + 1); },
      ...
      async mountDeck(replace){
        ...
        // compareFields off BranchSurvey. cm6-merge is fileReview's, which the
        ...
        try {
          for (const k of ['kits/swipe-deck.js', 'kits/branch-survey.js', 'kits/guide-render.js',
      ...
      // only (show-repo.html, `g.pulls('open', 30)`), so a branch whose PR
