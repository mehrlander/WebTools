  ...
  Alpine.data('estate', function() {
    ...
    return {
                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <span class="flex items-center gap-3 font-mono tabular-nums shrink-0">
                      ...
                      <button x-show="row.nMissing" @click.stop="openBranchFiles(row, 'missing')"
              ...
              <div class="flex flex-col">
                <div class="flex items-baseline gap-2 px-3 pt-2.5 pb-2 border-b border-base-200 sticky top-0 bg-base-100">
                  <i class="ph text-base self-center"
                     :class="fileCard.cls === 'added' ? 'ph-file-plus text-success' : 'ph-files text-base-content/50'"></i>
                  <span class="font-mono font-semibold tabular-nums" x-text="fileCardSummary?.count"></span>
                  <span class="text-sm text-base-content/70"
                        x-text="fileCard.cls === 'added' ? 'new' : 'changed'"></span>
                ...
                <div class="flex flex-col py-1">
                  ...
                  <template x-for="f in fileCardList" :key="f.path">
                    <a :href="fileBlobUrl(fileCard.repo, fileCard.name, f.path)" target="_blank" rel="noopener"
                       :title="f.path + (f.prev ? '\\nwas ' + f.prev : '')"
                       class="flex items-baseline gap-2 px-3 py-1 hover:bg-base-200 transition-colors">
      ...
      openFileCard(row, cls, ev){
        ...
        this.branchMenuAt = null;
        const f = this.fileStats(row) || {};
        const shape = f.shape?.[cls] || { exts: [], dirs: [] };
        this.fileCard = {
          repo: row.repo, name: row.name, base: row.def || '', cls,
          count: cls === 'added' ? (f.added || 0) : (f.changed || 0),
          shape, split: f.split !== false,
        };
      ...
      get fileCardList(){
        const c = this.fileCard, r = this.fileCardMine;
        return (c && r) ? r.files.filter(f => f.cls === c.cls) : [];
      },
