document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
      ...
      //
      // It names the repo rather than saying "this repo", since the menu is
      // read after the pointer has left the row it belongs to, and it carries
      // no group label of its own: one row above a rule needs no heading.
      chipOpts(repo){
        const on = this.activeRepoFilter === repo;
        return { align: 'left', extra: [{
          key: 'only', strong: true,
          label: on ? 'All repos' : 'Only ' + this.repoShort(repo),
