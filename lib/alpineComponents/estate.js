document.addEventListener('alpine:init', function() {
  ...
  //   turnOpen  — which turns the reader has opened to full size, by index.
  //               Cleared when the card changes, since the indices are the
  //               card's own.
  //   cardRow   — the session row the open card was built from, which is what
  //               a tap needs to reach the record and read the turn whole. Off
  //               the descriptor deliberately: rowCard is reactive and a row
  //               carries the whole summary.
  //   turnFull  — the record's own text for a turn the reader opened, keyed
  //               the same way, so closing and reopening one costs no read.
  let turnMarks = [], turnOpen = new Set(), turnFull = new Map();
  let cardRow = null, hereFrame = 0;
...
${opts.nested ? '' : `
    ...
    return {
          ...
          <div x-show="rowCard" x-cloak x-ref="rowCardBox" @click.outside="closeRowCard()"
            ...
            <template x-if="rowCard">
              <div class="flex flex-col">
                ...
                <div data-card-head
                     class="flex items-baseline gap-2 px-3 pt-2.5 pb-2 border-b border-base-200 sticky top-0 z-10 bg-base-100">
                  ...
                  <span x-show="rowCardHere?.asked" x-cloak
                        class="hidden sm:inline truncate max-w-[15rem] shrink text-[11px] text-base-content/30"
                        x-text="rowCardHere?.asked"
                        title="the ask this stretch of the transcript answers"></span>
                  <span x-show="rowCardHere" x-cloak
                        class="shrink-0 font-mono text-[10px] tabular-nums text-base-content/40"
                        x-text="rowCardHere ? rowCardHere.n + '/' + rowCardHere.total
                                  + (rowCardHere.ts ? ' · ' + rowCardHere.ts : '') : ''"
                        title="the turn under this bar, and when it was said"></span>
      ...
      async mountReplyCard(card){
        ...
        this.$nextTick(() => { this.pinRowCard(true); this.readRowCardHere(); });
      },
      // ── One turn, at whichever size it is currently showing ──────────────
      // Dense until the reader taps it, then the kit's standalone form: full
      // type, its own role head with the clock on it, and the coloured rail
      // dense mode drops. The rail is what makes an opened turn obvious
      // without adding chrome for it, and the clock is a fact the card has
      // nowhere else to put, since dense mode hangs it on a title.
      //
      // `md` is swapped for the record's own text where the tap has fetched it
      // (fullText), so opening a turn is not only bigger type: it is the rest
      // of the sentence. `dropped` goes with it, since the chip would then be
      // counting characters that are on screen.
      drawCardTurn(t){
        const key = this.turnKey(t);
        const open = turnOpen.has(key);
        const whole = open ? turnFull.get(key) : '';
        const full = whole ? { md: whole, dropped: 0 } : null;
        return window.chatRender.message({ ...t, ...full },
          { collapse: 0, dense: !open, tap: () => this.toggleCardTurn(t) });
      },
      turnKey(t){ return (t.at || []).join(':'); },
      // ── Open one turn, and only that one ─────────────────────────────────
      // Each turn keeps its own state, because the reader opening a turn is
      // reading THAT turn: expanding its neighbours would push it off screen,
      // and a card that opened everything at once is the session page, which
      // is one tap away and a different act.
      //
      // The redraw happens twice on a cut turn, deliberately. The first is
      // instant and is the size change alone, so the tap answers immediately;
      // the second lands when the record has been read and carries the rest of
      // the sentence. A spinner for the interval between them would be chrome
      // for a fetch that is usually already cached: the record is read on open
      // for any lean row, and `readRecord` memoises per session.
      async toggleCardTurn(t){
        const key = this.turnKey(t);
        if (turnOpen.has(key)) { turnOpen.delete(key); this.redrawCardTurn(key); return; }
        turnOpen.add(key);
        this.redrawCardTurn(key);
        this.$nextTick(() => this.scrollCardTurn(key));
        if (!this.turnHasMore(t) || turnFull.has(key)) return;
        const md = await this.fullTurnMd(t);
        // The reader may have closed it again, or moved to another card, while
        // the record was in flight.
        if (!md || !turnOpen.has(key) || !turnMarks.some(m => this.turnKey(m.turn) === key)) return;
        turnFull.set(key, md);
        this.redrawCardTurn(key);
        this.$nextTick(() => this.scrollCardTurn(key));
      },
      // Swap one turn's element for a fresh one at the new size, in place, and
      // keep the mark pointing at what is actually in the document: the scroll
      // reader measures offsetTop off these, so a stale node would report the
      // reader's position against geometry that is no longer on screen.
      redrawCardTurn(key){
        const mark = turnMarks.find(m => this.turnKey(m.turn) === key);
        if (!mark?.el?.isConnected) return;
        const next = this.drawCardTurn(mark.turn);
        next.className += ' ' + [...mark.el.classList].filter(c => /^mt-/.test(c)).join(' ');
        mark.el.replaceWith(next);
        mark.el = next;
        this.readRowCardHere();
      },
      // The opened turn goes to the TOP of the card rather than staying where
      // it was. A turn that grows in place pushes its own first line off the
      // top as often as not, so the tap would appear to scroll away from what
      // was tapped.
      scrollCardTurn(key){
      ...
      // Whether the record can be expected to hold more of this turn than the
      // card drew. A scroll-back turn says so itself, in the count the chip
      // renders. THE OPENING ASK DOES NOT, and that is the one case a
      // `dropped` test alone gets wrong: the row carries it as a hard
      // `slice(0, ASK_CHARS)` with no count beside it, so the most truncated
      // turn on the card is the one that looks whole. Its length against the
      // cap is the only evidence there is, and it is enough to be worth a
      // read; a turn that happens to end exactly on the cap costs one fetch
      // that returns the same sentence.
      turnHasMore(t){
        if (t.dropped) return true;
        const cap = window.RepoSessionsCache?.ASK_CHARS || 240;
        return (t.at || [])[0] === 'ask' && String(t.md || '').length >= cap;
      },
      // The turn as the RECORD holds it. The cache heads every scroll-back
      // turn at PROMPT_HEAD/TURN_HEAD and the opening ask at ASK_CHARS, so on
      // a cut turn this is the difference between two or three sentences and
      // the answer. The closing reply is carried whole already and the states
      // are capped only above 2,000 characters, so neither has a route here.
      //
      // INDEXED, not matched: `fullTurns` is the same list `priorTurns` heads,
      // so entry i of one is entry i of the other and no timestamp has to be
      // trusted to line them up.
      async fullTurnMd(t){
        const S = window.RepoSessionsCache;
        const [kind, i] = t.at || [];
        if (!cardRow || !S || !kind) return '';
        let rec;
        try { rec = await this.readRecord(cardRow); } catch { return ''; }
        if (kind === 'ask') return String(rec?.opening_ask || (rec?.prompts || [])[0]?.text || '');
        if (kind === 'turn') return String(S.fullTurns(rec).slice(-S.TURNS_KEPT)[i]?.md || '');
        return '';
      },
      ...
      readRowCardHere(){
        ...
        for (const m of turnMarks) {
          ...
          if (m.el.offsetTop <= y) cur = m; else break;
        }
        const t = cur.turn;
      ...
      resetCardTurns(){
        turnMarks = []; turnOpen.clear(); turnFull.clear();
        cardRow = null; this.rowCardHere = null;
      },
