document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    // Verdict styling for the per-card adoption read (lib/portable-align.js
    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div x-show="tab==='activity' || tab==='sessions'"
            ...
            <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 shrink-0" role="tablist">
              ...
              <button role="tab" @click="goSub('sessions')"
                ...
                <i class="ph ph-terminal-window text-lg"></i>Sessions
                <span x-show="authed && sessionRows.length" class="font-mono text-sm opacity-60"
                      x-text="sessionRows.length"></span></button>
            </div>
            <div class="grow"></div>
                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <button @click="openSession(row)"
                            ...
                            :title="'Open this session record'" x-text="row.id"></button>
                    <!-- The branches this session was sitting on, which is the
                         join to the Branches pane. Tapping one filters that
                         list to its repo and switches panes. -->
                    <template x-for="b in row.branches.slice(0, 2)" :key="b">
                      <button @click="showBranch(row, b)" :title="'Find ' + b + ' in Branches'"
                  ...
                  <p x-show="row.ask" class="text-base text-base-content/60 truncate mt-0.5"
                     :title="row.ask" x-text="row.ask"></p>
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <!-- The Claude session itself, when the record could name
                         it. Empty for a session that made no commits, because
                         the id is recovered from its own commit trailers. -->
                  ...
                  <!-- The opened record, inline. A takeover would need its own
                       swipe deck and back stack for a pane whose rows are read
                       one at a time; expanding in place costs one fetch and
                       keeps the list's position. -->
                  <template x-if="openSessionId === row.id">
                    <div class="mt-3 pt-3 border-t border-base-300/60">
                      <div x-show="sessionDetailLoading" class="flex justify-center py-6">
                        <span class="loading loading-dots loading-md opacity-30"></span>
                      </div>
                      <div x-show="sessionDetailErr" class="text-base text-error font-mono"
                           x-text="sessionDetailErr"></div>
                      <template x-if="sessionDetail && !sessionDetailLoading">
                        <div class="flex flex-col gap-3">
                          <!-- The conversation itself, as the paged deck
                               (session-render.js). This pane and that deck
                               answer different questions and both earn their
                               place: the sections below are a scan (what was
                               this about, what did it touch, what did it
                               fight), while the deck is the reading. Its
                               libraries load on the first tap rather than in
                               show-repo's boot chain, since most visits here
                               never open a record at all.

                               The old note here said a takeover would need its
                               own swipe deck and back stack. It does, and
                               swipe-deck.js is it: one history entry, so the
                               phone back button, Escape and the arrows all
                               dismiss, and the list keeps its position. -->
                          <button @click="openSessionDeck()" :disabled="deckLoading"
                                  class="btn btn-sm btn-primary gap-2 self-start">
                            <i class="ph text-base" :class="deckLoading ? 'ph-circle-notch animate-spin' : 'ph-cards-three'"></i>
                            <span x-text="deckLoading ? 'Loading…' : 'Read the conversation'"></span>
                          </button>
                          <div x-show="deckErr" class="text-base text-error font-mono" x-text="deckErr"></div>
                          <!-- What the user asked, in order. The record stores
                               the first 200 prompts at 400 characters each;
                               this is the substance a search hits on. -->
                          <div x-show="sessionDetail.prompts?.length">
                            <div class="text-sm uppercase tracking-wide text-base-content/40 mb-1">Asks</div>
                            <div class="flex flex-col gap-1">
                              <template x-for="(p, i) in sessionDetail.prompts" :key="i">
                                <p class="text-base text-base-content/70 pl-2 border-l-2 border-base-300"
                                   x-text="p.text"></p>
                              </template>
                            </div>
                          </div>
                          <!-- Files, busiest first, with the kind breakdown.
                               This is the per-session read of the same data the
                               Attention rollup aggregates across sessions. -->
                          <div x-show="detailFiles.length">
                            <div class="text-sm uppercase tracking-wide text-base-content/40 mb-1"
                                 x-text="'Files (' + sessionDetail.files_total + ')'"></div>
                            <div class="flex flex-col gap-0.5">
                              <template x-for="f in detailFiles" :key="f.path">
                                <div class="flex items-center gap-2 text-base font-mono">
                                  <span class="text-base-content/70 truncate" x-text="f.path"></span>
                                  <span class="text-base-content/40 shrink-0 tabular-nums" x-text="f.label"></span>
                                </div>
                              </template>
                            </div>
                          </div>
                          <!-- Failures, which are why call bodies are kept at
                               all: every failing call keeps its full body where
                               a succeeding one may not. -->
                          <div x-show="detailFailures.length">
                            <div class="text-sm uppercase tracking-wide text-warning/70 mb-1"
                                 x-text="'Failures (' + detailFailures.length + ')'"></div>
                            <div class="flex flex-col gap-1">
                              <template x-for="(c, i) in detailFailures" :key="i">
                                <div class="text-base font-mono">
                                  <div class="text-base-content/70 truncate" x-text="c.name + ': ' + (c.arg || '')"></div>
                                  <div class="text-warning/80 whitespace-pre-wrap break-words text-sm"
                                       x-text="(c.body || '').slice(0, 400)"></div>
                                </div>
                              </template>
                            </div>
                          </div>
                          <div class="text-sm text-base-content/40 font-mono"
                               x-text="detailFootnote"></div>
                        </div>
                      </template>
                    </div>
                  </template>
          ...
          <!-- ── Lists view ────────────────────────────────────────────────
               To-do over Jot, both at once. They were two of Activity's pills
               and they were never activity: a personal checklist and an idea
               pile are things you keep, not the estate moving. Combined they
               are one stop, and combining them is what makes the tab
               unnecessary rather than merely fewer: the reason to switch was
               to see the other one, and now both are on screen.
      ...
      // lib/repo-sessions-cache.js), one file read like the activity cache. The
      ...
      get tab(){
        ...
        return (v === 'activity' || v === 'sessions') ? v : 'repos';
      },
      ...
      // Open one session's full record, fetched from the store on demand. A
      // second tap closes it, so the row is a toggle and the list never grows a
      // back stack.
      async openSession(row){
        if (this.openSessionId === row.id){ this.openSessionId = ''; this.sessionDetail = null; return; }
        ...
        try {
          ...
          this.sessionDetail = JSON.parse((await reg.get(path)).text);
        } catch(e){
          this.sessionDetailErr = 'Could not read the record: ' + (e?.message || e);
        } finally { this.sessionDetailLoading = false; }
      },
      // The opened record as a paged conversation. The chain is pulled on the
      // first tap and deduped by the loader's registry afterwards, so a second
      // record opens with no fetch at all, and a visit that never opens one
      // pays nothing. proof.js backs chat-render's sandboxed Render frames;
      // order matters, since each file below reaches the one above it.
      deckLoading: false,
      deckErr: '',
      async openSessionDeck(){
        if (!this.sessionDetail || this.deckLoading) return;
        this.deckLoading = true; this.deckErr = '';
        try {
          if (!window.sessionRender){
            await gh.load('kits/proof.js');
            await gh.load('swipe-deck.js');
            await gh.load('chat-render.js');
            await gh.load('session-render.js');
          }
          await window.sessionRender.open(this.sessionDetail);
        } catch(e){
          this.deckErr = 'Could not open the conversation: ' + (e?.message || e);
        } finally { this.deckLoading = false; }
      },
      // The opened record's files, busiest first, with the per-kind breakdown
      // spelled out ("9 edit, 2 read") rather than reduced to a total: which
      // way a file was touched is the difference between working it and
      // consulting it.
      get detailFiles(){
        const f = this.sessionDetail?.files || {};
        return Object.entries(f)
          .map(([path, kinds]) => ({
            path,
            weight: window.RepoSessionsCache.fileWeight(kinds),
            label: Object.entries(kinds || {}).sort().map(([k, n]) => n + ' ' + k).join(', '),
          }))
          .sort((a, b) => (b.weight - a.weight) || a.path.localeCompare(b.path))
          .slice(0, 30);
      },
      get detailFailures(){
        return (this.sessionDetail?.calls || []).filter(c => !c.ok).slice(0, 20);
      },
      // What the record does NOT hold, said where the record is read. A schema-2
      // record has no files and a schema-1 record has no calls at all, so a pane
      // that just showed empty sections would read as "this session did nothing"
      // rather than "this was not captured then".
      get detailFootnote(){
        const d = this.sessionDetail;
        if (!d) return '';
        const bits = ['schema ' + (d.schema || 1)];
        // Each bit names something the record COULD NOT have held, so an empty
        // section reads as "not captured then" rather than "nothing happened".
        // The prose one matters most: through schema 3 the answering half of
        // every conversation was counted and discarded, so a record without it
        // is not a quiet session, it is half a session.
        if ((d.schema || 1) < 4) bits.push('predates assistant-prose capture');
        if ((d.schema || 1) < 3) bits.push('predates file capture');
        if ((d.schema || 1) < 2) bits.push('predates tool-call capture');
        if (d.prompts_stored < d.exchanges) bits.push(d.prompts_stored + ' of ' + d.exchanges + ' asks stored');
        if (d.replies_elided) bits.push(d.replies_elided + ' replies elided at the prose budget');
        if (d.bodies_elided) bits.push(d.bodies_elided + ' results elided at the record budget');
        return bits.join(' · ');
      },
      ...
      showBranch(row, branch){
        ...
        this.openRepoFilter = full ? full.repo : '';
      ...
      // .web-tools.json), graded by lib/portable-align.js, which is pure and
      ...
      // already loads (lib/repo-config-cache.js, state/configs.json), computed
