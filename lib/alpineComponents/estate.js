...
${opts.nested ? '' : `
                      ...
                      <span class="flex items-center gap-1 flex-wrap min-w-0">
                        ...
                        <template x-for="rt in branchRoutes(row).near" :key="rt.key">
                          <button @click.stop="openRoute(rt)"
                                  ...
                                  class="shrink-0 rounded-full px-2 py-0.5 text-sm bg-base-200/70 text-base-content/40 hover:text-base-content transition-colors"
                                  x-text="rt.label"></button>
                        </template>
                      </span>
                    </template>
    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div x-show="isActivityTab"
            ...
            <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 w-fit flex-wrap" role="tablist">
              <!-- Branches was a pill here until 2026-08-19. It is a LENS of
                   Sessions now: the same list, the same controls, one tap
                   inside the pane that already nests each session's branches.
                   ?view=activity still resolves, landing on that lens, so every
                   saved link and every openAbandoned jump keeps working. -->
            ...
            <template x-if="tab==='sessions' && sessionLens !== 'branches' && authed">
              <div class="flex items-center gap-2">
          ...
          <div x-show="isActivityTab" x-effect="landActivityAlias()"
               class="flex flex-col">
            ...
            <div x-show="authed" class="flex items-center gap-2 mb-3 flex-wrap">
              <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 w-fit flex-wrap" role="tablist">
                <template x-for="l in [
                    ...
                    { key: 'branches', label: 'Branches', icon: 'ph-git-branch', note: 'Every branch, freshest first, whatever session made it. The list the Branches tab used to be.' },
              ...
              <span x-show="sessionLens !== 'branches'" class="text-base text-base-content/40"
                    :title="sessionJoinNote" x-text="sessionJoinLabel"></span>
            </div>

          <!-- ── Branches lens (was the Activity sub-tab) ─────────────────
          ...
          <!-- The route chips on each row need the manifest and the open PRs'
            ...
            <div x-show="authed && sessionLens === 'branches'" class="flex-1 min-w-0">
            ...
            <div x-show="authed && sessionLens === 'list' && sessionsTitlesAt && sessionRows.length"
                 class="text-base text-base-content/45 mb-2"
            ...
            <div x-show="authed && sessionLens === 'list' && sessionNodes.length" class="flex flex-col gap-2">
              <template x-for="n in sessionNodes" :key="n.key">
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                     :class="n.row && n.row.failures ? 'border-warning bg-warning/10'
                ...
                <div x-data="{ row: n.row }">
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <!-- What the session is CALLED: the title from the export
                         ...
                         failure rail. -->
                    <span x-show="sessionLabel(row)"
                          class="text-base text-base-content/70 truncate min-w-0"
                          :title="sessionLabelNote(row)"
                          x-text="sessionLabel(row)"></span>
                ...
                <div class="rounded-lg border border-base-300 bg-base-100 flex flex-col overflow-hidden"
                  ...
                  <div class="flex items-start gap-2">
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="font-mono text-sm text-base-content/40 truncate" x-text="g.repo"></div>
                    </div>
                          ...
                          <template x-for="it in g.items" :key="it.id">
                            <div class="flex items-start gap-2 px-2 py-1 rounded-lg hover:bg-base-200/60 group min-w-0">
                              ...
                              <div class="flex-1 min-w-0">
                                ...
                                <p x-show="it.note" class="text-sm text-base-content/40 truncate" x-text="it.note"></p>
                              </div>
      ...
      // `activity` is an ALIAS now, not a pane. The Branches pill retired on
      // 2026-08-19 and its list became a lens of Sessions, so a saved
      // ?view=activity link (or openAbandoned's jump) lands on that lens and
      // the address is rewritten to the one that describes it. The route key
      // stays alive on purpose: an address a reader has bookmarked is a
      // contract, and answering it costs less than breaking it.
      landActivityAlias(){
        if (this.tab !== 'activity') return;
        this.sessionLens = 'branches';
        this.goSub('sessions');
      },
