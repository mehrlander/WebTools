  ...
  Alpine.data('estate', function() {
    ...
    return {
      ...
      template: `
        <div>
          ...
          <div x-show="tab==='stage'">
            <!-- ── THE BENCH ──────────────────────────────────────────────
                 The working surface, fixed at the top of the view and always
                 open. It is not a card in the list and no saved card ever
                 becomes it: THE BENCH DOES NOT MOVE. That is the whole
                 correction. It used to be the first card on the shelf, opened
                 by a pencil and mounted under whichever surface was being
                 edited, which cost three things at once: the word "stage"
                 disappeared from the UI, the workspace had no fixed place, and
                 it took its name from its own contents ("README.md +2"), so
                 there was nothing on screen that was recognizably the Stage.
                 Now it is titled, permanent, and needs no gesture to reach.
                 A saved surface is LOADED onto it rather than becoming it. -->
            <section class="mb-8">
              <div class="flex items-baseline gap-2 mb-2">
                <h2 class="text-lg font-semibold text-primary">Stage</h2>
                <!-- Where the set came from, when it came from somewhere. This
                     is what the moving bench used to say by sitting under a
                     card; said in words, it survives the bench holding still. -->
                <span x-show="benchOriginName" class="text-base text-base-content/50 truncate">
                  from <span class="font-mono" x-text="benchOriginName"></span>
                </span>
                <div class="grow"></div>
                <button x-show="benchOrigin" @click="detachBench()"
                  ...
                  <i class="ph ph-link-break text-base leading-none"></i></button>
                <span class="badge badge-ghost badge-sm font-mono">working</span>
              </div>
              <!-- Mounted on first visit to the Stage, then kept: x-if rather
                   ...
                   and NOT re-evaluated on tab change, so leaving the Stage and
                   coming back does not reset the lens, the diff pair, or the
                   built bundle. Same lazy-mount idiom as the estate itself. -->
            ...
            <!-- ── THE SHELF ──────────────────────────────────────────────
                 Headed unconditionally, so the signed-out reader gets the
                 label its message belongs to rather than one stranded line
                 under the bench with nothing to attach it to. -->
            <div class="flex items-center gap-2 mb-4">
              <div class="text-base font-mono uppercase tracking-widest text-base-content/40">Saved</div>
              <div class="grow"></div>
      ...
      loadOntoStage(s){
        ...
        document.querySelector('[data-pane="estate"]')?.scrollTo?.({ top: 0, behavior: 'smooth' });
        if (skipped.length)
      ...
      // Which estate view is showing, from the shell (Repos | Stage | To-do |
      // Jots | Activity). 'surfaces' is the Stage's alias, kept because old
      // links carry it; both resolve to the one key this component switches on.
      get tab(){
        const v = window.__shell?.view;
        if (v === 'stage' || v === 'surfaces') return 'stage';
        return (v === 'activity' || v === 'todo' || v === 'jots') ? v : 'repos';
      },
