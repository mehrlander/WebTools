    ...
    return {
      ...
      template: `
        <div>
          ...
          <!-- ── Branch detail: a full-viewport takeover for one Open row ──
               ...
               Navigation: swipe on the header or the edge strips (the iframe
               owns touches over its own body, so the strips are the takeover's
               swipe surface), arrow keys and Escape on a keyboard, chevrons
               everywhere. Stepping swaps the iframe src with a per-branch
               query, because two addresses differing only in fragment would be
               a hash-only change the iframe never navigates on. -->
          <div x-show="detail" x-cloak x-transition.opacity.duration.150ms
               class="fixed inset-0 z-[70] bg-base-100 flex flex-col"
               @keydown.window="detailKeys($event)">
            <div class="h-12 shrink-0 flex items-center gap-1.5 px-2 border-b border-base-300"
                 @touchstart.passive="dTouchStart($event)" @touchend.passive="dTouchEnd($event)">
            ...
            <div class="relative flex-1 min-h-0">
              <template x-if="detail">
                <iframe :src="detailUrl" class="absolute inset-0 w-full h-full border-0 bg-base-100"
                        sandbox="allow-scripts allow-same-origin allow-popups allow-downloads"></iframe>
              </template>
              <div class="absolute inset-y-0 left-0 w-6"
                   @touchstart.passive="dTouchStart($event)" @touchend.passive="dTouchEnd($event)"></div>
      ...
      get branchMenuItems(){
        ...
        return [
          ...
          { key: 'copyCompare', label: 'Copy compare link', icon: 'ph-link' },
        ].filter(Boolean);
      },
      runBranchMenu(key){
        const r = this.menuBranch;
        ...
        if (key === 'copyCompare') return this.copyText(cmp, 'Compare link copied');
      },
      ...
      detailStep(d){
        ...
        if (i !== this.detail.i) this.detail = { ...this.detail, i };
      },
      detailKeys(e){
        if (!this.detail) return;
      ...
      // Touch swipe for the header and the edge strips. A horizontal move of
      // 48px+ that is clearly more horizontal than vertical steps; anything
      // else is a scroll or a tap and is left alone.
      _dT: null,
      dTouchStart(e){ const t = e.touches && e.touches[0]; if (t) this._dT = { x: t.clientX, y: t.clientY }; },
      dTouchEnd(e){
        const s = this._dT, t = e.changedTouches && e.changedTouches[0];
        this._dT = null;
        if (!s || !t || !this.detail) return;
        const dx = t.clientX - s.x, dy = t.clientY - s.y;
        if (Math.abs(dx) > 48 && Math.abs(dx) > Math.abs(dy) * 1.5) this.detailStep(dx < 0 ? 1 : -1);
      },
