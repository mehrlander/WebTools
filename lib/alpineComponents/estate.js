document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
                    ...
                    <div class="card bg-base-100 border border-base-300 shadow-sm hover:border-primary/40 transition-colors w-full">
                      <div class="card-body p-4 gap-1.5">
                        <div class="flex items-center gap-1.5">
                          ...
                          <!-- Public jsDelivr browse retired from the card: it is
                               a signed-out affordance (the Repos-view Public
                               browse button and the config dialog's Links tab
                               still carry it), and on a card with a token it is
                               noise, more so for a private repo where it can't
                               work. GitHub jump's tooltip names the repo. -->
                          <a :href="'https://github.com/' + face(e).repo" target="_blank" @click.stop
                             ...
                             :title="face(e).repo"><i class="ph ph-github-logo text-base leading-none"></i></a>
                        </div>
                        ...
                        <!-- PROJECTS: the repo's workspaces, hung off the same
                             1 px rule the two sidebar lists use, so the three
                             places a project is listed read alike. Names only
                             here: a card is a summary, and the board and the
                             GitHub folder are one tap further in, where the
                             sidebar row carries them. -->
                        <div x-show="face(e).projects && face(e).projects.length"
                             class="flex flex-col mt-0.5 ml-1 pl-2 border-l border-base-300">
                          <template x-for="p in (face(e).projects || [])" :key="p.path">
                            <button @click="openProjectFrom(face(e), p)" :title="p.path"
                                    class="flex min-w-0 text-left text-sm text-base-content/60
      ...
      async load(){
        ...
        this.entries = members.map(name => {
          ...
          return {
            ...
            // The repo's workspaces, normalized by the shell so the card, the
            // estate sidebar, and the repo's own sidebar all agree on what a
            // project row is (label, board path). The cfg is passed rather than
            // looked up, because on a cold estate this map came from a live
            // scan and the shell's cache may still be empty.
            projects: window.__shell?.repoProjects?.(name, cfg) || [],
      ...
      // The leading half of an Open row's lifespan: how long ago the branch's
      // first commit landed, or '' when there is nothing to say. Empty when the
      // crawl could not know the start (a branch with no merge base, or a
      // compare past GitHub's 250-commit cap; see BranchSurvey.firstCommitDate)
      // and when it rounds to the same label as the tip, since "2h → 2h" is
      // noise where "2h" is the answer.
      branchStart(row){
        if (!row.first) return '';
        const s = this.agoShort(row.first);
        return s === this.agoShort(row.date) ? '' : s;
      },
      branchSpanTitle(row){
        const latest = 'latest ' + this.agoOf(row.date);
        return row.first ? 'started ' + this.agoOf(row.first) + ', ' + latest : latest;
      },
      ...
      // A card's project row opens the repo's PROJECT VIEW, the same destination
      // the two sidebar lists reach; the shell switches the repo first.
      async openProjectFrom(en, p){
        await window.__shell?.openProject?.(en.repo, p);
      },
      ...
      itemPeek(it){
        const r = this.itemRef(it);
        if (r && !r.dir) return window.SourcePeek?.addr(r.repo, r.ref, r.path) || null;
        if (!r && this.isEmbed(it) && it.repo && it.path)
          return window.SourcePeek?.addr(it.repo, it.ref || 'main', it.path) || null;
        return null;
      },
      // An embed's title opens the same render full screen (the routed
      // toss-render URL); a url item opens its external link, as before.
      itemExt(it){
