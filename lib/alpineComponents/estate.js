document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <!-- FILES: the route the row was missing. The branch name
                         ...
                         ONE number, and the same one on every row: how many
                         files this branch changed. It is free, since the crawl
                         compares every open PR head against its default for the
                         ahead/behind pair and that response lists the files, and
                         a surveyed branch answers from its own touched-path set.
                         A row with stranded content adds ONE more thing, the
                         missing count in amber, which opens the pane already
                         ...
                         the branch itself. -->
                    <span class="flex items-center gap-2 font-mono tabular-nums shrink-0">
                      <button @click.stop="openBranchFiles(row, '')" :title="filesTitle(row)"
                        ...
                        <span x-text="fileCount(row)" class="hover:underline underline-offset-2"></span>
                      </button>
      ...
      //
      // A no-merge-base row keeps its count rather than blanking: there the
      // compare fell back to the branch's recent history, so the number spans
      // more than the branch, and the row's asterisk says exactly that about
      // every number on it. Blanking made the one row that most needs a route
      // into its files the one row whose glyph stood bare.
      fileCount(row){
        if (!row) return null;
        return row.nFiles ?? (row.nUnique || null);
      },
      // The files control's hover, and the only place the full verdict is
      // stated now that the row carries a count instead of a ratio. Where the
      // survey reached the branch that is the three-way partition; otherwise it
      // is the plain count.
      filesTitle(row){
