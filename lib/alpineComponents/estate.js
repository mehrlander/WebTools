  ...
  Alpine.data('estate', function() {
    ...
    const branchRowBody = (opts = {}) => `
                  ...
                  <div class="flex items-start gap-x-3 mt-2 text-base">
                    ...
                    ` : ''}
                    <!-- ── The row's actions, leading the control line ───────
                         ...
                         FIRST on the line, beside the GitHub menu and the
                         Stage: everything here that ACTS now sits together at
                         the head, and the counts and route chips wrap after
                         them as they always did. Past the chips these orphaned
            ...
            <div x-show="authed && sessionLens === 'list' && sessionNodes.length" class="flex flex-col gap-2">
              <template x-for="n in sessionNodes" :key="n.key">
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                     :class="sessionAccent(n)" :title="sessionOutcomeNote(n)">
                ...
                <div x-data="{ row: n.row }">
                  ...
                  <!-- The ask, and it is PROSE. It was a button opening the
                       ...
                       button are for. cursor-help says as much, and a row whose
                       cache predates the reply field gets no handler and no
                       pointer, only its ask as a plain title. -->
                  <p x-show="row.ask"
                     @click.stop="row.reply && openSessionCard(row, 'reply', $event)"
                     @mouseenter="row.reply && hoverSessionCard(row, 'reply', $event)"
                     @mouseleave="hoverLeaveRowCard()"
                     class="text-base text-base-content/60 truncate mt-0.5"
                     :class="row.reply && 'cursor-help hover:text-base-content/80 transition-colors'"
                     :title="row.reply ? '' : row.ask" x-text="row.ask"></p>
                  <div x-show="sessionDetailErr && openSessionId === row.id"
                       class="text-base text-error font-mono mt-1" x-text="sessionDetailErr"></div>
                  <!-- The token headline keeps its ml-auto and takes a line of its own
                       at 430px, now that three action glyphs share this line.
                       gap-x-3 was tried to win the space back and did not: the run
                       is about 40px too wide either way, so the tightening was
                       reverted rather than kept as a restyle that solved nothing.
                       Right-aligned on its own line reads as a trailing figure,
                       and the identity row got the width back where it mattered,
                       which is the session title. -->
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    <!-- ── The row's actions, leading the control line ───────
                         ...
                         FIRST on the line, not last, and the tail was tried and
                         rendered. Past the counts they sat behind the Claude
                         mark and the ml-auto token headline, which at 430px
                         pushed the tokens onto a line of their own and left the
                         three glyphs orphaned on it. Leading, the primary
                         action leads, and it is the COUNTS that wrap, which is
                         what this line already did before any of this. -->
                    ...
                    <button x-show="row.tokens" @click.stop="openSessionCard(row, 'tokens', $event)"
                            ...
                            @mouseleave="hoverLeaveRowCard()"
                            class="ml-auto flex items-center gap-2.5 font-mono tabular-nums text-base-content/40 hover:text-primary transition-colors"
              ...
              <div class="flex flex-col">
                ...
                <template x-if="rowCard.kind === 'prose'">
                  <div class="flex flex-col gap-2 px-3 py-2.5">
                    ...
                    <p x-show="rowCard.ask" class="text-xs text-base-content/40 leading-snug"
                       x-text="rowCard.ask"></p>
                    <p class="text-xs leading-relaxed whitespace-pre-line text-base-content/80"
      ...
      openSessionCard(row, cls, ev){
        ...
        if (cls === 'turns') {
          ...
          this.rowCard = {
            ...
            cls, icon: spec.icon, label: this.replyLabel(row),
            text: String(row.reply || '').trim(),
            cut: row.replyCut === 'cut',
            ask: String(row.ask || ''),
            shape: { exts: [], dirs: [] },
          };
          this.rowCardAt = window.__shell?.anchorMenu?.(ev, 9, { width: this.ROW_CARD_W, align: 'left' }) || null;
