document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div class="flex-1 min-w-0" :class="tab==='chats' ? '' : 'hidden'"
            ...
            <div x-show="authed && chatsErr" class="rounded-xl bg-base-200/40 p-4 text-base max-w-lg">
              <span class="text-error font-mono" x-text="chatsErr"></span>
            </div>
      ...
      async loadChats(){
        if (!this.authed || this.chatsBusy || this.chatFrontier) return;
        if (!window.chatArchive) {
          try { await window.gh.load('kits/chat-archive.js'); } catch { return; }
        }
        this.chatsBusy = true;
        this.chatsErr = '';
        try {
          ...
          this.chatsErr = 'Could not read the archive: ' + (e?.message || e);
