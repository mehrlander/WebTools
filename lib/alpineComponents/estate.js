...
${opts.nested ? '' : `
    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div x-show="rowCard" x-cloak x-ref="rowCardBox" @click.outside="closeRowCard()"
               ...
               class="fixed z-[56] max-w-[calc(100vw-1rem)] max-h-[60vh] overflow-y-auto
                      rounded-lg border border-base-300 bg-base-100 shadow-lg"
               :class="rowCard?.kind === 'prose' ? 'w-[35rem]' : 'w-96'"
               :style="rowCardStyle">
              ...
              <div class="flex flex-col">
                ...
                <template x-if="rowCard.kind === 'prose'">
                  ...
                  <div class="px-3 py-2.5">
                    <div x-ref="replyBody" class="flex flex-col gap-3"
      ...
      //
      // `collapse: 0` turns off chat-render's 460px height clamp. Everything
      // here is already bounded by the CACHE (ASK_CHARS, TURN_HEAD, then
      // REPLY_CHARS), so a second bound would only cut a short turn again and
      // offer to expand what is not there. The card's own max-h scrolls.
      replyTurns(row){
        const out = [];
        const ask = String(row?.ask || '').trim();
        if (ask) out.push({ role: 'user', md: ask, ts: row.askAt || '' });
        for (const [k, t, ts] of row?.turns || []) {
          out.push({ role: k === 'u' ? 'user' : 'assistant', md: String(t || ''), ts: ts || '' });
        }
        ...
        if (reply) {
          out.push({
            ...
            // so where the prose cannot.
            label: 'Assistant · ' + this.replyLabel(row),
          });
        }
        return out;
      },
      ...
      async mountReplyCard(card){
        ...
        for (const t of card.turns) host.append(window.chatRender.message(t, { collapse: 0 }));
        if (card.cut) host.append(this._replyFoot(
          'trimmed at ' + (window.RepoSessionsCache?.REPLY_CHARS || 600)
          + ' characters. The whole reply is in the session, on its last card.'));
      ...
      _replyFoot(text){
        ...
        el.className = 'text-[10px] text-base-content/40 leading-snug';
        el.textContent = '… ' + text;
        return el;
      },
      ...
      openSessionCard(row, cls, ev){
        ...
        if (cls === 'turns') {
          ...
          this.rowCardAt = window.__shell?.anchorMenu?.(ev, 9, { width: this.REPLY_CARD_W, align: 'left' }) || null;
