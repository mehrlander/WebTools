document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div x-show="isActivityTab"
            ...
            <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 w-fit flex-wrap" role="tablist">
              <button role="tab" @click="goSub('activity')"
                ...
                <i class="ph ph-tree-structure text-lg"></i>Activity</button>
          ...
          <div class="flex-1 min-w-0" :class="tab==='activity' ? '' : 'hidden'"
            ...
            <!-- ── Orientation: the same edges, read from either end ─────────
                 A session is the ACT and a branch is what it left behind, and
                 the relation between them is a fan rather than a mesh: 87 of
                 132 recorded sessions hold more than one branch, while 346 of
                 390 branch rows hold exactly one session. So the default
                 nests branches INSIDE the session that made them, and the
                 branch-first list this pane has always been is one tap away.
                 Both readings are built from the same two caches; neither is a
                 second copy of anything.

                 It is a switch rather than a second sub-tab because the two
                 answer one question at different grains. What the branch-first
                 list does that nesting cannot is sort by branch recency, so a
                 push an hour ago leads; that is why it stays. -->
            <div x-show="authed" class="flex items-center gap-2 mb-3 flex-wrap">
              <div class="flex items-center gap-0.5 rounded-lg bg-base-200/60 p-0.5 w-fit" role="tablist">
                <button role="tab" @click="actDir = 'session'"
                        title="Sessions, with the branches each one left behind nested under it"
                        class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors"
                        :class="actDir === 'session' ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60 hover:text-base-content'">
                  <i class="ph ph-terminal-window text-lg"></i>Sessions</button>
                <button role="tab" @click="actDir = 'branch'"
                        title="Branches, freshest first: what is in flight right now"
                        class="flex items-center gap-1.5 px-3 py-1.5 rounded-md text-base font-medium transition-colors"
                        :class="actDir === 'branch' ? 'bg-base-100 text-primary shadow-sm' : 'text-base-content/60 hover:text-base-content'">
                  <i class="ph ph-git-branch text-lg"></i>Branches</button>
              </div>
              <span x-show="actDir === 'session'" class="text-base text-base-content/45"
                    :title="actJoinNote" x-text="actJoinLabel"></span>
            </div>

            <!-- ── Sessions, branches nested ─────────────────────────────── -->
            <div x-show="authed && actDir === 'session'" class="flex flex-col">
              <div class="flex items-center gap-1.5 mb-2 -mx-1 px-1 pb-0.5 overflow-x-auto">
                <template x-for="sc in actScopes" :key="sc.key">
                  <button @click="actScope = sc.key" :title="sc.note"
                          class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-base font-medium transition-colors"
                          :class="actScope === sc.key ? 'bg-primary/10 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
                    <i class="ph text-lg" :class="sc.icon"></i>
                    <span x-text="sc.label"></span>
                    <span class="font-mono opacity-60" x-text="sc.count"></span></button>
                </template>
              </div>

              <div class="flex flex-col gap-2">
                <template x-for="n in actRows" :key="n.key">
                  <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                    ...
                    <div class="flex items-center gap-2 min-w-0">
                      ...
                      <!-- A recorded session opens its CONVERSATION, exactly as
                           the Sessions pane does; a stub has no record to open,
                           so its id links the harness session instead. -->
                      <button x-show="n.kind === 'record'" @click="openSession(n.row)"
                              class="font-mono text-base font-semibold hover:text-primary transition-colors text-left shrink-0"
                              title="Read this session as a conversation" x-text="n.id"></button>
                      <a x-show="n.kind === 'stub'" :href="n.url" target="_blank" rel="noopener"
                         class="font-mono text-base text-base-content/45 hover:text-primary transition-colors shrink-0"
                         x-text="n.id"></a>
                      <!-- The stub is what lets this list reach past the
                           recorder. A branch's Claude-Session commit trailer
                           names its session whether or not a record was ever
                           written, so a branch older than the store still
                           nests under the act that made it, with an honest
                           badge saying there is nothing to read. -->
                      <span x-show="n.kind === 'stub'"
                            title="Named by a branch's Claude-Session commit trailer; no record in the sessions store"
                            class="font-mono text-xs text-base-content/35 shrink-0">no&nbsp;record</span>
                      <div class="grow"></div>
                      <span x-show="n.row && n.row.mins" class="text-base text-base-content/50 shrink-0 tabular-nums"
                            x-text="n.row ? durLabel(n.row.mins) : ''"></span>
                    </div>

                    <button x-show="n.row && n.row.ask" @click="openSession(n.row)"
                            class="block w-full text-left text-base text-base-content/60 truncate mt-0.5 hover:text-primary transition-colors"
                            :title="n.row ? n.row.ask : ''">
                      <i x-show="sessionDetailLoading && openSessionId === n.id"
                         class="ph ph-circle-notch animate-spin mr-1"></i><span x-text="n.row ? n.row.ask : ''"></span></button>

                    <!-- The branches this session left behind, indented behind
                         a hairline: the nesting IS the claim, that a branch is
                         not a peer of the act that made it. Each row carries
                         what a branch row carries at this width, and the whole
                         branch-first row is one tap away on the switch. -->
                    <div x-show="n.children.length" class="mt-2 pl-2 border-l border-base-300/70 flex flex-col gap-1">
                      <template x-for="b in n.children" :key="b.repo + '/' + b.name">
                        <div class="flex items-center gap-2 min-w-0 text-base">
                          ...
                          <a :href="branchPageFor(b.repo, b.name)" target="_blank" rel="noopener"
                             :title="'Open ' + b.name" class="font-mono truncate min-w-0 hover:text-primary transition-colors"
                             x-text="b.name"></a>
                          <!-- The one branch in 390 that carries two sessions
                               appears under both, which is the whole cost of
                               nesting a many-to-many as a tree, and it is
                          ...
                          <template x-if="rowPR(b)">
                            <a :href="prUrl(b.repo, rowPR(b).number)" target="_blank" rel="noopener"
                               :title="rowPR(b).title" class="font-mono text-base shrink-0 hover:underline underline-offset-2"
                    ...
                    <p x-show="!n.children.length" class="text-base text-base-content/30 mt-1">
                      no branch in the crawl's window</p>
                  </div>
                </template>
              </div>

              <!-- The rows the tree cannot hold, named rather than dropped. A
                   branch whose commits carry no session trailer and whose name
                   matches no record has nothing to nest under, and that is a
                   gap in attribution, not evidence that no session made it. -->
              <div x-show="actScope === 'all' && actTree.orphans.length"
                   class="rounded-lg bg-base-200/40 p-3 mt-2">
                <div class="flex items-center gap-2 mb-1">
                  <i class="ph ph-question text-base-content/50 text-lg"></i>
                  <span class="text-base font-medium"
                        x-text="'Unattributed branches (' + actTree.orphans.length + ')'"></span>
                </div>
                <p class="text-base text-base-content/50 mb-2">
                  No commit trailer and no matching record: unattributed, not sessionless.</p>
                <div class="flex flex-col gap-1">
                  <template x-for="b in actTree.orphans" :key="b.repo + '/' + b.name">
                    <div class="flex items-center gap-2 min-w-0 text-base">
            ...
            <!-- ── Branches, freshest first (this pane's original list) ───── -->
            <div x-show="actDir === 'branch'" class="flex flex-col">
      ...
      // ── The unified reading: sessions, with their branches nested ────────
      // Two caches, one tree. The join runs on TWO keys and needs both: a
      // record carries the harness session URL (`agent`) only since 2026-08-07,
      // so 44 of 142 records can be reached from a branch by nothing but the
      // branch NAME, and 75 branch rows are placed by that fallback alone.
      //
      ...
      // exists, so the list reaches every branch the trailers cover rather than
      // only the weeks the recorder has been running. What is left over is
      // genuinely unattributed and is shown as such.
      //
      ...
      // their row counts) can repeat across two genuinely different caches.
      // A list that silently shows the previous crawl is a worse bug than any
      ...
      ACT_SCOPES: [
        { key: 'live', label: 'Live', icon: 'ph-git-pull-request',
          note: 'Sessions holding a branch whose pull request is still open.' },
        { key: 'all', label: 'All', icon: 'ph-list-bullets',
          note: 'Every session the two caches can name, recorded or not.' },
        { key: 'record', label: 'Recorded', icon: 'ph-book-open-text',
          note: 'Sessions with a record in the store, so there is a conversation to read.' },
        { key: 'stub', label: 'No record', icon: 'ph-git-commit',
          note: 'Sessions known only from a branch commit trailer: older than the recorder, or never recorded.' },
        { key: 'barren', label: 'No branch', icon: 'ph-circle-dashed',
          note: 'Recorded sessions whose branches are outside the branch crawl s window, or that committed nothing.' },
      ],
      inActScope(n, scope){
        if (scope === 'all') return true;
        if (scope === 'record') return n.kind === 'record';
        if (scope === 'stub') return n.kind === 'stub';
        if (scope === 'barren') return n.kind === 'record' && !n.children.length;
        if (scope === 'live') return n.children.some(b => !!b.pr);
        return true;
      },
      get actScopes(){
        const all = this.actTree.nodes;
        return this.ACT_SCOPES.map(sc => ({ ...sc, count: all.filter(n => this.inActScope(n, sc.key)).length }));
      },
      get actRows(){
        return this.actTree.nodes.filter(n => this.inActScope(n, this.actScope));
      },
      // What the join reached, on the pane rather than in a doc: a tree that
      // cannot place every branch should say so where it is read.
      get actJoinLabel(){
        const st = this.actTree.stats;
      ...
      get actJoinNote(){
        ...
        return st.viaAgent + ' branches reach a session record through the record s own session URL, '
             ...
             + st.orphan + ' reach no session at all.';
      },
