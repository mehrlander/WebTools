                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <span class="flex items-center gap-3 font-mono tabular-nums shrink-0">
                      <button @click.stop="openBranchFiles(row, '')" :title="filesTitle(row)"
                        ...
                        <!-- New files as their OWN pair, on the ahead/behind
                             idiom this row already uses: a glyph and a number,
                             no word. GREEN, and green now means exactly this
                             everywhere on the row, which is what the colour had
                             to be freed up to say. It used to tint the whole
                             control when the survey found nothing missing, a
                             signal the absent missing count and the Landed chip
                             were already carrying twice over; spending it here
                             is what lets a file-plus glyph read as a different
                             thing from a files glyph at eighteen pixels. The
                             row's palette is now one idea: neutral changed,
                             green added, amber stranded. -->
                        <template x-if="fileParts(row)?.added">
                          <span class="flex items-center gap-1 ml-2 text-success">
