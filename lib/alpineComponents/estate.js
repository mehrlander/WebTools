document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    const ACTIVITY_TABS = ['activity', 'sessions', 'guides', 'chats', 'routes', 'state'];
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <!-- ── State view (Activity sub-tab) ─────────────────────────────
               ...
               refreshes it. In DOM order where it sits in the pill row, after
               Guides, so reading the panes in the file reads them in the order
               the row offers them.
