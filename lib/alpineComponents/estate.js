document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
      ...
      // The shell's one progress channel, this pane's slot. The verb and the
      // unit ride with the numbers (the crawl names its own phase, since only
      // it knows whether it is on the quick pass or the survey), so nothing
      // here decodes a state it cannot see.
      get activityProgress(){ return window.__shell?.crawlProgress?.activity || null; },
      ...
      get activityProgressLabel(){
        ...
        if (!p) return '';
        return p.total ? `${p.verb} · ${p.done} of ${p.total} ${p.unit}` : p.verb;
      },
