document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <!-- FILES: the route the row was missing. The branch name
                         ...
                         Bare, it is the COUNT, and the count is free: the crawl
                         compares every open PR head against its default for the
                         ahead/behind pair, and that response lists the files.
                         Where the survey reached the branch it grows into the
                         VERDICT, of the paths this branch touched how many hold
                         content the default branch has now, and its missing half
                         opens the pane already filtered to them.

                         A landed row shows 6/6 and is safe to delete; a stranded
                         row shows what is still only here. The word "landed"
                         rides the ratio because a bare 28/80 says nothing about
                         which direction is good, and the third count (what
                         differs) is in the title and in the pane's own strip.
                         Until 2026-08-18 this was inert text whose tooltip
                         pasted up to twelve missing paths under a sentence
                         describing the paths that were present.

                         No number at all on a no-merge-base row: there the
                         compare fell back to the branch's recent history, so a
                         count would span more than the branch and the pane it
                         opens has no diff to render. The asterisk says so. -->
                    <span class="flex items-center gap-1 font-mono tabular-nums shrink-0">
                      <button @click.stop="openBranchFiles(row, '')" :title="filesTitle(row)"
                              class="flex items-center gap-1 hover:text-primary transition-colors"
                              :class="row.nUnique && !row.nMissing ? 'text-success' : 'text-base-content/60'">
                        <i class="ph ph-files text-lg opacity-60"></i>
                        <span x-show="row.nUnique" class="hover:underline underline-offset-2"><span
                            x-text="row.nLanded + '/' + row.nUnique"></span><span
                            class="hidden sm:inline font-sans font-normal opacity-70"> landed</span></span>
                        <span x-show="!row.nUnique" x-text="fileCount(row)"
        ...
        for (const [repo, e] of Object.entries(this.activity)){
          ...
          for (const b of (e.survey?.branches || [])){
            ...
            out.push({ repo, def, name: b.name, date: b.date || '', subject: b.subject || '', pr,
                       ...
                       sha: b.sha || '',
                       // How many files the branch changed, off the PR compare
                       // the crawl runs for ahead/behind. Only a PR row has one;
                       // a surveyed row answers from its own unique-path set,
                       // which is the same count read another way (fileCount).
                       nFiles: pr?.nFiles ?? null,
      ...
      // How many files this branch changed, when something already knows, and
      // null when nothing does. The glyph alone is an honest "open the files";
      // a 0 would be a claim, and the survey's own rule holds here too, that
      // "not measured" and "measured zero" are different answers.
      //
      // It costs no call. The crawl compares every open PR head against its
      // default for the ahead/behind pair, and that response lists the files;
      // a surveyed row's nUnique is the same count read off the same kind of
      // response. GitHub caps a compare's file list at 300, so a sweeping
      // branch reports a floor.
      //
      // Null on a no-merge-base row, where the compare fell back to the
      // branch's recent history: the number would span more than the branch,
      // and the pane it opens has no diff to render anyway.
      fileCount(row){
        if (!row || row.noBase) return null;
        return row.nFiles ?? (row.nUnique || null);
      },
      // The files control's hover. It is the verdict where there is one, since
      // that says strictly more, and the plain count otherwise.
      filesTitle(row){
        if (row?.nUnique) return this.verdictTitle(row);
        const n = this.fileCount(row);
        return (n ? n + (n === 1 ? ' file' : ' files') + ' changed against ' + row.def + '. ' : '')
