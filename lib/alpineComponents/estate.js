    ...
    return {
      ...
      async mountReplyCard(card){
        ...
        for (const t of card.turns) {
          ...
          // rather than by a uniform gap; see _stateRule. Everything else keeps
          // the exchange rhythm below.
          if (t.gap !== null && t.gap !== undefined) host.append(this._stateRule(t.gap));
          const msg = window.chatRender.message(t, { collapse: 0, dense: true });
          if (prev && (t.gap === null || t.gap === undefined))
            msg.className += t.role === 'user' ? ' mt-3.5' : ' mt-1.5';
          prev = t.role;
          host.append(msg);
        }
      ...
      // A hairline carrying the number of user prompts in the interval, which
      // is the one fact that tells two identical pairs of glyphs apart: a
      // session that closed twice inside one turn and a session that closed,
      // was answered, and closed again read the same otherwise.
      //
      // LABELLED ONLY WHERE IT SAYS SOMETHING. 73% of the store's 2,411
      // consecutive pairs sit one prompt apart, which is the ordinary rhythm,
      // so a "1 prompt" on three dividers in four is the page-of-green-rails
      // failure this pane has already made once: the common case loudest and
      // the list saying nothing. It gets a bare rule. The two informative
      // cases speak: 15% at zero, where nobody spoke and the session closed
      // twice in one breath, and 12% at two or more, where a stretch ran long.
      _stateRule(n){
        ...
        if (n !== 1) {
          ...
          lab.className = 'shrink-0 font-mono text-[9.5px] uppercase tracking-widest '
            + (n === 0 ? 'text-base-content/40' : 'text-base-content/50');
          lab.textContent = n === 0 ? 'same turn' : n + ' prompts';
          lab.title = n === 0
            ? 'No user turn between these two: the session closed twice inside one exchange.'
