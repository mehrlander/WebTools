document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    const ACTIVITY_TABS = ['activity', 'sessions', 'guides', 'chats'];
    ...
    const agePill = (item, ago, busy, busyLabel, what) => `
    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div x-show="isActivityTab"
            ...
            <template x-if="tab==='activity' && authed">
              <div class="flex items-center gap-2">
                ...
                ${agePill('activity', 'activityGeneratedAt', 'activityBusy', 'Crawling…', 'Branch activity cache')}
              </div>
            </template>
          </div>
          ...
          <!-- ── Branch detail: a full-viewport takeover for one Open row ──
               ...
               Stepping does NOT reload the frame. It used to swap the src,
               which meant a document load per step: the 2.4 MB pre-build
               re-parsed, Alpine re-booted, the DOM re-walked, before the first
               API call went out, and the reader watched a facts card until all
               of that finished. The frame is now opened once and asked for the
               next branch over postMessage, which the embedded component
               answers by reloading itself in place (alpineComponents/
               branch-brief.js, listen()). The message carries the two
               neighbours so the page can warm their compares, so a step in
               either direction usually has its data already. -->
          <div x-show="detail" x-cloak x-transition.opacity.duration.150ms
            ...
            <div class="absolute inset-0 bg-base-100 overflow-hidden flex flex-col
              ...
              <div x-show="!detailDrilled"
                   class="h-12 shrink-0 flex items-center gap-1.5 px-2 border-b border-base-300"
                ...
                <a x-show="detailPrNumber" :href="detailRow ? prUrl(detailRow.repo, detailPrNumber) : '#'"
                   target="_blank" class="font-mono text-base font-bold shrink-0 hover:text-primary transition-colors"
                   :class="detailPrState === 'merged' && 'text-secondary'"
                   :title="'Pull request #' + detailPrNumber + (detailPrState ? ' (' + detailPrState + ')' : '')"
                   x-text="detailPrNumber ? '#' + detailPrNumber : ''"></a>
              ...
              <div class="relative flex-1 min-h-0 overflow-hidden">
                ...
                <template x-if="detail">
                  <iframe :src="detailSrc" x-ref="detailFrame" @load="onDetailFrame($event)"
                          class="absolute inset-0 w-full h-full border-0 bg-base-100 transition-opacity duration-200"
                          :class="(detailReady || detailSeen) ? 'opacity-100' : 'opacity-0'"
                          sandbox="allow-scripts allow-same-origin allow-popups allow-downloads"></iframe>
                </template>
                ...
                <div x-show="!detailReady && !detailSeen" x-transition.opacity.duration.200ms
      ...
      init(){
        ...
        // The embedded branch page reporting on itself: loading, ready, and
        // which PR it found. See onBranchMessage.
        window.addEventListener('message', (e) => this.onBranchMessage(e.data));
      ...
      get activityProgressLabel(){
        ...
        if (!p) return '';
        // Two phases since the split refresh: the quick pass (commits, PRs,
        // recent branches) and the survey true-up behind it.
        const verb = p.phase === 'survey' ? 'Surveying branches' : 'Refreshing activity';
        return p.total ? `${verb} · ${p.done} of ${p.total} repos` : verb;
      },
      ...
      detailSrc: '',       // the frame's address, set ONCE per takeover (see detailStep)
      // Whether the embedded page has ever rendered in this takeover. It is
      // what retires the instant facts card after the first open: the card and
      // the frame's fade exist because a src swap left the frame BLANK while a
      // whole document loaded, and a frame that reloads no longer goes blank.
      // Covering it on every step, when the page now keeps its own head up and
      // spins only the pane, is the flash a reader saw between branches.
      detailSeen: false,
      detailPr: null,      // { repo, branch, number, state } as reported by the frame
      detailDrilled: false,// the embedded page has a deck of its own open over it
      openBranchDetail(row){
        ...
        this.detailPr = null;
        // A step queued against the LAST takeover's frame is not a step in this
        // one, and delivering it on the next load would open a branch the
        ...
        // The frame's address is fixed here and never again for the life of
        // this takeover. Steps talk to the page instead (detailStep).
        this.detailSrc = this.detailUrl;
        this.stampDetail();
      },
      closeDetail(){
        this.detail = null; this.detailReady = false; this.detailSeen = false;
        this.detailPr = null; this.detailDrilled = false;
        this.detailSrc = ''; this._detailFrame = null; this._detailPending = null;
        this.stampDetail();
      },
      ...
      onDetailFrame(e){
        ...
        // A step taken before the frame finished loading: deliver it now.
        // The gap is real, since the first document load is the slow one and
        // the reader can swipe straight through it.
        const q = this._detailPending;
        this._detailPending = null;
        if (q) this.postDetail(q); else this.watchDetailReady();
      },
      // The frame reports through both channels, and the shell listens on both
      // for reasons that are not redundancy. The message (`branch-state`)
      // arrives at once and carries the PR number, which the shell's own row
      // cannot know for a merged PR. The attribute (`data-brief-ready`) is the
      // older signal and the only one available for a frame that predates the
      // channel, which is any deployment where the shell has been updated
      // ahead of the page it embeds.
      watchDetailReady(){
        const fr = this.detailFrame();
        const t0 = Date.now();
        const tick = ++this._readyTick;
        const poll = () => {
          if (!this.detail || tick !== this._readyTick || this.detailReady) return;
          let ok = false;
          try { ok = fr?.contentDocument?.documentElement?.hasAttribute('data-brief-ready'); } catch {}
          if (ok || Date.now() - t0 > 2500) { this.detailReady = true; this.detailSeen = true; }
      ...
      _detailFrame: null,
      _detailPending: null,
      // Ask the embedded page for a branch. The neighbours ride along so it can
      // warm them once it has settled: the reader's next step is almost always
      // one of the two, and a compare already in flight is the difference
      // between a step and a wait.
      // The frame, from the ref first and the load event second. The load event
      // alone is not enough, and the gap it leaves is not hypothetical: closing
      // and reopening the takeover inside one tick leaves Alpine's x-if with
      // the same element to reuse, so no second `load` fires, `_detailFrame`
      // stays null from the close, and every step after that queues into
      // `_detailPending` and is never delivered. The reader sees a header that
      // steps over a page that does not. Measured 2026-08-13 with
      // tools/render/scenarios/branch-step-cost.mjs, which is what a real
      // browser buys over jsdom here: the reuse is a rendering fact.
      detailFrame(){
        const fr = this.$refs?.detailFrame || this._detailFrame;
        return (fr && fr.contentWindow) ? fr : null;
      },
      postDetail(row){
        const fr = this.detailFrame();
        if (!fr) { this._detailPending = row; return; }
        const at = (r) => r && ({ repo: r.repo, branch: r.name, base: r.def || '' });
        const rows = this.detail?.rows || [];
        const i = rows.indexOf(row);
        const warm = i >= 0 ? [at(rows[i - 1]), at(rows[i + 1])].filter(Boolean) : [];
        try {
          fr.contentWindow.postMessage({
            source: 'web-tools', type: 'branch-open', ...at(row), warm,
          }, location.origin);
        } catch { this._detailPending = row; }
      },
      // The frame's own report. Guarded on the branch it names rather than
      // trusted blind: a late `ready` from the branch before last must not
      // reveal the page while the current one is still reading.
      onBranchMessage(m){
        if (!m || m.source !== 'web-tools') return;
        // The embedded page drilled into a deck of its own, or came back up.
        // While it is down there the shell's header names a level the reader
        // has left, so it stands down entirely: no arrows to step a branch
        // from inside a file, no counter describing the wrong sequence, and
        // no drag competing with the deck's own.
        if (m.type === 'branch-drill') { this.detailDrilled = !!m.on; return; }
        if (m.type !== 'branch-state') return;
        const r = this.detailRow;
        if (!r || m.repo !== r.repo || m.branch !== r.name) return;
        if (m.phase === 'loading') { this.detailReady = false; this.detailPr = null; return; }
        this.detailReady = true;
        this.detailSeen = true;
        this.detailPr = m.pr ? { repo: m.repo, branch: m.branch, number: m.pr, state: m.prState || '' } : null;
      },
      // The number the header shows. The row's own PR is preferred because it
      // is there the instant the takeover opens, and the frame's report fills
      // the case the row cannot cover: the activity crawl asks GitHub for OPEN
      // pull requests only (show-repo.html, `g.pulls('open', 30)`), so a branch
      // whose PR merged has no number in the cache at all, and the header sat
      // blank on exactly the branches whose work is finished. The frame reads
      // `state=all` at open time, so it knows; it just cannot say so until it
      // has loaded, which is why this is a fallback and not the source.
      get detailPrNumber(){
        const r = this.detailRow;
        if (r?.pr?.number) return r.pr.number;
        const p = this.detailPr;
        return (p && r && p.repo === r.repo && p.branch === r.name) ? p.number : 0;
      },
      get detailPrState(){
        const r = this.detailRow;
        if (r?.pr?.number) return r.pr.draft ? 'draft' : 'open';
        return this.detailPr?.state || '';
      },
      get detailRow(){ return this.detail ? this.detail.rows[this.detail.i] : null; },
      // The frame's opening address. Steps no longer go through here (they are
      // messages), so this runs once per takeover; `?swipe=` survives as the
      // marker that says which branch the frame was opened at.
      //
      // `&base=` is the row's own default branch, and passing it removes a
      // round trip the page would otherwise spend asking GitHub for something
      // the shell already had: with no base in the address, branch.html reads
      // the repo meta before it can compare anything, and that call was first
      // in the queue on every open.
      get detailUrl(){
        const r = this.detailRow;
        if (!r) return '';
        const use = new URLSearchParams(location.search).get('use');
        return '../branch.html?swipe=' + encodeURIComponent(r.repo + '@' + r.name)
          ...
          + '#gh=' + r.repo + '@' + r.name + (r.def ? '&base=' + encodeURIComponent(r.def) : '');
      },
      // Clamped, not wrapped: the ends are real, and a swipe past them should
      // feel like an edge, not teleport across the list.
      //
      // The step used to change the iframe's src, which is a whole document
      // load: the pre-build re-parsed and re-executed, Alpine booted, the DOM
      // walked, all before the first API call went out. The frame is now
      // persistent and the step is a message, so what a step costs is what the
      // data costs, and with the neighbours warmed it is usually nothing.
      detailStep(d){
        ...
        if (i !== this.detail.i) {
          ...
          this.postDetail(this.detailRow);
          this.watchDetailReady();
        }
      },
      detailKeys(e){
        if (!this.detail || this.detailDrilled) return;   // the deck below owns them
      ...
      // The finger, in the SHELL's coordinates, whichever document the touch
      // was born in.
      //
      // This is the fix for a shake, and the shake was a feedback loop. The
      // drag reaches inside the frame (that is what made the whole page a
      // swipe surface rather than two 24px strips), so most touches originate
      // in the frame's document and report `clientX` relative to the FRAME's
      // viewport. The frame is also the thing being translated. So the moment
      // the surface moves, the next measurement of a stationary finger moves
      // with it, by exactly the amount just applied, and the offset oscillates
      // between two values instead of tracking. Measured 2026-08-13: a finger
      // walking left in even 8px steps was read as 284, 292, 276, 284, 268,
      // 276, which is precisely `page ± the offset last written`.
      //
      // `getBoundingClientRect().left` on the frame carries the live transform,
      // so adding it converts a frame-relative reading back to the shell's
      // frame of reference and the loop closes. A touch on the header or an
      // edge strip is already in shell coordinates and is left alone.
      _fingerX(e, t){
        const doc = e && e.target && e.target.ownerDocument;
        if (doc && doc !== document){
          const fr = this.detailFrame();
          const r = fr && fr.getBoundingClientRect();
          if (r) return t.clientX + r.left;
        }
        return t.clientX;
      },
      ...
      dTouchStart(e){
        ...
        if (!t || !this.detail || this._dBusy || this.detailDrilled || e.touches.length !== 1
            || this._dInHScroll(e.target)) { this._dT = null; return; }
        const pane = this._detailPane();
        this._dT = { x: this._fingerX(e, t), y: t.clientY,
      ...
      dTouchMove(e){
        ...
        const dx = this._fingerX(e, t) - s.x, dy = t.clientY - s.y;
        if (this._dLock === null){
      ...
      dTouchEnd(e){
        ...
        const pane = this._detailPane();
        const dx = this._fingerX(e, t) - s.x;
        const dir = dx < 0 ? 1 : -1;
      ...
      // lives in BranchSurvey.dropFileUrl, shared with the branch page.
      dropFileUrl(r){
        const cfg = window.__shell?.estateConfigs?.[r.repo] || {};
        return window.BranchSurvey.dropFileUrl(r.repo, r.name, cfg.inbox);
      },
