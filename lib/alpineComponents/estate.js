  ...
  Alpine.data('estate', function() {
    ...
    const branchRowBody = (opts = {}) => `
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <!-- PLAIN GLYPHS at gap-2, and the alternative was tried
                         and rendered. Padding each control (gap-1 with p-1)
                         grows the hit box from about 20px to about 28px for the
                         same 8px between glyphs, which sounds free and is not:
                         at 430px it took twelve pixels off the identity row and
                         cut the session title from show-rep… to show-r…. The
                         title is both what identifies the row and what opens
                         its brief, so a control you cannot read beats a control
                         you can read and might miss. 44px per glyph is further
                         out of reach still: it would make the cluster taller
                         than the row.
                         Measured 2026-08-27 at 430px, both ways, on this pane
                         and on the Branches one. Do not re-try it blind. -->
...
${opts.nested ? '' : `
    ...
    return {
              ...
              <template x-for="n in sessionNodes" :key="n.key">
                ...
                <div x-data="{ row: n.row }">
                  ...
                  <!-- The ask, and it is PROSE. It was a button opening the
                       ...
                       What it gains by giving up the click is the TOOLTIP, and
                       the pairing is the point: the line says how the session
                       opened, the hover says how it closed. Two ends of one
                       session, in the space a row already had. The full reply
                       is in the brief and in the deck's own last card; this is
                       capped in the cache at 600 characters and says so when it
                       was cut (sessionAskTitle). -->
                  <p x-show="row.ask" class="text-base text-base-content/60 truncate mt-0.5"
                     :title="sessionAskTitle(row)" x-text="row.ask"></p>
      ...
      // The ask line's tooltip: how the session CLOSED. The line itself is the
      // opening ask, so the pair is the session in two readings, and the hover
      // is where the second one fits without costing the row a line.
      //
      // Three states and they are not the same claim. A row from a cache built
      // before the field existed has nothing to say and falls back to the ask,
      // which is what the line showed before this existed. A schema-1-to-3
      // record never held the assistant's prose at all, so what comes back is
      // the recorder's 500-character tail of the final turn and the label says
      // so. And a reply the cache cut at REPLY_CHARS is marked, because the
      // store's own schema-5 lesson is that a silent bound is the damage.
      sessionAskTitle(row){
        const ask = String(row?.ask || '');
        const reply = String(row?.reply || '').trim();
        if (!reply) return ask;
        const head = row.replyCut === 'tail' ? 'Final turn (tail only)'
