...
${opts.nested ? '' : `
    ...
    return {
      ...
      sessionRepoRows(row){
        return (row?.repos || []).map((x) => {
          const full = this.entries.find((e) => e.repo.endsWith('/' + x.name));
          return { name: x.name, branch: x.branch || '', lines: x.lines || 0,
                   slug: full ? full.repo : '' };
        });
      },
