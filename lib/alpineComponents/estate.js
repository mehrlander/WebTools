...
${opts.nested ? '' : `
                      ...
                      <span class="flex items-center gap-1 flex-wrap min-w-0">
                        ...
                        <template x-for="rt in branchRoutes(row).on" :key="rt.key">
                          <button @click.stop="openRoute(rt)"
                                  ...
                                  class="shrink-0 rounded-full px-2 py-0.5 text-sm font-medium bg-primary/5 text-primary hover:bg-primary/20 transition-colors"
    ...
    return {
                        ...
                        <div x-show="adopt(e)" class="flex flex-col gap-1 mt-0.5">
                          <div class="flex items-center gap-1.5 flex-wrap">
                            ...
                            <button x-show="scopeOf(e)" @click.stop="scopeOpen = scopeOpen === face(e).repo ? '' : face(e).repo"
                                    class="badge badge-sm badge-ghost gap-1 cursor-pointer hover:bg-primary/5 hover:text-primary transition-colors"
                        ...
                        <template x-if="cardActivity(face(e).repo)">
                          <div class="flex flex-wrap items-center gap-1 mt-0.5">
                            <button @click="openRepoBranches(face(e).repo)"
                                    class="badge badge-sm badge-ghost gap-1 font-mono cursor-pointer hover:bg-primary/5 hover:text-primary transition-colors"
                            ...
                            <button x-show="cardAbandoned(face(e).repo)"
                                    ...
                                    class="badge badge-sm badge-ghost gap-1 font-mono text-error cursor-pointer hover:bg-error/5 transition-colors"
          ...
          <div class="flex-1 min-w-0" :class="tab==='activity' ? '' : 'hidden'"
            ...
            <div x-show="authed && !activityLoading"
              ...
              <template x-for="s in branchScopes" :key="s.key">
                <button @click="branchScope = s.key" :title="s.note"
                        ...
                        :class="branchScope === s.key ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
            ...
            <div x-show="authed && !activityLoading && openRepos.length > 1"
              ...
              <button @click="openRepoFilter = ''"
                      class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-sm font-medium transition-colors"
                      :class="!activeRepoFilter ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
              ...
              <template x-for="r in openRepos" :key="r.repo">
                <button @click="openRepoFilter = (activeRepoFilter === r.repo ? '' : r.repo)" :title="r.repo"
                        class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-sm font-medium transition-colors"
                        :class="activeRepoFilter === r.repo ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
          ...
          <div class="flex-1 min-w-0" :class="tab==='sessions' ? '' : 'hidden'"
            ...
            <div x-show="authed && sessionLens === 'list' && !sessionsLoading"
              ...
              <template x-for="s in sessionScopes" :key="s.key">
                <button @click="sessionScope = s.key" :title="s.note"
                        class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-sm font-medium transition-colors"
                        :class="sessionScope === s.key ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
                  <i class="ph text-base" :class="s.icon"></i>
                  <span x-text="s.label"></span>
                  <span class="font-mono opacity-60" x-text="s.count"></span></button>
              </template>
            ...
            <div x-show="authed && sessionLens === 'list' && !sessionsLoading && sessionRepos.length > 1"
              ...
              <button @click="sessionRepoFilter = ''"
                      class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-sm font-medium transition-colors"
                      :class="!activeSessionRepo ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
              ...
              <template x-for="r in sessionRepos" :key="r.repo">
                <button @click="sessionRepoFilter = (activeSessionRepo === r.repo ? '' : r.repo)"
                        ...
                        class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-sm font-medium transition-colors"
                        :class="activeSessionRepo === r.repo ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
          ...
          <div class="flex-1 min-w-0" :class="tab==='chats' ? '' : 'hidden'"
            ...
            <div x-show="authed && chatRows.length"
              ...
              <button @click="chatProvider = ''"
                      class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-sm font-medium transition-colors"
                      :class="!chatProvider ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
              ...
              <template x-for="p in chatProviders" :key="p.key">
                <button @click="chatProvider = (chatProvider === p.key ? '' : p.key)"
                        class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-sm font-medium transition-colors"
                        :class="chatProvider === p.key ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
              ...
              <button @click="chatHandOnly = !chatHandOnly"
                      ...
                      class="shrink-0 flex items-center gap-1.5 rounded-full px-2.5 py-1.5 text-sm font-medium transition-colors"
                      :class="chatHandOnly ? 'bg-primary/5 text-primary' : 'bg-base-200/60 text-base-content/60 hover:text-base-content'">
      ...
      // stylesheet: bg-success/5, /15, /25 and /45 all compute to transparent,
      // and text-base-content/45 to full strength, so a step the build does not
