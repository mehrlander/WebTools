    ...
    return {
      ...
      async mountReplyCard(card){
        ...
        for (const t of card.turns) {
          host.append(window.chatRender.message(t, { collapse: 0 }));
          if (t.dropped) host.append(this._replyFoot(
            t.dropped.toLocaleString() + ' more characters in this turn'));
        }
      ...
      _replyFoot(text){
        ...
        // Indented to the turn's own text column (the rail is 2px plus pl-3.5),
        // so it reads as that turn's footnote rather than as a new turn.
        el.className = 'text-[10px] text-base-content/40 leading-snug pl-4 -mt-1';
        el.textContent = '… ' + text;
        return el;
      },
