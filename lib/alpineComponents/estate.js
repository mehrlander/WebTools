document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
              ...
              <div class="flex-1 min-h-0 flex flex-col">
                ...
                <section class="flex-1 min-h-0 flex flex-col">
                  <div class="flex items-center gap-x-2 gap-y-1.5 flex-wrap mb-2">
                    ...
                    <span x-show="todoUrgent.length"
                          class="badge badge-error badge-sm gap-1 font-mono"
                          :title="todoUrgent.length + ' urgent'">
                      <i class="ph-fill ph-flag text-xs"></i><span x-text="todoUrgent.length"></span></span>
                    <div class="grow"></div>
                    ...
                    <div class="flex flex-col gap-1">
                      ...
                      <template x-for="it in todoOpen" :key="it.id">
                        <label class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg border-l-4 hover:bg-base-200/60 group"
                               :class="it.urgent ? 'border-error bg-error/5' : 'border-transparent'">
                          <input type="checkbox" :checked="it.done" @change="toggleTodo(it)"
                                 class="checkbox checkbox-sm">
                          <span class="text-base flex-1" :class="it.urgent && 'font-medium'" x-text="it.text"></span>
      ...
      // {items:[{id,text,done,created_at,done_at?,urgent?}]}. Not a surface: no
      // kind/curation, just text, done, and one flag, so it gets the plainest
      // shape rather than reusing the surfaces schema. 404 (no file yet) is a
      // quiet empty list, matching loadSurfaces' no-dir case.
      //
      ...
      // Urgent first, and within each band the file's own order, which is the
      // order things were added. A stable partition rather than a sort: two
      // urgent items keep the sequence you wrote them in, so flagging one does
      // not quietly reshuffle the rest of the list around it.
      get todoOpen(){
        const open = this.todoItems.filter(it => !it.done);
        return [...open.filter(it => it.urgent), ...open.filter(it => !it.urgent)];
      },
      get todoUrgent(){ return this.todoOpen.filter(it => it.urgent); },
