                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center flex-wrap gap-x-4 gap-y-1.5 mt-2 text-base">
                    ...
                    <!-- The content survey's verdict, on the rows that have
                         ...
                         many hold content the default branch has now. A landed
                         ...
                         "measured zero" are different answers.

                         IT IS A ROUTE, not a readout, and that is the 2026-08-18
                         change. It read 28/80 beside 11 missing, the two did
                         not add up (forty-one paths sat in a third class nothing
                         named), and the only way to see WHICH files were missing
                         was a tooltip that pasted up to twelve paths under a
                         sentence describing the ones that were present. Both
                         halves now open the branch detail on its Files pane,
                         filtered to the class that was tapped, where the files
                         are diffs a reader can actually read. The word "landed"
                         rides the ratio because a bare 28/80 says nothing
                         about which direction is good, and the third count
                         lives in the title and in the pane's own strip.

                         The asterisk is the survey's own caveat: no shared
                         ancestor, so the compare fell back to the branch's
                         recent history and the counts span more than this
                         branch. -->
                    <span x-show="row.nUnique" class="flex items-center gap-1 font-mono tabular-nums shrink-0">
                      ...
                      <button @click.stop="openBranchFiles(row, '')" :title="verdictTitle(row)"
                              class="hover:underline underline-offset-2 transition-colors"
                              :class="row.nMissing ? 'text-base-content/60 hover:text-primary' : 'text-success'">
                        <span x-text="row.nLanded + '/' + row.nUnique"></span><span
                          class="hidden sm:inline font-sans font-normal opacity-70"> landed</span></button>
                      ...
                      <span x-show="row.noBase" class="text-warning"
                            title="No shared ancestor with the default branch, so the compare fell back to this branch's recent history and these counts span more than the branch itself">*</span>
                    </span>
