  ...
  Alpine.data('estate', function() {
    ...
    return {
      ...
      crawlLabel(key){
        ...
        if (!p) return '';
        // A multi-pass run names its pass: the activity refresh is a quick pass
        // and a survey behind it, and without this the second reads as the first
        // one repeating.
        const pass = p.passes > 1 ? ` · pass ${p.pass}/${p.passes}` : '';
        return p.verb + pass + (p.total ? ` · ${p.done} of ${p.total} ${p.unit}` : '');
      },
      ...
      // The bar spans the whole RUN, passes included, so it cannot fill, reach
      // the end, and start over: that said the run had finished when it had not.
      // Equal shares because each pass covers the same items; they do not cost
      // the same, so this counts item-passes rather than time.
      crawlPct(key){
        const p = this.crawl(key);
        if (!p?.total) return 0;
        const passes = p.passes || 1, pass = p.pass || 1;
        return Math.round(((pass - 1) + p.done / p.total) / passes * 100);
      },
      ...
      branchAccent(row){
        ...
        return s === 'ready' ? 'border-success bg-success/5'
             ...
             : s === 'merged' ? 'border-primary/40 bg-primary/5'
      ...
      // without a legend.
      BRANCH_STATE_MARK: {
        merged: { icon: 'ph-git-merge', cls: 'text-primary/70' },
