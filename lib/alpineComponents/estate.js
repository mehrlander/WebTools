      ...
      template: `
        <div>
          ...
          <!-- ── To-do view (Activity sub-tab) ─────────────────────────────
               ...
               Full width, like the branch list: an item's trailing control sits
               at the row's right edge either way, so a cap only moves where the
               row ends, and a list that stops halfway across a wide screen
               reads as a rail that lost its neighbor. -->
          <div :class="tab==='todo' ? '' : 'hidden'">
            <p x-show="!authed" class="text-base text-base-content/60">
