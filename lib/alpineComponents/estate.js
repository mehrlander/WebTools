    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div class="flex-1 min-w-0" :class="tab==='routes' ? '' : 'hidden'"
            ...
            <!-- ONE RANKED LIST, not a section per group. The pane was grouped
                 first and the grouping quietly cost it its headline: a route
                 touched an hour ago sat below one touched six days ago because
                 they were in different sections, which is precisely the reading
                 "recently updated" is supposed to give. The group survives as a
                 chip on the row, where it labels without reordering. -->
            <div x-show="authed && routeRows.length"
                 class="flex flex-col divide-y divide-base-300/50 border-y border-base-300/50 mb-4">
                  <template x-for="r in routeRows" :key="r.key">
                    <div class="py-2">
                      ...
                      <div class="flex items-baseline gap-2 flex-wrap">
                        <button x-show="routeIsOpenable(r)" @click="openRoute(r)"
                                class="text-base font-medium text-primary hover:underline shrink-0"
                                x-text="r.label"></button>
                        <span x-show="!routeIsOpenable(r)"
                              class="text-base font-medium shrink-0" x-text="r.label"></span>
                        <code class="text-sm text-base-content/45 font-mono" x-text="r.address"></code>
                        <span class="text-sm text-base-content/35" x-text="routeGroupLabel(r.group)"></span>
                        <span x-show="r.branches.length"
                              class="badge badge-sm badge-primary badge-outline gap-1 font-mono"
                        ...
                        <span x-show="r.nearBranches.length"
                              class="badge badge-sm badge-ghost gap-1 font-mono"
      ...
      async loadRoutes(force){
        ...
        try {
          ...
          let manifest;
          try {
            manifest = JSON.parse((await gh.get(ROUTES_MANIFEST)).text);
          } catch (e) {
            // Name the address that failed. A bare "404: Not Found" sent the
            // first diagnosis of this to auth, which the rate-limit figure in
          ...
          // What is open against a route right now. Bounded by OPEN PRs rather
          // than by branches, on the measurement the Guides pane already made:
          // the estate carries hundreds of branches against a dozen open PRs,
          // and work in flight on a route lives on a PR branch by construction.
          let prs = [];
