...
${opts.nested ? '' : `
    ...
    return {
          ...
          <div x-show="tab==='stage'">
            ...
            <div class="flex items-center gap-2 mb-4 flex-wrap">
              ...
              <template x-if="stageTab === 'bench' && benchOrigin">
                <div class="flex items-center gap-2 min-w-0">
                  <span class="text-base text-base-content/40 truncate">
            ...
            <div x-show="authed && sessionLens === 'list' && sessionNodes.length" class="flex flex-col gap-2">
              <template x-for="n in sessionNodes" :key="n.key">
                ...
                <!-- ── The branches this session left behind ─────────────────
                     ...
                     a tree elbow, a bottom shelf) on the live pane. -->
                ...
                <template x-if="rowCard.kind === 'prose'">
                  <div class="flex flex-col gap-2 px-3 py-2.5">
                    ...
                    <p x-show="rowCard.pending" class="text-[10px] text-base-content/40 leading-snug">
                      No closing reply on this row yet. The field arrived with row version 5
                      and one crawl pass reads 120 records, so a store this size heals over
                      two: press Refresh on this pane again.
                    </p>
      ...
      // Pill taps (Activity's pills and the Stage's two): route through the
      ...
      replyHtml(row, text){
        ...
        (async () => {
          ...
          if (this.rowCard?.kind === 'prose' && this.rowCard.text === text) {
            this.rowCard = { ...this.rowCard, html: out.html,
                             htmlClass: window.GuideRender.bodyClass('drawer') };
          }
        })();
        return '';
      },
      ...
      openSessionCard(row, cls, ev){
        ...
        if (cls === 'turns') {
          ...
          this.rowCard = {
            ...
            // and not about the session: the field arrived with ROW_V 5 and one
            // crawl pass reads 120 records, so a store of 225 heals over two.
            pending: !reply,
            shape: { exts: [], dirs: [] },
          };
          this.rowCardAt = window.__shell?.anchorMenu?.(ev, 9, { width: this.ROW_CARD_W, align: 'left' }) || null;
      ...
      embedUrl(it){
        ...
        if (!r || !r.path) return '';
        // ABSOLUTE, like every other page address this component builds. It
        // was '../toss-render.html', which was right while the app lived at
        // pages/show-repo/show-repo.html and has resolved to
        // /web-tools/toss-render.html since the 2026-08-16 move to app/. The
        // move's stub redirect covers links coming IN; nothing covered the
        // links this file writes going out. Found 2026-08-27 next to the same
        // fallout in the session pane's branch link, which was dead code and
        // is now deleted.
        return 'https://mehrlander.github.io/web-tools/pages/toss-render.html'
             + '#' + this.embedPage(it) + '=' + window.Surface.key(it);
      },
