document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
      ...
      template: `
        <div>
          ...
          <div x-show="tab==='surfaces'">
            <!-- No mode pill and no second tab. There is one list, and the
                 working set is the first thing on it: display and edit are
                 states of whichever surface you are looking at, not two places
                 to be. Opening a card's bench is what "edit" means, and only
                 one can be open, since there is one bench. -->
                  ...
                  <template x-for="s in sec.surfaces" :key="s.uid">
                    <div>
                      <div class="flex items-baseline gap-2 mb-1">
                        <h2 class="text-lg font-semibold" :class="s.bench && 'text-primary'"
                            x-text="s.manifest.name || s.file"></h2>
                        <span class="text-base font-mono text-base-content/30" x-text="s.file"></span>
                        <div class="grow"></div>
                        <!-- The one edit affordance, on every card including the
                             working one. Opening it puts that surface on the
                             bench: the working set is already there, a saved one
                             is read onto it and remembers where it came from, so
                             saving writes back instead of minting a stray copy.
                             Prose items have no file behind them and are
                             reported, not dropped. A repo's own surface opens
                             here too: reading one onto the bench needs no write
                             access. Only one card can be open, because there is
                             one bench. -->
                        <button x-show="s.bench || stageableCount(s)" @click="toggleBench(s)"
                                class="self-center transition-colors shrink-0"
                                :class="benchOn(s) ? 'text-primary' : 'text-base-content/30 hover:text-primary'"
                                :title="benchOn(s) ? 'Done editing' : (s.bench ? 'Edit the working set' : 'Edit on the bench')">
                          <i class="ph text-base leading-none"
                             :class="benchOn(s) ? 'ph-eye' : 'ph-pencil-simple'"></i></button>
                        ...
                        <button x-show="authed && !sec.repo && !s.bench" @click="editSurface(s)"
                        ...
                        <button x-show="authed && !sec.repo && !s.bench" @click="deleteSurface(s)"
                                class="self-center transition-colors shrink-0"
                        ...
                        <a x-show="sec.repo && !s.bench" :href="s.blob" :data-peek="s.repo + '@' + s.ref + ':' + s.path" target="_blank"
                      ...
                      <!-- The bench, in place of the card's item list. Mounted
                           once and shown where it belongs rather than mounted
                           per card: there is one working set, so one stager,
                           and it appears under whichever card claims it. -->
                      <template x-if="benchOn(s)">
                        <div class="mb-2"><div x-data="stager()"></div></div>
                      </template>

                      <div x-show="!benchOn(s)" class="flex flex-col gap-2">
      ...
      adoptRows: {},   // repo -> the portable-align row, probed live per card
      adoptLoading: false,
      scopeOpen: '',   // repo whose scope paragraph is expanded (one at a time)
      // The bench: one working set, so one stager, appearing under whichever
      // card claims it. benchOrigin names the saved surface it was read from
      // ('' = the working set itself), which is what lets a save write back
      // rather than mint a stray copy.
      benchOpen: false,
      benchOrigin: '',
      ...
      init(){
        ...
        // ?view=stage, a #stage= link, and the explorer's "+" all still ask for
        // the bench. It is no longer a view, so what they get is this shelf with
        // the working card open. The shell raises openBench; this lowers it,
        // since it is a request, not a state.
        const takeBenchRequest = () => {
          if (!window.__shell?.openBench) return;
          window.__shell.openBench = false;
          this.benchOrigin = '';
          this.benchOpen = true;
        };
        this.$watch(() => window.__shell?.openBench, takeBenchRequest);
        takeBenchRequest();
      },

      // The working set as a surface card, so it sits in the one list rather
      // than behind a tab. Present even when empty: it is where things are
      // added, so it has to be somewhere to open. Absent only while the bench
      // is showing a SAVED surface, since then the working set is that surface
      // and two cards would be one thing drawn twice.
      get benchCard(){
        if (this.benchOrigin) return null;
        const items = window.Surface.fromStage(Alpine.store('browser')?.stage || []).surface.items;
        return { uid: 'bench', bench: true, file: '',
                 manifest: { name: items.length ? window.Surface.autoName(items) : 'Working set',
                             category: 'working' },
                 items };
      },
      benchOn(s){ return this.benchOpen && (this.benchOrigin ? this.benchOrigin === s.uid : !!s.bench); },

      // A surface the bench was read from shows what is ON the bench, not what
      // is in its file, whether the bench is open or closed. Otherwise closing
      // an edit would swap the card back to the saved version and quietly
      // disagree with the set you are still holding.
      live(s){
      ...
      // The one edit gesture. Closing returns the card to display; opening a
      // saved surface reads it onto the bench and remembers where it came from.
      toggleBench(s){
        if (this.benchOn(s)) { this.benchOpen = false; return; }
        if (s.bench) { this.benchOrigin = ''; this.benchOpen = true; return; }
      ...
      get tab(){
        ...
        // 'stage' is an address for the shelf with the bench open, not a view
        // of its own, so it resolves here.
        if (v === 'stage') return 'surfaces';
      ...
      async load(){
        ...
        this.loading = false;
        this.enrichMeta();
        // Grade the same roster the cards already name, rather than the Map
        // rebuilding its own list from the cache. Membership is the canonical
        // answer to "which repos are in the constellation", so asking it once
        // is also what stops the two lists drifting the way they did when a
        // repo joined the estate and never reached the Map's authored array.
        this.loadAdoption([...new Set(this.entries.flatMap(e => [e.repo, e.child?.repo]).filter(Boolean))]);
      ...
      get surfaceSections(){
        const secs = [];
        // The working set leads the list rather than living behind a tab: it is
        // a surface like the rest, just one that has not been saved.
        const bench = this.benchCard;
        const general = bench ? [bench, ...this.surfaces.map(s => this.live(s))] : this.surfaces.map(s => this.live(s));
        if (general.length)
      ...
      async probeAdoption(repo){
        const gh = new window.GH({ token: window.TOKEN, repo, ref: '' });
        const grab = async (path, parse) => {
          try { const s = (await gh.get(path)).text; return parse ? JSON.parse(s) : s; }
          catch { return null; }
        };
        const [settings, claudeMd, config] = await Promise.all([
          grab('.claude/settings.json', true), grab('CLAUDE.md', false), grab('.web-tools.json', true),
        ]);
        const hub = window.PortableAlign?.HUB || 'mehrlander/web-tools';
        const role = repo === hub ? 'hub' : repo === this.registry() ? 'registry' : null;
      ...
      // Fire and forget per repo: a card fills in when its probe lands rather
      // than the grid waiting on the slowest one.
      async loadAdoption(repos){
        if (!this.hasToken() || !window.PortableAlign) return;
        this.adoptLoading = true;
        await Promise.all((repos || []).map(async (repo) => {
          if (this.adoptRows[repo]) return;
          try { this.adoptRows[repo] = await this.probeAdoption(repo); }
          catch { /* a card simply shows no verdict */ }
        }));
