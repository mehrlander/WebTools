document.addEventListener('alpine:init', function() {
  ...
  // Rendered closing replies, keyed by session id and the raw text. A Claude
  // reply IS markdown, so the reply card renders it rather than printing its
  // syntax; the render is asynchronous, since marked loads on first ask. The
  // memo is what keeps a second hover over the same row from showing the raw
  // source for a frame while the parser answers again, which reads as a
  // flicker rather than as a load. Shared across every card, not per row.
  const REPLY_HTML = new Map();
...
${opts.nested ? '' : `
    ...
    return {
      ...
      template: `
        <div :class="tab==='lists' && 'flex-1 min-h-0 flex flex-col'">
          ...
          <div class="flex-1 min-w-0" :class="tab==='sessions' ? '' : 'hidden'"
            ...
            <!-- How old the titles are, and how far they reach. A dated export
                 behind a live view is exactly the case that reads as current,
                 so the date is on the pane and not only in the cache; the count
                 beside it is there because the join's reach is uneven and a
                 reader seeing four titles and six slugs would otherwise have no
                 way to tell a stale export from a structural gap. It is the
                 latter: the key is the record's own session URL, which nothing
                 written before 2026-08-06 has. -->
            <div x-show="authed && sessionLens === 'list' && sessionsTitlesAt && sessionRows.length"
                 class="text-base text-base-content/40 mb-2"
                 title="Titles are scraped from the claude.ai sidebar on the desktop and committed to mehrlander/chat-histories; a record cannot carry its own title. Sessions the export does not name show the branch-derived slug instead.">
              <i class="ph ph-tag mr-1 opacity-60"></i>
              <span x-text="'Titles as of ' + sessionsTitlesAt + ' · ' + sessionTitledCount + ' of ' + sessionRows.length + ' named; the rest derived from the branch'"></span>
            </div>
              ...
              <template x-for="n in sessionNodes" :key="n.key">
                ...
                <template x-if="n.kind === 'stub'">
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <a :href="n.url" target="_blank" rel="noopener"
                       ...
                       class="font-mono text-base text-base-content/40 hover:text-primary transition-colors shrink-0"
                       x-text="n.id"></a>
                ...
                <div x-data="{ row: n.row }">
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <!-- The id opens the session ITSELF: its brief, swipeable
                         across the list this row was tapped in from. It used to
                         open the conversation, which made this pane the mirror
                         image of the Branches pane beside it, where a name
                         opens the branch and its contents are one level down.
                         The cards button below is that level here.

                         Two elements carry it, the id and the title, because
                         between them they are what a branch name is on the
                         other row: the identifier and the label. Giving the
                         action to eight mono characters alone is the tap target
                         the ask line used to be compensating for. -->
                    <button @click="openSessionDetail(row)"
                            ...
                            :title="'Open this session here; swipe or arrow through the list'" x-text="row.id"></button>
                    <!-- What the session is CALLED: the title from the export
                         where it names this one, the branch-derived slug
                         otherwise. The fallback is per ROW, never per pane, so
                         a list never goes blank for the records that predate
                    ...
                    <button x-show="sessionLabel(row)" @click="openSessionDetail(row)"
                            class="text-base text-base-content/70 truncate min-w-0 text-left
                  ...
                  <!-- The ask, and it is PROSE. It was a button opening the
                       ...
                       button are for. cursor-help says as much.
                  ...
                  <p x-show="row.ask"
                     ...
                     @mouseleave="hoverLeaveRowCard()"
                     class="text-base text-base-content/60 truncate mt-0.5
                            cursor-help hover:text-base-content/80 transition-colors"
                     x-text="row.ask"></p>
            ...
            <template x-if="rowCard">
              <div class="flex flex-col">
                <div class="flex items-baseline gap-2 px-3 pt-2.5 pb-2 border-b border-base-200 sticky top-0 bg-base-100">
                  ...
                  <!-- Prose has no count, so it shows none: a leading dash
                       where a number belongs reads as a measurement that failed
                       rather than as a card that never had one. -->
                  <span x-show="rowCard.kind !== 'prose'" class="font-mono font-semibold tabular-nums"
                        x-text="rowCardSummary?.count ?? '–'"></span>
                  <span class="text-sm text-base-content/70"
                        x-text="rowCard.kind === 'prose' ? rowCard.label
                ...
                <template x-if="rowCard.kind === 'prose'">
                  <div class="flex flex-col gap-2 px-3 py-2.5">
                    <!-- /40, not /45. Only multiples of ten generate against this app's
                         stylesheet, so an off-step renders at FULL strength and the
                         quiet context line would have shouted over the reply it is
                         context for. npm run opacity-scan is the gate. -->
                    <!-- The ask is quiet BESIDE a reply and plain WITHOUT one:
                         context for the thing you opened this for, or the thing
                         itself when the row has nothing else. Either way it is
                         the whole ask, which the row truncates. -->
                    <p x-show="rowCard.ask" class="leading-snug"
                       :class="rowCard.text ? 'text-xs text-base-content/40'
                                            : 'text-xs leading-relaxed text-base-content/80'"
                       x-text="rowCard.ask"></p>

                    <!-- ── The scroll back ────────────────────────────────
                         Everything said between the opening ask and the
                         closing reply, oldest first, so the card reads down
                         the way the session ran. Only the turns the swipe
                         deck leaves expanded: a short assistant turn that
                         announces work folds there, and carrying it here
                         would fill the card with narration the deck hides.
                         The cache does that selection (priorTurns), because
                         it needs the record's calls interleaved with its
                         replies and the row has neither.

                         An ask is a RULE with its text on it, not a
                         paragraph. Twenty reply openings in a column have no
                         structure; the asks are what make them exchanges.
                         Not uppercased, which is where this landed first: a
                         separator that is one word can be shouted and one that
                         is a whole question cannot. Small and muted does the
                         same job and stays readable. -->
                    <div x-show="rowCard.priorCut" class="text-[10px] text-base-content/40 leading-snug">
                      … earlier turns not kept. The card holds the last 20; the whole
                      conversation is in the session.
                    </div>
                    <template x-for="t in (rowCard.prior || [])" :key="t.key">
                      <div>
                        <div x-show="t.role === 'u'"
                             class="flex items-center gap-2 mt-1 mb-1.5 text-[11px]
                                    text-base-content/40">
                          <i class="ph ph-user text-xs shrink-0"></i>
                          <span class="truncate" x-text="t.text"></span>
                          <span class="grow border-t border-base-200"></span>
                        </div>
                        <template x-if="t.role === 'a'">
                          <div>
                            <div x-show="t.html" x-html="t.html"
                                 class="text-base-content/70" :class="rowCard.htmlClass"></div>
                            <p x-show="!t.html"
                               class="text-xs leading-relaxed whitespace-pre-line text-base-content/70"
                               x-text="t.text"></p>
                          </div>
                        </template>
                      </div>
                    ...
                    <!-- The closing reply sits below them all and reads a
                         shade darker (/80 against /70). It is what the card is
                         opened for; the scroll back is how you got here. -->
                    <!-- Rendered, with the raw text as the floor. The reply
                         is markdown and was being printed as source: a fence
                         showed as three backticks and a bold run as four
                         asterisks. The parser loads on first ask, so the plain
                         line is what a cold card shows for the moment before
                         the render lands, not a fallback for failure alone. -->
                    <div x-show="rowCard.html" x-html="rowCard.html"
                         class="text-base-content/80" :class="rowCard.htmlClass"></div>
                    <p x-show="rowCard.text && !rowCard.html"
                       class="text-xs leading-relaxed whitespace-pre-line text-base-content/80"
                       x-text="rowCard.text"></p>
                    <p x-show="rowCard.pending" class="text-[10px] text-base-content/40 leading-snug">
                      Nothing said on this row yet. The reply arrived with row version 5 and
                      the turns above it with 6; one crawl pass reads 120 records, so a store
                      this size heals over two. Press Refresh on this pane again.
                    </p>
                    <!-- The cut is marked where it happened, not only in the
                         label: the store's own schema-5 lesson is that a silent
                         bound is the damage. -->
                    <p x-show="rowCard.cut" class="text-[10px] text-base-content/40"
                       title="The sessions cache keeps the first 600 characters of a closing reply">
                      … trimmed at 600 characters. The whole reply is in the session, on its last card.
                    </p>
                  </div>
                </template>
      ...
      // How much of the visible list the export actually names. Shown rather
      // than assumed, because the join's reach is uneven in a way no row can
      // reveal on its own: it keys on the record's session URL, which no record
      // written before 2026-08-06 carries at all.
      get sessionTitledCount(){
        return this.sessionRows.filter(r => r && r.title).length;
      },
      ...
      // not decoration. Three states and they are not the same thing. A
      // schema-1-to-3 record never held the assistant's prose, so what comes
      // back is the recorder's 500-character tail of the final turn. A reply
      // the cache cut at REPLY_CHARS is marked, because the store's own
      // schema-5 lesson is that a silent bound is the damage. Anything else is
      // the closing reply whole.
      replyLabel(row){
        return row?.replyCut === 'tail' ? 'final turn, tail only'
             : row?.replyCut === 'cut' ? 'closing reply, trimmed'
             : 'closing reply';
      },

      // The reply, rendered. It is a Claude reply, so it arrives as markdown:
      // fenced blocks, code spans, bold, lists. Printed as text it showed the
      // syntax instead of the formatting, and a fence read as three stray
      // backticks.
      //
      // kits/guide-render.js is the estate's markdown path already, and this
      // is its fourth reader rather than a fourth copy: it loads marked once
      // for the page, injects the guide-body rules, re-aims a blob link at
      // this app and styles a bare path as code. bodyClass('drawer') is its
      // small scale, which is what a card this narrow wants.
      //
      // Returns SYNCHRONOUSLY from the memo or empty, and kicks the render
      // when it has neither. The empty answer is the raw text's cue to show
      // itself, so a card opened before the parser lands is readable rather
      // than blank, and swaps to the render when it arrives.
      //
      // `slot` names where on the card the answer belongs: '' for the closing
      // reply, 'prior:<i>' for one of the turns above it. A card now holds up
      // to twenty of these, so the patch has to say WHICH one it is patching;
      // matching on the text was fine when there was one.
      replyHtml(row, text, slot){
        if (!text) return '';
        const key = (row?.id || '') + '\u0000' + text;
        if (REPLY_HTML.has(key)) return REPLY_HTML.get(key);
        REPLY_HTML.set(key, '');              // one render per reply, not one per hover
        // The card this render was kicked for. A reader hovering down a list
        // opens several before the first parse lands, so the answer has to
        // check it is still wanted rather than patching whatever is open.
        const cardKey = 'session:' + (row?.id || '') + ':reply';
        (async () => {
          try {
            if (!window.GuideRender) await window.gh?.load('kits/guide-render.js');
            await window.GuideRender.needMarked();
          } catch { REPLY_HTML.delete(key); return; }   // let a later hover retry
          // The session's own branches, so a blob link inside a reply splits
          // its ref correctly: every session branch is claude/<slug>, and the
          // naive first-segment split yields ref `claude` and a path starting
          // with the rest of the name. A session row carries `branches`, not a
          // `name`; that is the branch row's field.
          const refs = row?.branches || [];
          const out = window.GuideRender.render(text, {
            knownRefs: [...refs, 'main'],
            preferRef: refs[0] || '',
          });
          if (!out.html) { REPLY_HTML.delete(key); return; }
          REPLY_HTML.set(key, out.html);
          // The class rides ON the card, not on a getter. x-if keeps this
          // subtree alive across a card swap (kind stays 'prose'), so a
          // :class effect that reads nothing reactive would never re-run.
          const c = this.rowCard;
          if (c?.kind !== 'prose' || c.key !== cardKey) return;
          const cls = window.GuideRender.bodyClass('drawer');
          if (!slot) {
            if (c.text === text) {
              this.rowCard = { ...c, html: out.html, htmlClass: cls };
              this.$nextTick(() => this.pinRowCard());
            }
            return;
          }
          const i = Number(slot.slice(6));
          if (c.prior?.[i]?.text !== text) return;
          // A new array, not a mutation: Alpine tracks the assignment, and a
          // card rebuilt between the kick and the answer must not be patched
          // through a stale reference, which the key check above already
          // caught but the array identity would have hidden.
          this.rowCard = { ...c, htmlClass: cls,
                           prior: c.prior.map((e, j) => (j === i ? { ...e, html: out.html } : e)) };
          this.$nextTick(() => this.pinRowCard());
        })();
        return '';
      },
      ...
      openSessionCard(row, cls, ev){
        ...
        if (cls === 'turns') {
          ...
          // Markdown, rendered where the parser has already answered for this
          // reply and empty on a first open, which the raw text covers until
          // replyHtml patches the card. See replyHtml.
          const html = this.replyHtml(row, reply);
          this.rowCard = {
            ...
            text: reply,
            html,
            htmlClass: html ? (window.GuideRender?.bodyClass('drawer') || '') : '',
            // !!reply, not reply: a bare && hands back the empty STRING when
            // there is no reply, which x-show reads as false and a test reads
            // as a type error. A boolean field is a boolean.
            cut: !!reply && row.replyCut === 'cut',
            ask: String(row.ask || ''),
            // THE SCROLL BACK. `reply` is the last thing said; these are the
            // main turns before it, each cut to its opening, asks interleaved
            // so twenty replies read as a conversation rather than as a wall.
            // Rendered as markdown like the reply, memoised the same way.
            prior: (row.turns || []).map(([k, t], i) => ({
              key: i, role: k, text: String(t || ''),
              html: k === 'a' ? this.replyHtml(row, String(t || ''), 'prior:' + i) : '',
            })),
