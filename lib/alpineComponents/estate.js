  ...
  Alpine.data('estate', function() {
    ...
    const branchRowBody = (opts = {}) => `
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    ${opts.nested ? `
                    <!-- NESTED: the subject rides the identity row rather than
                         taking a line of its own, so one branch is one row at
                         desktop width and wraps at phone width. The standalone
                         paragraph below is dropped in this mode. -->
                    <span x-show="row.subject" :title="row.subject" x-text="row.subject"
                          class="hidden sm:inline text-base text-base-content/60 truncate min-w-0 grow"></span>
                    <div class="grow sm:hidden"></div>
                    <div x-show="!row.subject" class="hidden sm:block grow"></div>
                    ` : `
                    <div class="grow"></div>
                    `}
                  ...
                  ${opts.nested ? `
                  <!-- The subject rides the identity row from sm up, where
                       there is width for it, and takes its own line below that,
                       where competing with the branch name truncated both to a
                       letter. One rule, two layouts, no breakpoint in the JS. -->
                  <p x-show="row.subject" class="sm:hidden text-base text-base-content/60 truncate"
          ...
          <div class="flex-1 min-w-0" :class="tab==='sessions' ? '' : 'hidden'"
            ...
            <div x-show="authed && sessionLens === 'list' && sessionNodes.length" class="flex flex-col gap-2">
              <template x-for="n in sessionNodes" :key="n.key">
                ...
                <div x-show="n.children.length" class="mt-2 pl-2.5 border-l-2 border-base-300/60 flex flex-col gap-2.5">
