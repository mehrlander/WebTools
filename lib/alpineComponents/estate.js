...
${opts.nested ? '' : `
    ...
    return {
                ...
                <div x-data="{ get row(){ return n.row } }">
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <!-- The closing state, in a slot the day's fixed width
                         ...
                         Not a control. It is two characters wide, which is the
                         tap target this row's comments already complain about,
                         and the thing it would open is the reply card the ask
                         line directly below it opens on the whole width. -->
                    <span class="shrink-0 w-5 text-center text-base leading-none"
                          :title="sessionStateNote(row)"
                          x-text="sessionStateMark(row)"></span>
      ...
      sessionStateMark(row){ return (this.SESSION_STATE[row?.state] || [])[0] || ''; },
      ...
      sessionStateNote(row){
        ...
        return row?.v && row.v !== window.RepoSessionsCache?.ROW_V
