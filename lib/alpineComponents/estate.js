document.addEventListener('alpine:init', function() {
  Alpine.data('estate', function() {
    ...
    return {
                ...
                <div class="rounded-lg border-l-4 pl-3 pr-3 py-2 transition-colors hover:brightness-[1.02]"
                  ...
                  <div class="flex items-center gap-2 min-w-0">
                    ...
                    <button @click.stop="repoChipMenu(row.repo, $event)"
                            ...
                            :title="'GitHub links for ' + row.repo"
      ...
      // ── Repo filter ──────────────────────────────────────────────────────
      // `openBranches` stays the whole cross-repo list, so the tab badge and
      // the empty state keep counting everything; `openRows` is what renders.
      openRepoFilter: '',
      ...
      hoverBranchMenu(row, ev){
        ...
        if (this.branchMenuAt) return this.openBranchMenu(row, el);
      ...
      // ── The row's repo chip ──────────────────────────────────────────────
      // The GitHub menu for the repo a row is in, filling the shell's one
      // anchored panel (the sidebar's), not a third one of this view's own.
      // Same list, same geometry, same hover: the reader learns the control
      // once. Left-aligned, since the chip leads its row.
      repoChipMenu(repo, ev){
        this.branchMenuAt = null;
        window.__shell?.toggleRepoMenu?.(repo, ev, 'github', { align: 'left' });
      },
      repoChipHover(repo, ev){
        this.cancelBranchClose();
        this.branchMenuAt = null;
        window.__shell?.hoverRepoMenu?.(repo, ev, 'github', { align: 'left' });
      },
