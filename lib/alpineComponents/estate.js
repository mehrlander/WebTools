...
${opts.nested ? '' : `
                    ...
                    <template x-if="branchRoutes(row)">
                      <span class="flex items-center gap-1 flex-wrap min-w-0">
                        ...
                        <span x-show="branchRoutes(row).near.length"
                              :title="nearRoutesNote(row)"
                              class="shrink-0 cursor-help rounded-full px-2 py-0.5 text-sm
    ...
    return {
              ...
              <template x-for="n in sessionNodes" :key="n.key">
                ...
                <div x-data="{ get row(){ return n.row } }">
                  ...
                  <!-- ── THE SCOPE: which repos this session stood in ────────
                       ...
                       "where was this". The Claude+ button below says "another
                       session in the same place", and a button naming a set the
                       row never printed is a button nobody can check.
                    ...
                    <span class="flex items-center gap-2 shrink-0 ml-auto">
                      ...
                      <button @click.stop="openSession(row)"
                        ...
                        <i class="ph text-xl" :class="sessionDetailLoading && openSessionId === row.id
                                                        ? 'ph-circle-notch animate-spin' : 'ph-cards-three'"></i></button>
                      <!-- THE SAME PLACE, AGAIN: a new Claude Code session
                           with this row's repositories preselected. The mark is
                           the house idiom for "a Claude session" (kits/
                           claude-mark.js) and the plus is the only thing that
                           separates this from the mark on the counts line
                           above, which opens the session this row IS. Two
                           readings of one glyph, which is right: they differ by
                           tense, not by kind.

                           NO PROMPT RIDES. claude.ai/code opens its own
                           composer focused, so a text box here would be a
                           second one whose contents then have to survive a
                           navigation. Where words DO need composing first, that
                           is the dictation page's job and it has the room.

                           EVERYTHING IT COULD NOT CARRY IS IN THE TITLE, and
                           there is always something: a multi-repo scope cannot
                           carry a branch at all (kits/prompt-link.js), and the
                           scope itself is where the shell stood rather than
                           what was attached. A button that quietly narrows the
                           thing it claims to copy would be worse than no
                           button, so the losses are named rather than implied.

                           Absent, not dimmed, when nothing resolved: a link
                           that preselects nothing is the plain new-session page
                           with extra steps, and the reader has that already. -->
                      <!-- The same scope, spoken. See sessionDictateUrl: the
                           Claude mark opens a composer to type into, and this
                           opens the one to talk at, with the repositories
                           already chosen at the far end. Two destinations for
                           one intent, so they sit together, and the microphone
                           is the only thing that separates them. -->
                      <a x-show="sessionDictateUrl(row)"
                         :href="sessionDictateUrl(row)" target="_blank" rel="noopener"
                         @click.stop
                         :title="'Speak a prompt for these repositories, then send it: '
                                 + sessionRepoSlugs(row).join(', ')"
                         class="flex items-center shrink-0 text-base-content/30 hover:text-primary transition-colors">
                        <i class="ph ph-microphone text-lg"></i></a>
                      <a x-show="newSessionFor(row).url && sessionRepoSlugs(row).length"
                         :href="newSessionFor(row).url" target="_blank" rel="noopener"
                         @click.stop
                         :title="newSessionNote(row)"
                         class="flex items-center gap-0.5 shrink-0 opacity-60 hover:opacity-100 transition-opacity">
                        <span class="flex" x-html="window.claudeMark.svg({ cls: 'w-5 h-5 shrink-0' })"></span>
                        <i class="ph ph-plus text-[11px]" :style="'color:' + window.claudeMark.CLAY"></i></a>
            ...
            <template x-if="rowCard">
              <div class="flex flex-col">
                ...
                <div class="flex items-baseline gap-2 px-3 pt-2.5 pb-2 border-b border-base-200 sticky top-0 z-10 bg-base-100">
                  ...
                  <span x-show="rowCard.aside" class="text-sm text-base-content/40"
      ...
      // ── The scope, and starting another session in it ────────────────────
      // A record names the repos it worked in, so the row can say what a
      // session's scope WAS. That is the fact; the button under it is the act,
      // and they are one thought: you read a session, you have an idea about
      // it, and the next move is another session in the same place. Until now
      // that meant opening claude.ai/code in another tab and re-picking by hand
      // what this row already had on screen.
      //
      ...
      // The chips carry that caveat in their title and so does the dispatch
      // button, because a scope copied forward copies the omission with it.
      //
      ...
      // Only the ones that resolved to an owner/repo the estate knows. A
      // checkout with no estate entry cannot be preselected, and handing the
      // bare name to the link would preselect nothing while looking like it
      // had, so it is left out here and counted in the note below.
      sessionRepoSlugs(row){
        return this.sessionRepoRows(row).map((r) => r.slug).filter(Boolean);
      },

      // The link, plus what could not ride. See kits/prompt-link.js: the
      // prompt is deliberately empty, because claude.ai/code opens its own composer
      // focused and a second one here would be a text box that has to be
      // carried across a navigation to reach the first.
      newSessionFor(row){
        const D = window.PromptLink;
        if (!D) return { url: '', carried: [], dropped: ['prompt-link kit not loaded'] };
        const rows = this.sessionRepoRows(row);
        return D.code({
          repos: rows.map((r) => r.slug),
          // Passed even when it cannot ride: with several repos the kit drops
          // it and says why, which is the sentence the tooltip below wants.
      ...
      // AND THE SAME SCOPE, SPOKEN. The link above opens a composer at the far
      // end, which is right when the prompt is a sentence you will type. It is
      // the wrong surface for a thought you would rather say, and saying it is
      // the case this row exists for: you are reading a session, an idea about
      // it arrives, and the phone in your hand is better at hearing it than at
      // taking dictation into somebody else's text box.
      //
      // So this goes to the dictation page carrying the repositories, and its
      // Send sheet does the rest. `?to=send` paints that destination as the
      // lead rather than acting on it, which is the page's own rule: an address
      // that filed something on arrival would be a different and much less safe
      // thing.
      //
      // A FIXED ADDRESS, like the launcher's Home row and for the same reason:
      // the app runs under a ?use= pin and inside a toss, and a relative hop
      // from either lands somewhere the reader did not ask for.
      sessionDictateUrl(row){
        const slugs = this.sessionRepoSlugs(row);
        if (!slugs.length) return '';
        return 'https://mehrlander.github.io/web-tools/pages/dictate.html'
             + '?to=send&target=code&repo=' + encodeURIComponent(slugs.join(','));
      },

      newSessionNote(row){
        const r = this.newSessionFor(row);
        const rows = this.sessionRepoRows(row);
        const unresolved = rows.filter((x) => !x.slug).map((x) => x.name);
        const parts = ['New Claude Code session'];
        parts.push(r.carried.length ? 'carrying ' + r.carried.join(', ') : 'with nothing preselected');
        for (const d of r.dropped) parts.push('Not carried: ' + d);
        if (unresolved.length)
          parts.push('Not carried: ' + unresolved.join(', ') + ' (no repository of that name in the estate)');
        parts.push('The scope is the checkouts this session stood in, which misses one worked only by absolute path.');
        parts.push('It prefills the form; nothing starts until you send it.');
        return parts.join('. ');
      },
      ...
      stateTurns(row){
        return (row?.states || []).map(([, t, ts, gap, dropped], i) => ({
