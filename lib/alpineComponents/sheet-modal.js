...
//
// CAVEAT, measured 2026-08-07 in the Map view's doc-sheet cut: in a real
// browser, slot bindings can end up FROZEN at their initial values — the
// mutation observer destroys Alpine effects on the nodes init() moves into
// the chrome, while the chrome's own bindings (added once, never moved) stay
// live. jsdom does not reproduce this, so a component test passes while the
// pixels fail. If the slot must reflect outer reactive state, paint it
// imperatively from that state instead of binding it, or use a surface that
// never moves nodes (the Map's doc reader switched to swipe-deck for this).
//
// Usage:
