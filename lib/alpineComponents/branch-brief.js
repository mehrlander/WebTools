...
// name the neighbours worth reading ahead, and `onMeta` to be told what only a
    ...
    return {
          ...
          <div class="shrink-0 flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-0.5 min-w-0">
              ...
              <div class="flex items-center gap-2 min-w-0 text-xs opacity-55 font-mono">
                <span class="badge badge-sm shrink-0" x-show="framed && brief"
            ...
            <div class="flex items-center gap-2">
              <div role="tablist" class="tabs tabs-box tabs-sm flex-nowrap bg-base-200 p-0.5 min-w-0">
                <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap" x-show="showGuideTab"
                   :class="pane === 'guide' && 'tab-active'" @click="pane = 'guide'">Guide</a>
                <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap" :class="pane === 'files' && 'tab-active'" @click="pane = 'files'">
                  Files<span class="font-mono opacity-50" x-text="brief?.files.length ?? ''"></span></a>
                <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap" :class="pane === 'commits' && 'tab-active'" @click="pane = 'commits'">
      ...
      async load() {
        ...
        try {
          ...
          if (pass === this._pass) {
            this.loading = false;
            // What the host could not know until now. The PR number is the
            // case that matters: the activity crawl asks GitHub for OPEN pull
            // requests only, so a branch whose PR merged has none in the
            // cache, and a deck header wanting to show it has to be told.
            // A plain callback, because the host mounts this component
            // directly; it used to be a postMessage across an iframe.
