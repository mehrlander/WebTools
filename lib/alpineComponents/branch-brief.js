    ...
    return {
      ...
      template: `
        ...
        <div class="mx-auto w-full flex flex-col px-4 pt-3 pb-4 gap-2"
          ...
          <!-- THE SECTIONS, IN READING ORDER: why, then what, then the
               documents themselves. The guide led from 2026-09-06; before that
               the files led and the guide sat under them, on the reading that
               the list is what cannot be read anywhere else. That was right
               while the guide was the only prose here. It stopped being right
               once the readable files were presented on the page, because then
               the page opened on a document with no statement of what the
               branch was for, and the one unautomated thing on the page was
               last.
               ...
               guide off the screen. The reorder answers that directly: the
               guide is FIRST and clipped, so it is reachable without a pane of
               its own, and the reason for the second scrollbar is gone. What is
               left is one region that scrolls, the same shape a phone has, with
               the heading row pinned inside it.
               (No backticks in this markup: it is a JS template literal.) -->
          <div class="flex flex-col gap-4"
            ...
            <!-- CLIPPED TO ITS HEAD, with the page's one expander, because
                 leading with the guide only helps if leading with it is cheap.
                 A two-thousand-word body at the top of the page is the same
                 failure the files had when they led: everything after it is
                 screens away. Twenty-two rem is the clip the presented files
                 take, so the page has one answer to "how much of a block do I
                 get before I ask for more" rather than one per section.

                 It was a PANE until 2026-09-06, growing into whatever the file
                 list left and scrolling the prose inside it, which is what kept
                 it reachable under a long list. Being first is what keeps it
                 reachable now, so the pane and the second scrollbar are both
                 gone. See the container note above.
                 (No backticks in this markup: it is a JS template literal.) -->
            <div x-ref="guide" x-show="!loading" class="relative flex flex-col gap-4">
             <div class="flex flex-col gap-4 overflow-hidden rounded-lg"
                  x-init="watchClip($el, 'guide')"
                  :class="clipExpanded('guide') ? '' : 'max-h-[22rem]'">
            ...
            <template x-if="!hasGuide">
             ...
             <button type="button" x-show="!clipExpanded('guide') && !!clipTall['guide']"
                     ...
                     class="absolute inset-x-0 bottom-0 h-16 flex items-end justify-center pb-2
                            rounded-b-lg bg-gradient-to-b from-transparent via-base-100/80 to-base-100
              ...
              <div class="flex items-center gap-2 shrink-0 sticky top-0 z-20 -mx-4 px-4 py-2 bg-base-200">
                ...
                <div class="flex items-baseline gap-1.5 min-w-0 shrink">
                  ...
                  <!-- The marker carries the guide's TITLE where there is room
                       ...
                       IT POINTS UP. The guide sat below this row until
                       2026-09-06 and the marker was how a reader learned there
                       was one; the guide leads now, so the arrow is a way back
                       to it from a row that is pinned while the list and the
                       documents scroll under it. An arrow is cheap and pointing
                       it the wrong way is not. -->
                  <button x-show="hasGuide" @click="setPane('guide')"
                    ...
                    <i class="ph ph-arrow-up text-[10px] opacity-50 shrink-0"></i></button>
                </div>
                <div class="grow"></div>
            ...
            <!-- NO HEIGHT RULE. This carried roomy:max-h-[45%] to stop a long
                 ...
                 now and the guide is above this and clipped, so there is
            ...
            <!-- ══ Reviewable ═══════════════════════════════════════════════
                 ...
                 LAST, because the page now reads why, then what, then the
                 documents themselves. Above the list they were the first thing
                 met, which put a document in front of a reader who had not yet
                 been told what the branch was for.
