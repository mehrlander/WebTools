  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
      ...
      get fileGroups() {
        ...
        const byMode = (!this.registry || !window.ContentRegistry)
          ? [{ mode: 'all', files, collapsed: false, note: '', labeled: false }]
          : window.ContentRegistry.group(files, this.registry).map(g => ({ ...g, labeled: true }));
        return this.liftReviewable(byMode);
      },

      // ── Reviewable first, the rest collapsed ─────────────────────────────
      //
      // TWO AXES IN ONE LIST, deliberately. `ContentRegistry.group` sorts by
      // WHO MADE IT, which is the registry's whole question and stays its own;
      // this splits by WHETHER YOU READ IT, which is the page's. A branch page
      // is opened to review, and what a reader reviews by reading is the pages
      // and the prose. The rest is code, read as a diff and mostly skimmed, so
      // it starts closed.
      //
      // Three things this must not break, each of which the simpler versions
      // of the idea did break:
      //
      //   THE COUNT. Every file is still in exactly one group and the heading
      //   still reads `brief.files.length`, so the groups sum to it. This is a
      //   LIFT, not the removal considered first: the Look row's chip opens the
      //   RENDER and a row opens the DIFF, which are different questions, and a
      //   deleted page has a row and no chip at all.
      //
      //   THE DECK. `deckFiles` reads the OPEN groups, so collapsing everything
      //   empties it, and its button keys `x-show` on `deckFiles.length` and
      //   would not render. The reviewable group is open, so the deck holds it
      //   on load and grows as groups are opened. That is the rule a collapsed
      //   `mechanical` group has always worked under; what changes is how much
      //   starts open, not the rule.
      //
      //   NOTHING TO OPEN IN THEIR PLACE. A branch with no page and no doc
      //   would collapse to nothing but headers. Measured over 20 merged
      //   branches: 12 changed no .html at all and 2 changed neither .html nor
      //   .md, so this is a real case, not a defensive one. There the lift
      //   stands down whole and the list is exactly what it was.
      //
      // Mechanical is never lifted from. A generated .md is machine output
      ...
      liftReviewable(groups) {
        const lifted = [], keep = [];
        for (const g of groups) {
          if (g.mode === 'mechanical') { keep.push(g); continue; }
          const rest = [];
          for (const f of g.files) (this.REVIEWABLE.test(f.path) ? lifted : rest).push(f);
          // A group emptied by the lift goes rather than drawing a header over
          // nothing; the registry's own note goes with it, since it described
          // files that are no longer there.
          if (rest.length) keep.push({ ...g, files: rest, labeled: true,
                                       mode: g.labeled ? g.mode : 'other' });
        }
        if (!lifted.length) return groups;
        return [{ mode: 'reviewable', files: lifted, collapsed: false, labeled: true,
                  note: 'html and markdown' },
                ...keep.map(g => ({ ...g, collapsed: true }))];
      },
