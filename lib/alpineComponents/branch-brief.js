...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
      ...
      inbox: null,   // the repo's declared inbox dir, read best-effort at init; feeds dropUrl
      deposit: { open: false, name: '', text: '', busy: false, msg: '', err: '' },
          ...
          <template x-if="brief && !loading">
            <div class="flex flex-col gap-4">
              ...
              <div class="flex items-center gap-2">
                ...
                <div class="grow"></div>
                <!-- One quiet row: the PR exit matches its siblings (ghost),
                     since the guide header's chip already announces
                     draft/ready louder than a button variant could. -->
                <a x-show="brief.pr && !framed" :href="prUrl" target="_blank"
                   class="btn btn-sm btn-square btn-ghost max-sm:h-11 max-sm:w-11"
                   :title="brief.pr ? ('PR #' + brief.pr.number + (brief.pr.draft ? ' (draft)' : '') + ' on GitHub') : ''">
                  <i class="ph ph-git-pull-request text-lg max-sm:text-xl"></i></a>
                ...
                <!-- max-sm bumps: these are thumb targets on a phone, and
                     btn-sm's 32px box with a 14px glyph undershoots both. -->
                <a :href="treeUrl" target="_blank" class="btn btn-sm btn-square btn-ghost max-sm:h-11 max-sm:w-11" title="Browse the tree on GitHub">
                  <i class="ph ph-git-branch text-lg max-sm:text-xl"></i></a>
                <a :href="compareUrl" target="_blank" class="btn btn-sm btn-square btn-ghost max-sm:h-11 max-sm:w-11" title="Compare on GitHub">
                  <i class="ph ph-git-diff text-lg max-sm:text-xl"></i></a>
                <!-- Add a file: opens the in-page deposit sheet below, scoped
                     to THIS branch. The branch page is where a session sends
                     its reader, so the deposit affordance belongs here; the
                     stage stays the general mover for content that already
                     has an address. A plus, because that is the glyph a
                     reader looks for. -->
                <button @click="openDeposit()" class="btn btn-sm btn-square max-sm:h-11 max-sm:w-11"
                        :class="deposit.open ? 'btn-primary' : 'btn-ghost'"
                        title="Add a file to this branch (paste or type, commits here)">
                  <i class="ph ph-plus text-lg max-sm:text-xl"></i></button>
              </div>

              <!-- ── The deposit sheet ────────────────────────────────────
                   Add a file to this branch without leaving the app: name
                   prefilled into the repo's inbox (else dump/), content typed
                   or pasted (kits/io.js paste, which carries the iOS
                   hidden-textarea fallback), committed via gh-transfer's
                   saveRaw with the viewer's token. GitHub's editor stays one
                   tap away for binaries and long edits. -->
              <div x-show="deposit.open" x-cloak
                   class="mt-2 p-3 rounded-xl border border-base-300 bg-base-100 flex flex-col gap-2">
                <div class="text-xs opacity-60">New file on <span class="font-mono" x-text="branch"></span></div>
                <input x-model="deposit.name" spellcheck="false"
                       class="input input-bordered input-sm w-full font-mono text-[16px]">
                <textarea x-model="deposit.text" rows="5" spellcheck="false"
                          placeholder="File content… (type, or tap Paste)"
                          class="textarea textarea-bordered w-full font-mono text-[16px] leading-snug"></textarea>
                <div class="flex flex-wrap items-center gap-2">
                  <button @click="depositPaste()" class="btn btn-sm" title="Paste the clipboard as the file content">
                    <i class="ph ph-clipboard"></i>Paste</button>
                  <span class="text-xs opacity-60"
                        x-text="deposit.text ? deposit.text.length.toLocaleString() + ' chars' : ''"></span>
                  <div class="grow"></div>
                  <a :href="dropUrl" target="_blank" rel="noopener" class="btn btn-sm btn-ghost"
                     title="Use GitHub's editor instead (for a binary, its upload form)">GitHub…</a>
                  <button @click="depositSave()"
                          :disabled="deposit.busy || !deposit.text || !deposit.name.trim()"
                          class="btn btn-sm btn-primary"
                          x-text="deposit.busy ? 'Committing…' : 'Commit to branch'"></button>
                </div>
                <div class="text-xs" :class="deposit.err ? 'text-error' : 'opacity-60'"
                     x-text="deposit.err || deposit.msg"></div>
              </div>
      ...
      async load() {
        ...
        try {
          ...
          try {
            if (!window.ContentRegistry && window.gh?.load) await window.gh.load('content-registry.js');
      ...
      // Guarded: the template can evaluate this before the page's gh.load
      // chain lands BranchSurvey (invariant 8 in docs/loader.md).
      get dropUrl() {
        return window.BranchSurvey ? window.BranchSurvey.dropFileUrl(this.repo, this.branch, this.inbox) : '#';
      },

      // ── The deposit sheet ─────────────────────────────────────────────────
      openDeposit() {
        this.deposit.open = !this.deposit.open;
        this.deposit.err = '';
        if (this.deposit.open && !this.deposit.name) {
          this.deposit.name = window.BranchSurvey?.dropFileName?.(this.inbox) || 'dump/drop.md';
        }
      },
      // kits/io.js owns the clipboard read, including the iOS hidden-textarea
      // fallback; lazy-loaded like the other on-demand kits (literal path, so
      // the build walker sees it).
      async depositPaste() {
        this.deposit.err = '';
        try {
          if (!window.io?.paste && window.gh?.load) await window.gh.load('kits/io.js');
          const t = await window.io.paste();
          if (t == null || t === '') throw new Error('clipboard empty or unreadable');
          this.deposit.text = t;
          // A pasted .html deserves an .html name; retag only the untouched
          // default, never a name the user edited.
          if (/^\s*(<!doctype|<html)/i.test(t) && /-drop\.md$/.test(this.deposit.name)) {
            this.deposit.name = this.deposit.name.replace(/\.md$/, '.html');
          }
        } catch (e) { this.deposit.err = 'Paste: ' + (e?.message || e); }
      },
      async depositSave() {
        const d = this.deposit;
        d.err = ''; d.msg = ''; d.busy = true;
        try {
          const gh2 = new window.GH({ token: window.TOKEN, repo: this.repo, ref: this.branch });
          if (typeof gh2.saveRaw !== 'function' && window.gh?.load) await window.gh.load('gh-transfer.js');
          if (typeof gh2.saveRaw !== 'function') throw new Error('gh-transfer.js unavailable');
          const name = d.name.trim().replace(/^\/+/, '');
          const b64 = btoa(unescape(encodeURIComponent(d.text)));   // utf8-safe; the contents API wants base64
          await gh2.saveRaw(name, b64, 'Drop ' + name + ' via branch page', this.branch);
          // Stay open: the confirmation line lives in the sheet, and closing
          // it would hide the one sentence that says the commit happened.
          d.msg = 'Committed ' + name + ' to ' + this.branch;
          d.text = '';
          d.name = '';
          this.load();   // the Files/Commits counts now include the deposit
        } catch (e) {
          d.err = (e?.status === 401 || e?.status === 403)
            ? 'Commit failed: this browser has no write token for ' + this.repo
            : 'Commit failed: ' + (e?.message || e);
