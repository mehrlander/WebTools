...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
      ...
      //
      // FRAMED, IT STAYS QUIET. As a slide of show-repo's branch deck three of
      // these are mounted at once and this component cannot tell which one the
      // reader is on. The deck knows and has not been asked, so the host that
      // supplies the chrome owns the subject too; three announcers racing is
      // worse than none.
      get subjectKey() {
        const f = this.reviewableFiles[this.revAt];
        return f ? [this.repo, this.branch, f.path, this.base].join('|') : '';
      },
      ...
      async announceSubject() {
        if (this.framed || this._gone) return;
        if (window.swipeDeck?.top()) return;
        if (!this.reviewableFiles[this.revAt]) {
        ...
        if (!this._chan) {
          ...
          if (!this.reviewableFiles[this.revAt]) return;
        ...
        this._chan.announce({ repo: this.repo, ref: this.branch, path: f.path,
                              route: 'deck', base: this.base, baseName: this.base });
      },
