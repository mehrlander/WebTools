          ...
          <template x-if="brief && !loading">
            <div class="flex flex-col gap-4">
              ...
              <div class="flex items-center gap-2">
                ...
                <!-- Drop a file: GitHub's new-file form on THIS branch, name
                     prefilled into the repo's inbox (else dump/). The branch
                     page is where a session sends its reader, so the deposit
                     affordance belongs here, not only on the Activity menu.
                     A plus, because that is the glyph a reader looks for. -->
                <a :href="dropUrl" target="_blank" rel="noopener" class="btn btn-sm btn-square btn-ghost max-sm:h-11 max-sm:w-11"
                   title="Add a file to this branch (GitHub's new-file form, name prefilled)">
