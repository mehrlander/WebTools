    ...
    return {
      ...
      template: `
        <!-- The layout, and the one thing it decides: WHERE the scrollbar is.
             ...
             while the sections under them scroll, so a long guide or a
             three-hundred-file list never carries away the branch name or the
             controls. Standalone it stays a page and scrolls as one, because a
             page that pins its own header costs a phone its URL-bar collapse
             and gains nothing.
        ...
        <div class="mx-auto w-full max-w-4xl flex flex-col p-4 gap-4"
            ...
            <div class="flex items-center gap-2">
              ...
              <div class="flex items-baseline gap-1.5 min-w-0 shrink">
                ...
                <button x-show="hasGuide" @click="setPane('guide')"
                        class="btn btn-xs btn-ghost gap-1 font-mono normal-case min-w-0"
                        :title="'Jump to the guide' + (guidePr ? ': #' + guidePr.number : '')">
                  <span class="truncate" x-text="guidePr ? '#' + guidePr.number : 'guide'"></span>
                  <i class="ph ph-arrow-down text-[10px] opacity-50"></i></button>
              </div>
              <div class="grow"></div>
          ...
          <!-- The scrolling region, and the only one. Both sections live in
               it, files first; the heading row above carries why. -->
          <div class="flex flex-col gap-4" :class="framed && 'flex-1 min-h-0 overflow-y-auto pb-4'">
            ...
            <div x-ref="files" x-show="!loading" class="flex flex-col gap-1.5">
              ...
              <div x-show="!!brief && !brief.pending && !filesLoading && !brief.files.length && !lentMissing.length"
      ...
      async load() {
        ...
        try {
          ...
          if (!this.hasGuide || this.pane === 'files' || o.file) this.ensureCompare();
