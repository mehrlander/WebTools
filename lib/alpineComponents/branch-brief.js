          ...
          <template x-if="brief && !loading">
            <div class="flex flex-col gap-4">
              ...
              <!-- Identity, and how much of it depends on who is asking.
                   ...
                   Inside show-repo's takeover the frame's own header already
                   carries the repo, the branch name and the PR number, so
                   repeating them here spent a phone's first screen on a second
                   copy, and the branch name wrapped to two lines doing it. When
                   framed this collapses to the chip and the base, which is the
                   one identity fact the header does not have. Standalone it
                   stays whole, on ONE line: a name too long for the width is
                   truncated with the full string on hover, since a wrapped
                   mono name reads as two names. -->
              <div class="flex items-center gap-2 min-w-0">
                <span class="badge shrink-0" :class="stateClass" x-text="brief.state"></span>
                <span x-show="!framed" class="font-mono text-lg font-medium truncate min-w-0"
                      :title="brief.branch" x-text="brief.branch"></span>
                <span class="text-sm opacity-60 font-mono truncate shrink-0">
              ...
              <div x-show="brief.files.length && pane === 'files'" class="flex flex-col gap-1.5">
                <!-- No heading: the tab that got you here says "Files" and
                     carries the count, so a row repeating both is a line of
                     phone screen spent on nothing. The totals are what the tab
                     cannot hold. -->
                <div class="font-mono text-[10px] tabular-nums self-end">
