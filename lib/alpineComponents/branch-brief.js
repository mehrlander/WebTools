    ...
    return {
      ...
      template: `
        ...
        <div class="mx-auto w-full flex flex-col px-4 pt-3 pb-4 gap-2"
          ...
          <!-- ══ Reviewable ═══════════════════════════════════════════════
               The pages and the prose, out of the file list and above it. They
               were mid-list until 2026-09-05, sorted by path among the code,
               and then briefly a group inside the list; a section is what they
               are. What a reader reviews by READING is a different question
               ...
               Locked, it takes its content height and no more than a third of
               the box, scrolling inside that: a branch with a dozen docs must
               not squeeze the guide, which is the one thing on this page that
               is not derived.
               (No backticks in this markup: it is a JS template literal.) -->
            ...
            <template x-for="f in reviewableFiles" :key="f.path">
              <div class="relative">
                ...
                <div class="overflow-hidden rounded-lg"
                     x-init="watchRev($el, f.path)"
                     :class="revExpanded(f.path) ? '' : 'max-h-[22rem]'">
                ...
                <!-- The fade IS the clip saying so, which is why it overlays
                     ...
                     the suite runs). -->
                <button type="button" x-show="!revExpanded(f.path) && !!revTall[f.path]"
                        @click="toggleRev(f.path)"
                ...
                <button type="button" x-show="revExpanded(f.path)" @click="toggleRev(f.path)"
      ...
      reviewCardOpts(f) {
        return { ...this.cardOpts(f), open: true, read: true };
      },

      // HOW TALL A PRESENTED FILE GETS BEFORE IT IS THE WHOLE PAGE. A rendered
      // SNAGS.md is fifteen thousand words; three of those out in the open put
      // the guide five screens down and undo the afternoon. So each card is
      // clipped to a preview and says so with a fade, and one tap gives it the
      // room it wants. The clip is overflow-hidden and not a scroller: the card
      // already owns one inside its pane, and a scrollbar inside a scrollbar is
      // the shape both this file and the house style refuse.
      revOpen: {},
      revTall: {},
      revExpanded(path) { return !!this.revOpen[path]; },
      toggleRev(path) { this.revOpen = { ...this.revOpen, [path]: !this.revOpen[path] }; },
      ...
      watchRev(el, path) {
        if (!window.ResizeObserver) { this.revTall = { ...this.revTall, [path]: true }; return; }
        const set = () => {
          const tall = el.scrollHeight > el.clientHeight + 4;
          if (!!this.revTall[path] !== tall) this.revTall = { ...this.revTall, [path]: tall };
        };
        const ro = new ResizeObserver(set);
        ro.observe(el);
        for (const c of el.children) ro.observe(c);
        this._revObs = (this._revObs || []).concat(ro);
        set();
      },
      ...
      destroy() {
        for (const ro of this._revObs || []) ro.disconnect();
        this._revObs = [];
      },
    };
  });
