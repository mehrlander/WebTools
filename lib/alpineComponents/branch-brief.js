...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
      ...
      // in the repo that declares them. Feeds the Look row above the tabs.
      routeManifest: null,
      // ── The content verdict ──────────────────────────────────────────────
      ...
      // Files pane counts it, filters by it, and marks the missing ones.
      //
      ...
      template: `
        ...
        <div class="mx-auto w-full flex flex-col p-4 gap-4"
          ...
          <div class="shrink-0 flex flex-col gap-4">
            ...
            <!-- ── Look: the branch, running ──────────────────────────────
                 Above the tabs, because it is not a reading of the branch but
            ...
            <div x-show="!loading && hasLook" class="flex items-center gap-1.5 flex-wrap min-w-0">
              ...
              <span x-show="bundleStale" class="shrink-0 text-xs text-warning/80"
                    title="?use= fetches dist/web-tools.js, which this branch did not rebuild: run npm run build:lib and commit, or these links serve the old bundle">
            ...
            <div class="flex items-center gap-2">
              ...
              <div role="tablist" class="tabs tabs-box tabs-sm flex-nowrap bg-base-200 p-0.5 min-w-0
                ...
                <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap"
                   ...
                   @click="setPane('guide'); if (!hasGuide) ensureCompare()">Guide</a>
                <a role="tab" class="tab gap-1 flex-nowrap whitespace-nowrap" :class="pane === 'files' && 'tab-active'"
                   @click="setPane('files'); ensureCompare()">
                  Files<span class="font-mono opacity-50" x-text="filesTabCount"></span></a>
              </div>
              <div class="grow"></div>
          ...
          <div class="flex flex-col gap-4" :class="framed && 'flex-1 min-h-0 overflow-y-auto pb-4'">
            ...
            <!-- ── The content verdict, as this pane's filter ───────────────
                 The question a branch LIST cannot answer and this pane can: for
                 each file the branch changed, is that content on the base
                 branch already? Three answers whose counts sum to the total,
                 and each one a filter, so "11 missing" stops being a number in
                 a tooltip somewhere else and becomes the eleven files, open and
                 readable as diffs. The estate row's chip links straight into
                 one of these filters, which is the whole reason it exists here
                 rather than as a longer tooltip there.

                 A FILTER, not a badge per row. Thirty rows are read by
                 scanning, and the density pass that took a control off every
                 row applies to a glyph on every row too. The one exception is
                 the missing mark below, which shows only while the list is
                 unfiltered: that class is worth spotting unasked, and once a
                 filter is on the strip has already said what every row is.
            ...
            <div x-show="pane === 'files' && (verdict || scanLoading || scanError)"
            ...
            <div x-show="pane === 'files' && verdictNote"
                 class="text-xs text-warning/90 px-1 pb-1 leading-snug"
                 x-text="verdictNoteText"></div>

            <!-- A filter that matches nothing says so, rather than leaving the
                 reader looking at empty group boxes. -->
            <div x-show="pane === 'files' && fileState && !filteredFiles.length && !scanLoading"
            ...
            <!-- No merge base, so no compare, so no diff to render here. The
                 estate crawl scanned the branch anyway, through a fallback
                 that reaches back into whatever line it came from, and the
                 paths it found are the half worth acting on. Listed rather than
                 left as a count on a row in another view, and linked, since the
                 point of naming a file is being able to open it. -->
            <div x-show="pane === 'files' && lentMissing.length" class="flex flex-col gap-2">
              <p class="text-sm opacity-70">
                No merge base with <span class="font-mono" x-text="base"></span>, so there is no diff
                to render here. The estate scan found
      ...
      async load() {
        ...
        this.scan = null; this.scanError = ''; this.fileState = o.fileState || '';
        try {
          ...
          // render: the Look row appears above the tabs when it lands, and a
          ...
          //
          // A host may ask for a pane, and that wins: the estate's verdict chip
          // opens this view because the reader tapped a count of FILES, and
          // landing on the guide would answer a question nobody asked.
          this.pane = o.pane || (this.hasGuide ? 'guide' : 'files');
          this.renderGuide();
          ...
          if (this.pane !== 'guide' || o.file) this.ensureCompare();
          // An address may also name a FILE, and that is the deck rather than a
          // pane: a link naming one path is asking to READ it, where the list
          // is the route to a file and never the destination. Not awaited, so
          // the head and the pane render while the compare that turns a path
          // into a slide is still in flight.
      ...
      // The tab reads what the pane SHOWS, and says what it is showing out of
      // when that is not everything. A count on a tab that disagrees with the
      // list under it is how a filter goes unnoticed.
      get filesTabCount() {
      ...
      // but the crawl scanned it through a fallback and its missing paths are
      // the actionable half of that. Linked, because the whole point of naming
      // a file is being able to open it.
      get lentMissing() {
        return (!this.brief || this.brief.files.length) ? [] : (o.scan?.missingPaths || []);
      },
      blobUrl(path) {
      ...
      // Switching panes is a reported fact, not just local state, because the
      // host stamps the address from what the slide is showing: a reader who
      // opened Files and copied the link should get a link that opens Files.
      // Assignment alone cannot do that, which is why the tabs call this
      // rather than setting `pane` inline.
      setPane(p) { this.pane = p; this.reportMeta(); },
      ...
      get routeChips() {
        ...
        return {
          ...
          // The near set is a COUNT, not chips, and the pixels are the argument.
          // One wide file is wide precisely by being declared many times, so a
          // branch touching estate.js brushes nine routes at once: rendered as
          // chips that was two lines of ghosted labels above the one line that
          // answers the question, on a phone, every time. The fact is worth
          // keeping (it says the branch is not as narrow as the solid chips
          // suggest) and worth exactly one slot.
          //
          // Not a link, deliberately. Every route here is one the rule says the
          // branch cannot be claimed to change, so offering to open one would
          // hand a reader the link this row exists to stop them opening.
          nearCount: near.length,
          nearTitle: near.length
            ? near.map(r => r.label).join(', ')
              + ' — touched only through '
              + [...new Set(near.flatMap(r => r.hits))].join(', ')
