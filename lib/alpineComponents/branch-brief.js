...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
      ...
      // Whether the branch on screen before this load had a guide. It exists
      // only to hold the tab strip still across a swap; see openBranch.
      hadGuide: false,
          ...
          <div class="shrink-0 flex flex-col gap-4">
            ...
            <div class="flex flex-col gap-0.5 min-w-0">
              <div class="flex items-center gap-2 min-w-0">
                <span class="badge shrink-0" x-show="brief" :class="stateClass" x-text="brief?.state"></span>
              ...
              <div class="text-xs opacity-55 font-mono truncate">
                <span x-show="!framed" x-text="repo + ' '"></span>vs <span x-text="base"></span>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2">
              <div role="tablist" class="tabs tabs-box tabs-sm bg-base-200 p-0.5 min-w-0">
                <a role="tab" class="tab gap-1" x-show="showGuideTab"
              ...
              <div class="grow"></div>
              <!-- Read the files one at a time. ONE control, on the tab row,
                   not an icon per row: a list of thirty rows is read by
                   scanning, and the collapsed-density pass rejected a second
                   glyph per row for exactly that reason. It shows only on the
                   Files pane, since that is the list it opens, and only when
                   there is something visible to open (a fully collapsed set of
                   groups is a reader's choice, and the deck honours it). -->
              <button x-show="pane === 'files' && deckFiles.length"
                      @click="openFileDeck(0)"
                      class="btn btn-sm btn-square btn-ghost max-sm:h-11 max-sm:w-11"
                      :title="'Read ' + plural(deckFiles.length, 'file') + ' one at a time'">
      ...
      init() {
        ...
        if (this.framed) { this.fitDocument(); this.listen(); }
        this.load();
      },

      // Give the mount a height to fill, so the pane's `flex-1 min-h-0` has
      // something to resolve against and the scrollbar lands inside the view
      // rather than on the document.
      //
      // Stamped from here rather than written into pages/branch.html for the
      // reason the message channel is: ?use= swaps the library while github.io
      // serves the page file from main, so a shell-side rule cannot be
      // previewed from a branch. It is also the more robust half. The page's
      // body holds a masthead as well as this mount, and reading the layout off
      // whatever shell is actually serving beats assuming one: the body becomes
      // a column, the masthead keeps its natural height, and the mount takes
      // the rest. A shell that has neither is unaffected, since only the mount
      // is told to flex.
      //
      // Framed only. Standalone, the document is the scroller, which is what a
      // page should be and what a phone needs to collapse its URL bar.
      fitDocument() {
        try {
          const b = document.body;
          b.style.height = '100dvh';
      ...
      openBranch(m) {
        ...
        // `pane` is deliberately NOT reset here, and neither is the sticky
        // guide-tab flag. Both are read by the head, which stays mounted
        // through the load, so clearing them would collapse the switch to two
        // tabs and move the selection, then put both back a round trip later:
        // a flicker in the one strip that should hold still while the pane
        // under it changes. load() settles them once the brief lands.
        this.load();
      },
      ...
      // Tell the embedder a deck is covering this page.
      //
      // The deck is opened INSIDE the frame, so it covers the frame's viewport
      // and not the shell's own header strip above it. Left alone, that strip
      // keeps its branch arrows and its position counter live over a file the
      // reader has drilled into, which is exactly the incoherence the drill
      // shape exists to avoid: "next branch" has no meaning at "file 3 of 6",
      // and the shell's own drag would page branches out from under a swipe
      // meant for files. So the shell hides the strip and stops listening
      // while this is true, and the drilled deck's header is the only one.
      reportDrill(on) {
        if (!this.framed) return;
        try {
          parent.postMessage({ source: 'web-tools', type: 'branch-drill',
                               on: !!on, repo: this.repo, branch: this.branch }, location.origin);
        } catch {}
      },
      ...
      async load() {
        ...
        try {
          ...
          // there is one, Files when there is not. `hadGuide` is what keeps
          // the tab from vanishing and returning during the NEXT load.
          this.hadGuide = this.hasGuide;
      ...
      // What the tab strip renders, which is not the same question. Mid-load
      // there is no brief and so no guide, and answering literally would drop
      // the tab and put it back a round trip later. The previous branch's
      // answer is the better guess for that gap, and it is only a gap.
      get showGuideTab() { return this.loading ? this.hadGuide : this.hasGuide; },
      ...
      // Drill into the files. The parent is whatever deck the reader is already
      // in, so the header becomes the file's and Back returns them here; inside
      // the branch takeover there is no parent DECK (the takeover is its own
      // chrome), so this opens as a root deck over it and its ✕ closes back to
      // the branch, which is the same journey by a different mechanism.
      //
      ...
      async openFileDeck(start) {
        ...
        try {
          ...
          window.fileDeck.open({
            ...
            parent: window.swipeDeck.top(),
            // No parent deck inside the takeover, but dismissing still goes UP
            // to the branch rather than closing the reader out of anything, so
            // the button is a chevron.
            back: true,
            onClose: () => this.reportDrill(false),
          });
          this.reportDrill(true);
