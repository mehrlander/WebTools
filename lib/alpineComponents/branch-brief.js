...
(function () {
  ...
  Alpine.data('branchBrief', function (opts) {
    ...
    return {
      ...
      // now? BranchSurvey answers it three ways (landed / differs / missing),
      ...
      // row chip, so a host that has one LENDS it (o.survey) on the same
      ...
      survey: null,               // measured: { states: Map } plus countStates fields
      surveyLoading: false, surveyError: '',
      ...
      template: `
        ...
        <div class="mx-auto w-full max-w-4xl flex flex-col p-4 gap-4"
          ...
          <div class="flex flex-col gap-4" :class="framed && 'flex-1 min-h-0 overflow-y-auto pb-4'">
            ...
            <div x-show="pane === 'files' && (verdict || surveyLoading || surveyError)"
              ...
              <span x-show="surveyLoading" class="flex items-center gap-1 opacity-50">
                <span class="loading loading-spinner loading-xs"></span>measuring</span>
              <span x-show="verdict?.truncated" class="flex items-center gap-1 text-warning"
                    title="GitHub truncated a tree listing for this repo, so a path it left out reads as missing here">
                <i class="ph ph-warning"></i>tree truncated</span>
              <span x-show="surveyError" class="flex items-center gap-1 text-warning" :title="surveyError">
                <i class="ph ph-warning"></i>not measured</span>
            </div>

            <!-- A filter that matches nothing says so, rather than leaving the
                 reader looking at empty group boxes. -->
            <div x-show="pane === 'files' && fileState && !filteredFiles.length && !surveyLoading"
            ...
            <!-- No merge base, so no compare, so no diff to render here. The
                 estate crawl surveyed the branch anyway, through a fallback
            ...
            <div x-show="pane === 'files' && lentMissing.length" class="flex flex-col gap-2">
              <p class="text-sm opacity-70">
                ...
                to render here. The estate survey found
      ...
      async load() {
        ...
        this.survey = null; this.surveyError = ''; this.fileState = o.fileState || '';
        try {
      ...
      async ensureCompare() {
        ...
        if (!this.brief.pending) { this.ensureSurvey(); return; }
        ...
        try {
          ...
          this.reportMeta();
          this.ensureSurvey();          // not awaited: the file list paints first
      ...
      // per reading pass, since `survey` is set and this returns early after.
      async ensureSurvey() {
        if (this.survey || this.surveyLoading) return;
        if (!window.BranchSurvey || !this.base) return;
        const files = this.brief?.files || [];
        if (!files.length) return;                     // nothing to classify
        const pass = this._pass;
        this.surveyLoading = true; this.surveyError = '';
        try {
          const gh = new window.GH({ token: window.TOKEN, repo: this.repo, ref: this.branch });
          const [base, tip] = await Promise.all([
            window.BranchSurvey.defaultTree(gh, this.base),
            window.BranchSurvey.defaultTree(gh, this.tipRef),
          ]);
          if (pass !== this._pass) return;             // a step landed while reading
          const states = window.BranchSurvey.pathStates(
            files.map(f => f.path), tip.sets, base.sets);
          this.survey = { ...window.BranchSurvey.countStates(states), states,
          ...
          if (pass === this._pass) this.surveyError = e?.message || String(e);
        } finally {
          if (pass === this._pass) this.surveyLoading = false;
        }
      },
      ...
      get verdict() {
        if (this.survey) return { ...this.survey, lent: false };
        const s = o.survey;
      ...
      // One file's verdict, or '' when nothing can answer yet. A lent survey
      ...
      stateOf(path) {
        if (this.survey?.states) return this.survey.states.get(path) || '';
        const lent = o.survey?.missingPaths;
        return (lent && lent.length && lent.includes(path)) ? 'missing' : '';
      },
      // The strip: one chip per state, plus a leading All that clears the
      // filter. Built from BranchSurvey.PATH_STATES so the vocabulary and the
      ...
      get pathStateChips() {
        ...
        return (window.BranchSurvey?.PATH_STATES || [])
      ...
      // but the crawl surveyed it through a fallback and its missing paths are
      // the actionable half of that. Linked, because the whole point of naming
      // a file is being able to open it.
      get lentMissing() {
        return (!this.brief || this.brief.files.length) ? [] : (o.survey?.missingPaths || []);
      },
      blobUrl(path) {
