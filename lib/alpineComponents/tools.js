document.addEventListener('alpine:init', function() {
  Alpine.data('tools', function() {
    ...
    // stage.files.
    //
    // The title and the description are NOT in that entry. docs/tools.json is a
    ...
    // pages/pages.json at load. It used to carry its own copies, and they
    ...
    return {
      ...
      async load(){
        ...
        try {
          ...
          // A pages.json that fails to load costs the titles, not the view.
          const [shelf, pages] = await Promise.all([
            gh.get('docs/tools.json'),
            gh.get('pages/pages.json').catch(() => null),
          ]);
          this.items = (() => { const j = JSON.parse(shelf.text); return Array.isArray(j.items) ? j.items : []; })();
          this.pageIdentity = pages ? Object.fromEntries(
            (JSON.parse(pages.text) || []).flatMap(g => (g.items || []).map(
