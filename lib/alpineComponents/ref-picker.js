...
// not in the list (a tag, a sha, a branch past the survey's reach) is still
...
//               re-pointed at `repo`
//
// The survey runs ON DEMAND (hover, focus, or open) and once per repo: a view
...
document.addEventListener('alpine:init', function () {
  Alpine.data('refPicker', function (opts) {
    ...
    return {
      ...
      typed: '',
      rows: [],
      loadedFor: '',   // the repo the rows describe, so a repo switch re-surveys
      loading: false,
      error: '',
      dated: true,     // false once a survey has degraded to the REST list
          ...
          <div x-show="open" x-cloak x-transition.opacity.duration.120ms
            ...
            <div x-show="!loading" class="max-h-72 overflow-y-auto py-1">
              ...
              <p x-show="!dated && matches.length" class="px-3 pt-1 pb-2 text-sm text-base-content/40">
                Undated and alphabetical: this survey fell back to the REST list.
              </p>
            </div>
          </div>
        </div>`,
