document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    const OFFLINE = {
      ...
      why: 'Not a page load, and not a check either: checks answer questions about data, they never run code. A session runs the builder; a 30-day file-age check on the repo card is what makes forgetting loud.',
    };
    ...
    return {
      ...
      template: `
        <div class="w-full">
          ...
          <p class="text-base text-base-content/50 mb-5 max-w-2xl">
            What the estate keeps derived, when each piece was last built, and what builds it.
            <span class="text-base-content/40">Built is the last commit on the file; checked is this browser's
            throttle stamp. A crawl that found nothing does not commit, so a recent check over an older build
            means current, not stale.</span>
          </p>
          ...
          <div class="flex flex-col gap-2 mb-8">
            <template x-for="r in rows" :key="r.key">
              <div class="rounded-box border border-base-300 bg-base-100 p-3 flex flex-col gap-2">
                <div class="flex items-start gap-2 min-w-0">
                  ...
                  <button @click="run(r)" :disabled="!authed() || busy(r)"
                          class="btn btn-sm gap-1.5 shrink-0 disabled:opacity-50"
                          :class="busy(r) ? 'btn-ghost' : 'btn-outline'"
                          :title="'Force the crawl now (normally every ' + r.throttle + ')'">
                    <i class="ph ph-arrows-clockwise" :class="busy(r) && 'animate-spin'"></i>
            ...
            <template x-if="offline">
              <div class="rounded-box border border-base-300 border-dashed bg-base-100 p-3 flex flex-col gap-2">
                <div class="flex items-start gap-2 min-w-0">
                ...
                <div class="text-sm text-base-content/40 pl-7 flex flex-col gap-0.5">
                  ...
                  <span class="text-base-content/50 mt-0.5" x-text="offline.why"></span>
                </div>
              </div>
            </template>
          </div>
          ...
          <h3 class="text-base font-mono uppercase tracking-widest text-base-content/40 mb-2">Read live</h3>
          <p class="text-base text-base-content/50 mb-2 max-w-2xl">
            Cheap enough to redo on demand, so nothing is committed and nothing throttles.
          </p>
          <div class="rounded-box border border-base-300 bg-base-100 p-3 flex flex-col gap-2 mb-8">
            <div class="flex items-start gap-2 min-w-0">
              ...
              <button @click="refreshGuides()" :disabled="!authed()"
                      class="btn btn-sm btn-outline gap-1.5 shrink-0 disabled:opacity-50"
                      title="Re-read the shelf">
                <i class="ph ph-arrows-clockwise"></i><span>Refresh</span>
              </button>
            </div>
            <div class="flex items-center gap-x-4 gap-y-1 flex-wrap text-base pl-7">
          ...
          <!-- ── This browser ────────────────────────────────────────────── -->
          <h3 class="text-base font-mono uppercase tracking-widest text-base-content/40 mb-2">This browser</h3>
          <p class="text-base text-base-content/50 mb-2 max-w-2xl">
            Held in memory or by the browser, gone on reload. Nothing here is estate state, and neither
            control writes anything.
          </p>
          ...
          <p class="text-sm text-base-content/40 mt-6 max-w-2xl">
            The registry also holds authored content (lists, surfaces, the private config) and captured
            records (sessions, mailbox, proposals). Neither is derived, so neither has an age to report
            or a crawl to run; they are each their own source of truth.
          </p>
        </div>
      `,
      ...
      async load() {
        ...
        if (!this.authed()) { this.err = 'Sign in with a GitHub token to read the registry.'; return; }
