document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    //
    //   Duration is nowhere. The only party that knows it is the browser that
    //   ran the crawl, and it reports it to a four-second toast and drops it.
    //
    // All three are limits of READING rather than writing: the fix for each is
    // to have the crawl record something, which is a different decision from
    // this panel and should not be smuggled in by leaving them unsaid.
    //
          ...
          <template x-if="!histErr">
            <div class="flex flex-col min-h-0 gap-2">
              ...
              <p class="text-sm text-base-content/40 leading-snug">
                ...
                How long a crawl took is recorded nowhere.
              </p>
                ...
                <template x-for="(h, i) in histRows" :key="h.sha">
                  <div class="border-b border-base-200 last:border-0 py-1.5">
                    <div class="flex items-baseline gap-2 text-sm">
                      <span class="font-mono text-base-content/70 shrink-0" x-text="h.stamp"></span>
    ...
    return {
      ...
      // to the interval at which it is checked. A median rather than a mean,
      // since one long quiet stretch (a fortnight nobody opened the page)
      // otherwise sets the whole figure.
      histSummary() {
        const n = this.histRows.length;
        if (!n) return 'no commits found';
        const gaps = this.histRows.map(h => h.gapMs).filter(Boolean).sort((a, b) => a - b);
      ...
      stamp(iso) { try { return new Date(iso).toISOString().slice(0, 16).replace('T', ' '); } catch { return ''; } },
