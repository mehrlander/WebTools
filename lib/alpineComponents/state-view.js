  ...
  Alpine.data('stateView', function () {
    ...
    const TICKS = (r) => `
      ...
      <div class="-my-0.5 flex items-center gap-2">
        ...
        <span class="shrink-0 text-[10px] leading-none text-base-content/40 tabular-nums"
              x-text="${r}.window"></span>
...
`;
    ...
    return {
      ...
      WINDOW_H: 24,
      // The label the rail carries, derived from the window rather than typed
      // beside it: two copies of one number is how a rail comes to say 24h over
      // a week of events.
      get windowLabel() { return this.WINDOW_H + 'h'; },
      async loadPulse() {
        if (!this.authed()) return;
        const reg = new window.GH({ token: window.TOKEN, repo: REGISTRY(), ref: 'main' });
