    ...
    const FILE_LINK = (r) => `
      <a :href="fileGh(${r}.file)" :data-peek="peekAddr(${r}.file)"
        ...
        <!-- The nudge is MEASURED, not judged. Centring the glyph box beside
             ...
             centred mark hung 2px BELOW the baseline of the name it sits
             against, which reads as sagging. Raising it 1px lands its mass
             0.35px off the text's x-height centre and leaves a 1px overshoot
             under the baseline, which is what a round glyph wants. Measured off
             the rendered pixels at 14px/ui-monospace; -0.5 and -1.5 render
             identically to 0 and -1, so the choice is integers only. -->
        <i class="ph ph-github-logo text-sm relative -top-px"></i>
      </a>`;
