document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    // sources with two throttles and two histories, and every reading below is
    ...
    const CACHES = [
      { key: 'configs', file: 'configs.json', label: 'Repo configs', icon: 'ph-sliders-horizontal',
        ...
        throttleKey: 'CONFIG_CACHE_INTERVAL_MS', throttleMs: 6 * 3600 * 1000, checkedKey: 'wt:configCacheCheckedAt',
      ...
      { key: 'activity', file: 'activity.json', label: 'Branches', icon: 'ph-git-branch',
        ...
        throttleKey: 'ACTIVITY_CACHE_INTERVAL_MS', throttleMs: 30 * 60 * 1000, checkedKey: 'wt:activityCacheCheckedAt',
      ...
      { key: 'sessions', file: 'sessions.json', label: 'Sessions', icon: 'ph-terminal-window',
        ...
        throttleKey: 'SESSIONS_CACHE_INTERVAL_MS', throttleMs: 15 * 60 * 1000, checkedKey: 'wt:sessionsCacheCheckedAt',
...
`;
    ...
    const GROUP_HEAD = `
      <template x-if="g.group">
        <div class="flex items-center gap-2 px-1 pt-0.5">
          ...
          <!-- The note explains the group on first sight; a live reading beats
               it the moment there is one, so they take turns rather than
               competing for the same slot.
               BOTH stand down below sm, and the probe's reason is measured: at
               390px the line truncates to "2 pus…", which is not a shorter fact
               but a useless one. Nothing is lost, because each row carries its
               own probe two lines below and the button's colour already says
               whether either source moved. The heading's copy is a wide-screen
               convenience, so it goes where it cannot be read. -->
          <template x-if="!groupProbe(g)">
            <span class="text-sm text-base-content/30 hidden sm:inline" x-text="g.group.note"></span>
          </template>
          <template x-if="groupProbe(g)">
            <span class="hidden sm:flex items-center gap-1.5 text-sm min-w-0" :title="groupProbeWhy(g)">
              <i class="ph shrink-0 text-base-content/40"
                 :class="groupProbe(g).moved ? 'ph-arrow-fat-line-up' : 'ph-equals'"></i>
              <span class="text-base-content/40">since</span>
              <span class="truncate"
                    :class="groupProbe(g).moved ? 'text-warning font-medium' : 'text-base-content/70'"
                    x-text="groupProbe(g).line"></span>
            </span>
          </template>
          <div class="grow"></div>
          <button @click="runGroup(g)" :disabled="!authed() || groupBusy(g)"
                  class="btn btn-sm gap-1.5 shrink-0 min-w-[7rem] disabled:opacity-40 disabled:border-base-300"
                  ${REFRESH_BTN('groupMatters(g)', 'groupBusy(g)')}
                  :title="groupWhy(g)">
    ...
    return {
      ...
      groupMatters(g) { return g.rows.some(r => this.matters(r)); },
      // Both halves, each with its own cost and its own throttle, since that is
      // exactly what a shared button stops the rows from saying for themselves.
      // The staleness preamble matches refreshWhy's, and names WHICH half is
      // past its throttle: on a shared button "past twice its throttle" without
      // a name is a sentence the reader cannot act on, because the two
      // throttles are 12h and 3h and the one that tripped is the whole story.
      groupWhy(g) {
        const stale = g.rows.filter(r => !this.probe[r.key] && r.stale).map(r => r.label);
        const lead = stale.length ? stale.join(' and ') + ' past twice its throttle. ' : '';
        return lead + 'Force both crawls now. ' + g.rows.map(r =>
          r.label + ': ' + r.cost + ', normally every ' + r.throttle).join('. ') + '.';
      },

      // The group's own probe: BOTH members' readings, side by side, and NEVER
      // added together. The two counts do not share a unit (repos pushed
      // against session records written), so a sum would be a number with no
      // referent, and concatenation is the only fold here that stays a fact.
      // `moved` counts how many members reported motion and exists to colour
      // the line; it is never rendered, for the same reason.
      //
      // Null until every member has reported. The probe's two calls fail
      // independently, and a heading that showed one reading while the other
      // was missing would read as the whole group's answer, which is the one
      // thing a half-read must not do. The rows still show whichever half
      // arrived, where it is labelled with the file it belongs to.
      groupProbe(g) {
        const parts = g.rows.map(r => this.probe[r.key]);
        if (parts.some(p => !p)) return null;
        return { moved: parts.filter(p => p.n > 0).length,
                 line: parts.map(p => p.line).join(', ') };
      },
      // Each member's own probe sentence, kept whole and attributed. probeWhy
      // already says what each reading over- and under-counts, and those
      // caveats are per file: the config listing is an upper bound, a PR opened
      // without a push moves nothing in the branch reading. Merging them into
      // one sentence would drop exactly the qualifications that make the
      // numbers safe to act on.
      groupProbeWhy(g) {
        return g.rows.map(r => r.label + ': ' + this.probeWhy(r)).join(' ');
      },
      ...
      // The shell owns every throttle, because the shell is what enforces it.
      // The row's own `throttleMs` is a fallback for the seconds before the
      // shell is up and for a jsdom harness that never mounts one, never a
      // second authority: two numbers for one fact is how a row comes to
      // promise "auto every 12h" over a crawl that runs every half hour.
      // tools/test/state-view-throttles.test.mjs holds the fallbacks to the
      // shell's constants so the pair cannot part in silence.
      throttleOf(c) {
        const live = window.__shell?.[c.throttleKey];
        return Number.isFinite(live) ? live : c.throttleMs;
      },

      async load() {
        const build = (c, extra) => ({ ...c, ...extra, throttle: this.humanMs(this.throttleOf(c)) });
        ...
        try {
          ...
          const stamp = (c, i) => ({
            ...
            stale: dates[i] ? (Date.now() - +new Date(dates[i])) >
              (c.throttleKey ? this.throttleOf(c) * 2 : 30 * 86400 * 1000) : false,
          });
