document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    return {
      ...
      // THE BAR SPANS THE WHOLE RUN, not one pass of it. It used to fill, reach
      // the end, and start over, which said the run had finished when it had
      // not: the one thing a progress bar must never say. Each pass is an equal
      // share of the length because each covers the same items; they do not cost
      // the same (the survey is the slow one), so this counts item-passes and
      // not time, which is the same honesty the per-pass bar already had.
      progPct(r) {
        const p = this.prog(r);
        if (!p?.total) return 0;
      ...
      progLabel(r) {
        const p = this.prog(r);
        if (!p) return '';
        // A multi-pass run says which pass, because the bar alone cannot: the
        // activity refresh is a quick pass and a survey behind it, and without
        // this the second one reads as the first one repeating.
