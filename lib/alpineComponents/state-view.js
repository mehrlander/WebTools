document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    //
    // NOT SUCCESS, AND NOT ACCENT. Green means one verb on this view (bring
    ...
    // something; winter's accent sits near enough to green to blur that.
    // Secondary is the theme's one remaining accent that is neither.
    //
    ...
    const STREAM_TONE = {
      ...
      sessions: { tick: 'bg-secondary/60', mark: 'bg-secondary', band: 'bg-secondary/10' },
    };
      ...
      template: `
        <div class="w-full">
          ...
          <div class="flex flex-col gap-2 mb-8">
            ...
            <!-- The second row with no button. Shorter than the one above it,
                 because it has less paperwork and not because it matters less:
                 it is not a state/ file, so it has no history to open and no
                 bytes to expand here. Same dashed border, so the two read as
                 the pair they are: the things this page reports and cannot
                 fix. -->
