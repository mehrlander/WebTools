document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    // THE TWO FILE CONTROLS, on every registry row. `{}` opens the JSON in the
    // app; the github mark is the way out. Tapping a path used to go straight
    // to GitHub, which is the wrong default for a page whose whole subject is
    // data the app already has a token for and a viewer for.
    const FILE_CONTROLS = (r) => `
      <button @click="togglePeek(${r})" :disabled="!authed()"
              class="btn btn-xs btn-ghost px-1.5 font-mono disabled:opacity-40"
              :class="peek === ${r}.key && 'text-primary bg-primary/10'"
              :title="peek === ${r}.key ? 'Close the data' : 'Read the JSON here (' + (${r}.size || 'unknown size') + ')'">
        <i class="ph text-sm" :class="peekBusy === ${r}.key ? 'ph-circle-notch animate-spin' : 'ph-brackets-curly'"></i>
      </button>
    ...
    const PEEK_PANEL = (r) => `
      <template x-if="peek === ${r}.key">
        <div class="ml-7 rounded-box border border-base-300 bg-base-200/30 p-2 flex flex-col min-h-0">
          <div x-show="peekErr" class="text-sm text-error font-mono px-1 py-2" x-text="peekErr"></div>
          <div x-show="!peekErr" class="flex flex-col h-[26rem]"
                ...
                <div class="flex items-start gap-2 min-w-0">
                  ...
                  <div class="min-w-0 flex-1">
                    <!-- The path is a LABEL, not a link. It used to be an
                         ...
                         the app can render it. The peek below reads it in
                         place; the github mark is the deliberate way out. -->
      ...
      async togglePeek(r) {
        ...
        try {
          ...
          this.$nextTick(() => document.getElementById('state-peek-' + r.key)?.__viewer
            ?.show(path, text, { repo: REGISTRY(), ref: 'main', path }));
        } catch (e) {
