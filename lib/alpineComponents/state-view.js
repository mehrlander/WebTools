  ...
  Alpine.data('stateView', function () {
    ...
    const FILE_CONTROLS = (r) => `
      <div class="flex items-center gap-0.5 justify-end">
        <!-- The word is the INVITATION, so it belongs only on the closed state.
             Open, the panel below is the answer to what the control does, and
             "Collapse" is a label for something already evident; the caret
             alone closes it, at a size that survives losing its word. -->
        <button @click="toggleOpen(${r})" :disabled="!authed()"
                class="btn btn-xs btn-ghost gap-1 px-1.5 disabled:opacity-40"
                :class="open === ${r}.key ? 'text-primary' : 'text-base-content/60 hover:text-primary'"
                :title="open === ${r}.key ? 'Collapse' : 'Read ' + ${r}.file + ' here, and when it changed'">
          <i class="ph transition-transform"
             :class="(busyRow(${r}) ? 'ph-circle-notch animate-spin'
                     : (open === ${r}.key ? 'ph-caret-down' : 'ph-caret-right'))
                     + (open === ${r}.key ? ' text-lg' : ' text-sm')"></i>
          <span x-show="open !== ${r}.key">Expand</span>
        </button>
      </div>`;
    ...
    return {
                ...
                <div class="flex items-start gap-2 min-w-0">
                  <div class="min-w-0 flex-1">
                    ...
                    <div class="flex items-baseline gap-2 flex-wrap">
                      ...
                      <!-- Path, mark, and size read as one unit, so the mark
                           is tight against the name it opens rather than an
                           equal member of the title line's wider rhythm. -->
      ...
      busyRow(r) { return this.peekBusy === r.key || this.histBusy === r.key; },
