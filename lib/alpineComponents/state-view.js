document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    // The panel, mounted only while open. It drives the shared multi-mode
    // viewer (alpineComponents/viewer.js, the same component the Files view and
    // the stage preview use), so tree, table, code, and raw all come along and
    // nothing here is a second JSON renderer. bindStore is off because this is
    // an embedded host, not the Files page.
    //
    // ONE BOX, not three. It used to sit in its own bordered, tinted, padded
    // box, indented under the icon, inside the card, wrapping a viewer that
    // draws its own frame: four nested edges squeezing an editor that then had
    // to truncate its own filename. Now a hairline separates it, the negative
    // margin lets it use the card's full width, and the height is a share of
    // the viewport rather than a fixed 26rem that was cramped on a phone and
    // stingy on a desktop.
    const PEEK_PANEL = (r) => `
      <template x-if="peek === ${r}.key">
        <div class="flex flex-col min-h-0 border-t border-base-300 pt-2 -mx-3 px-3">
          ...
          <div x-show="!peekErr" class="flex flex-col h-[60vh] min-h-[20rem]"
               :id="'state-peek-' + ${r}.key" x-data="viewer({ bindStore: false, fill: true, defaultMode: 'tree' })"></div>
        </div>
      </template>`;
    ...
    return {
        ...
        <div class="w-full">
            ...
            <template x-for="r in rows" :key="r.key">
              <div :id="'state-' + r.key"
                ...
                <!-- The icon rides the TITLE LINE rather than a gutter of its
                     ...
                     blurb into three wrapped lines on a phone, and left the
                ...
                <div class="flex items-start gap-2 min-w-0">
                  ...
                  <div class="flex flex-col items-stretch gap-0.5 shrink-0">
                    <button @click="run(r)" :disabled="!authed() || busy(r)"
                            class="btn btn-sm gap-1.5 min-w-[7rem] disabled:opacity-40 disabled:border-base-300"
              ...
              <div :id="'state-' + offline.key"
                ...
                <div class="flex items-start gap-2 min-w-0">
                  ...
                  <div class="flex flex-col items-end gap-0.5 shrink-0">
                    <span class="text-sm text-base-content/40 italic pt-1.5"
          ...
          <div id="state-guides"
            ...
            <!-- No file controls: the shelf is assembled in memory from a
                 listing per repo, so there is nothing committed to peek at and
      ...
      async togglePeek(r) {
        if (this.peek === r.key) { this.peek = ''; this.peekErr = ''; return; }
        if (!this.authed()) return;
        this.peek = r.key; this.peekErr = ''; this.peekBusy = r.key;
        const path = 'state/' + r.file;
        try {
          const reg = new window.GH({ token: window.TOKEN, repo: REGISTRY(), ref: 'main' });
          const text = (await reg.get(path)).text;
          if (this.peek !== r.key) return;   // closed while it was in flight
          // The viewer is handed the BASENAME, not the path. Its header shows
          // the name beside the stats and the mode buttons, and on a phone the
          // full `state/activity.json` truncated to `state/…`, which is the
          // least useful half. The row two lines up already names the file, and
          // `origin` still carries repo, ref, and the real path, so the header's
          // links are unaffected. The extension is what mode detection reads,
          // and the basename keeps it.
          this.$nextTick(() => document.getElementById('state-peek-' + r.key)?.__viewer
            ?.show(r.file, text, { repo: REGISTRY(), ref: 'main', path }));
        } catch (e) {
