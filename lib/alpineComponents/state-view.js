document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    const TICKS = (r) => `
      <!-- The rail sits INSIDE the gap the row already had, rather than adding a
           band of its own: measured 2026-08-22, its own padding plus one flex
           gap cost 22px a row, which on a 175px card is not "fits inside the
           existing card". A negative margin gives the marks back the space the
           layout was already spending, and the net cost is the 8px of rail. -->
      <template x-if="${r}.stream && pulse[${r}.key]">
      <div class="-my-1">
        <div class="relative h-2 w-full" :title="pulseTitle(${r})">
          <!-- The rule sits at the vertical middle and the ticks straddle it,
               so the strip reads as one object 8px tall rather than as a row of
               marks with a line under them. -->
...
`;
    ...
    return {
      ...
      pulse: {},          // the three registry caches, resolved
      ...
      async loadPulse() {
        ...
        const jobs = CACHES.filter(c => c.stream).map(async (c) => {
          try {
            ...
            const { times, capped } = this.eventsOf(c, doc);
            this.pulse = { ...this.pulse, [c.key]: this.strip(times, since, capped) };
      ...
      eventsOf(c, doc) {
        if (c.key === 'activity') {
          ...
          return { times: all.map(x => Date.parse(x.date)).filter(Number.isFinite),
                   capped: all.length ? Date.parse(all[all.length - 1].date) : null };
        }
        // A session's event is when it STARTED. Its end is a second event of a
        // different kind, and drawing both would double every session into a
        // pair of ticks that reads as twice the activity.
        const rows = doc?.rows || [];
        const times = rows.map(x => Date.parse(x.started)).filter(Number.isFinite);
        return { times, capped: times.length ? Math.min(...times) : null };
      },
      ...
      strip(times, since, capped) {
        const span = this.WINDOW_H * 3600 * 1000;
        const inWindow = times.filter(t => t >= since).sort((a, b) => a - b);
        return {
          ticks: inWindow.map(t => Math.min(100, Math.max(0, (t - since) / span * 100))),
          n: inWindow.length,
      ...
      pulseTitle(r) {
        ...
        if (!p) return '';
        const noun = r.stream === 'commits' ? 'commit' : 'session';
        const head = p.n ? p.n + ' ' + noun + (p.n === 1 ? '' : 's') + ' in the last '
                           + this.WINDOW_H + ' hours'
                         : 'No ' + noun + ' in the last ' + this.WINDOW_H + ' hours';
      ...
      changeTitle(r) {
        ...
        if (r.stream && p?.newest)
          return 'The newest ' + (r.stream === 'commits' ? 'commit' : 'session')
               + ' this cache holds (' + p.newest + '), which is when the source last moved.';
