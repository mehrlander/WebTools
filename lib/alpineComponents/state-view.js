document.addEventListener('alpine:init', function () {
  Alpine.data('stateView', function () {
    ...
    const FILE_CONTROLS = (r) => `
      <div class="flex items-center gap-0.5 justify-end">
        <button @click="toggleOpen(${r})" :disabled="!authed()"
                ...
                :title="'Read ' + ${r}.file + ' here, and when it changed'">
          ...
          <span x-text="open === ${r}.key ? 'Collapse' : 'Expand'"></span>
        </button>
    ...
    // The tab strip, at the head of the open panel. Two words, because the two
    // readings genuinely differ and the reader has already committed to looking:
    ...
    const TABS = (r) => `
      <div role="tablist" class="tabs tabs-sm tabs-bordered self-start">
        <button role="tab" @click="showTab(${r}, 'contents')"
                class="tab gap-1.5" :class="tab === 'contents' && 'tab-active'">
          <i class="ph ph-brackets-curly text-sm"></i><span>Contents</span>
        </button>
        <button role="tab" @click="showTab(${r}, 'history')"
                class="tab gap-1.5" :class="tab === 'history' && 'tab-active'">
          <i class="ph ph-clock-counter-clockwise text-sm"></i><span>History</span>
        </button>
      </div>`;
