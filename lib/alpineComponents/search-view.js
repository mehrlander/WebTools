...
document.addEventListener('alpine:init', function () {
  Alpine.data('searchView', function () {
    ...
    return {
      ...
      get facts() {
        if (this.mode === 'names') {
          // The one state where the button is dead says why, where the reason
          // is: a greyed control with no account of itself is the defect this
          // view's front door already had once.
          if (!this.canRun) {
            return 'Every file of every repo is not a listing worth reading. Pick a repo or name a folder to list one, or type to search names across all of them.';
          }
