...
document.addEventListener('alpine:init', function () {
  Alpine.data('searchView', function () {
    ...
    return {
      ...
      // Which mode the reader opens a file in. The Files view's viewer defaults
      // to `raw`, which is right there: you walked a tree to a file and the
      // question is what is in it, plainly. Here you searched for it, so the
      // question is already what it SAYS, and raw markdown or a wall of
      // unhighlighted JSON puts a mode switch between the reader and the
      ...
      // this estate holds files in the megabytes (dist/web-tools.js is 2.4 MB
      // and is browsable here). Past the cut a file opens raw, which is what
      // it did everywhere until today, and the mode switch is one tap.
      READ_MODE: (f) => {
      ...
      // ── The scope controls ───────────────────────────────────────────────
      // Known only for the repo the shell surveyed. Elsewhere '' is the honest
        ...
        try {
          if (this.mode === 'names' && this.browsing) {
            ...
            const dirs = res.dirs.map(d => ({
              key: 'd:' + d.path, icon: 'ph-folder', mono: true, label: d.name,
              sub: d.n + (d.n === 1 ? ' file' : ' files'),
              kind: 'dir', path: d.path,
            }));
            const files = res.files.slice(0, this.cap).map(p =>
              this.fileRow({ repo: this.repo, ref: this.ref.trim(), path: p }, { key: 'n:' }));
