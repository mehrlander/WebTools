...
document.addEventListener('alpine:init', function () {
  Alpine.data('fileReview', function (opts) {
    ...
    // the estate uses for docs.json's reach and surfacing.json's membership.
    ...
    const PANE_LABEL = { read: 'Read', image: 'Image', inside: 'Inside', binary: 'File' };
    const KB = (n) => n == null ? ''
      : n < 1024 ? n + ' B'
    ...
    return {
          ...
          <div x-show="open" x-collapse>
            ...
            <div class="flex flex-col gap-2"
              ...
              <div x-show="!loading" class="flex items-center gap-1">
                <div role="tablist" x-show="panes.length > 1"
                ...
                <div class="grow"></div>
                <div class="join shrink-0" x-show="tab==='diff' && diffable">
                ...
                <details class="dropdown dropdown-end shrink-0" x-ref="ghMenu">
                  <summary class="btn btn-ghost btn-xs gap-0.5 cursor-pointer" title="This file on GitHub">
      ...
      async _reloadBase() {
        if (this.binary || (this.kind && this.kind !== 'markdown')) return;
        this.baseText = null;
        this._teardown();
        this.loading = true;
        this.error = '';
      ...
      get comparable() {
        if (this.compareOff || this.kind === 'image' || this.kind === 'binary') return false;
      ...
      get copyTitle() {
        const what = { read: this.namePart, new: this.namePart, base: 'the base version',
                       patch: 'the patch', diff: 'the patch', inside: 'what is inside' }[this.tab];
