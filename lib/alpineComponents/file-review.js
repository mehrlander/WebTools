    ...
    return {
            ...
            <div class="flex flex-col gap-2"
              ...
              <div x-show="!loading" class="flex items-center gap-1">
                ...
                <button x-show="read && !bare" @click="toggle()"
                        class="flex items-center gap-1.5 min-w-0 grow text-left cursor-pointer">
                  <span class="font-mono text-xs w-3 text-center shrink-0 font-semibold"
                        :class="statusClass" :title="status" x-text="statusTag"></span>
                  <span class="font-mono text-sm min-w-0 flex items-baseline" :title="path">
                    <span x-show="!!dirPart" class="opacity-40 truncate shrink-[9999]" x-text="dirPart"></span>
                ...
                <span x-show="read && !bare && additions != null"
                ...
                <div class="grow" x-show="!read"></div>
                <div class="join shrink-0" x-show="!read && tab==='diff' && diffable">
                ...
                <div class="join shrink-0" x-show="read && viewModes.length > 0">
                ...
                <button x-show="!read && copyable !== null" @click="$clip(copyable)" :title="copyTitle"
                        class="btn btn-xs btn-square btn-ghost shrink-0">
                  <i class="ph ph-copy text-sm"></i></button>
                <button x-show="!read && action" @click="action && action.onClick(path)"
                        :title="action?.label"
                        class="btn btn-xs btn-square btn-ghost shrink-0">
                  <i class="ph" :class="action?.icon || 'ph-cards-three'"></i></button>
                <details class="dropdown dropdown-end shrink-0" x-ref="ghMenu">
                  <summary class="btn btn-ghost btn-xs gap-0.5 cursor-pointer"
                           :title="read ? 'What else this file can do' : 'This file on GitHub'">
                    <i class="ph text-base" :class="read ? 'ph-dots-three-outline' : 'ph-github-logo'"></i>
                    <i class="ph ph-caret-down text-[10px] opacity-50" x-show="!read"></i>
                  </summary>
                  <ul class="dropdown-content menu menu-sm z-20 mt-1 w-56 rounded-box border border-base-300 bg-base-100 p-1 shadow-lg">
                    <!-- x-if, not x-show: a hidden row is still in the DOM, and
                         a list card would then carry two copy controls, which
                         is the exact defect the one-copy-button pass removed
                         and which its test still watches. A surface that does
                         not fold these in should not build them. -->
                    <template x-if="read && copyable !== null">
                      <li><button type="button" @click="$clip(copyable); $refs.ghMenu.open = false"
                                  class="gap-2 flex-nowrap">
                        <i class="ph ph-copy shrink-0"></i><span x-text="copyTitle"></span></button></li>
                    </template>
                    <template x-if="read && !!action">
                      <li><button type="button"
                                  @click="$refs.ghMenu.open = false; action && action.onClick(path)"
                                  class="gap-2 flex-nowrap">
                        <i class="ph shrink-0" :class="action?.icon || 'ph-cards-three'"></i>
                        <span x-text="action?.label"></span></button></li>
                    </template>
              ...
              <template x-if="tab==='render'">
                <div class="rounded border border-base-200 overflow-hidden bg-base-100">
              ...
              <!-- On a reading surface the pane is called "Compare" and the
                   other half of the comparison is chosen in the sidebar, which
                   on a phone is closed. So the pane says what it is against;
                   without it the reader is looking at a diff whose second side
                   is named nowhere on screen. -->
              <div x-show="tab==='diff' && read" class="text-xs opacity-55 flex items-center gap-1.5 pb-1">
                <i class="ph ph-git-diff"></i>
                <span>against <span class="font-mono" x-text="baseName"></span></span>
                <span x-show="status==='added'" class="opacity-70">(not there at all)</span>
              </div>
      ...
      get paneClass() {
        return this.bare
          ...
          : 'border border-base-200 rounded max-h-[70vh] overflow-auto bg-base-100 p-2';
      },
