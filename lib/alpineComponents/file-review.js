...
//   - action:    { label, icon?, onClick(path) }, an optional labelled button
//                in the expanded card's action strip. The hosts use it for
//                "Read from here", which opens kits/file-deck.js at this file.
    ...
    return {
      ...
      template: `
        <div class="bg-base-100 border-b border-base-200 last:border-b-0">
          ...
          <div x-show="!bare" class="flex items-center gap-1.5 px-2 py-1 hover:bg-base-200/50">
          ...
          <div x-show="open" x-collapse>
            <div class="border-t border-base-200 bg-base-200/20 px-3 py-2 flex flex-col gap-2">
              ...
              <div class="flex items-center gap-2 text-sm">
                ...
                <span class="grow"></span>
                <!-- An optional host action, and the reason it is here rather
                     than in the collapsed row: the row is read by SCANNING and
                     the density pass rejected a second glyph per row, while
                     this strip is already a row of labelled buttons and can
                     take one more. The branch page and the review page both
                     pass "Read from here", which opens the file deck at this
                     file (kits/file-deck.js). A host that passes nothing gets
                     nothing, so the card is unchanged where it is not wanted. -->
                <button x-show="action" @click="action && action.onClick(path)"
                        class="btn btn-ghost btn-xs gap-1">
                  <i class="ph" :class="action?.icon || 'ph-cards-three'"></i><span x-text="action?.label"></span></button>
              ...
              <!-- The tab strip, computed. Which panes exist depends on what
                   the file IS, so writing them out meant four x-show clauses
                   that between them still could not say "this is a PNG". The
                   presentation pane leads where there is one; Diff and the
                   source panes follow it rather than being replaced by it. -->
              <div x-show="!loading && panes.length > 1" class="flex items-center gap-2">
                <div role="tablist" class="tabs tabs-box tabs-xs bg-base-200 p-0.5 flex-nowrap
              ...
              <div x-show="tab==='read'" class="max-h-[75vh] overflow-auto rounded border border-base-200 bg-base-100 px-3 py-2">
                <div :class="proseClass" x-html="readHtml"></div>
              </div>
              ...
              <div x-show="tab==='inside'" class="flex flex-col gap-1">
                ...
                <pre class="border border-base-200 rounded max-h-[70vh] overflow-auto text-[11px] leading-snug font-mono p-2 m-0 whitespace-pre-wrap break-words"
                     x-text="insideText"></pre>
              </div>
      ...
      init() {
        ...
        if (this.open) {
          if (this.patch && !(this.read && this.shownPane)) this.tab = 'patch';
      ...
      async load() {
        ...
        if (this.kind && this.kind !== 'markdown') {
          try {
      ...
      get shownPane() {
        return { markdown: 'read', image: 'image', svg: 'image',
      ...
      get panes() {
        ...
        if (shown) p.push({ id: shown, label: { read: 'Read', image: 'Image',
                                                inside: 'Inside', binary: 'File' }[shown] });
        if (this.kind === 'binary' || this.kind === 'image') {
      ...
      _defaultTab() {
        const shown = this.shownPane;
        if (shown && (shown !== 'read' || this.read)) return shown;
      ...
      async _loadShown() {
        ...
        const { bytes, size } = await this._gh(ref).bytes(this.path);
      ...
      async _renderRead() {
        ...
        const out = window.GuideRender.render(this.newText, {
