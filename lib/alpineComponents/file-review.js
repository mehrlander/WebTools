      ...
      template: `
        <div class="bg-base-100 border-b border-base-200 last:border-b-0">
          <!-- The collapsed row. A changed-file list is read by SCANNING, so
               ...
               wall, and the filename is what the eye is looking for. Truncation
               eats the LEFT (direction-rtl on the dir span), so a deep path
               loses the part that repeats rather than the part that
               distinguishes.
            ...
            <div class="border-t border-base-200 bg-base-200/20 px-3 py-2 flex flex-col gap-2">
              ...
              <div x-show="!loading && !binary" class="flex items-center gap-2">
                <div class="tabs tabs-boxed tabs-xs bg-base-200 p-0.5">
                  ...
                  <a class="tab tab-xs" :class="tab==='diff' && 'tab-active'" x-show="diffable || !loaded" @click="setTab('diff')">Diff</a>
                  <a class="tab tab-xs" :class="tab==='patch' && 'tab-active'" x-show="patch" @click="setTab('patch')">Patch</a>
                  <a class="tab tab-xs" :class="tab==='new' && 'tab-active'" x-show="newText !== null || !loaded" @click="setTab('new')">New</a>
                  <a class="tab tab-xs" :class="tab==='base' && 'tab-active'" x-show="baseText !== null || !loaded" @click="setTab('base')">Base</a>
                </div>
                <div class="grow"></div>
