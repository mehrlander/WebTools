...
document.addEventListener('alpine:init', function () {
  Alpine.data('repoMenu', function () {
    return {
      ...
      // A rule sits above a head row that FOLLOWS A NON-HEAD ROW, so it
      // separates blocks and not peers: two head rows in a row are one
      // question with two answers (open this repo here, or on GitHub) and
      // belong side by side.
      //
      ...
      // look at the other. The stroke is 1.44 because Phosphor's regular
      // weight is a 16-unit stroke on a 256 canvas, which on this 23-unit
      // viewBox is 23 * 16/256; the first pass used 1.7 and read visibly
      // heavier than the glyphs beside it. No backticks in here: this markup is a JS template
              ...
              <button @click="run(item.key)" :title="item.title || ''"
                ...
                <template x-if="item.mark === 'app'">
                  <svg viewBox="4.5 4.5 23 23" fill="none" stroke="currentColor" aria-hidden="true"
                       stroke-width="1.44" stroke-linejoin="round" stroke-linecap="round"
