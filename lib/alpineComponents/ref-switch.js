      ...
      return {
        ...
        template: `
          <div class="relative flex items-center gap-0.5" @click.outside="close()" @keydown.escape="close()">
            ...
            <!-- Narrower on a phone, where this shares a 390 px header with a
                 scrolling nav and the drawer toggle. 10 rem still holds a
                 branch's tail (the memorable half) and the whole warning tint,
                 which are what the box is read for; the full name is one tap
                 away in the list, and the caret is what a thumb aims at
                 anyway. -->
            <label :title="openerTitle"
                   class="input input-sm input-bordered flex items-center gap-1.5 w-40 sm:w-64 shrink-0 font-mono text-sm transition-colors"
            ...
            <div x-show="open" x-cloak x-transition.opacity.duration.120ms
              ...
              <!-- No second input here. The box lives in the header, where it is
                   reachable without opening anything; a copy inside the panel
                   would be two fields holding the same value, and the one that
                   is always visible is the one to keep. This panel is the list. -->
