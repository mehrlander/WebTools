        ...
        template: `
          <div class="relative flex items-center gap-0.5" @click.outside="close()" @keydown.escape="close()">
            ...
            <!-- ── The phone trigger: one glyph, and the panel is the control ──
                 ...
                 this component is for: off the default branch the glyph goes
                 warning-tinted, so "am I running something other than main" is
                 answered at a glance on every viewport. The color is the whole
                 signal; it carried an outline and a tinted background at first,
                 which drew a box around one glyph in a row of bare ones and
                 said nothing the color had not already said. -->
            <button type="button" @click="toggle()" @pointerenter="load()"
                    :title="openerTitle"
                    class="sm:hidden shrink-0 p-1.5 rounded-lg transition-colors"
              ...
              <div class="flex items-start gap-2 px-3 pt-2.5 pb-1.5 border-b border-base-300">
                ...
                <!-- ── The newest slot, phone only: one corner, three states ──
                     ...
                     is true:
                       ...
                       you are on it      the word, muted, not a control

                     The third state is the one that used to disappear, and it
                     is worth a word: knowing you are on the newest branch is an
                     answer, not the absence of one. A fourth case, no dated
                     non-default branch at all, leaves the slot empty, since
                ...
                <div class="sm:hidden shrink-0 flex items-center justify-end h-6 min-w-[5rem]">
                  ...
                  <!-- Both read showNewest rather than restating its rule, so
                       the corner and the desktop bolt cannot drift: after the
                       survey it means "there is somewhere to jump", which makes
                       its negation, with a newest in hand, exactly "you are
                       standing on it". -->
                  <span x-show="!loading && loaded && newest && !showNewest"
                        :title="'Running the most recently committed branch, ' + (newest ? newest.name : '')"
                        class="font-mono text-sm text-base-content/40">newest</span>

                  <button x-show="!loading && loaded && showNewest"
                          type="button" @click="goNewest()" :title="newestTitle"
                          class="flex items-center gap-1 rounded-lg px-1.5 py-0.5 font-mono text-sm text-primary hover:bg-primary/10 transition-colors">
