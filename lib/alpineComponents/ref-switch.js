...
// scans every branch carrying a different copy of the page. What the fab does
...
//
// It is deliberately NOT a second scan. Three things only:
//
...
//      the case the fab's scan list serves worst, since a branch you know the
...
//      itself once the scan says the newest branch IS the default one.
//
...
//   defaultRef  the guess until the scan lands; 'main'
      ...
      return {
        ...
        template: `
          <div class="relative flex items-center gap-0.5" @click.outside="close()" @keydown.escape="close()">
            <!-- Newest. Shown until the scan says otherwise, since before the
            ...
            <button x-show="showNewest" type="button" @click="goNewest()" @pointerenter="load()"
                    :title="newestTitle"
                    class="hidden sm:flex shrink-0 p-1.5 rounded-lg text-base-content/40 hover:text-primary hover:bg-base-200 transition-colors">
            ...
            <button type="button" @click="toggle()" @pointerenter="load()"
                    ...
                    :class="riding ? 'text-warning hover:bg-warning/10'
                                   : 'text-base-content/40 hover:text-primary hover:bg-base-200'">
            ...
            <button x-show="riding" type="button" @click="returnToLive()"
                    ...
                    class="hidden sm:flex shrink-0 p-1.5 rounded-lg text-warning/70 hover:text-warning hover:bg-warning/10 transition-colors">
            ...
            <div x-show="open" x-cloak x-transition.opacity.duration.120ms
              ...
              <div class="flex items-start gap-2 px-3 pt-2.5 pb-1.5 border-b border-base-300">
                <div class="min-w-0 grow">
                  ...
                  <div class="truncate font-mono text-sm text-base-content/40" :title="path" x-text="path"></div>
                </div>
              ...
              <!-- ── The list, and why a phone does without it ────────────────
                   ...
                   genuinely duplicated elsewhere: the fab's Render tab scans
                ...
                <template x-for="b in matches" :key="b.name">
                  <button type="button" @click="go(b.name)"
                    ...
                    <!-- A branch carrying a different copy of THIS FILE. Not the
                         ...
                         narrower one the scan can actually answer. -->
                    ...
                    <span x-show="b.name === defaultRef"
                          class="shrink-0 text-sm text-base-content/30">default</span>
                    <span class="shrink-0 text-sm text-base-content/30 tabular-nums" x-text="b.ago"></span>
                  </button>
                </template>
        ...
        // Hidden once the scan says there is nowhere to jump: either the
        ...
        async load() {
          ...
          try {
            ...
            let rows = null;
            // The path-aware scan when it is available: same one call, and it
        ...
        // the order: before the scan lands showNewest is true by design, so
        ...
        init() {
          ...
          // and it cannot answer anything before the scan lands. Desktop gets
          ...
          //
          // So the scan runs itself, once, and ONLY WHILE RIDING: on the
          ...
          // exists to report, so one scan is proportionate. On idle rather
