...
(() => {
  ...
  // The grammar lives in one module now (lib/repo-address.js). Read at call
  ...
  const grammar = () => {
    if (!window.RepoAddress) throw new Error('data-payload.js requires window.RepoAddress (load repo-address.js first)');
  ...
  //
  // Delegates to lib/repo-address.js, which owns the grammar. This copy used to
