...
// gh.load('swipe-deck.js') or drop a plain <script src> from jsDelivr, in
...
(() => {
  ...
  function core(count, render, o = {}) {
    ensureStyle();
    const track = h('div', {
      class: 'sd-track flex h-full w-full min-h-0 snap-x snap-mandatory overflow-x-auto scroll-smooth overscroll-x-contain',
      style: 'scrollbar-width:none',
    });
    const slides = [];
    const built = new Array(count).fill(false);
    for (let i = 0; i < count; i++) {
      const inner = h('div', { class: o.innerClass || 'mx-auto max-w-2xl w-full' });
      slides.push(inner);
      track.append(h('section', {
        class: 'h-full min-w-full snap-center overflow-y-auto px-4 py-5 sm:px-8 sm:py-8 '
          + (o.slideClass || ''),
      }, inner));
    }
    const build = i => {
