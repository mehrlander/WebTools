...
return (async () => {
  ...
  // tools, the way __loadedScripts answers "is X loaded". lib/traffic.js holds
  ...
  if (!window.__trafficWrapped && typeof window.fetch === 'function') {
    ...
    window.__trafficTotals = { calls: 0, wire: 0, unknown: 0, errors: 0, ms: 0, trimmed: 0 };
    const CAP = 400;
    ...
    window.fetch = function (input, init) {
      ...
      const entry = { url, t: Date.now(), ms: 0, status: 0, wire: null, error: null };
  ...
  // read() goes through, and the pre-build (lib/kits/build.js) overrides it to
  ...
  if (!proto.__getWrapped) {
    ...
    proto.get = async function (p) {
      const res = await origGet.call(this, p);
      try {
  ...
  try {
    ...
    if (typeof document !== 'undefined' && !optedOut) {
      ...
      try { await gh.load('alpineComponents/path-picker.js'); } catch (e) {}
      (async () => {
  ...
  // The source peek is standing equipment for the same reason the FAB is: it
  // belongs wherever an exact-file GitHub jump-over is rendered, and a page's
  // own boot list is the wrong owner of that. It first hung off show-repo's
  // boot block, which failed the one test that matters here — a page shell is
  // served from main even under ?use=, so a preview of the branch loaded the
  // new components with no SourcePeek behind them, every :data-peek bound to
  // null, and no card could appear. In lib it rides the ref like everything
  // else. repo-address.js comes with it, being the address grammar the peek
  // parses with (and the module the estate's parsers are converging on, so
  // this loads it rather than restating the regex).
  await gh.load('repo-address.js');
  // The surface envelope, standing equipment for the same reason: it is the
  // shared item vocabulary two components read through (the Surfaces shelf and
  // the Stage, which are one view in two modes), so a page's own boot list is
  // the wrong owner of it. Loads beside repo-address.js, the address grammar
  // its sources are written in.
  await gh.load('surface.js');
  await gh.load('source-peek.js');
  // The read over the ledger this file collects, standing equipment for the
  // same reason source-peek is: the fab's readout strip is on every tab of
  // every page, so the module behind it cannot be a lazy load that arrives
  // after the number was wanted. Pure functions, no side effects on load.
  await gh.load('traffic.js');
