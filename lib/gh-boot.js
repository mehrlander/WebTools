  ...
  try {
    ...
    if (typeof document !== 'undefined' && !optedOut) {
      ...
      (async () => {
        ...
        await sleep(1500);
        // The load-race guard. A page whose own chain is slower than this
        // timer (measured live: shorter.html on a phone through a #gh= toss,
        // four sequential API round trips) would get our Alpine started
        // against helpers that have not loaded, throwing from its inline
        // x-data. A page publishes its chain as window.__pageBoot (see the
        // canonical boot block in README.md) and we wait for it; a failed
        // chain still gets the FAB, and a page that publishes nothing
