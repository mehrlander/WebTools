...
(() => {
  ...
  function openTranscript(messages, opts = {}) {
    ...
    // link goes to the live conversation. Absent for a provider that has no
    // addressable session (Gemini), so it is conditional, not a placeholder.
    const openLink = opts.url ? h('a', {
      class: 'btn btn-ghost btn-sm btn-circle shrink-0', href: opts.url,
