...
(() => {
  ...
  function openTranscript(messages, opts = {}) {
    ...
    const sub = [opts.provider, opts.date, turns && `${turns} messages`].filter(Boolean).join('  ·  ');
    ...
    // that, derive it from the uuid the header note just gave up, so the link
    // works for any caller rather than only one that thought to pass it. Absent
    // for a provider with no addressable session (Gemini) and when there is no
    // uuid to build from, so it is conditional, not a dead placeholder.
    const href = opts.url || chatUrl(opts.provider, facts?.uuid);
    const openLink = href ? h('a', {
      class: 'btn btn-ghost btn-sm btn-circle shrink-0', href,
      target: '_blank', rel: 'noopener', title: 'Open the chat', 'aria-label': 'Open the chat',
    }, h('i', { class: 'ph ph-arrow-square-out text-base' })) : '';
    const header = h('div', { class: 'flex items-center gap-3 border-b border-base-300 bg-base-100/90 px-4 py-3 backdrop-blur sm:px-6' },
      ...
      h('div', { class: 'min-w-0 flex-1' },
        ...
        h('p', { class: 'truncate text-xs text-base-content/50' }, sub || 'Swipe through the exchanges')),
      openLink, pill);
