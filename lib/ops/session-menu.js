...
//           bold for the lead word or the age; see bold() below
...
(function sessionMenu(input) {
  ...
  function label(r, here) {
    ...
    return (here ? HERE : RECENT) + ' ' + bold(ago(r.started)) + '  ' + ask;
  }
  ...
  function build(idx) {
    ...
    // session or that none was on the clipboard. The index's age appears only
    // when it is old enough to explain a missing session.
    ...
    if (isFinite(age) && age > 6 * 3600000) caption += ' · index ' + ago(idx.generatedAt) + ' old';
