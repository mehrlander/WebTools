...
(function () {
  ...
  // ── Classification ────────────────────────────────────────────────────────
  // Attribution by endpoint SHAPE, deliberately, and the alternative is worth
  // naming: there is no reliable async caller context in a browser, and the
  // scripts registry already refused stack sniffing for the same reason
  // (gh-api.js runs loaded files as anonymous Function bodies, so a stack names
  // nobody). An endpoint shape cannot lie about who it is, and in practice it
  // answers the question anyway: forty calls to `commits` is the activity
  // crawl, whatever function issued them.
  const API_SHAPES = [
  ...
  function apiGroups(entries) {
    ...
    for (const e of entries || []) {
      ...
      const g = by.get(c.group) || { group: c.group, label: c.label, host: c.host, calls: 0, wire: 0, unknown: 0, ms: 0, errors: 0, cached: 0 };
      g.calls++;
      g.ms += num(e.ms);
  ...
  function summary({ boot: b, api: a, rate }) {
    const bits = [];
    if (b && b.count) bits.push(fmtBytesShort(b.wire));
